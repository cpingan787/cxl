...
if __name__ == "__main__":
    ...
    bus._bmapi.BM_SetLogLevel(bus._bmapi.BM_LOG_ERR)
    bus.config_isotp(tester_msg_id=UDS_REQUEST_ID, ecu_msg_id=UDS_RESPONSE_ID, mode=bmapi.BM_ISOTP_NORMAL_ECU, fd=False, dlc=8, functional=False)
    print("ECU UDS simulator started on %s, listening on 0x7DF and 0x%x ..." % (str(bus), UDS_REQUEST_ID))

    while True:
        request = bus.receive_isotp(timeout=10.0)
        print("UDS Request:", request)
        response = None
        if request[0] == 0x10:
            p2_server_max = 1000 #uint:ms
            p2_star_server_max = 100 #uint:10ms
