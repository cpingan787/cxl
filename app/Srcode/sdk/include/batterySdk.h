...
typedef enum
{
    ...
    E_BatteryState_DeepLowVoltageRecover,                //深度亏电boost恢复状态
    ...
    E_BatteryState_TrickleChargeCheckTem,                //涓流充电温度检查
...
typedef enum
{
  E_BatteryResisState_Init,                                 //初始化状态
	E_BatteryResisState_IntVoltageCheck,                      //采集空载电压状态
	E_BatteryResisState_Discharge,                            //放电状态，采集放电电压
	E_BatteryResisState_DischargeVolCheck,                    //放电电压确认状态
	E_BatteryResisState_End,                                  //结束状态
}BatteryResisState_e;


typedef enum
{
  E_BatteryAgeResult_Invalid = 0,                 // 老化检测结果无效，表示未完成或检测失败
  E_BatteryAgeResult_Normal,                      // 老化检测结果正常，表示未老化
  E_BatteryAgeResult_Aging,                       // 老化检测结果老化，表示达到老化门限
}BatteryAgeResult_e;

typedef struct
{
  int16_t temperature;                    // 温度
  uint16_t eolResistance;                 // 老化门限
}BatteryAgeMap_t;
...
/*************************************************
  Function:       BatterySdkGetBatteryEnableFlag
  Description:    获取备用电池的启用状态
  Input:          无
  Output:         无
  Return:         0：未启用
                  1：已启用
...
/*************************************************
  Function:       BatterySdkSetBatteryEnableFlag
  Description:    设置备用电池的启用状态
  Input:          enable: 0：未启用 1：已启用
  Output:         无
  Return:         0：未启用
                  1：已启用
  Others:         VIN写入后改变备用电池使能状态标志位
*************************************************/
void BatterySdkSetBatteryEnableFlag(uint8_t enable);
#endif
