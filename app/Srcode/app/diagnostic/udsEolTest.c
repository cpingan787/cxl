...
static const ReadDidMap_t g_readDidMap[] = 
{
...
static const WriteDidMap_t g_writeDidMap[] = 
{
...
static const WriteDidMap_t g_routineStartDidMap[] = 
{
...
static const ReadDidMap_t g_routineResultDidMap[] = 
{
...
static TestUdsSecurityLevel_e g_securityLevel = TestUdsSecurity_LevelNone;
static TestUdsSecurityLevel_e g_SeedGetLevel = TestUdsSecurity_LevelNone;
static uint8_t g_returnKey[10];
static uint8_t g_seedArray[10];
static uint32_t g_securityTimeOutCount = 0;
static uint8_t g_diagnosticCanChannel = TBOX_CAN_CHANNEL_1;
...
int16_t ServiceTestSecurityProcess(uint8_t *pUdsDataIn,uint16_t lenIn,uint8_t *pUdsDataOut,uint16_t *pLenOut)
{
  ...
  {
    uint8_t seedSize;
    GetRandomKey(g_seedArray,&seedSize);
    SeedToKey(g_seedArray,g_returnKey,TEST_SECURITY_LEVEL1);
    pUdsDataOut[0] = 0xFA;
    pUdsDataOut[1] = 0x01;
    memcpy(&pUdsDataOut[2],g_seedArray,4);
    *pLenOut = 6;
    g_SeedGetLevel = TestUdsSecurityLevel_Level1;
    //g_securityTimeOutCount = 0; 
  }
  else if(0x02==subFunction)//extended security key
  {
    if(g_SeedGetLevel == TestUdsSecurityLevel_Level1)
    {
      if(CheckByteArrayIsSame(g_returnKey,pUdsDataIn+2,4)==0)//key is valid
      {
        g_securityLevel = TestUdsSecurityLevel_Level1;
        g_SeedGetLevel = TestUdsSecurity_LevelNone;
        //g_securityTimeOutCount = 0;
        pUdsDataOut[0] = 0xFA;
        pUdsDataOut[1] = 0x02;
        *pLenOut = 2;
  ...
  else if(0x09==subFunction)//FBL seed
  {
    uint8_t seedSize;
    GetRandomKey(g_seedArray,&seedSize);
    SeedToKey(g_seedArray,g_returnKey,TEST_SECURITY_LEVEL3);
    pUdsDataOut[0] = 0xFA;
    pUdsDataOut[1] = 0x09;
    memcpy(&pUdsDataOut[2],g_seedArray,4);
    *pLenOut = 6;
    g_SeedGetLevel = TestUdsSecurityLevel_Level2;
    g_securityTimeOutCount = 0;
  }
  else if(0x0A==subFunction)//FBL key
  {
    if(g_SeedGetLevel == TestUdsSecurityLevel_Level2)
    {
      if(CheckByteArrayIsSame(g_returnKey,pUdsDataIn+2,4)==0)//key is valid
      {
        g_securityLevel = TestUdsSecurityLevel_Level2;
        g_SeedGetLevel = TestUdsSecurity_LevelNone;
        pUdsDataOut[0] = 0xFA;
        pUdsDataOut[1] = 0x0A;
        *pLenOut = 2;
...
static  int16_t ServiceGetWriteDidFunction(uint16_t WDID,pWrite_DIDFunction *pfun)
{
    ...
    listSize = sizeof(g_writeDidMap) / sizeof(g_writeDidMap[0]);
    ret = -1;//invalid did 
    *pfun = NULL;
    
    for(i = 0; i < listSize; i++)
    {
        if(g_writeDidMap[i].u16DID == WDID)
        {
            if(g_writeDidMap[i].pFun!=NULL)
            {
              *pfun = g_writeDidMap[i].pFun;
              ret = 0;                   
            }
            break;
        }
...
static int16_t ServiceGetReadDidFunction(uint16_t RDID,pRead_DIDFunction *pfun)
{
    ...
    *pfun = NULL;
    ret = -1;
    listSize = sizeof(g_readDidMap) / sizeof(g_readDidMap[0]);
    
    for(i = 0; i < listSize; i++)
    {
        if(g_readDidMap[i].u16DID == RDID)
        {
          if(g_readDidMap[i].pFun!=NULL)
          {
            *pfun = g_readDidMap[i].pFun;
            ret = 0;
          }
          break;          
        }
...
static  int16_t ServiceGetRoutineStartDidFunction(uint16_t WDID,pWrite_DIDFunction *pfun)
{
    ...
    *pfun = NULL;
    ...
    for(i = 0; i < listSize; i++)
    {
        if(g_routineStartDidMap[i].u16DID == WDID)
        {
...
static int16_t ServiceGetRoutineResultDidFunction(uint16_t RDID,pRead_DIDFunction *pfun)
{
    ...
    *pfun = NULL;
    ...
    for(i = 0; i < listSize; i++)
    {
        if(g_routineResultDidMap[i].u16DID == RDID)
        {
...
{
  g_diagnosticCanChannel = canChannel;
}
static uint8_t ServiceTestProcessSecurityCheck(uint8_t *pUdsDataIn,uint16_t lenIn,uint8_t *pUdsDataOut, uint16_t *pLenOut)
{
    ...
    if(TestUdsSecurityLevel_Level1==g_securityLevel)//security check///////
    {
      uint16_t did;
...
{
  if((g_securityTimeOutCount*cycleTime)<40000)
  {
    g_securityTimeOutCount++;    
  }
  else
  {
	if(g_SeedGetLevel != TestUdsSecurity_LevelNone)
	{
    ...
    g_securityTimeOutCount = 0;
    g_SeedGetLevel = TestUdsSecurity_LevelNone;
    g_securityLevel = TestUdsSecurity_LevelNone;
  }
}
...
static int16_t ServiceSetTboxTestMode(uint8_t *pData,uint16_t length)
{
  ...
  {
    if(TBOX_CAN_CHANNEL_1==g_diagnosticCanChannel)
    {
    ...
    else if(TBOX_CAN_CHANNEL_2==g_diagnosticCanChannel)
    {
    ...
    else if(TBOX_CAN_CHANNEL_3==g_diagnosticCanChannel)
    {
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_1,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_2,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_4,1); 
    ...
    else if(TBOX_CAN_CHANNEL_4==g_diagnosticCanChannel)
    {
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_1,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_2,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_3,1); 
    ...
    else if(TBOX_CAN_CHANNEL_5==g_diagnosticCanChannel)
    {
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_1,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_2,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_3,1); 
    ...
    else if(TBOX_CAN_CHANNEL_6==g_diagnosticCanChannel)
    {
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_1,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_2,1);
      CanHalSetCanTestMode(TBOX_CAN_CHANNEL_3,1); 
