...
#define DEM_EVT_5G_DIV2_SHORT ((Dem_EventIdType)EventParameter_0x95A111) /*B15A1 FTB:11 */
#define DEM_EVT_5G_DIV2_OPEN ((Dem_EventIdType)EventParameter_0x95A113)  /*B15A1 FTB:13 */
...
#define DEM_EVT_5G_DIV3_SHORT ((Dem_EventIdType)EventParameter_0x95A411) /*B15A4 FTB:11 */
#define DEM_EVT_5G_DIV3_OPEN ((Dem_EventIdType)EventParameter_0x95A413)  /*B15A4 FTB:13 */

#define ANT5G_SWITCH_TO_BACKUP() R_PORT_SetGpioOutput(Port9, 0, 0)  /* 切换到备天线 */
#define ANT5G_SWITCH_TO_MAIN() R_PORT_SetGpioOutput(Port9, 0, 1)    /* 切换到主天线 */
...
static const ANT_DetectConfig_t g_antDetectCfg[ANT5G_ANT_MAX] =
    {
        ...
        {DEM_EVT_5G_DIV2_SHORT, DEM_EVT_5G_DIV2_OPEN, AD0_CHANNEL_MIMO3_ANT_ADC, FALSE}, /* 第二分集配置 */
        {DEM_EVT_5G_DIV1_SHORT, DEM_EVT_5G_DIV1_OPEN, AD0_CHANNEL_DIV_ANT_ADC, FALSE},   /* 第一分集配置 */
        {DEM_EVT_5G_DIV3_SHORT, DEM_EVT_5G_DIV3_OPEN, AD0_CHANNEL_MIMO4_ANT_ADC, FALSE}  /* 第三分集配置 */
};
...
{
    return ((antVoltage >= ANT_DTC_OPEN_LOW_TH_MV) && (antVoltage <= ANT_DTC_OPEN_HIGH_TH_MV)) ? TRUE : FALSE; 
}
...
static boolean MICIN_PassedReportedThisCycle = FALSE;
...
static void MICIN_ReportPassedOncePerCycle(void)               
{
    if (MICIN_PassedReportedThisCycle == TRUE)                  
    {
        return;                                                 
    }
    ...
    if (MICIN_ShortGnd_Flag == TRUE)                            
    {
        ...
        MICIN_PassedReportedThisCycle = TRUE;                   
    }
    else if (MICIN_Open_Flag == TRUE)                           
    {
        Dem_SetEventStatus(DEM_EVT_MICIN_OPEN, DEM_EVENT_STATUS_PASSED); 
        MICIN_PassedReportedThisCycle = TRUE;                   
    }
    else if (MICIN_ShortBat_Flag == TRUE)                       
    {
...
void MICIN_DtcInit(void)
{
    ...
    MICIN_Open_Flag = FALSE;
    MICIN_ShortBat_Flag = FALSE;
    MICIN_PassedReportedThisCycle = FALSE;
}
...
void MICIN_DetectProcess_2s(void)
{
    ...
    if ((Dtc_IsCommonMonitorEnable())&&(ParameterSyncSdkGetFromCpuIsFinished()==0)) //满足监测使能条件以及mpu上电完成
    {
                 ...
                 (micInVoltage <= MICIN_DTC_NORMAL_HIGH_TH_MV)) 
        {
            ...
            MICIN_Open_Cnt = 0; 
            MICIN_ShortBat_Cnt = 0;
            MICIN_ReportPassedOncePerCycle(); 
        }
        else 
        {
            MICIN_ShortGnd_Cnt = 0; 
...
static boolean LED_PassedReportedThisCycle = FALSE;
...
static void LED_ReportPassedOncePerCycle(void) 
{
    if (LED_PassedReportedThisCycle == TRUE) 
    {
        return;
    }
    ...
    if (LED_ShortGnd_Flag == TRUE)
    {
        ...
        LED_PassedReportedThisCycle = TRUE;
    }
    else if (LED_ShortBat_Flag == TRUE) 
    {
...
static void LED_SyncMonitorStateWithDem(void) 
{
    ...
    if ((LED_ShortGnd_Flag == FALSE) && (LED_ShortBat_Flag == FALSE))
    {
        if (Dem_GetEventStatus(DEM_EVT_LED_SHORT_GND, &eventStatus) == E_OK)
        {
            if ((eventStatus & (DEM_UDS_STATUS_TF | DEM_UDS_STATUS_PDTC | DEM_UDS_STATUS_CDTC)) != 0)
            {
                ...
                LED_PassedReportedThisCycle = FALSE;
                return;
            }
        }
...
void LED_DetectProcess_200ms(void)
{
    ...
    if ((LED_ShortGnd_Cnt >= LED_DTC_CONFIRM_CNT) && (LED_ShortGnd_Flag == FALSE))
    {
        LED_ShortGnd_Flag = TRUE;
        LED_PassedReportedThisCycle = FALSE;
        Dem_SetEventStatus(DEM_EVT_LED_SHORT_GND, DEM_EVENT_STATUS_FAILED);
    }
    if ((LED_ShortBat_Cnt >= LED_DTC_CONFIRM_CNT) && (LED_ShortBat_Flag == FALSE))
    {
        LED_ShortBat_Flag = TRUE;
        LED_PassedReportedThisCycle = FALSE;
...
static boolean ECALL_PassedReportedThisCycle = FALSE;

static void ECALL_ReportPassedOncePerCycle(void)                 
{
    if (ECALL_PassedReportedThisCycle == TRUE)                   
    {
        return;                                                  
    }
    ...
    if (ECALL_Stuck_Flag == TRUE)                                
    {
        ...
        ECALL_PassedReportedThisCycle = TRUE;                    
    }
}
    ...
    if (ECALL_Stuck_Flag == FALSE)                               
    {
        if (Dem_GetEventStatus(DEM_EVT_ECALL_BUTTON_STUCK, &eventStatus) == E_OK)
        {
            if ((eventStatus & (DEM_UDS_STATUS_TF | DEM_UDS_STATUS_PDTC | DEM_UDS_STATUS_CDTC)) != 0)
            {
                ...
                ECALL_PassedReportedThisCycle = FALSE;            
                return;                                           
            }
        }
    }
...
void ECALL_DetectProcess_200ms(void)
{
    ...
    if (Dtc_IsCommonMonitorEnable() == TRUE)
    {
        ...
        else if (ecallButtonStatus == ECALL_SOS_BUTTON_NORMAL_LEVEL) /* 如果当前按键状态为未按下，说明按键处于正常状态判定 */
        {
            ...
            ECALL_Stuck_Cnt = 0;
            ECALL_ReportPassedOncePerCycle();
        }
        else
        {
            ECALL_Stuck_Cnt = 0;
