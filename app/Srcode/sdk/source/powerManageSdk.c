...
#define PM_KL30_UNDERVOLTAGE_MV         6000UL
#define PM_KL30_OVERVOLTAGE_WAKE_ADC_WAIT_MS 20UL
#define PM_VOLTAGE_CHECK_MS             (200UL)   /* 过欠压后等待NM状态切换的缓冲时间 */
/* RTC唤醒相关 */
#define PM_RTC_WAKEUP_DELAY_MS          (30000UL)   /* RTC唤醒后 30s 维持窗口 */
...
typedef struct
{
    ...
    Kl30Voltage_e kl30VoltageState;
    ...
    uint8_t deepstopSleepFlag; //deepstop休眠flag ：1 进入deepstop
}PmSdkManage_t;
...
static uint32_t g_mpuSleepAckCount = 0; // MPU休眠回复计数器
static uint8_t g_rtcAlarmResetFlag = 1; // RTC闹钟重置标志：上电=1(首次必设)，设闹钟后清0，条件满足时置1
...
static int16_t SaveUserModeParam(void);
...
{
    g_rtcAlarmResetFlag = 1;
    TimerHalDisableRtcAlarm(); // 此处主要作用为保存RTC闹钟刷新标志位到NVM 避免24H重启后RTC闹钟刷新标志位被清除
}

uint8_t GetListenTimerResetFlag(void)
{
    return g_rtcAlarmResetFlag;
}
...
int16_t PowerManageSdkInit(const PmSdkConfig_t* pmConfig)
{
    ...
    if(pmConfig!=NULL)
    {
        ...
        g_pmManage.kl30VoltageState = E_KL30_LOW_VOLTAGE;
        ...
        g_rtcAlarmResetFlag  = TimerHalGetRtcAlarmFlagInNvm()?TimerHalGetRtcAlarmResetFlagInNvm():1; // 判断是否已经设置过闹钟
...
static void PmStateWakeDelayProcess(uint32_t cycleTime)
{
    ...
    else if((NM_STATE_BUS_SLEEP == nmStatePtr) && (g_pmManage.wakeupSource != PM_HAL_WAKEUP_SOURCE_MCURTC))
    {
        PmNmGotoSleepMode();
        g_pmManage.wakeDelayCount = 0;
        g_pmManage.pmState = E_PM_STATE_PRE_SLEEP_NOTICE;
...
static uint8_t CheckVinIsInvalid(void)
{
    ...
    uint8_t vinFF[VIN_LEN] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
...
static uint8_t PmBackupModeProcess(uint32_t cycleTime)
{
    ...
    // if(BatterySdkGetBatteryEnableFlag() == 0)
    // {
    //     g_pmManage.backupModeDelayCount = 0;
    //     return 0;
    // }
    ...
    if(sleepDisableEvent == 0)
    {
        ...
        TBOX_PRINT("enter backup mode , wakeup hold 2 min\r\n");
        return 1;
    }
...
static void StopCommunication(void)
{
    ...
    Com_SetIpduGroup(ipduGroupVector, Com_RxPduGroup_CONTROLLER_0_IAM, FALSE);
...
static uint8_t PmKl30OverVoltageSleepProcess(void)
{
    ...
    Nm_ModeType nmModePtr = 0;
    uint8_t ret = 0;
    static uint16_t voltageCheckMs = 0U;

    if(PeripheralHalAdGet(AD0_CHANNEL_KL30, &kl30Voltage) == 0)
    {
        switch(g_pmManage.kl30VoltageState)
        {
            case E_KL30_NORMAL_VOLTAGE:
            {
                if(kl30Voltage >= PM_KL30_OVERVOLTAGE_MV)
                {   
                    voltageCheckMs++;
                    if(voltageCheckMs >= (PM_VOLTAGE_CHECK_MS/5)) //5ms周期task
                    {
                        voltageCheckMs = 0;
                        g_pmManage.kl30VoltageState = E_KL30_HIGH_VOLTAGE;
                        g_pmManage.pmState = E_PM_STATE_CHECK_NM_STATUS;
                        CanIf_SetPduMode(0 ,CANIF_OFFLINE);
                        TBOX_PRINT("KL30 over voltage: %u mV, close communication\r\n", kl30Voltage);
                        ret = 1;
                    }
                }
                else if(kl30Voltage <= PM_KL30_UNDERVOLTAGE_MV)
                {
                    voltageCheckMs++;
                    if(voltageCheckMs >= (PM_VOLTAGE_CHECK_MS/5)) //5ms周期task
                    {
                        ...
                        g_pmManage.kl30VoltageState = E_KL30_LOW_VOLTAGE;
                        CanIf_SetPduMode(0 ,CANIF_OFFLINE);
                        TBOX_PRINT("KL30 under voltage: %u mV, close communication\r\n", kl30Voltage);
                        ret = 1;
                    }
                }
                else
            ...
            case E_KL30_HIGH_VOLTAGE:
            {
                if(kl30Voltage < (PM_KL30_OVERVOLTAGE_MV - 500))
                {
                    voltageCheckMs++;
                    if(voltageCheckMs >= (PM_VOLTAGE_CHECK_MS/5)) //5ms周期task
                    {
                        voltageCheckMs = 0;
                        g_pmManage.kl30VoltageState = E_KL30_NORMAL_VOLTAGE;
                        CanIf_SetPduMode(0 ,CANIF_ONLINE);
                        TBOX_PRINT("KL30 normal voltage: %u mV, open communication\r\n", kl30Voltage);
                        ret = 0;
                        break;
                    }
                }
            ...
            case E_KL30_LOW_VOLTAGE:
            {
                if(kl30Voltage >= (PM_KL30_UNDERVOLTAGE_MV + 500))
                {
                    voltageCheckMs++;
                    if(voltageCheckMs >= (PM_VOLTAGE_CHECK_MS/5)) //5ms周期task
                    {
...
static void PmStatePreSleepNoticeProcess(uint32_t cycleTime)
{
    ...
    if(g_pmManage.testMode == 2)
    {
        ...
        // MpuHalPowerOff();
        /* 禁用RTC闹钟 避免诊断禁用listen模式后，deepstop被RTC唤醒 */
        TimerHalDisableRtcAlarm();
        /*通知MPU休眠*/
        MpuPowerSyncSdkSetSleep(1);
        g_pmManage.pmState = E_PM_STATE_PRE_SLEEP_WAIT;
        g_pmManage.deepstopSleepFlag = 1;
        APP_SetSleepMode(APP_SLEEP_MODE);
    ...
    else
    {
        ...
        g_pmManage.pmState = E_PM_STATE_PRE_SLEEP_WAIT;
        APP_SetSleepMode(APP_LISTEN_MODE);
        g_pmManage.deepstopSleepFlag = 0;
...
static void PmStatePreSleepWaitProcess(uint32_t cycleTime)
{
    ...
    /* MPU未回复休眠ACK处理 每秒重发一次休眠请求，最多重发两分钟后，MCU直接进休眠 */
    g_mpuSleepAckCount++;
    /* MPU1s无回复，重发休眠请求 */
    if((g_mpuSleepAckCount % 200) == 0)
    {
        MpuPowerSyncSdkSetSleep(g_pmManage.deepstopSleepFlag);
    }

    /* 2min后依旧无回复，MCU直接进休眠 */
    if(g_mpuSleepAckCount >= 24000)
    {
        g_mpuSleepAckCount = 0;
        g_pmManage.pmState = E_PM_STATE_MCU_SLEEP;
        TBOX_PRINT("MPU sleep ack timeout, MCU go sleep\r\n");
    }
    return;
}
...
static void PmStateMcuSleepProcess(uint32_t cycleTime)
{
    ...
    if(APP_GetSleepMode() == APP_LISTEN_MODE)
    {
        if(g_listenMode == E_PM_LISTEN_MODE_NORMAL && g_rtcAlarmResetFlag == 1)
        {
            if(TimerHalListenSleepSetAlarm(g_pmManage.listenWakeupTimer) == 0)
            {
                g_rtcAlarmResetFlag = 0;
            }
        }
        else
        {
    ...
    {
        PeripheralHalSetDeepsleep_PWR_EN_Low();
    }
    TimerHalSetMode(0);
    //EcallHalSetMode(0);
    LogHalSetMode(0);
...
static void PmStateCheckWakeupSourceProcess(uint32_t cycleTime)
{
    ...
    uint32_t sleepTime;
    static uint16_t wakeupFallbackCount = 0;
    ...
    {
        wakeupFallbackCount++;
        if(wakeupFallbackCount >= 200)
        {
            APP_ClearWakeupHold();
            APP_ReleaseNetWork();
            wakeupFallbackCount = 0;
            g_pmManage.pmState = E_PM_STATE_MCU_SLEEP;
        }
        return;
    }
    ...
    g_pmManage.restart24hTimer = (sleepTime >= g_pmManage.restart24hTimer)?0:(g_pmManage.restart24hTimer - sleepTime);
    /*MPU进入正常模式*/
    MpuHalSetMode(1);
    ...
    else 
    {
        ...
        /* RTC唤醒后30s内无其他维持源，关闭listen模式以进入DeepStop。从DeepStop唤醒时会初始化为E_PM_LISTEN_MODE_NORMAL */
        TimerHalClearRtcAlarmFlag(); // 清除闹钟触发标志
        TimerHalClearRtcAlarmFlagInNvm(); // 清除NvM当前闹钟状态
        g_listenMode = E_PM_LISTEN_MODE_CLOSE;// RTC唤醒后再次休眠进入DeepStop模式
        g_pmManage.pmState =E_PM_STATE_WAKE_DELAY;
        g_pmManage.wakeDelayTime = PM_RTC_WAKEUP_DELAY_MS;   //RTC唤醒后 30s 维持窗口
        // StartCommunication();
    }
...
void PowerManageSdkCycleProcess(uint32_t cycleTime)
{
    ...
    if((E_PM_STATE_UNPOWED != g_pmManage.pmState) &&
       ...
       (E_PM_STATE_BACKUP_MODE != g_pmManage.pmState))
    {
        if(PmKl30OverVoltageSleepProcess() != 0)
        {
            return;
...
/*************************************************
  ...
  Others:         
*************************************************/
int16_t PowerManageSdkIsrSetWakeUpSource(uint8_t wakeupSource)
{
    if((wakeupSource >= PM_HAL_WAKEUP_SOURCE_MAX) || (wakeupSource == PM_HAL_WAKEUP_SOURCE_NONE))
    {
        return -1;
    }
    g_pmManage.wakeupSource = wakeupSource;
...
/*************************************************
  ...
  Others:         
*************************************************/
int16_t PowerManageSdkClearWakeUpSource(void)
{
    g_pmManage.wakeupSource = PM_HAL_WAKEUP_SOURCE_NONE;
    return 0;
}
