...
typedef struct
{
  ...
  volatile uint32 * pCTR;
   
  volatile uint32 * pID;
   
  volatile uint32 * pPTR;
   
  volatile uint32 * pDF;
   
  volatile uint8 * pTrig;
...
static  Can_ReturnType Can_GetTxRegPointers(
  Can_TxRegSetType * const LpRegSet,
  const Can_HohConfigType * const LpHoh);

static  void Can_WriteDataReg(
  const uint8 * const LpSrc,
  volatile uint32 * const LpDF,
 ...
 
{
  ...
  {
    (void)Det_ReportError(80U, (uint8)0x00, (uint8)0x06,
      (uint8)0x05);
  ...
  else if ((((void *)0) == PduInfo) || ((((void *)0) == PduInfo->sdu)
    ))
  {
    (void)Det_ReportError(80U, (uint8)0x00, (uint8)0x06,
      (uint8)0x01);
    ...
    (8U < PduInfo->length) &&
      (
      (0UL == (PduInfo->id & (Can_IdType)0x40000000UL)) ||
        ((boolean)0x00 ==
          Can_GpPBController[
  ...
  else
  {
    ...
    if ((boolean)0x01 != Can_GaaHwAccessFlag[LpHoh->usHohId])
    {
       
      Can_GaaHwAccessFlag[LpHoh->usHohId] = (boolean)0x01;
       
      LblMutexAcquired = (boolean)0x01;
    }
    else
    {
    ...
    if ((boolean)0x01 == LblMutexAcquired)
    {
      ...
      if (CAN_OK == LenReturnValue)
      {
        ...
        if (0UL != (PduInfo->id & (Can_IdType)0x80000000UL))
        {
          { { (*(LstTxRegSet . pID)) = (((uint32)PduInfo->id & (Can_IdType)0x1FFFFFFFUL) | 0x20000000UL | 0x80000000UL);} }
           
        }
        else
        {
          { { (*(LstTxRegSet . pID)) = (((uint32)PduInfo->id & (Can_IdType)0x1FFFFFFFUL) | 0x20000000UL);} }
           
        }

        if (0UL != (PduInfo->id & (Can_IdType)0x40000000UL))
        {
           
          if ((boolean)0x01 == Can_GpPBController[LucController].pBaudrateConfig
            [Can_GaaCtrlState[LucController].ulBaudrateIndex].blBRS)
           
          {
            { { (*(LstTxRegSet . pCTR)) = (0x00000004UL | 0x00000002UL);} }
             
          }
          else
          {
            { { (*(LstTxRegSet . pCTR)) = (0x00000004UL);} }
        ...
        { { (*(LstTxRegSet . pPTR)) = ((uint32)((uint32)(LulDLC) << 28UL) | (uint32)((uint32)(PduInfo->swPduHandle) << 16UL));} }
      ...
      Can_GaaHwAccessFlag[LpHoh->usHohId] = (boolean)0x00;
  ...
  const Can_HohConfigType * const LpHoh)
{
  ...
  if (CAN_BUFFERTYPE_TXRXFIFO == LpHoh->enBufferType)
  {
    ...
    if (0UL !=
      (Can_GaaRegs[LucUnit].pCmn->aaCFSTS[LulBufIdx] & 0x00000002UL))
    {
    ...
    else
    {
      ...
      { (*(&LpRegSet->ucTrigValue)) = ((uint8)0x000000FFUL);}
  ...
  const uint32 LulSrcLength, const uint32 LulDestLength, const uint8 LucPadding)
{
     ...
     LulWordIndex < LulAvailableWords; LulWordIndex++)
  {
    ...
    LulDataRegValue = ( (uint32)(LpSrc[LulByteIndex]) | ((uint32)(LpSrc[LulByteIndex + 1UL]) << 8UL) | ((uint32)(LpSrc[LulByteIndex + 2UL]) << 16UL) | ((uint32)(LpSrc[LulByteIndex + 3UL]) << 24UL));
  ...
  case 1UL:  
    ...
    LulDataRegValue = ( (uint32)(LpSrc[LulByteIndex]) | ((uint32)(LucPadding) << 8UL) | ((uint32)(LucPadding) << 16UL) | ((uint32)(LucPadding) << 24UL));
    { { (*(&LpDF[LulWordIndex])) = (LulDataRegValue);} }
  ...
  case 2UL:  
    ...
    LulDataRegValue = ( (uint32)(LpSrc[LulByteIndex]) | ((uint32)(LpSrc[LulByteIndex + 1UL]) << 8UL) | ((uint32)(LucPadding) << 16UL) | ((uint32)(LucPadding) << 24UL));
    { { (*(&LpDF[LulWordIndex])) = (LulDataRegValue);} }
  ...
  default:  
    ...
    LulDataRegValue = ( (uint32)(LpSrc[LulByteIndex]) | ((uint32)(LpSrc[LulByteIndex + 1UL]) << 8UL) | ((uint32)(LpSrc[LulByteIndex + 2UL]) << 16UL) | ((uint32)(LucPadding) << 24UL));
    { { (*(&LpDF[LulWordIndex])) = (LulDataRegValue);} }
  ...
  LulDataRegValue = ( (uint32)(LucPadding) | ((uint32)(LucPadding) << 8UL) | ((uint32)(LucPadding) << 16UL) | ((uint32)(LucPadding) << 24UL));
