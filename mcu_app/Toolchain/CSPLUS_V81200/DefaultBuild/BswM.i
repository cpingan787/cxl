...
static  void BswM_InitEventRequestPort(void);

 
static  void BswM_InitModeRequestPort(void);

 
static  void BswM_InitRules(void);

 
static  void BswM_DoActionList(ApplicationType partIdx, BswM_ActionListIndexType actionListIdx);
...
volatile BswM_RuntimeStatusType BswM_RuntimeStatus;
...
BswM_RequestMode(BswM_UserType requestingUser, uint16 requestedMode)
{
    ...
    if ((Std_ReturnType)0U == BswM_DetChkGenericRqst(requestingUser))
    {
        result = BswM_GetPartitionIdx(&partIdx);
        if ((boolean)1u == result)    
        {
...
BswM_InitGenericRequestPorts(void)
{
    ...
    if ((boolean)1u == result)    
    {
        ...
        for (userIdx = 0u; userIdx < numOfGenericReq; userIdx++)
        {
            if (((void *)0) != modeRqstPCfgPtr->genRqstInitMode)
            {
...
BswM_GetGeneRqstStatus(BswM_UserType userIdx)
{
    uint16 retStatus = (uint16)(0xffffu);
    ApplicationType partIdx;
    boolean result;
 ...
 boolean BswM_GetPartitionIdx(ApplicationType * partitionIdx)
{
    boolean foundResult = 1u;
...
{
    const BswM_RuleLcCfgType * ruleLCfgPtr;
    const BswM_ActionListLCfgType * actionListPtr;
    ...
    if ((BSWM_TRUE == ruleRet) && (((void *)0) != ruleLCfgPtr->trueActList))
    {
        actListIdx = *(ruleLCfgPtr->trueActList);
    }
    else if ((BSWM_FALSE == ruleRet) && (((void *)0) != ruleLCfgPtr->falseActList))
    {
...
{
    const BswM_RuleLcCfgType * ruleLCfgPtr;
    BswM_RuleRunTimeType * ruleStatusPtr;
    ApplicationType partIdx;
    BswM_RuleIndexType numOfRules;
    BswM_RuleIndexType ruleIdx;
    ...
    if ((boolean)1u == result)    
    {
        ...
        {
            ruleStatusPtr[ruleIdx].ruleNeedArbitrate = 0u;
        ...
        for (ruleIdx = 0u; ruleIdx < numOfRules; ruleIdx++)
        {
            ...
            ruleStatusPtr[defRuleIdx].ruleNeedArbitrate = 1u;
        }
    }
}
...
{
    const BswM_ActionListLCfgType * actionListPtr;
    const BswM_ActionListItemType * actionItemPtr;
    ...
    for (itemIdx = 0u; itemIdx < numOfActionItem; itemIdx++)
    {
        ...
        case BSWM_ACTIONITEM_ACTION:
            ...
            if ((Std_ReturnType)1U == itemResult)
            {
                if (((void *)0) != actionItemPtr->runtimeErrId)
                {
                ...
                if ((boolean)1u == actionItemPtr->abortOnFail)
                {
