...
static MpuTime_t g_gnssTimeData;
static MpuTime_t g_ntpTimeData;
    ...
    else
    {
        if(msgData != NULL && msgData->pDataBuffer != NULL  &&  msgData->dataLength >0)
        {
            ...
            **************/
            ...
            {
                g_gnssTimeData.timeSyncStat = (pRxData[5] >> 4) & 0x0F; // TODO guanyuan
                g_gnssTimeData.timeSrc = pRxData[5] & 0x0F;
                g_gnssTimeData.timestamp = (pRxData[0] << 24) + (pRxData[1] << 16) + (pRxData[2] << 8) + pRxData[3];
                g_gnssTimeData.timeValidity = 1;
            }
            else if(((pRxData[5] & 0xF0) == 0x00))
            {
                g_ntpTimeData.timeSyncStat = (pRxData[5] >> 4) & 0x0F;
                g_ntpTimeData.timeSrc = pRxData[5] & 0x0F;
                g_ntpTimeData.timestamp = (pRxData[0] << 24) + (pRxData[1] << 16) + (pRxData[2] << 8) + pRxData[3];
                g_ntpTimeData.timeValidity = 1;
            }

            if(g_gnssTimeData.timeValidity == 1)
            {
                ...
                memcpy(&g_timeData, &g_gnssTimeData, sizeof(MpuTime_t));
                // TimerHalSetRtcTime(g_timeData.timestamp);    // TODO guanyuan
            }
            else if(g_ntpTimeData.timeValidity == 1)
            {
                // g_timeData.timeSyncStat = (pRxData[5]>>4) & 0x0F;
                // g_timeData.timeSrc = pRxData[5] & 0x0F;
                // g_timeData.timestamp = (pRxData[0]<<24)+(pRxData[1]<<16)+(pRxData[2]<<8) + pRxData[3];
                // g_timeData.timeValidity = 1;
                memcpy(&g_timeData, &g_ntpTimeData, sizeof(MpuTime_t));
                // TimerHalSetRtcTime(g_timeData.timestamp);    // TODO guanyuan
            }
            else
            {
...
int16_t TimeSyncSdkGetRealTime(uint8_t timeZone,uint32_t *pYear,uint8_t *pMonth,uint8_t *pDay,uint8_t *pHour,uint8_t *pMin,uint8_t *pSecond)
{
    ...
    // TimerHalGetRtcTime(&time);    // TODO guanyuan
