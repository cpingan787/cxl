...
static uint8 Dcm_0x22DidReadNvmFlag = 0xFF; /* PRQA S 3218 */ /* VL_Dcm_3218 */
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) pNrc)
{
        ...
        if (E_OK == ret)
        {
            ...
            else
            {
                if ((dataoffset + *ResOffset - pDcmChannelCfg->offset) > pDcmChannelCfg->Dcm_DslBufferSize)
                {
                    ...
                    /**< Read operation not executed, but the datalength continues to be calculated*/
                    ret = E_OK;
#else
...
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
    if (((*ResOffset + dataoffset - pDcmChannelCfg->offset) > pDcmChannelCfg->Dcm_DslBufferSize))
    {
        /* check single did datalength */
        if (((dataoffset + 2u) <= pDcmChannelCfg->Dcm_DslBufferSize) && (E_OK == ret))
        {
            if (Dcm_PageBufferData.BufferOverFlowed == FALSE)
            {
                Dcm_PageBufferData.BufferOverFlowed = TRUE;
                if (Dcm_PageBufferData.PagedBufferStarted == FALSE)
                {
                    Dcm_PageBufferData.FilledSize = *ResOffset - 2u - pDcmChannelCfg->offset;
                    /** init response offset */
                    Dcm_PageBufferData.ResOffset = pDcmChannelCfg->offset;
                }
                else
                {
                    Dcm_PageBufferData.NextFilledSize = *ResOffset - 2u - pDcmChannelCfg->offset;
                }
            }
        }
        else
    ...
    if (ret == DCM_PENDING)
    {
        Dcm_PageBufferData.ResOffset = *ResOffset;
    }
#endif
...
#if (DCM_DSP_PID_FUNC_ENABLED == STD_ON)
static FUNC(Std_ReturnType, DCM_CODE)
    DspInternalUDS0x22_ObdDidDealF4(P2CONST(Dcm_0x22Types, AUTOMATIC, DCM_VAR) Dcm_0x22Type)
{
    Std_ReturnType ret = E_OK;
    uint32 SupportBuffer = 0;
    const Dcm_DspCfgType* pDcmDspCfg = DcmPbCfgPtr->pDcmDspCfg;
...
/*************************************************************************/
static FUNC(Std_ReturnType, DCM_CODE)
    DspInternalUDS0x22_ObdDidDealF8(P2CONST(Dcm_0x22Types, AUTOMATIC, DCM_VAR) Dcm_0x22Type)
{
    Std_ReturnType ret = E_OK;
    uint32 SupportBuffer = 0;
    const Dcm_DspCfgType* pDcmDspCfg = DcmPbCfgPtr->pDcmDspCfg;
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode)
{
    ...
    {
        ret = DspInternalUDS0x22_ObdDidDealF4(Dcm_0x22Type);
    }
    else if ((Dcm_0x22Type->RecDid >= 0xF800u) && (Dcm_0x22Type->RecDid <= 0xF8FFu))
    {
        ret = DspInternalUDS0x22_ObdDidDealF8(Dcm_0x22Type);
    }
...
/*************************************************************************/
static FUNC(Std_ReturnType, DCM_CODE)
    Dcm_UDS0x22_StartPagedProcessing(uint8 ProtocolCtrlId, uint32 ResOffset, Dcm_NegativeResponseCodeType* ErrorCode)
{
    uint8 MsgCtrlId = Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex;
    ...
    {
        /** start paged-transmit,release Dcm_Channel for next page*/
        Dcm_MemoryCopy(&(Dcm_Channel[Offset]), Dcm_PageBufferData.PagedBuffer, Dcm_PageBufferData.FilledSize);
        ...
        Dcm_PageBufferData.IsRecord = FALSE;
    }
    return ret;
}
#endif
    ...
    *ErrorCode = DCM_E_REQUESTOUTOFRANGE;
    ...
    else
    {
        ...
        if (TRUE == Dcm_PageBufferData.PagedBufferStarted)
        {
            ...
            ReqOffset = (uint32)((uint32)Index0 << 1uL) + 1uL;
            ResOffset = Dcm_PageBufferData.ResOffset; // buffer offset position when pending
        }
        else
#endif
        {
...
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
                if ((Dcm_PageBufferData.BufferOverFlowed == TRUE) && (Dcm_PageBufferData.IsRecord == FALSE))
                {
                    /**< record the current loop index */
                    Dcm_PageBufferData.IloopOne = Index0;
                    Dcm_PageBufferData.IsRecord = TRUE;
                }
#endif
            }
        ...
        {
#if (TRUE == DCM_PAGEDBUFFER_ENABLED)
            if ((Dcm_PageBufferData.PagedBufferStarted == FALSE) && (Dcm_PageBufferData.BufferOverFlowed == TRUE))
            {
                /** active pagedBuffer and the first page is filled */
                ret = Dcm_UDS0x22_StartPagedProcessing(ProtocolCtrlId, ResOffset, ErrorCode);
            }
            else if ((Dcm_PageBufferData.PagedBufferStarted == TRUE) && (Dcm_PageBufferData.BufferOverFlowed == TRUE))
            {
                /**<  the next page is filled */
                Dcm_PageBufferData.IsNextFilled = TRUE;
                Dcm_PageBufferData.IsBufferOccupied = TRUE;
                Dcm_PageBufferData.IsRecord = FALSE;
            }
            else if ((Dcm_PageBufferData.PagedBufferStarted == TRUE) && (Index0 == DidNum))
            {
                /**<  the last page is filled */
                Dcm_PageBufferData.IsNextFilled = TRUE;
                Dcm_PageBufferData.NextFilledSize = ResOffset - Offset;
                Dcm_PageBufferData.IsBufferOccupied = TRUE;
                Dcm_PageBufferData.IsRecord = FALSE;
