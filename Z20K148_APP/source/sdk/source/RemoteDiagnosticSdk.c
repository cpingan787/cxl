...
static int16_t g_remoteDiagnosticTimerHandle = -1;
static uint8_t g_remoteDiagnosticOnlineStatus = 0; // 0=离线, 1=在线
...
static uint8_t RemoteDiagnosticSdkTpTransmit(CanIdConfig_t *pEcuConfigure, MpuHalDataPack_t *rxMsg, int16_t virtualTpHandle)
{
    ...
    if (pUdsData[0] == 0x3E && pUdsData[1] == 0x80)
    {
        for (i = 0; i < canChannelList->canChanelListSize; i++)
        {
            UdsTpTransmit(g_udsTpHandle[canChannelList->canChanelList[i]], 1, pUdsData, udsDataLen); // 1表示7df广播
        }
    ...
    {
        // LogHalUpLoadLog("[MCU] Functional ID: 0x%X\r\n", canId);
        if (responseId != 0)
        {
        ...
        for (i = 0; i < canChannelList->canChanelListSize; i++) // 只有1路can
        {
            UdsTpTransmit(g_udsTpHandle[canChannelList->canChanelList[i]], 1, pUdsData, udsDataLen); // 1表示7df广播
        }
        VirtualTpSdkClientTransmit(virtualTpHandle, pUdsData, udsDataLen);
...
void RemoteDiagnosticSdkProcess(CanIdConfig_t *pEcuConfigure, MpuBuffer_t *pMpuBuffer, UdsTpParameter_t *pTpParameter, CanChanel_t *canChannelList)
{
    ...
    g_remoteDiagnosticTimerHandle = TimerHalOpen();
    ...
    while (1)
    {
        ...
        if (g_udsFlag == 1)
        {
            ...
            {
                uint8_t currentTargetEcu = s_pendingEcuId;

                if (currentTargetEcu != 0xFF)
                {
                    udsRecvLen = 0;
                    uint32_t currentReqId = pEcuConfigure->pEcuList[currentTargetEcu].requestId;
                    if (currentReqId == 0x067 || currentReqId == 0x069)
                    {
                        ret = UdsTpReceiveRaw(g_udsTpHandle[pEcuConfigure->pEcuList[currentTargetEcu].channel], udsRxbuf, &udsRecvLen);
                    }
                    else
                    {
            ...
            if (g_udsTimeCount > 20000)
            {
                LogHalUpLoadLog("[cxl6");
                if (g_udsReceiveFlag == 1)
                {
...
{
    return g_remoteDiagnosticOnlineStatus;
}
