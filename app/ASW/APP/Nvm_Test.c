...
// Global array: Stores the RAM starting address of each Block
uint8* NvmBlockRamAddrs[NVM_BLOCK_NUM_ALL];
// Global array: Stores the length of each Block
...
void TC001_NvM_WriteAndReadAll(void)
{
    ...
    //  Fill the RAM data of all blocks
    FillAllBlocksWithIncrementPattern(All_TC01);
        ...
        // Read the RAM data of all blocks and verify it
        init_expired_time = 0;
        InitNvMWriteAllStatus = NVM_REQ_PENDING;
        NvM_ReadAll();
        do
...
void TC002_WriteAndReadBlock_Multi(void)
{
    ...
    for (blkIdx = 0; blkIdx < numBlocks; blkIdx++)
    {
        ...
        /* Set all RAM data to 0xE1 */
        for (i = 0; i < length; i++)
        {
            ramAddr[i] = 0xE1;
        }

        /* Call NvM_WriteBlock for writing */
    ...
    for (blkIdx = 0; blkIdx < numBlocks; blkIdx++)
    {
      ...
      for (blkIdx = 0; blkIdx < numBlocks; blkIdx++)
      {
        ...
        uint8* ramAddr = blocks[blkIdx].addr;
          /* Call NvM_ReadBlock to read the recovery.*/
...
void TC003_NvM_WriteAndReadPRAMBlock(void)
{
    ...
    for (blkIdx = 0; blkIdx < numBlocks; blkIdx++)
    {
        ...
        /* Set all the RAM data to 0xE2 */
        for (i = 0; i < length; i++)
        {
            ramAddr[i] = 0xE2;
        }

        /* Call NvM_WritePRAMBlock for writing */
    ...
    for (blkIdx = 0; blkIdx < numBlocks; blkIdx++)
    {
        ...
        /* Call NvM_ReadPRAMBlock to read and restore */
    ...
    //  Fill the RAM data of all Blocks
     FillAllBlocksWithIncrementPattern(All_TC04_E3);
       ...
       FillAllBlocksWithIncrementPattern(All_TC04_E4);
         ...
         /* Set write protection for all Blocks*/
...
void TC006_NvM_CancelWriteAll(void)
{
    ...
    //  Fill RAM data for all Blocks
     FillAllBlocksWithIncrementPattern(All_TC06_E6);
        ...
        // Immediately cancel the current batch write request
        NvM_CancelWriteAll();
...
void TC007_NvM_GetErrorStatus_OK(void)
{
    ...
    /* Asynchronous polling to obtain status */
    init_expired_time = 0;
    do {
        Wdg_59_DriverB_TriggerFunc(WDG_59_DRIVERB_INCLUDE_CRITICAL_SECTION);
        init_expired_time++;
    ...
    /* If the status is NVM_REQ_OK, set NVRAM Block2 (ID=2) to 0xAA */
    if (InitNvMWriteAllStatus == NVM_REQ_OK)
    {
