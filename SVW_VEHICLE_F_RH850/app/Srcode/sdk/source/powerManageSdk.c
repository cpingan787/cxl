...
#define PM_KL30_UNDERVOLTAGE_MV         5500UL
...
#define PM_KL30_OVERVOLTAGE_NM_CHECK_MS 2000UL   /* 过压后等待NM状态切换的缓冲时间 */
...
typedef struct
{
    ...
    Kl30Voltage_e kl30OVoltageFlag;
        ...
        switch(g_pmManage.kl30OVoltageFlag)
        {
            case E_KL30_NORMAL_VOLTAGE:
            {
                ...
                {   /* 过压: 若总线上有网管活动则不休眠，否则进入过压休眠 */
                    ...
                    else if(nmLostCheckMs < PM_KL30_OVERVOLTAGE_NM_CHECK_MS)
                    {
                        /* 网管未活动，2S内未检测到网管活动，进入过压休眠 */
                        nmLostCheckMs += 5u;//当前电源管理周期为5ms
                        ret = 0;
                    }
                    else
                    {
                        nmLostCheckMs = 0U;
                        g_pmManage.kl30OVoltageFlag = E_KL30_HIGH_VOLTAGE;
                ...
                {   /* 欠压 */
                    g_pmManage.kl30OVoltageFlag = E_KL30_LOW_VOLTAGE;
                    TBOX_PRINT("KL30 under voltage: %u mV, force bus sleep\r\n", kl30Voltage);
                    StopCommunication();
                    ret = 1;
                }
                else
            ...
            case E_KL30_HIGH_VOLTAGE:
            {
                ...
                {
                    StartCommunication();
                    g_pmManage.kl30OVoltageFlag = E_KL30_NORMAL_VOLTAGE;
                    ret = 0;
                    break;
                }
            ...
            case E_KL30_LOW_VOLTAGE:
            {
                ...
                {   /* 欠压恢复 */
                    g_pmManage.kl30OVoltageFlag = E_KL30_NORMAL_VOLTAGE;
                    TBOX_PRINT("KL30 under voltage: %u mV, force bus wake\r\n", kl30Voltage);
                    StartCommunication();
                    ret = 0;
                    break;
                }
...
static void PmStatePreSleepWaitProcess(uint32_t cycleTime)
{
    ...
    if(mpuSleepStatus==0)
    {
        ...
        if(g_pmManage.deepstopSleepFlag == 1)
        {
            g_pmManage.pmState = E_PM_STATE_MCU_SLEEP;
            return;
        }
...
static void PmStatePreCheckCanProcess(uint32_t cycleTime)
{
    ...
    CanNm_GetState(0, &nmStatePtr, &nmModePtr);
    if ((nmStatePtr == NM_STATE_BUS_SLEEP) && (validNmMsgFlag == TRUE))
    {
        /* 如果接收到NM报文，但CanNm依旧在睡眠状态，则主动发起网络请求 */
