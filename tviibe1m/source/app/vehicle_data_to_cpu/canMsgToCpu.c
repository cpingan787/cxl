...
#define TX_BUFFER_CAN_MSG_NUM_MAX               150
...
int16_t CanMsgTransmitToCpu(uint8_t canChannel,int16_t mpuHandle)
{
    ...
    g_txBuffer[0] = g_txBufferCount>>8;//canChannel;//channel start from 1,0 is reserved
    g_txBuffer[1] = g_txBufferCount;//canChannel;//channel start from 1,0 is reserved
