...
static uint8_t flashAppFlag = 0x00;
...
static uint8_t FirmwareUpdateSdkEraseFlash(volatile uint8_t *dataPack)
{
    ...
    if (dataPack[2] == 0x01 && dataPack[3] == 0x02 && dataPack[4] == 0x03 && dataPack[5] == 0x04)
    {
        ...
        LogHalPrint("boot erase start address is 0x%x\r\n",address);
        
        if (address == 0x00080200)
        {
            flashAppFlag = 0x01;
        }
        else
        {
            flashAppFlag = 0x00;
            LogHalPrint("05 address error\r\n");
            return 1;
        }
        address = 0x00080000;
        uint32_t eraseLength = 0x00110000;
        ...
        if (retValue == 0U)
        {
            LogHalPrint("Erase Bank %d success\r\n", flashAppFlag);
...
static uint8_t FirmwareUpdateSdkLoadCode(volatile uint8_t *dataPack, uint16_t dataBufferSize)
{
    ...
    if(dataPack != ((void *) 0)) 
    {
        if(g_flashState == E_FlashState_FlashErase)
        {
            ...
            LogHalPrint("--------------------06 addr=%08x len=%d cnt=%02x %02x------------------\r\n", addr, len, dataPack[0], dataPack[1]);
                      
            
            if((len <= 0 || len > (1024*1024)) || 
               (addr < 0x00080200) || 
               (addr > 0x00190000U) ||  
               ((addr + len) > 0x00190000U)) 
            {
                if(len <= 0 || len > (1024*1024))
                {
                    LogHalPrint("06 invalid data length: %d\r\n", len);
                }
                else if((addr < 0x00080200) || (addr > 0x00190000U))
                {
                    LogHalPrint("06 address out of range: %08x\r\n", addr);
                }
                else if((addr + len) > 0x00190000U)
                {
                    LogHalPrint("FLASH_APP_BANKA_END_ADDRESS = %08x\r\n", 0x00190000U);
                    LogHalPrint("0666 address+length exceeds A Bank boundary: %08x\r\n", (addr + len));
                }
                ret = 0x0A; 
            }
            else
            {
                ...
                {
                    LogHalPrint("06 Fls_Write request rejected\r\n");
                    ret = 0x03; 
                }
            }
        }
        ...
        {
            LogHalPrint("--------------------06 Invalid flash state: %d (expected: %d)------------------\r\n", g_flashState, E_FlashState_FlashErase);
            ret = 0x06; 
        }
    }
    else
    {
        LogHalPrint("FirmwareUpdateSdkLoadCode: dataPack is NULL\r\n");
    }
    return ret;
}
...
static uint8_t FirmwareUpdateSdkCodeCheck(volatile uint8_t *dataPack)
{
    ...
    LogHalPrint("--------------------07 Download Complete size=%d------------------\r\n", g_otaCurSize);
    ...
    {
        if (0x01 == flashAppFlag)  
        {
            uint32_t WriteData = 0xFE;
            uint32_t retValue = FlsIf_Write(0x00080000, 4, (uint8*)&WriteData);
            retValue = FlsIf_Read(0x00080000, 4, (uint8*)&dataRead);
            if(dataRead != WriteData)
            {
                LogHalPrint("FlsIf_Read failed %08x\r\n", dataRead);
                ret = 0x01; 
            }
            if (retValue != 0U)
            {
                LogHalPrint("APP ACTIVE failed %02x\r\n", retValue);
                ret = 0x02;
            }
            else 
            {
                LogHalPrint("virify success\r\n");
                ...
                if (dataRead != WriteData)
                {
                    LogHalPrint("EEIf_Read failed %08x\r\n", dataRead);
                    ret = 0x01; 
                }
                ret = 0x00;
            }
    ...
    {
        LogHalPrint("CRC check failed %08x != %08x\r\n", checksum, g_crcData);
        ret = 0x01; 
    }
    return ret;
}
...
void FirmwareUpdateSdkCycleProcess(int16_t handle, MpuHalDataPack_t *pRxMsg)
{
    ...
    if ((pRxMsg == ((void *) 0)) || (pRxMsg->pDataBuffer == ((void *) 0)) || (pRxMsg->dataBufferSize < (8U))) 
    {
        return;
    }
    ...
    if(curAid == (0x03U))
    {
        ...
        switch(s_UpdateMid)
        {
            ...
            case E_FirmwareUpdateSdkCmd_EnterDownloadMode:
                LogHalPrint("---04 Enter Download Mode---\r\n");
                if ((pRxMsg->pDataBuffer[2] == 0x01) && (pRxMsg->pDataBuffer[3] == 0x02) && (pRxMsg->pDataBuffer[4] == 0x03) && (pRxMsg->pDataBuffer[5] == 0x04))
                {
            ...
            case E_FirmwareUpdateSdkCmd_SoftwareResetMcu:
                LogHalPrint("---08 Reset---\r\n");
                dataTxAraay[2] = 0x00; 
                dataTxLen = 3U;
                break;
            case E_FirmwareUpdateSdkCmd_GetMcuBankId:
                LogHalPrint("---09 Get Bank Id---\r\n");
                dataTxAraay[2] = 0x00; 
...
void FirmwareUpdate_UnlockMcuFlashAck(void)
{
    ...
    if (otaFlag == 1U)
    {
        LogHalPrint("Mpu OTA To Bootloader\r\n");
    }
}
