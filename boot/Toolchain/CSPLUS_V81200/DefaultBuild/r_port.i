...
void R_PORT_SetOpenDrain(enum port_t Port, uint32_t Pin)
{
  ...
  loc_podcValue |= 1<<Pin;
  do{ (*PortList[Port]. PPCMD_Reg)=0xa5u; (*PortList[Port]. PODC_Reg)=(loc_podcValue); (*PortList[Port]. PODC_Reg)=~(loc_podcValue); (*PortList[Port]. PODC_Reg)=(loc_podcValue); }while((*PortList[Port]. PPROTS_Reg)==1u);
}  
