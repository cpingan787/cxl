...
int16_t CanPeriodMessage3F8(uint8_t *pData)
{
    McuSelfcheckMsg_t* res = NULL;
    MpuSelfcheckMsg_t* res1 = NULL;

    res = AlarmSdkGetMcuSelfcheckResult();
    pData[0] = res->self_test << 4;
    pData[0] |= res->kl30_fault;
    pData[1] = res->mic_fault;
    pData[2] = res->sos_fault;
    pData[3] = res->can_fault;
    pData[4] = res->abm_fault;
    pData[5] = res->speaker_fault;
    pData[6] = res->battery_fault;
    pData[7] = res->nad_fault;
    
    res1 = AlarmSdkGetMpuSelfcheckResult();
    pData[8] = res1->gsmMainAnt_fault;
    pData[9] = res1->gsmSecAnt_fault;
    pData[10] = res1->gpsAnt_fault;
    pData[11] = res1->simCard_fault;
    pData[12] = 0;
    pData[13] = 0;
    pData[14] = 0;
    pData[15] = 0;
...
#if(SELFCHECK_RESULT_SEND == 1)
int16_t CanPeriodMessage400(uint8_t *pData)
{
    pData[0] = AlarmSdkGetEcallTriggerType();
    pData[1] = EcallHalGetAmpFaultStatus();;
    pData[2] = EcallHalGetAmpDiagnosticStatus();
    pData[3] = EcallHalGetAmpControlStatus();;
    pData[4] = 0x00;
    pData[5] = 0x00;
    pData[6] = 0x00;
    pData[7] = 0x00;
...
/****************************time, id ,fdFlag   length  canfunction canll back******/
  CAN_CYCLE_SEND_CONFIGURE_CAN(1000, 0x53D, 0, 8,      CanPeriodMessage53D)  
  // CAN_CYCLE_SEND_CONFIGURE_CAN(1000, 0x3E9, 1, 16,      CanPeriodMessage3E9)  
  // CAN_CYCLE_SEND_CONFIGURE_CAN(100,  0x2F0, 0, 8,      CanPeriodMessage2F0)
#if(SELFCHECK_RESULT_SEND == 1)
  CAN_CYCLE_SEND_CONFIGURE_CAN(100,  0x3F7, 1, 16,      CanPeriodMessage3F8)
#endif
CAN_CYCLE_SEND_CONFIGURE_END(1)

CAN_CYCLE_SEND_CONFIGURE_BEGIN(2)
/****************************time, id ,fdFlag   length  canfunction canll back******/
  CAN_CYCLE_SEND_CONFIGURE_CAN(1000, 0x53E, 0, 8,      CanPeriodMessage53D)  
  // CAN_CYCLE_SEND_CONFIGURE_CAN(1000, 0x3E9, 1, 16,      CanPeriodMessage3E9)  
  // CAN_CYCLE_SEND_CONFIGURE_CAN(100,  0x2F0, 0, 8,      CanPeriodMessage2F0)
#if(SELFCHECK_RESULT_SEND == 1)
  CAN_CYCLE_SEND_CONFIGURE_CAN(1000,  0x3F8, 1, 16,      CanPeriodMessage3F8)
  //CAN_CYCLE_SEND_CONFIGURE_CAN(1500,  0x400, 0, 8,      CanPeriodMessage400)
#endif
CAN_CYCLE_SEND_CONFIGURE_END(2)

CAN_CYCLE_SEND_CONFIGURE_BEGIN(3)
/****************************time, id ,fdFlag   length  canfunction canll back******/
  CAN_CYCLE_SEND_CONFIGURE_CAN(1000, 0x53F, 0, 8,      CanPeriodMessage53D)  
  // CAN_CYCLE_SEND_CONFIGURE_CAN(1000, 0x3E9, 1, 16,      CanPeriodMessage3E9)  
  // CAN_CYCLE_SEND_CONFIGURE_CAN(100,  0x2F0, 0, 8,      CanPeriodMessage2F0)
#if(SELFCHECK_RESULT_SEND == 1)
  CAN_CYCLE_SEND_CONFIGURE_CAN(100,  0x3F9, 1, 16,      CanPeriodMessage3F8)
#endif
CAN_CYCLE_SEND_CONFIGURE_END(3)
...
const CanChannelCycleSendConfigure_t g_canCycleConfigureList[] = {
    ...
    },
    {
     ...
     .msgStartTime = 5,
     .pCycleConfigureList = CYCLE_SEND_CAN_CONFIGURE(3),
     .cycleConfigureListSize = CYCLE_SEND_CAN_CONFIGURE_SIZE(3),
     .pMsgBuffer = CYCLE_SEND_CAN_BUFFER(3)
    },
};
