...
static void CanTp_Prv_GetTxBuffer(const CanTp_TxContextType *Context, PduInfoType* SduInfoPtr)
{
    ...
    if(Context->ElapsedValue < Context->CsTicks)
    {
        ...
        {
            switch(Dcm_CopyTxData(Context->PduRPduHandleId, SduInfoPtr, NULL_PTR, &RemBufSize))
            {
...
static void CanTp_Prv_ProcessRxSingleFrame(const CanTp_RxContextType *Context, const PduInfoType *PduInfoPtr)
{
    ...
    /*added by lhy*/
    if((SduInfo.SduDataPtr[0] == 0x3EU)&&(SduInfo.SduDataPtr[1] == 0x80U)&&(SduInfo.SduLength == 0x02U))/*modify the question-3e 80 limit next service does not respond*/
    {
        /*Ignore*/
        Clr_FunReq_Flag();
    ...
    else if(CanTp_Prv_GetConnectionAcceptance(Context, PduInfoPtr) == E_OK)
    {
...
static Std_ReturnType CanTp_Prv_CreateTxSingleFrame(CanTp_ChannelIdType ChannelId, PduInfoType *PduInfoPtr)
{
    ...
    if(CanTp_SubState[ChannelId] == CANTP_TX_WAIT_FOR_TRANSMIT_CONFIRMATION)
    {
        ...
        if(Dcm_CopyTxData(Connection->PduRPduHandleId, &SduInfo, NULL_PTR, &RemBufSize) != BUFREQ_OK)
        {
