...
#define SOS_KEY_RESET_TBOX_TIME ( 30000 )                       /*按键复位TBOX的时间*/    
...
static void SosButtonDetection( void )
{
    ...
    else if(SosButtonState == E_SOS_BUTTON_STATE_RELEASED )
    {
        ...
        else
        {
            ...
            // {
                uint8_t triggerRet = 0xFF;
                triggerRet = AlarmSdkEcallTriger(E_ECALL_TRIGGER_BTN_MANN);
                EcallLogReport(14, &triggerRet,1);
...
static uint8_t getBattery(void) {
    ...
    uint8_t battAgeStatus = 0;
    BatterySdkGetAgeCheckResult(&battAgeStatus);

    if (Dem_GetEventStatus(DEM_EVT_BACKUP_BAT_OPEN, &eventStatus) == E_OK)
    {
        if ((eventStatus & DEM_UDS_STATUS_TF) == 1u)
        {
            return 2;
        }
    }
    else if (Dem_GetEventStatus(DEM_EVT_BACKUP_BAT_SHORT_GND, &eventStatus) == E_OK)
    {
        if ((eventStatus & DEM_UDS_STATUS_TF) == 1u)
        {
            return 1;
    ...
    else if (battAgeStatus == E_BatteryAgeResult_Aging)
    {
        return 3;
    }
    else
...
static uint8_t getMIC(void) {
    ...
    // uint32_t ANT_Value = 0;
    // PeripheralHalAdGet(AD1_CHANNEL_MIC_IN_ADC_DET, &ANT_Value);
    // TBOX_PRINT("MIC ADC = %d\r\n",ANT_Value);
...
static uint8_t getSpeaker(void) {
    DtcQueryState_e AMP_Value = DtcGetObjState(E_DTC_QUERY_SPEAKER);

    if(AMP_Value == E_DTC_QUERY_STATE_OPEN)
    {
        return 1;
    }
    else if(AMP_Value == E_DTC_QUERY_STATE_SHORT_GND)
    {
        return 2;
    }
    else if(AMP_Value == E_DTC_QUERY_STATE_NORMAL)
    {
        return 0;
    }
}
...
static uint8_t getSIMCard(void) {
    DtcQueryState_e SIM_Value = DtcGetObjState(E_DTC_QUERY_SIM_CARD);

    if(SIM_Value == E_DTC_QUERY_STATE_SIM_NOT_ONLINE)
    {
        return 1;
    }
    else if(SIM_Value == E_DTC_QUERY_STATE_SIM_INVALID)
    {
        return 2;
    }
    else if(SIM_Value == E_DTC_QUERY_STATE_NORMAL)
    {
        return 0;
    }
}
...
static void SelfcheckCycleProcess( void )
{
    ...
    switch( AlarmSdkGetSelfcheckState() )
    {
        ...
        case E_SELFCHECK_RUN_STOP:
            ...
            {
                if((( GetSosLedState() == E_SOS_LED_STATE_SELFCHECK_ON ) || ( GetSosLedState() == E_SOS_LED_STATE_NO_ECALL )) && ( selfcheckLevel == 2 ))
                {
                    EcallHalSosLedControlSend( E_SOS_LED_STATE_SELFCHECK_ERR_HEAVY );
                }
                if((( GetSosLedState() == E_SOS_LED_STATE_SELFCHECK_ON ) || ( GetSosLedState() == E_SOS_LED_STATE_NO_ECALL )) && ( selfcheckLevel == 1 ))
                {
...
void AirbagSingleProcess(void)
{
    ...
    if(airbagSingal != 0)
    {
        if(SRStriggerFlag == 0)
        {
            if(adasStatus != 0)
            {
                triggerRet = AlarmSdkEcallTriger(E_ECALL_TRIGGER_SRS_AUTO);
                //SetECALLWorkingState(1);
            ...
            {
                triggerRet = AlarmSdkEcallTriger(E_ECALL_TRIGGER_CAN_AUTO);
                //SetECALLWorkingState(1);
                TBOX_PRINT("==Ecall Airbag %d \r\n",(airbagSingal-adasStatus));
                EcallLogReport(15,&triggerRet,1);
            }
...
void TaskEcallEnableInit(void)
{
    uint8_t ReadData[8] = {0};
    NvM_ReadBlock(NvMBlock_DIDC30C,NvMBlockRamBuffer41);
    memcpy(ReadData, NvMBlockRamBuffer41, 8);
    if(ReadData[1] == 0x00)
    {
        SetEcallEnableState(0);
    }
...
void ControlAmpStatusProcess(uint16_t cycleTimeCount)
{
    if((AlarmGetControlAmpStatus() == 1) && (AlarmSdkIsEcallInProgress() == 0))
    {
        /* AMP STANDBY引脚(SDZ)控制：非ECALL状态下每1秒刷新一次，低电平有效，间隔100ms */
        if(cycleTimeCount == (1000 / ECALL_PROCESS_CYCLE_TIME))
        {
            EcallHalSetAmpSdz(0);  /* 拉低，使能STANDBY */ 
        }
        if(cycleTimeCount == (1100 / ECALL_PROCESS_CYCLE_TIME))
        {
            EcallHalSetAmpSdz(1);  /* 拉高，完成脉冲（间隔100ms） */
        }
    }
    else
    {
...
void TaskEcallProcess(void)
{
    ...
    ControlAmpStatusProcess(cycleTimeCount);
    if(cycleTimeCount == (1500 / ECALL_PROCESS_CYCLE_TIME))
    {
    ...
    if(cycleTimeCount == (2000 / ECALL_PROCESS_CYCLE_TIME))
    {
        ...
        TBOX_PRINT("=V0617=Ecall Selfcheck\r\n");
    ...
    if(ecallLogtimeCount == ((500) / ECALL_PROCESS_CYCLE_TIME))
    {
        ...
        */
         ...
         * g_SendECALLLog[11] MainAnt
         * g_SendECALLLog[12] core->state
         * g_SendECALLLog[13] evt->event
         * g_SendECALLLog[14] SOS按键触发结果
         * g_SendECALLLog[15] CAN触发结果
         */

        g_SendECALLLog[16] = 0x06;
        AlarmSdkLogReport(g_SendECALLLog,17);
    }
}
