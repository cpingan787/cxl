...
// #define DCM_APP_10_02_EPTRDYV_VALID_VALUE    ((uint8)0u)   
#define DCM_APP_10_02_EPTRDY_NOT_READY_VALUE ((uint8)0u)   
#define DCM_APP_10_02_VEHSPD_VALID_VALUE     ((uint8)0u)   
#define DCM_APP_10_02_VEHSPD_THRESHOLD       ((uint16)256u)   

#define DCM_APP_0X10_NON_02_VEHSPD_VALID_VALUE    ((uint8)0u)  
#define DCM_APP_0X10_NON_02_VEHSPD_THRESHOLD_RAW  ((uint16)256u)

static FUNC(Std_ReturnType, DCM_CODE) Dcm_App_10_02_PreconditionCheck(      /*��� 10 02 ��ҵ��ǰ������*/
...
{ 
    ...
    uint32 supplyVoltage = 0u;
    if (PeripheralHalAdGet(AD0_CHANNEL_KL30, &supplyVoltage) == 0)
    {
        if (supplyVoltage > 18000u)
        {
            *ErrorCode = (Dcm_NegativeResponseCodeType)0x92u;
            return E_NOT_OK;
        }

        if (supplyVoltage < 7000u)
        {
            *ErrorCode = (Dcm_NegativeResponseCodeType)0x93u;
            return E_NOT_OK;
        }
    }
    ...
    // if ((comRet != E_OK) || (EPTRdyV != DCM_APP_10_02_EPTRDYV_VALID_VALUE))  
    // {                                                                         
    //     return E_OK;                                                         
    // }                                                                         
    ...
    if ((comRet != E_OK) || (VehSpdAvgV != DCM_APP_10_02_VEHSPD_VALID_VALUE))
    {                                                                         
        return E_OK;                                                          
    }                                                                         
...
static FUNC(Std_ReturnType, DCM_CODE) Dcm_App_0x10Non02_PreconditionCheck(   /*���10����(�ų�10 02)ǰ������ */
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode)       
{                                                                             
    Std_ReturnType comRet = E_NOT_OK;
    uint8 VehSpdAvgV = 0u;
    ...
    if (VehSpdAvg >= DCM_APP_0X10_NON_02_VEHSPD_THRESHOLD_RAW)
    {
        *ErrorCode = (Dcm_NegativeResponseCodeType)0x88u;
        return E_NOT_OK;
    }
    ...
    P2VAR(Dcm_NegativeResponseCodeType, AUTOMATIC, DCM_VAR) ErrorCode)
{
    ...
    *ErrorCode = DCM_E_SUBFUNCTIONNOTSUPPORTED;
    ...
    if ((E_OK == ret) && (Dcm_MsgCtrl[MsgCtrlId].Subfunction == 0x02u))  //10 02�����Ⱦ�����֮����������Ӧ��
    {
        ret = Dcm_App_10_02_PreconditionCheck(ErrorCode);
    }

    if ((E_OK == ret)
        && ((Dcm_MsgCtrl[MsgCtrlId].Subfunction == 0x01u) || (Dcm_MsgCtrl[MsgCtrlId].Subfunction == 0x03u)))
    {
