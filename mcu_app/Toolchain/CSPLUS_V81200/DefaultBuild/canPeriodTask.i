...
static volatile uint8_t g_isCanSending=0;
...
static void SendLocationDefaultInfo(void)
{
    ...
    Com_SendSignal(258u, &defaultValue);                 
    Com_SendSignal(260u, &defaultValue);                 
    Com_SendSignal(259u, &defaultValue);                 
    Com_SendSignal(280u, &defaultValue);    
    Com_SendSignal(254u, &defaultValue);        
    Com_SendSignal(252u, &defaultValue);   
    Com_SendSignal(274u, &defaultValue); 
    Com_SendSignal(279u, &defaultValue);     
    uint8 diffAgeInfo = 0xFF;
    Com_SendSignal(220u, &diffAgeInfo);                                                                                                    
    Com_SendSignal(218u, &defaultValue);                                                                                                   
    Com_SendSignal(216u, &defaultValue);                                                                                               
    Com_SendSignal(203u, &defaultValue);                                                                                               
    Com_SendSignal(202u, &defaultValue);                                                                                              
    Com_SendSignal(205u, &defaultValue);                                                                                            
    Com_SendSignal(217u, &defaultValue);                                                                                                  
    Com_SendSignal(211u, &defaultValue);                                                                                    
    Com_SendSignal(210u, &defaultValue);                                                                                            
    Com_SendSignal(208u, &defaultValue);                                                                                              
    Com_SendSignal(285u, &defaultValue); 
    Com_SendSignal(206u, &defaultValue);                                                                                                  
    Com_SendSignal(207u, &defaultValue);                                                                                                 
    Com_SendSignal(241u, &defaultValue);                     
    Com_SendSignal(242u, &defaultValue);                         
    Com_SendSignal(240u, &defaultValue);                       
    Com_SendSignal(247u, &defaultValue);        
}
...
static void SendLocationInfo(void)
{
    ...
    Com_SendSignal(209u, &locationInfo.wireState);                                                                        
    Com_SendSignal(253u, &gnssSeq); 
    gnssSeq++;
    Com_SendSignal(275u, &locationInfo.svwTimestamp); 
    uint8 gnssSignalStatus = 0;
    if (statusLocation == 3)
    {
        gnssSignalStatus = 3;
        Com_SendSignal(248u, &gnssSignalStatus); 
    }
    ...
    Com_SendSignal(204u, &locationInfo.useSvsnum); 
    Com_SendSignal(42u, &locationInfo.svsNum);                   
    uint16 hdop = locationInfo.svwHdop;
    Com_SendSignal(265u, &hdop); 
    uint16 pdop = locationInfo.svwPdop;
    Com_SendSignal(269u, &pdop); 
    uint16 vdop = locationInfo.svwVdop;
    Com_SendSignal(267u, &vdop); 
}
...
static void SendSatInfo(void)
{
    ...
    svGroupCount = (svGroupNum << 4) | (svGroupCount & 0x0F); 
    while (1)
    {
        if ((svGroupCount & 0x0F) * 20 < totalSatNum)
        {
            Com_SendSignal(41u, &svGroupCount); 
            ...
            for (uint8 i = 0; i < 20; i++)
            {
                ...
                Com_SendSignal(satelliteID[i], &satelliteInfo.svList[sentCount + i].svId);       
                Com_SendSignal(satelliteAz[i], &satelliteInfo.svList[sentCount + i].azimuth);    
                Com_SendSignal(satelliteElv[i], &satelliteInfo.svList[sentCount + i].elevation); 
                Com_SendSignal(satelliteCNO[i], &satelliteInfo.svList[sentCount + i].cN0Dbhz);   
            }
            sentCount += 20;
            svGroupCount++;
        }
        else if ((svGroupCount & 0x0F) * 20 >= totalSatNum) 
        {
            Com_SendSignal(41u, &svGroupCount); 
...
{
    Com_ReceiveSignal(372u, &year);
    Com_ReceiveSignal(373u, &month);
    Com_ReceiveSignal(374u, &day);
    Com_ReceiveSignal(375u, &hour);
    Com_ReceiveSignal(376u, &min);
    Com_ReceiveSignal(377u, &sec);
...
uint8_t CanPeriodGetVehTime(uint8_t *year, uint8_t *month, uint8_t *day, uint8_t *hour, uint8_t *min, uint8_t *sec)
{
    if (year == ((void *) 0) || month == ((void *) 0) || day == ((void *) 0) || hour == ((void *) 0) || min == ((void *) 0) || sec == ((void *) 0))
    {
        return -1;
    }
