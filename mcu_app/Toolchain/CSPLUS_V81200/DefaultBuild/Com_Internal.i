...
Com_TxSignalGroupRunTimeStateType Com_TxSignalGroupState[8u];
...
uint16 Com_RxSignalTimeOutCnt[6u];
...
uint16 Com_RxSignalGroupTimeOutCnt[1u];
...
Com_TxSignalGroupTriggerFlagInit(void)
{
    ...
    for (uint16 cirCnt = 0u; cirCnt < 8u; ++cirCnt)
    {
        Com_TxSigGrpStPtr = &Com_TxSignalGroupState[cirCnt];
        Com_TxSigGrpStPtr->TxSigGrpRTStFlag &= 0xFEu;
    }
}
...

{
    ...
    if (isTrue)
    {
        ...
        signalValue = (signalValue >> lsbPosInByte) & ((0xFFFFFFFFFFFFFFFFu) >> (64u - bitSize));
        ...
        {
            signalValue = ((0xFFFFFFFFFFFFFFFFu) << bitSize) | signalValue;
        }
    }
...
Com_TxIpduTMSHandle(Com_TxIPduRunTimeStateType* TxIpduStatePtr, const Com_TxIPduType* TxIpduPtr, uint16* RptNum)
{
    ...
    {
        TxIpduStatePtr->TxIpduRTStFlag |= 0x4u;
