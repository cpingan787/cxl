...
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncBeh_EntryPrepareSleep(uint32 pncIdx);

#if COMM_PNC_GATEWAY_ENABLED == STD_ON
COMM_LOCAL void ComM_PncUpdateEra(uint32 chIdx, const uint8* pncBitEra);
#endif /* COMM_PNC_GATEWAY_ENABLED == STD_ON */
 ...
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncStateChangeHandleNotify(uint32 pncIdx)
{
    ...
    for (uint32 index = 0; index < pncCfgPtr->belongUserNum; index++)
    {
        ...
        for (uint32 i = 0u; i < usrCfgPtr->pncListNum; i++)
        {
            uint32 pncIdx = usrCfgPtr->pncList[i];
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncLearnSendIraByCh(uint32 chIdx)
{
    ...
    {
        for (uint32 i = 0u; i < COMM_PN_INFO_LENGTH; i++)
        {
...
#endif /* COMM_PNC_GATEWAY_ENABLED == STD_ON */
    for (uint32 index; index < ComM_PncCfgPtr->pncMapTotalChannelNum; index++)
    {
 ...
 * @return      NA
 */
/* PRQA S 1520 ++ */ /* VL_ComM_1520 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncBeh_EntryReadySleep(uint32 pncIdx)
/* PRQA S 1520 -- */ /* VL_ComM_1520 */
{
    ...
    for (uint32 portIdx = 0u; portIdx < pncCfgPtr->ethIfSwitchPortNum; portIdx++)
    {
    ...
    for (uint32 chIdx = 0; chIdx < ComM_PncCfgPtr->pncMapTotalChannelNum; chIdx++)
    {
     ...
     */
    BswM_ComM_CurrentPNCMode(pncCfgPtr->notifyBswMPncId, COMM_PNC_READY_SLEEP); /* PRQA S 1520 */ /* VL_ComM_1520 */
#endif /* COMM_BSWM_ENABLE == STD_ON */
#if ((COMM_USER_MODESWITCH_NOTIFY == STD_ON) && (COMM_USER_NUMBER > 0))
    ...
    {
        ComM_PncBeh_EntryPrepareSleep(pncIdx); /* PRQA S 1520 */ /* VL_ComM_1520 */
    }
}
/* PRQA S 1520 ++ */ /* VL_ComM_1520 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncBeh_EntryNoCom(uint32 pncIdx)
/* PRQA S 1520 -- */ /* VL_ComM_1520 */
{
    const ComM_PncItemConfigType* pncCfgPtr = &ComM_PncCfgPtr->pncListCfgPtr[pncIdx];
...
/* PRQA S 1520 -- */ /* VL_ComM_1520 */
{
     ...
     * notified by the API call BswM_ComM_CurrentPncMode() with the entered PNC state
     */
    BswM_ComM_CurrentPNCMode(pncCfgPtr->notifyBswMPncId, COMM_PNC_PREPARE_SLEEP); /* PRQA S 1520 */ /* VL_ComM_1520 */
#endif /* COMM_BSWM_ENABLE == STD_ON */
#if ((COMM_USER_MODESWITCH_NOTIFY == STD_ON) && (COMM_USER_NUMBER > 0))
    ComM_PncStateChangeHandleNotify(pncIdx);
#endif
    ...
    {
        ComM_PncBeh_EntryNoCom(pncIdx); /* PRQA S 1520 */ /* VL_ComM_1520 */
    }
}
/**
 * @ingroup     ComM pnc
 ...
 */
/* PRQA S 1520 ++ */ /* VL_ComM_1520 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncBeh_EntryRequst(uint32 pncIdx)
/* PRQA S 1520 -- */ /* VL_ComM_1520 */
{
    const ComM_PncItemConfigType* pncCfgPtr = &ComM_PncCfgPtr->pncListCfgPtr[pncIdx];
    ...
    else
    {
         ...
         */
#if (COMM_BSWM_ENABLE == STD_ON)
        BswM_ComM_CurrentPNCMode(pncCfgPtr->notifyBswMPncId, COMM_PNC_REQUESTED); /* PRQA S 1520 */ /* VL_ComM_1520 */
#endif /* COMM_BSWM_ENABLE == STD_ON */
    }
#if ((COMM_USER_MODESWITCH_NOTIFY == STD_ON) && (COMM_USER_NUMBER > 0))
    ComM_PncStateChangeHandleNotify(pncIdx);
    ...
    for (uint32 index = 0u; index < ComM_PncCfgPtr->pncMapTotalChannelNum; index++)
    {
...
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ComRxEraMainHandle(void)
{
    if (TRUE == ComM_PncRxComSigEraInd)
    {
        ...
        for (uint32 index = 0; index < ComM_PncCfgPtr->comRxEraSigNum; index++)
        {
            ...
            if (E_OK == ret)
            {
                for (uint32 i = eraSigCfgPtr->offset; i < eraSigCfgPtr->len; i++)
                {
...
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ComRxEiraMainHandle(void)
{
    if (TRUE == ComM_PncRxComSigEiraInd)
    {
        ...
        for (uint32 index = 0; index < ComM_PncCfgPtr->comRxEiraSigNum; index++)
        {
            ...
            {
                eiraEnCo = TRUE;
                for (uint32 i = eiraSigCfgPtr->offset; i < eiraSigCfgPtr->len; i++)
                {
 ...
 * @return      pncMode
 */
ComM_PncModeType ComM_PncGetStatus(uint32 pncIdx)
{
 ...
 * @return      NA
 */
FUNC(void, COMM_CODE) ComM_PncWakupIndPnc(uint32 pncIdx)
{
...
/* PRQA S 1520 ++ */ /* VL_ComM_1520 */
FUNC(void, COMM_CODE)
ComM_PncRequestComMode(uint32 pncIdx, uint32 usrInPncIdx, ComM_ModeType comMode)
/* PRQA S 1520 -- */ /* VL_ComM_1520 */
{
    ComM_PncVarType* pncVarPtr = &ComM_PncVar[pncIdx];
    ...
    if (modeReq == COMM_FULL_COMMUNICATION)
    {
        ...
        {
            ComM_PncBeh_EntryRequst(pncIdx); /* PRQA S 1520 */ /* VL_ComM_1520 */
            /** @ref SWS_ComM_00948
         ...
         * channel COMM_FULL_COMMUNICATION
         */
        for (uint32 chIdx = 0u; chIdx < ComM_PncCfgPtr->txOnlPncChNum; chIdx++)
        {
    ...
    else if (modeReq == COMM_NO_COMMUNICATION)
    {
        ...
        if (pncCfgPtr->pncGatewayEnable == TRUE)
        {
                ...
                && ComM_CommonGetBit(pncVarPtr->setMask, COMM_PNC_MAP_LEAST_CHANNEL))
            {
                 ...
                 */
                for (uint32 chIdx = 0u; chIdx < ComM_PncCfgPtr->pncMapTotalChannelNum; chIdx++)
                {
                    if (ComM_CommonGetBit(ComM_ChToPncMap[chIdx], pncCfgPtr->pncOffsetId)
#if (COMM_TX_ONLY_PNC_ENABLE == STD_ON)
                ...
                ComM_PncBeh_EntryReadySleep(pncIdx); /* PRQA S 1520 */ /* VL_ComM_1520 */
            }
        }
        else
#endif /* COMM_PNC_GATEWAY_ENABLED == STD_ON */
...
ComM_PncInit(P2CONST(ComM_PncConfigType, AUTOMATIC, COMM_APPL_CONST) pncCfgPtr)
{
    ...
    if (NULL_PTR != ComM_PncCfgPtr->chanToPncMap)
    {
        ...
        for (uint32 index = 0; index < copySize; index++)
        {
...
#endif /* COMM_DYN_PNC_TO_CH_MAPP_SUPPORT == STD_ON */
    /* per pnc init */
    for (uint32 index = 0; index < ComM_PncCfgPtr->pncListNum; index++)
    {
        ...
        pncVarPtr->eraCnt = 0u;
        for (uint32 i = 0u; i < pncItemCfgPtr->eraBufLen; i++)
        {
        ...
        for (uint32 i = 0u; i < pncItemCfgPtr->requestMaskLen; i++)
        {
            pncVarPtr->userReqTable[i] = 0x0u;
        }
    }
    for (uint32 index = 0; index < COMM_PN_INFO_LENGTH; index++)
    {
...
COMM_LOCAL FUNC(void, COMM_CODE) ComM_PncUpdateEira(P2CONST(uint8, AUTOMATIC, COMM_APPL_CONST) pncBitEira)
{
    for (uint32 pncIdx = 0; pncIdx < ComM_PncCfgPtr->pncListNum; pncIdx++)
    {
        uint32 pncId = ComM_PncCfgPtr->pncListCfgPtr[pncIdx].pncOffsetId;
        ComM_PncVarType* pncVarPtr = &ComM_PncVar[pncIdx];
...
/* PRQA S 6010 -- */
{
    ...
    for (uint32 pncIdx = 0; pncIdx < ComM_PncCfgPtr->pncListNum; pncIdx++)
    {
        ...
        {
            uint32 chInPncIdx = ComM_PncCfgPtr->pncListCfgPtr[pncIdx].chMapEraIndex[chIdx];
 ...
 */
FUNC(void, COMM_CODE)
ComM_PncForwardSyncPncShutdown(uint32 chIdx, P2CONST(uint8, AUTOMATIC, COMM_APPL_CONST) pncBitVectorPtr)
{
    /** @ref SWS_ComM_01050 update @ref SWS_ComM_01082
 ...
 * @return      NA
 */
FUNC(void, COMM_CODE) ComM_PncLearnInd(uint32 chIdx, boolean reqMode)
{
    if (TRUE == reqMode)
    {
        /** @ref SWS_ComM_01026
        ...
        if (0u != gatewayMask)
        {
            for (uint32 index = 0; index < ComM_PncCfgPtr->pncMapTotalChannelNum; index++)
            {
...
ComM_PncUpdateMembership(boolean Control, P2CONST(uint8, AUTOMATIC, COMM_APPL_DATA) PncMembership)
{
     ...
     */
    for (uint32 index = 0u; index < COMM_PN_INFO_LENGTH; index++)
    {
        if (Control == 0u)
        {
    ...
    P2VAR(uint8, AUTOMATIC, COMM_APPL_DATA) pncCnt)
{
    ...
    *pncCnt = ComM_PncCfgPtr->pncListNum;
    uint32 pncByteNum = COMM_VLU_MAPU8_BYTE(*pncCnt);
    for (uint32 chIdex = 0u; chIdex < ComM_PncCfgPtr->pncMapTotalChannelNum; chIdex++)
    {
        /** @ref SWS_ComM_01036
         */
        for (uint32 pncByteIdex = 0u; pncByteIdex < pncByteNum; pncByteIdex++)
        {
    ...
    VAR(uint8, COMM_APPL_DATA) pncCnt)
{
    ...
    if (E_OK == ret)
    {
        uint32 byteNum = COMM_VLU_MAPU8_NUM(pncCnt);
        for (uint32 index = 0; index < channelCnt; index++)
        {
            for (uint32 i = 0u; i < byteNum; i++)
            {
...
FUNC(Std_ReturnType, COMM_CODE) ComM_PncResetPncMap(void)
{
    ...
    if (E_OK == ret)
    {
        uint32 byteNum = COMM_VLU_MAPU8_NUM(ComM_PncCfgPtr->pncListNum);
        for (uint32 index = 0u; index < ComM_PncCfgPtr->pncMapTotalChannelNum; index++)
        {
            for (uint32 i = 0u; i < byteNum; i++)
            {
