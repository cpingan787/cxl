...
static int16_t Service0x2EProcess(uint8_t *udsData, uint16_t udsLen, uint8_t functionAddressFlag)
{
  ...
  if (did == 0x010D)
  {
      ...
      int16_t esKeyRet = ProcessWriteDidESKey(udsData, udsLen);

      if (esKeyRet == 0)
      {
          uint8_t positiveResponse[3];
          positiveResponse[0] = 0x6E;         
          positiveResponse[1] = udsData[1];   
          positiveResponse[2] = udsData[2];   
          DiagnosticDataTransmit(g_tpHandle, g_physicalTransmitCanId, positiveResponse, 3, 0);
      }
      else
      {
          uint8_t errorCode = (esKeyRet > 0 && esKeyRet <= 0xFF) ? (uint8_t)esKeyRet : 0x72;
          uint8_t negativeResponse[3] = {0x7F, 0x2E, errorCode};
  ...
  if ((did == 0x011B || did == 0x011C || did == 0x011D || did == 0x031C || did == 0x013C || did == 0x105F 
    || did == 0x105E || did == 0x1061 || did == 0x1053 || did == 0x1052 || did == 0xF190 || did == 0x011F || did == 0x0124)) 
  {
      uint8_t pendingResponse[3] = {0x7F, 0x2E, 0x78};
      DiagnosticDataTransmit(g_tpHandle, g_physicalTransmitCanId, pendingResponse, 3, 0);
      ...
      {
          uint8_t positiveResponse[3];
          positiveResponse[0] = 0x6E;         // Service ID + 0x40
          positiveResponse[1] = udsData[1];   // DID High
          positiveResponse[2] = udsData[2];   // DID Low
          DiagnosticDataTransmit(g_tpHandle, g_physicalTransmitCanId, positiveResponse, 3, 0);
      }
      else
      {
          uint8_t errorCode = (localWriteRet > 0 && localWriteRet <= 0xFF) ? (uint8_t)localWriteRet : 0x72;
          uint8_t negativeResponse[3] = {0x7F, 0x2E, errorCode};
          DiagnosticDataTransmit(g_tpHandle, g_physicalTransmitCanId, negativeResponse, 3, 0);
      }
      return 0;
  ...
  else
  {
    ...
    TimerHalStartTime(s_p2TimerHandle, 50);

    while (1)
    {
        // 4. 尝试接收 (每次只等 2ms，不阻塞主流程，确保能及时检查定时器)
        ...
        // 5. 检查是否超时 (TimerHalIsTimeout 返回 0 表示超时)
        if (TimerHalIsTimeout(s_p2TimerHandle) == 0)
        {
            if (pendingSent == 0)
            {
                ...
                pendingSent = 1; // 标记已发 78

                // 停止当前计时，立即重新启动 P2* 定时器 (5000ms)
                TimerHalStopTime(s_p2TimerHandle);
                TimerHalStartTime(s_p2TimerHandle, 5000);
            }
            else
            {
                // [P2* 超时] -> MPU 彻底没响应，报错退出
                TimerHalStopTime(s_p2TimerHandle);
                negativeNum = 0x72;
                break;
            }
