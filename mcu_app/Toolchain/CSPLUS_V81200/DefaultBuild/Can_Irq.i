...
static  void Can_RxGlobalIsr(
  const uint8 LucUnit);
static  void Can_TxIsr(const uint8 LucController);
static  void Can_BusOffIsr(
  const uint8 LucController);
static  void Can_WakeupIsr(
  const uint8 LucController);
...
static  void Can_RxGlobalIsr(const uint8 LucUnit)
 
{
  ...
  for (LulRxFIFOIndex = 0UL;
   LulRxFIFOIndex < (uint32) 8UL; LulRxFIFOIndex++)
  {
     
    LulIrqExist = LulIrqExist | (
      (((uint32)(Can_GaaRegs[LucUnitIdx]. pCmn->aaRFCC[LulRxFIFOIndex]) >> 1UL) & 1UL)
      &
      (((uint32)(Can_GaaRegs[LucUnitIdx]. pCmn->aaRFSTS[LulRxFIFOIndex]) >> 3UL) & 1UL)
      );
  }
   
  if ((0UL == LulIrqExist) ||
    ((uint16)(*Can_GaaRegs[LucUnitIdx].pICRxFIFO &
      ((uint16)0x0080U | (uint16)0x1000U)) != (uint16)0x1000U))
  {
  ...
  else
  {
    ...
    Can_RxProcessing((0x00000100UL << (LucUnitIdx)), 0x02U,
      0U);
     
     
    (*((volatile uint32*)(&Can_GaaRegs[LucUnitIdx]. pCmn->ulGSTS)) = ((*((volatile uint32*)(&Can_GaaRegs[LucUnitIdx]. pCmn->ulGSTS)))|(0UL))); (*((volatile uint32*)(&Can_GaaRegs[LucUnitIdx]. pCmn->ulGSTS))); __syncp();
...
void CAN_RSCAN0_RXFIFO_CAT2_ISR(void)
 ...
 
{
  ...
  uint8 LucCtrlIndex;
  const volatile uint8 * LpController;
  ...
  if ((0U ==
    ...
    ||
    ((uint16)(*LpPCController->pICTx & ((uint16)0x0080U | (uint16)0x1000U))
      != (uint16)0x1000U))
  {
    Dem_ReportErrorStatus((Dem_EventIdType)1, 0x01u);
  ...
  else
  {
    ...
    (*((volatile uint32*)(&Can_GaaRegs[LpPCController->ucUnitIndex]. pCmn->ulGSTS)) = ((*((volatile uint32*)(&Can_GaaRegs[LpPCController->ucUnitIndex]. pCmn->ulGSTS)))|(0UL))); (*((volatile uint32*)(&Can_GaaRegs[LpPCController->ucUnitIndex]. pCmn->ulGSTS))); __syncp();
...
void CAN_CONTROLLER1_TX_CAT2_ISR(void)
...
static  void Can_BusOffIsr(const uint8 LucController)
 
{
  ...
  if ((0UL == (
    (((uint32)(Can_GaaRegs[LucUnit]. pCmn->aaChReg[LucCh]. ulCTR) >> 8UL) & 0xFFUL) &
    ((uint32)(Can_GaaRegs[LucUnit]. pCmn->aaChReg[LucCh]. ulERFL) & 0xFFUL))
    ) ||
    ((uint16)(*LpPCController->pICErr & ((uint16)0x0080U | (uint16)0x1000U))
      != (uint16)0x1000U))
  {
    Dem_ReportErrorStatus((Dem_EventIdType)1, 0x01u);
  ...
  else
  {
    ...
    if (0UL !=
      (Can_GaaRegs[LucUnit].pCmn->aaChReg[LucCh].ulERFL & 0x00000008UL))
    {
          ...
          Can_GaaCtrlState[LucCtrlIndex].enSubState))
      {
        ...
        Can_GaaCtrlState[LucCtrlIndex].blBusOff = (boolean)0x01;
        ...
        LblNotificationRequired = (boolean)0x01;
      }
      else
      {
        
 
        LblNotificationRequired = (boolean)0x00;
      }
      SchM_Exit_Can_CAN_RAM_DATA_PROTECTION();
       

      if ((boolean)0x01 == LblNotificationRequired)
      {
        CanIf_ControllerBusOff((uint8)(LucCtrlIndex + 0U));
      }
      else
      {
    ...
    (*((volatile uint32*)(&Can_GaaRegs[LucUnit]. pCmn->ulGSTS)) = ((*((volatile uint32*)(&Can_GaaRegs[LucUnit]. pCmn->ulGSTS)))|(0UL))); (*((volatile uint32*)(&Can_GaaRegs[LucUnit]. pCmn->ulGSTS))); __syncp();
...
void CAN_CONTROLLER1_BUSOFF_CAT2_ISR(void)
...
static  void Can_WakeupIsr(const uint8 LucController)
{
  ...
  if (((void *)0) != LpPCController->pICWakeup)
  {
    ...
    if(0U != (*LpPCController->pICWakeup & (uint16)0x0080U))
    {
      Dem_ReportErrorStatus((Dem_EventIdType)1, 0x01u);
    ...
    else
    {
      ...
      {
        *(LpPCController->pFCLAReg) = (uint8)0;
        ...
        Can_GaaCtrlState[LucCtrlIndex].enSubState))
      {
        ...
        Can_GaaCtrlState[LucCtrlIndex].blWakeupEventOccurred = (boolean)0x01;
         
        Can_GaaCtrlState[LucCtrlIndex].blWakeupByHW = (boolean)0x01;
         
        LblCheckWakeupRequired = (boolean)0x01;
      }
      else
      {
         
        LblCheckWakeupRequired = (boolean)0x00;
      }
      ...
      if ((boolean)0x01 == LblCheckWakeupRequired)
      {
        Can_WakeupMode(LucCtrlIndex, (uint8)0x20);
        EcuM_CheckWakeup((EcuM_WakeupSourceType)(uint32)0x00000001UL <<
...
void CAN_CONTROLLER1_WAKEUP_CAT2_ISR(void)
