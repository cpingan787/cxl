...
static MpuTime_t g_gnssData; 
static MpuTime_t g_ntpData;  
...
static int16_t TimeSyncGetLocalTimeFromRtc(uint32_t rtc, uint8_t timeZone, uint32_t *year, uint8_t *mon, uint8_t *day, uint8_t *hour, uint8_t *min, uint8_t *sec)
{
    ...
    while (temp >= 365)
    {
        if ((temp1 % 400 == 0) || (temp1 % 4 == 0 && temp1 % 100 != 0)) 
        {
            if (temp >= 366)
            {
                temp -= 366; 
    ...
    *year = temp1; 
    ...
    while (temp >= 28) 
    {
        if (((*year % 400 == 0) || (*year % 4 == 0 && *year % 100 != 0)) && (temp1 == 1)) 
        {
            if (temp >= 29)
            {
                temp -= 29; 
...
static void TimeSyncSdkSendGnssTimeSignal()
{
    ...
    {
        status = 0;
        Com_SendSignal(52u, &status);
        return;
    }
    uint8 year8 = year32 - 2000;
    Com_SendSignal(51u, &year8);
    Com_SendSignal(57u, &month);
    Com_SendSignal(56u, &day);
    Com_SendSignal(55u, &hour);
    Com_SendSignal(54u, &min);
    Com_SendSignal(53u, &sec);
    Com_SendSignal(52u, &status);
}
...
static void TimeSyncSdkSendNtpTimeSignal()
{
    ...
    {
        status = 0;
        Com_SendSignal(63u, &status);
        return;
    }
    uint8 year8 = year32 - 2000;
    Com_SendSignal(64u, &year8);
    Com_SendSignal(58u, &month);
    Com_SendSignal(61u, &day);
    Com_SendSignal(59u, &hour);
    Com_SendSignal(62u, &min);
    Com_SendSignal(60u, &sec);
    Com_SendSignal(63u, &status);
}
...
void TimeSyncSdkCycleProcess(MpuHalDataPack_t *msgData)
{
    ...
    {
        if (msgData != ((void *) 0) && msgData->pDataBuffer != ((void *) 0) && msgData->dataLength > 0)
        {
...
int16_t TimeSyncSdkGetRealTime(uint8_t *timeSrc, uint32_t *pYear, uint8_t *pMonth, uint8_t *pDay, uint8_t *pHour, uint8_t *pMin, uint8_t *pSecond)
{
    ...
    int16_t ret = TimeSyncGetLocalTimeFromRtc(time, g_gnssData.timeZone, pYear, pMonth, pDay, pHour, pMin, pSecond);
    if (ret == 0)
    {
        return 1;
    }
...
int16_t TimeSyncSdkGetUtcTime(uint32_t *pTime)
{
    ...
    else if (g_gnssData.timeValidity == 1)
    {
        timeZone = g_gnssData.timeZone;
    }
    else if (g_ntpData.timeValidity == 1)
    {
        timeZone = g_ntpData.timeZone;
    }
    if (time < 946684800)
    {
        return -1;
