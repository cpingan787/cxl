...
const uint8 Can_GaaDLCFromPayloadTable[] =
{
  0x00U,  
  0x01U,  
  0x02U,  
...
const uint8 Can_GaaPayloadFromDLCTable[] =
{
  0U,   
  1U,   
  2U,   
...
static const uint8 Can_GaaPayloadFromPLSTable[] =
...
static  void Can_RxIndicationRxFIFO(
  const Can_HohConfigType * const LpHoh,
  const uint8 LucUnit);
static  void Can_RxIndicationCommonPart(
  const Can_HohConfigType * const LpHoh,
  uint32 LaaCanSdu[], uint32 LulDlc,
  const uint32 LulIDRegValue, const uint32 LulFDSts);
static  void Can_MainFunction_Write_Common(
  const uint8 LucIndex
);
static  void Can_MainFunction_Read_Common(
  const uint8 LucIndex
);
 ...
 
{
  ...
  Can_MainFunction_Write_Common(0U);
}
 ...
 
{
  ...
  Can_MainFunction_Read_Common(0U);
}
 ...
 
{
  ...
  if ((boolean)0x00 == Can_GblInitialized)
   
  {
    (void)Det_ReportError(80U, (uint8)0x00,
      (uint8)0x09, (uint8)0x05);
 ...
 
{
  ...
  else
  {
     ...
     
    {
      ...
      case CAN_PENDING_WAKEUP_REQUESTED:
        Can_WakeupMode((uint8) LucCtrlIndex, (uint8)0x0C);
        break;
 ...
 Can_ReturnType Can_CommonDetCheck(
  const uint8 LucSID, const uint8 LucController)
{
  ...
  else
  {
    ...
    if ((1U <= LucController) ||
      ((boolean)0x00 == Can_GpPCController[LucController].blActivation))
...
)
{
   ...
   
  {
    ...
    (void)Det_ReportError(80U, (uint8)0x00,
      (uint8)0x08, (uint8)0x05);
     
  }
  else
  {
    Can_RxProcessing(
      0x0000000FUL | 0x00000F00UL | 0x000000F0UL,
      0x00U, LucIndex);
  }
}
 ...
 void Can_TxConfirmationProcessing(
  const uint8 LucCtrlIndex)
 
{
   
  volatile uint32 * LpTHLACCReg;
  uint8 LucUnit;
  uint8 LucCh;
  uint8 LucCount;
  ...
  if (0x00000004UL ==
        (Can_GaaRegs[LucUnit].pCmn->aaTHLSTS[LucCh] & 0x00000004UL))
  {
  ...
  while ((LucCount < (uint8)16UL) && 
      (0UL == (Can_GaaRegs[LucUnit].pCmn->aaTHLSTS[LucCh] & 0x00000001UL)))
  {
     
    Can_GaaRegs[LucUnit].pCmn->aaTHLSTS[LucCh] = ~0xFFFFE0F8UL;
    LulHistoryData = *LpTHLACCReg;
     
    CanIf_TxConfirmation((PduIdType)(uint32)(((uint32)(LulHistoryData) & 0x0000FF00UL) >> 8UL));
     
    Can_GaaRegs[LucUnit].pCmn->aaTHLPCTR[LucCh] = 0x000000FFUL;
     

     
    LulBufferIndex = (uint32)(((uint32)(LulHistoryData) >> 3UL) & 0x0FUL);
    LulBufferType = (uint32)((uint32)(LulHistoryData) & 0x07UL);
    if (0x00000002UL == LulBufferType)
    {
        ...
        LulHohIndex < (uint32) 4U; LulHohIndex++)
      {
        ...
        if ((CAN_HOH_HTH == LpHoh->enHoh) &&
          ...
          ((((uint32)(LpHoh->ulXXCCRegValue) >> 20UL) & 0xFUL) == LulBufferIndex))
        {
      ...
      Can_GaaRegs[LucUnit].pCmn->aaCFSTS[LulTXRXFIFOIndex] = ~0xFFFF00F0UL;
 ...
 
{
    ...
    LulHohIndex < (uint32)4U; LulHohIndex++)
  {
    ...
    if ((CAN_HOH_HRH == LpHoh->enHoh) &&
      (
        ((LulMode & (uint32)0x02U) ==
          ((uint32)LpPCController->ucIntEnable & (uint32)0x02U))
      ))
    {
      ...
      case CAN_BUFFERTYPE_RXFIFO:
        if (0UL != (LulBufferBits & (0x00000100UL << (LucUnit))))
        {
  ...
  const uint8 LucUnit)
{
  ...
  volatile uint32 *LpDataReg;
   
  uint32 LaaCanSdu[(64U / sizeof(uint32))];
  ...
  LucRFDCValue = (uint8)(((uint32)(LpHoh->ulXXCCRegValue) >> 8UL) & 0x7UL);
   
  LucFIFOBufferDepth = (uint8)(uint8)(((uint8)(LucRFDCValue) == 1U) ? 4U : (((uint8)(LucRFDCValue) == 2U) ? 8U : (((uint8)(LucRFDCValue) == 3U) ? 16U : (((uint8)(LucRFDCValue) == 4U) ? 32U : (((uint8)(LucRFDCValue) == 5U) ? 48U : (((uint8)(LucRFDCValue) == 6U) ? 64U : (((uint8)(LucRFDCValue) == 7U) ? 128U : 0U )))))));
  LucCount = 0U;
   
  while ((0UL ==
    (Can_GaaRegs[LucUnit].pCmn->aaRFSTS[LpHoh->ucBufferIndex] &
      0x00000001UL)) && (LucCount < LucFIFOBufferDepth))
  {
    ...
    }
    {
      ...
      LulMessageDlc =
        (uint32)Can_GaaPayloadFromDLCTable[(uint32)((uint32)(LulMessageDlc) >> 28UL)];
       
      LulAvailableDlc =
        Can_GaaPayloadFromPLSTable[(((uint32)(LpHoh->ulXXCCRegValue) >> 4UL) & 0x7UL)];
    ...
    LulDlcWords = (((uint32)(LulAvailableDlc) + 3UL) & (uint32)(~3UL)) / (uint32)sizeof(uint32);
    ...
    { (*(&Can_GaaRegs[LucUnit]. pCmn->aaRFPCTR[LpHoh->ucBufferIndex])) = (0x000000FFUL);}
 ...
 
{
  ...
  if (0UL != (LulFDSts & 0x00000004UL))
  {
    LstMailbox.CanId = (Can_IdType)0x40000000UL;
  }
  else
  {
    LstMailbox.CanId = 0U;
  ...
  if (0UL != (LulIDRegValue & 0x80000000UL))
  {
     
    LstMailbox.CanId = LstMailbox.CanId |
      (Can_IdType)(
      (LulIDRegValue & (uint32)(Can_IdType)0x1FFFFFFFUL) |
        (uint32)(Can_IdType)0x80000000UL);
  ...
  else
  {
    ...
    LstMailbox.CanId = LstMailbox.CanId |
      (Can_IdType)(LulIDRegValue & (uint32)(Can_IdType)0x1FFFFFFFUL);
