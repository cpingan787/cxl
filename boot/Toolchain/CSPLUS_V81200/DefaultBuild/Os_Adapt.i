...
typedef enum
{
  OS_FASTTRUSTEDFUNCTIONID_COUNT = 0U,
...
static Os_ArchMsrType Os_GlobalKMInt[(1U)];
 ...
 
{
    ...
    if (Os_ObjectIDCheck((ObjectType)ISRID, (uint8)OS_OBJECT_ISR) != 1u)
    {
        err = 3U;
    }
 ...
 
{
    ...
    StatusType err = 0U;
    const Os_SCBType *pScb = Os_GetCurrentContext();
    if (((void *)0) == InterruptSource)
    {
        err = 11U;
    }
    else
    if (0u == pScb->UnhandledIrqReported)
    {
        err = 7U;
    }
    else
 ...
 
{
    ...
    StatusType err = 0U;
    const Os_SCBType *pScb = Os_GetCurrentContext();
    if (((void *)0) == ExceptionSource)
    {
        err = 11U;
    }
    else
    if (0u == pScb->UnhandledExcReported)
    {
        err = 7U;
    }
    else
