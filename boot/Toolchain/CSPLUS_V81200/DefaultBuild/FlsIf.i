...
static uint8 FlsIf_PageBuffer[0x100u];
typedef enum
{
    FlsIf_NoInit,
    FlsIf_OK,
...
uint8 FlsIf_Write(uint32 sAddr, uint32 size, uint8* buf)
{
    ...
    {
        if((0U == FlsIf_ConvertAddress(sAddr, &addr)) &&
            (0U == FlsIf_GetBlockNum(addr, &blocknum)) &&
        ...
        while( (len > 0U) && (tem == 0U) )
        {
            ...
            else
            {
                ...
                CommF_DataSet(&FlsIf_PageBuffer[len], 0xFF, (pageSize - len));
                len = pageSize;
            }
...
uint8 FlsIf_Erase(uint32 sAddr, uint32 size)
{
    ...
    if(FlsIf_State == FlsIf_OK)
    {
        if((0U == FlsIf_ConvertAddress(sAddr,&addr))&&
           ...
           (0U == FlsIf_GetSectorIdx(addr, &sectorIdx)))
        {
        ...
        while((len > 0) && (tem == 0U))
        {
            ...
            if(0U == tem)
            {
                ...
                if (len > 0)
                {
                    if (0U != FlsIf_GetSectorIdx(addr, &sectorIdx))
                    {
                        tem = 1U;
                    }
