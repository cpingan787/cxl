...
const FlashParamMap_t FlashParamMap[E_PARAMETER_INFO_MAX_NUM] = {
    ...
    {E_PARAMETER_INFO_ECallNumber,          11,     NvMBlock_DIDC006,   NvMBlockRamBuffer16},
    {E_PARAMETER_INFO_ParatNumber,          5,      NULL,               NULL},//通过EEIf读取
...
int16_t WorkFlashVehicleInforRead(FlashParaId_e parameterId, uint8_t *data, uint32_t *dataLength)
{
    ...
    {
        paramInfo.length = 5;
        ret = EEIf_Read(DID_F187_ADDR_IDX, paramInfo.length, data);
        *dataLength = 5;
        return ret;
    }
