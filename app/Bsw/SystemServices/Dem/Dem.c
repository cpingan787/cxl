...
Dem_ClearNonvolatileStatus = DEM_ClEAR_NONVOLATILE_INIT; /* PRQA S 1514 */ /* VL_Dem_1514 */
...
FUNC(void, DEM_CODE) Dem_Init(P2CONST(Dem_ConfigType, AUTOMATIC, DEM_CONST_PBCFG) ConfigPtr)
{
    ...
    else
    {
        ...
        if (NvmResult == E_NOT_OK)
        {
            ...
            for (uint8 permanentStart = 0u; permanentStart < DEM_MAX_NUMBER_EVENT_ENTRY_PERMANENT; ++permanentStart)
            {
...
#if (DEM_OPERATION_CYCLE_STATUS_STORAGE == STD_ON)
        const Dem_OperationCycleType* pDemOperationCycle = DemPbCfgPtr->OperationCycle;
...
FUNC(void, DEM_CODE) Dem_ReportErrorStatus(Dem_EventIdType EventId, Dem_EventStatusType EventStatus)
{
    ...
    else
    {
        ...
        /* req SWS_Dem_00676*/
            ...
            (Dem_InitState != DEM_STATE_INIT)
            && (DemPbCfg.DebounceCounterBasedClass[pEventCfg->AlgorithmRef].DemDebounceCounterStorage == TRUE))
        {
        ...
        else
        {
            ...
            if (ret == E_OK)
            {
                ...
                if (((DEM_BITS_ISNSET(DemOperationCycleStatus, OperationCycleId))
                     && (DemPbCfgPtr->OperationCycle[OperationCycleId].DemOperationCycleType != DEM_OPCYC_OBD_DCY))
                    || (0x00u == DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_DTC_SETTING))
                    || (0x00u == DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_AVAILABLE)))
                {
                    ret = E_NOT_OK;
...
FUNC(Std_ReturnType, DEM_CODE) Dem_ClearPrestoredFreezeFrame(Dem_EventIdType EventId)
{
    ...
    {
        IntId = Dem_GetEventInternalId(EventId);
        if (DemPbCfgPtr->pDemEventParameter[IntId].DemFFPrestorageSupported == FALSE)
        {
            DEM_DET_REPORT(DEM_SID_CLEARPRESTOREDFREEZEFRAME, DEM_E_WRONG_CONFIGURATION);
        }
        else
            ...
            *DTCOfEvent = 0x00u;
            ...
            if (DTCRef != DEM_DTC_REF_INVALID)
            {
                ...
                case DEM_DTC_FORMAT_UDS:
                    if (DEM_DTC_CFG_INVALID != DemPbCfgPtr->DemCfgDTC[DTCRef].DemDtcValue)
                    {
                ...
                case DEM_DTC_FORMAT_J1939:
                    if ((ObdDTCRef != DEM_OBD_DTC_INVALID)
                        && (DemPbCfgPtr->ObdDTC[ObdDTCRef].DemJ1939DTCValue != DEM_J1939_DTC_CFG_INVALID))
                    {
    ...
    for (iloop = 0u; iloop < EventNum; iloop++)
    {
        ...
        if (DEM_DTC_REF_INVALID != dtcRef)
        {
            ...
            else
            {
                ...
                const Dem_EventIdType* pDTCMapping = &DemPbCfgPtr->DTCMapping[EventRefStart];
#if (DEM_ONEDTCMAPPINGMUTILEVENT == STD_ON)
                for (uint16 num = 0u; num < EventRefNum; num++)
#endif /* DEM_ONEDTCMAPPINGMUTILEVENT == STD_ON */
                {
                    if (lEventParameter[*pDTCMapping].DemEventAvailable == TRUE)
                    {
                        DemDTCGeneralStatus[dtcRef].SuppressionStatus = FALSE;
#if (DEM_ONEDTCMAPPINGMUTILEVENT == STD_ON)
                        break;
         ...
         * related.*/
        const Dem_DTCType* pDemDTC = &DemPbCfgPtr->DemCfgDTC[pEventCfg->DemDTCRef];
...
/* PRQA S 6020 -- */
{
    ...
    {
#if (DEM_OBD_SUPPORT != DEM_OBD_NO_OBD_SUPPORT)
        IgnUpCycleCounter = DemOBDDataStorage.StoreIgnUpCycleCounter;
#endif
        WarmUpCycleCounter = DemOBDDataStorage.StoreWarmUpCycleCounter;
        OBDDistanceMILOn = DemOBDDataStorage.StoreOBDDistanceMILOn;
        DistSinceDtcCleared = DemOBDDataStorage.StoreDistSinceDtcCleared;
        OBDTimeMILOn = DemOBDDataStorage.StoreOBDTimeMILOn;
        OBDTimeDTCClear = DemOBDDataStorage.StoreOBDTimeDTCClear;
        ContinuousMICounter = DemOBDDataStorage.StoreContinuousMICounter;
        OBDB1Counter = DemOBDDataStorage.StoreOBDB1Counter;
#if (DEM_OBD_SUPPORT == DEM_OBD_MASTER_ECU)
        MasterContinuousMICounter = DemOBDDataStorage.StoreMasterContinuousMICounter;
#endif
    }
    else
    {
    ...
    P2VAR(Dem_EventMemEntryType, AUTOMATIC, DEM_APPL_DATA) * pEntry)
{
    ...
    uint8 iloop;
    Dem_EventMemEntryType* lEntry = DemMemDestCfg[memDest].EntryList;
    const Dem_EventParameterType* pEventParameter = DemPbCfgPtr->pDemEventParameter;
...
Dem_EventDisplacementProcess(Dem_EventIdType InternalId, uint8 memDest)
{
    ...
    for (uint8 iloop = 0; iloop < EntryNum; iloop++)
    {
        ...
        EventNewPriority = (pResEntry != NULL_PTR) ? Dem_GetEventPriority(Dem_GetEventInternalId(pResEntry->EventId))
                                                   : EventNewPriority;
    ...
    && (DEM_OBD_EVENT_DISPLACEMENT == STD_ON))
        ...
        /* PRQA S 3415 ++ */ /* VL_Dem_3415 */
            ...
            && (DemPbCfgPtr->DemCfgDTC[DTCRef].DTCKind == DEM_DTC_KIND_EMISSION_REL_DTCS)
            && (
        ...
        else
        {
            ...
            if (EventNewPriority <= EntryPriority)
            {
                ...
                if (PassiveFound == FALSE)
                {
                    ...
                    else if (EventNewPriority == EntryPriority) /* PRQA S 2995,2991 */ /* VL_Dem_2995,VL_Dem_2991 */
                    {
                ...
                if ((pEntry != NULL_PTR))
                {
                    if (((pDTCAttrCfg->DemAgingCycleRef == OpId) || (Dem_AgingCycle == OpId)
                         || ((DemPbCfgPtr->OperationCycle[DemOperationCycleRef].DemOperationCycleType
                             ...
                             && (DemPbCfgPtr->OperationCycle[OpId].DemOperationCycleType == DEM_OPCYC_WARMUP)))
    ...
    if ((0x00u == DEM_FLAGS_ISSET(pEvent->UdsStatus, DEM_UDS_STATUS_TFTOC)) && (pEventCfg != NULL_PTR))
    {
        if (0x00u != DEM_FLAGS_ISSET(pEvent->UdsStatus, DEM_UDS_STATUS_WIR))
        {
            ...
            if (((DemOperationCycleRef == OpId)
                 || ((DemPbCfgPtr->OperationCycle[DemOperationCycleRef].DemOperationCycleType == DEM_OPCYC_OBD_DCY)
                     && (DEM_BITS_ISNSET(DemOperationCycleStatus, DemOperationCycleRef))
                     && (DemPbCfgPtr->OperationCycle[OpId].DemOperationCycleType == DEM_OPCYC_WARMUP)))
                && (0x00u == DEM_FLAGS_ISSET(pEvent->UdsStatus, DEM_UDS_STATUS_TNCTOC)))
            {
...
#if (DEM_MAX_NUMBER_EVENT_ENTRY_PERMANENT > 0u)
    if (pClr->DTCOrigin == DEM_DTC_ORIGIN_PERMANENT_MEMORY)
    {
        for (uint8 permanentStart = 0u; permanentStart < DEM_MAX_NUMBER_EVENT_ENTRY_PERMANENT; ++permanentStart)
        {
            DemPermanentMemory[permanentStart] = 0u;
