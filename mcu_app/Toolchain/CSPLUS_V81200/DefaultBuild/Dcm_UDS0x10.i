...
static  Std_ReturnType DspInternalUDS0x10_SessionSubDeal(
    Dcm_OpStatusType OpStatus,
    uint8 ProtocolCtrlId,
    uint8 iloop,
    Dcm_NegativeResponseCodeType * ErrorCode)
{
    ...
    if ((pDcmDslProtocolRow->DcmSendRespPendOnTransToBoot == 0u)
...
static  Std_ReturnType DspInternalUDS0x10_SessionDeal(
    Dcm_OpStatusType OpStatus,
    uint8 ProtocolCtrlId,
    uint8 iloop,
    Dcm_NegativeResponseCodeType * ErrorCode)
{
    ...
    if ((DcmDspSessionForBoot == DCM_OEM_BOOT) || (DcmDspSessionForBoot == DCM_OEM_BOOT_RESPAPP))
    {
        ...
        ret = SchM_Switch_DcmEcuReset(((Rte_ModeType_DcmEcuReset)0x04u));
        if (ret == 0U)
        {
    ...
    else if ((DcmDspSessionForBoot == DCM_SYS_BOOT) || (DcmDspSessionForBoot == DCM_SYS_BOOT_RESPAPP))
    {
        ...
        ret = SchM_Switch_DcmEcuReset(((Rte_ModeType_DcmEcuReset)0x05u));
        if (ret == 0U)
        {
            ret = DspInternalUDS0x10_SessionSubDeal(OpStatus, ProtocolCtrlId, iloop, ErrorCode);
        }
    ...
    if (ret == 0U)
    {
        ...
        if (pDcmDslProtocolRow->DcmSendRespPendOnTransToBoot == 1u)
        {
            Dcm_MsgCtrl[MsgCtrlId].Dcm_OpStatus = ((Dcm_OpStatusType)0x01u);
            if (OpStatus == ((Dcm_OpStatusType)0x01u))
            {
                ret = ((Std_ReturnType)10);
            }
            else
            {
                *ErrorCode = ((Dcm_NegativeResponseCodeType)0x78u);
                ret = 1U;
            }
        }
        else
...
static  Std_ReturnType Dcm_UDS0x10_ConditionCheck(
    uint8 ProtocolCtrlId,
    uint8 * SesIndex,
    Dcm_NegativeResponseCodeType * ErrorCode)
{
    ...
    for (iloop = 0; (iloop < DspSessionRowNum) && (0u == Flag); iloop++)
    {
    ...
    if (0U == ret)
    {
        ret = DsdInternal_SubSesCheck(ProtocolCtrlId, 0x10u, ErrorCode);
        if (1U == ret)
        {
    ...
    if (0U == ret)
    {
        ...
        ret = DsdInternal_SubSecurityCheck(ProtocolCtrlId, 0x10u, ErrorCode);
        if (1U == ret)
        {
    ...
    if ((0U == ret) && ((2U) < Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen))
    {
    ...
    Dcm_NegativeResponseCodeType * ErrorCode)
{
    ...
    if ((0U == ret) && (OpStatus == ((Dcm_OpStatusType)0x00u)))
    {
    ...
    if ((0U == ret) && (OpStatus == ((Dcm_OpStatusType)0x01u)) && (pDcmDslProtocolRow->DcmSendRespPendOnTransToBoot == 1u))
    {
        for (iloop = 0; iloop < SessionRow_Num; iloop++)
        {
            if ((Subfunction == pDcm_DspSession->pDcmDspSessionRow[iloop].DcmDspSessionLevel)
