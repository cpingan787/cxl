...
static uint8_t g_respDataBuffer[200] = {0}; 
static MpuHalDataPack_t  g_respPack;        

static uint8_t g_reqDataBuffer[100] = {0} ; 
static MpuHalDataPack_t g_reqPack ;         
...
int8_t CanPassthrough_SendRequest(const uint8_t *pUdsRequest, uint16_t reqLength)
{
    if (pUdsRequest == ((void *) 0) || reqLength == 0) 
    {
        return -1;
    }
...
int8_t CanPassthrough_ReceiveResponse(uint8_t *pUdsResponse, uint16_t *pRespLength)
{
    ...
    if (pUdsResponse == ((void *) 0) || pRespLength == ((void *) 0)) {
        return -1;
    }
    ...
    if (ret == 0 && g_respPack.dataLength > 0) 
    {
        ...
        if (g_respPack.aid == 0x05 &&
            g_respPack.mid == 0x10 &&
            subCommand == 0x23 &&
            g_reqDataBuffer[0] == g_respDataBuffer[0] &&  
            g_reqDataBuffer[1] == g_respDataBuffer[1] &&  
