...
#define GET_BLOCK_OFFSET(block, manager) (size_type)((size_t)(block) - (size_t)(manager))
#define GET_BLOCK_ADDR(offset, manager)  ((BlockType*)((size_t)(manager) + (size_t)(offset)))
...
{
    BlockHeadType head;
    union { /* PRQA S 0750 */ /* VL_IStdLib_0750 */
        FreePtrType free;
        uint8 buffer[1];
    } ptr;
} BlockType;
...
static BlockType* MemHeap_FindFirstFreeBlock(const HeapManager* manager, uint32* fli, uint32* sli)
{
    ...
    if (tmp_sli > 0u)
    {
        *sli = MemHeap_LSBIT(tmp_sli);
        ...
        {
            block = GET_BLOCK_ADDR(manager->freeLinker[*fli][*sli], manager);
        }
        else
        {
            block = (BlockType*)(0u);
...
/* PRQA S 2889-- */
{
    ...
    if (0u != manager->freeLinker[fli][sli])
    {
        ...
        blocktmp = GET_BLOCK_ADDR(manager->freeLinker[fli][sli], manager);
        if (blocktmp == NULL_PTR)
        {
            return;
        }
    ...
    else
    {
        ...
        CLEAR_BIT_8(manager->sli[fli], sli); /* PRQA S 3472,3473 */ /* VL_IStdLib_3472,VL_IStdLib_3473 */
        if (0u == manager->sli[fli])
        {
            CLEAR_BIT_16(manager->fli, fli); /* PRQA S 3472,3473 */ /* VL_IStdLib_3472,VL_IStdLib_3473 */
        }
    }
...
/* PRQA S 2889-- */
{
    ...
    {
        tempBlock = GET_BLOCK_ADDR(block->ptr.free.next, manager);
        tempBlock->ptr.free.prev = block->ptr.free.prev;
    }

    if (0u != block->ptr.free.prev)
    {
        tempBlock = GET_BLOCK_ADDR(block->ptr.free.prev, manager);
        tempBlock->ptr.free.next = block->ptr.free.next;
    }

    /* the block is head of free block link list */
    if (manager->freeLinker[fli][sli] == GET_BLOCK_OFFSET(block, manager))
    {
        manager->freeLinker[fli][sli] = block->ptr.free.next;
...
/* PRQA S 2889-- */
{
    ...
    if (0u != manager->freeLinker[fli][sli])
    {
        ...
        if (manager->freeLinker[fli][sli] != 0u)
        {
            tempBlock = GET_BLOCK_ADDR(manager->freeLinker[fli][sli], manager);
        }
        else
        {
        ...
        if (tempBlock == NULL_PTR)
        {
            return;
        }
        tempBlock->ptr.free.prev = GET_BLOCK_OFFSET(block, manager);
    }

    /* point freeLinker to block as freeLinker's first node */
    manager->freeLinker[fli][sli] = GET_BLOCK_OFFSET(block, manager);

    SET_BIT_8(manager->sli[fli], sli); /* PRQA S 3472,3473 */ /* VL_IStdLib_3472,VL_IStdLib_3473 */
    SET_BIT_16(manager->fli, fli); /* PRQA S 3472,3473 */     /* VL_IStdLib_3472,VL_IStdLib_3473 */
}
/* PRQA S 0306-- */
...
//* PRQA S 2889-- */
{
    ...
    if (remainSize >= sizeof(BlockType))
    {
        ...
        splitBlock = GET_BLOCK(block->ptr.buffer, size);
        /*remainSize | FREE_BLOCK | PREV_USED*/
        splitBlock->head.size = remainSize | FREE_BLOCK;
        ...
        LINK_PREV_PHY_BLOCK(splitBlock, block, manager);
        LINK_PREV_PHY_BLOCK(nextBlock, splitBlock, manager);
    }
...
uint8 ILib_MemHeapInit(void* ram, uint32 size)
{
    ...
    HeapManager* manager = (HeapManager*)ram; /* PRQA S 0316 */ /* VL_QAC_0316 */
    ...
    else
    {
        ...
        /* user block */
        BlockType* block = GET_BLOCK(firstBlock, ROUNDUP_SIZE(BLOCK_HEAD_SIZE));
        LINK_PREV_PHY_BLOCK(block, firstBlock, manager);
        block->ptr.free.prev = 0u;
        block->ptr.free.next = 0u;
        /*block->head.size | USED_BLOCK | PREV_USED*/
        block->head.size = ROUNDDOWN_SIZE(
        ...
        LINK_PREV_PHY_BLOCK(endBlock, block, manager);
...
void* ILib_MemHeapMalloc(void* ram, uint32 size)
{
    ...
    manager = (HeapManager*)ram; /* PRQA S 0316 */ /* VL_QAC_0316 */
 ...
 */
/* PRQA S 0306,2889++ */                                       /* VL_IStdLib_0306,VL_QAC_MultiReturn */
uint8 ILib_MemHeapFree(void* ram, void* ptr) /* PRQA S 3673 */ /* VL_IStdLib_3673 */
/*PRQA S 2889 --*/
{
    StatusType ret = MEMHEAP_OK;
    HeapManager* manager = (HeapManager*)ram; /* PRQA S 0316 */                       /* VL_QAC_0316 */
    BlockType* block = (BlockType*)((size_t)ptr - BLOCK_HEAD_SIZE); /* PRQA S 0326 */ /* VL_IStdLib_0306 */
    ...
    else
    {
        ...
        if (0u != (block->head.size & PREV_STATE))
        {
            ...
            {
                tmp_block = GET_BLOCK_ADDR(block->head.prev, manager);
            }
            else
            {
        ...
        tmp_block->head.prev = GET_BLOCK_OFFSET(block, manager);
    }
    return ret;
}
/* PRQA S 0306-- */
...
{
    StatusType ret = MEMHEAP_OK;
    const HeapManager* manager = (const HeapManager*)ram; /* PRQA S 0316 */ /* VL_QAC_0316 */
