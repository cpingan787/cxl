...
typedef enum                                                      
{
    E_MPU_FAULT_REPORTED_UNKNOWN = 0,                            /* 初始未知状态，表示本地还没完成一次有效上报 */
    E_MPU_FAULT_REPORTED_PASSED  = 1,                            /* 已经向 Dem 上报过 PASSED */
    E_MPU_FAULT_REPORTED_FAILED  = 2                             /* 已经向 Dem 上报过 FAILED */
} MpuFaultReportedState_e;

typedef struct                                                    
{
    uint8_t confirmCnt;                                           
    uint8_t recoverCnt;                                           
} MpuFaultDebounceCfg_t;
