...
#endif
    {
        ...
        for (PduIdType index = 0u; index < destPduNumber; index++)
        {
            ...
            if (PduR_RoutingPathEnabled[destPduId])
            {
                ...
                PduR_QueueType* queuePtr = PduR_GetQueuePtrOfDestPdu(destPduId); /* PRQA S 0317 */ /* VL_QAC_0317*/
                if (queuePtr != NULL_PTR)
                {
                    PduR_BufferType* bufferPtr = PduR_QueueGetHead(queuePtr); /* PRQA S 3678 */ /* VL_PduR_3678*/
