...
static CanIf_IdType CanIf_TxPduDynamicCanIds[36u];
 ...
 void
CanIf_InitTxBuffer(void)
{
    PduIdType txBufferIndex;
    uint8 index;
    const uint8* bufferSizePtr = &CanIf_ConfigStd->CanIfBufferSize[(0)];
    for (txBufferIndex = 0u; txBufferIndex < 1u; txBufferIndex++)
    {
         
        for (index = 0u; index < *bufferSizePtr; index++)
        {
            CanIf_TxBuffer[txBufferIndex].CanIf_TxPduBufferRef[index].used = 0u;
        }
        bufferSizePtr++;
    }
}
 ...
 Std_ReturnType CanIf_TxBusyHandle(const Can_PduType * pduPtr)
{
    Std_ReturnType result = 1U;
    uint8 index;
    uint8 idleBufferIndex = (0xFFu);
    const CanIf_TxPduConfigType* txPduConfigPtr = &CanIf_ConfigStd->CanIfTxPduConfigRef[(pduPtr->swPduHandle)];
    PduIdType txBufIndex = txPduConfigPtr->TxBufferIdndex;
     
    if (txBufIndex < 1u)
    {
        uint8 txBufferSize = CanIf_ConfigStd->CanIfBufferSize[(txBufIndex)];
        CanIf_TxPduBufferType* txPduBufferPtr = CanIf_TxBuffer[txBufIndex].CanIf_TxPduBufferRef;
        for (index = 0u; (index < txBufferSize) && (1U == result); index++)
        {
            if (txPduBufferPtr[index].TxPduIndex == pduPtr->swPduHandle)
            {
                ...
                txPduBufferPtr[index].used = 1u;
                result = 0U;
            }
            if (((0xFFu) == idleBufferIndex) && (!txPduBufferPtr[index].used))
            {
                idleBufferIndex = index;
            }
        }
        if ((1U == result) && ((0xFFu) != idleBufferIndex))
        {
            ...
            txPduBufferPtr[idleBufferIndex].used = 1u;
            result = 0U;
        }
    }
    return result;
 ...
 void CanIf_FreeControllerTxBuffer(uint8 Controller)
{
    PduIdType txBufferIndex;
    Can_HwHandleType hthId;
    uint8 controllerId;
    uint8 index;
    const uint8* txBufferSizePtr = &CanIf_ConfigStd->CanIfBufferSize[(0)];
    ...
    for (txBufferIndex = 0u; txBufferIndex < 1u; txBufferIndex++)
    {
        ...
        for (index = 0u; index < *txBufferSizePtr; index++)
        {
            ...
            {
                hthId = CanIf_ConfigStd->CanIfTxPduConfigRef[(txPduBufferPtr->TxPduIndex)].CanIfTxPduHthId;
                controllerId = CanIf_ConfigStd->CanIfHthCfgRef[(hthId)].CanIfHthCanCtrlId;
                 
                if (Controller == controllerId)
                {
                     
                    txPduBufferPtr->used = 0u;
                }
            }
            txPduBufferPtr++;
        }
...
CanIf_TransmitBufferedPdu(PduIdType CanTxPduId)
{
    ...
    PduIdType txBufIndex = txPduConfigPtr->TxBufferIdndex;
    uint8 txBufSize = CanIf_ConfigStd->CanIfBufferSize[(txBufIndex)];
    if ((txBufIndex < 1u) && (0u < txBufSize))
    {
        Can_IdType canIdValue;
        Can_IdType canIdValueMin = (Can_IdType)(0xFFFFFFFFu);
        uint8 txIndex = (0xFFu);
        CanIf_TxPduBufferType* txPduBufferPtr = CanIf_TxBuffer[txBufIndex].CanIf_TxPduBufferRef;
        ...
        for (uint8 index = 0u; index < txBufSize; index++)
        {
            ...
            {
                canIdValue = (txPduBufferPtr[index].canId) & (0x1FFFFFFFu);
                if ((0xFFu) == txIndex)
                {
        ...
        if ((0xFFu) != txIndex)
        {
            ...
            const CanIfHthCfgType* hthPtr = &CanIf_ConfigStd->CanIfHthCfgRef[(hthIndex)];
            ...
            {
                txPduBufferPtr[txIndex].used = 0u;
            }
        }
    ...
    CanIf_IdType * CanId)
{
 ...
 void CanIf_FreeDynamicTxPduCanIds(uint8 Controller)
{
    PduIdType txPduIndex;
    Can_HwHandleType hthId;
    uint8 controllerId;
    ...
    {
        txPduConfigPtr = &CanIf_ConfigStd->CanIfTxPduConfigRef[(txPduIndex)];
 ...
 void
CanIf_InitDynamicTxPduCanId(void)
{
    PduIdType txPduIndex;
    const CanIf_TxPduConfigType* txPduCfgPtr = &CanIf_ConfigStd->CanIfTxPduConfigRef[(0)];
    for (txPduIndex = 0u; txPduIndex < CanIf_ConfigStd->CanIfTxPduNumber; txPduIndex++)
    {
