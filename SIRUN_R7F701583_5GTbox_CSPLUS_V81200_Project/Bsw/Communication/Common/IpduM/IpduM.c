 ...
 *  V2.0.5    [20250615]  [jianyu.yang]   QAC 2024.3 fix.
 *
 **********************************************************************************************************************/
...
static inline FUNC(PduIdType, IPDUM_CODE) IpduM_GetStartOfContainerTxPduInPartition(uint8 ipduMPartitionId);
#endif /* IPDUM_CONTAINER_TX_NUMBER_MAX >0u */
...
static inline FUNC(PduIdType, IPDUM_CODE) IpduM_GetStartOfContainerRxPduInPartition(uint8 ipduMPartitionId);
#endif /* IPDUM_CONTAINER_RX_NUMBER_MAX > 0u */
...
#endif /* IPDUM_MAX_BUFFER_SIZE > 0u */
#if (IPDUM_TX_REQUEST_NUMBER_MAX > 0u)
        for (index = 0u; index < IPDUM_TX_REQUEST_NUMBER_MAX; index++)
        {
            /* @SWS_IpduM_00223
...
#endif /* IPDUM_TX_REQUEST_NUMBER_MAX > 0u */
#if (IPDUM_CONTAINER_TX_NUMBER_MAX > 0u)
        for (index = 0u; index < IPDUM_CONTAINER_TX_NUMBER_MAX; index++)
        {
...
#endif /* IPDUM_CONTAINER_TX_NUMBER_MAX > 0u */
#if (IPDUM_CONTAINER_RX_NUMBER_MAX > 0u)
        for (index = 0u; index < IPDUM_CONTAINER_RX_NUMBER_MAX; index++)
        {
...
/* Multiplexer tx transmit handle */
            ...
            for (; index < IpduM_GetEndOfMainFunctionTx(mainFunctionId); index++)
            {
...
IpduM_MainFunctionRx(IpduM_MainFunctionType mainFunctionId)
{
    if ((IPDUM_INIT == IpduM_InitStauts) && (mainFunctionId < IPDUM_MAINFUNCTION_RX_NUM_MAX))
    {
        ...
        {
#if (IPDUM_CONTAINER_RX_NUMBER_MAX > 0u)
            IpduM_uintx index = (IpduM_uintx)IpduM_GetStartOfContainerRxPduInPartition(mainFunctionId);
            for (; index < IpduM_GetEndOfMainFunctionRx(mainFunctionId); index++)
            {
...
static inline FUNC(PduIdType, IPDUM_CODE) IpduM_GetStartOfContainerRxPduInPartition(uint8 ipduMPartitionId)
{
    return (((ipduMPartitionId) == 0u) ? 0u : IpduM_CfgPtr->IpduMMainFunctionRxRange[(ipduMPartitionId)-1u]);
}
#else
static inline FUNC(PduIdType, IPDUM_CODE) IpduM_GetStartOfContainerRxPduInPartition(uint8 ipduMPartitionId)
{
...
static inline FUNC(PduIdType, IPDUM_CODE) IpduM_GetStartOfContainerTxPduInPartition(uint8 ipduMPartitionId)
{
    return (((ipduMPartitionId) == 0u) ? 0u : IpduM_CfgPtr->IpduMMainFunctionTxRange[(ipduMPartitionId)-1u]);
}
#else
static inline FUNC(PduIdType, IPDUM_CODE) IpduM_GetStartOfContainerTxPduInPartition(uint8 ipduMPartitionId)
{
