...
static PeripheralTimer_t g_peripheralTimer[2];
                          ...
                          uint8_t hour_bcd, uint8_t minute_bcd, uint8_t second_bcd)
{
    ...
    uint16_t year = 2000 + bcd2dec(year_bcd);      
...
void TimestampToRtcRegisters(uint32_t timestamp, uint8_t regs[7]) {
    ...
    uint32_t remSec = timestamp % 86400;       
    ...
    regs[0] = dec2bcd(second);       
    regs[1] = dec2bcd(minute);       
    regs[2] = dec2bcd(hour);         
    regs[3] = TimerHalWeekdayToRtcMask(timestamp / 86400);       
    regs[4] = dec2bcd(day);           
    regs[5] = dec2bcd(month);         
    uint8_t year_low = year - 2000;   
    regs[6] = dec2bcd(year_low);       
}
...
uint8_t TimerHalSetRtcTime(uint32_t utc)
{
    ...
    LogHalPrint("regs = %02X %02X %02X %02X %02X %02X %02X\n", rtc_regs[6], rtc_regs[5], rtc_regs[4], rtc_regs[3], rtc_regs[2], rtc_regs[1], rtc_regs[0]);
...
uint8_t TimerHalGetRtcTime(uint32_t *pUtc)
{
    ...
    if(ret != 0)
    {
        LogHalPrint("rtc read time fail\r\n");
        return ret;
    }
...
static uint8_t TimerHalConfigDateAlarm(uint32_t utc)
{
    ...
    buf[0] = rtc_regs[1] & 0x7F;    

     
    buf[1] = rtc_regs[2] & 0x7F;    

     
    buf[2] = rtc_regs[4] & 0x7F;    
    ...
    regAddr = 0x0D;
    if (I2cReadRegisterValue(0x32, &regAddr, &ctr1, 1) != 0)
    {
        LogHalPrint("read CTR1 fail\r\n");
        return 1;
    }
    ctr1 |= 0x40;
    regAddr = 0x0D;
    if (I2cWriteRegisterValue(0x32, &regAddr, &ctr1, 1) != 0)
    {
        LogHalPrint("write CTR1 fail\r\n");
        return 1;
    }

     
    regAddr = 0x08;
    if (I2cWriteRegisterValue(0x32, &regAddr, buf, 3) != 0)
    {
        LogHalPrint("write alarm time fail\r\n");
        return 1;
    }

     
    regAddr = 0x0F;
    if (I2cReadRegisterValue(0x32, &regAddr, &ctr2, 1) != 0)
    {
        LogHalPrint("read CTR2 fail\r\n");
        return 1;
    }
    ctr2 |= 0x08;
    regAddr = 0x0F;
    if (I2cWriteRegisterValue(0x32, &regAddr, &ctr2, 1) != 0)
    {
        LogHalPrint("write CTR2 fail\r\n");
        return 1;
    }
...
uint8_t TimerHalPrepareSleep(uint32_t sleepSeconds)
{
    ...
    {
        LogHalPrint("prepare sleep alarm fail, now=%lu sleep=%lu alarm=%lu\r\n", (unsigned long)now, (unsigned long)sleepSeconds, (unsigned long)alarmUtc);
        return 1;
    }

    LogHalPrint("prepare sleep ok, now=%lu sleep=%lu alarm=%lu\r\n", (unsigned long)now, (unsigned long)sleepSeconds, (unsigned long)alarmUtc);
...
{
    uint8_t flag;
    uint8_t regAddr = 0x0E;

    if(I2cReadRegisterValue(0x32, &regAddr, &flag, 1) != 0)
    {
        return 0xFF;
    }

    return ((flag & 0x08) != 0U) ? 1U : 0U;
}
...
void TimerHalClearRtcAlarmFlag(void)
{
    ...
    regAddr = 0x0E;
    if (I2cWriteRegisterValue(0x32, &regAddr, &flag, 1) != 0)
    {
        LogHalPrint("clear INTAF fail\r\n");
    }
}
