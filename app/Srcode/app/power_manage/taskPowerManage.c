...
static boolean g_kl30PassedReportedThisCycle = FALSE;
...
static void KL30_SyncMonitorStateWithDem(void)         /* 当Dem状态已被外部清除时，同步复位KL30本地监控状态 */
{
    ...
    if ((g_kl30HighFlag == FALSE) && (g_kl30LowFlag == FALSE))          
    {
        if (Dem_GetEventStatus(kl30VOLTAGE_DTC_HIGH, &eventStatus) == E_OK)
        {
            if ((eventStatus & (DEM_UDS_STATUS_TF |                     
                                DEM_UDS_STATUS_PDTC |                   
                                DEM_UDS_STATUS_CDTC)) != 0)           
            {
                g_kl30HighFlag = TRUE;                                 
                g_kl30PassedReportedThisCycle = FALSE;                  
                return;                                                 
            }
        }
...
static void KL30_ReportPassedOncePerCycle(void)                         
{
    if (g_kl30PassedReportedThisCycle == TRUE)                          
    {
        return;                                                         
    }
    ...
    if (g_kl30HighFlag == TRUE)                                         
    {
        ...
        g_kl30PassedReportedThisCycle = TRUE;                           
    }
    else if (g_kl30LowFlag == TRUE)                                     
    {
...
static boolean BackupBat_VoltagePassedReportedThisCycle = FALSE;

static boolean BackupBat_IsCharging(void)         /* 判断当前备用电池是否处于充电状态 */
{
    int16_t batState = BatteryHalGetState();
    // TBOX_PRINT("[BAT][STATE] BatteryHalGetState=%d\r\n", batState);

    if (batState == BATTERY_HAL_STATE_CHARGING)
    {
        return TRUE;
    }
    return FALSE;
...
static void BackupBat_VoltageHighAndLowEnableDetectProcess(void)              /* 备用电池高低压DTC使能条件处理函数 */
{
    ...
    BackupBat_LowEnable = ((commonReady == TRUE) &&
                           (BackupBat_IsCharging() == TRUE)) ? TRUE : FALSE;
}
...
static void BackupBat_SyncMonitorStateWithDem(void)
{
    ...
    if ((BackupBat_High_Flag == FALSE) && (BackupBat_Low_Flag == FALSE)) 
    {
        if (Dem_GetEventStatus(DEM_EVT_BACKUP_BAT_HIGH, &eventStatus) == E_OK) 
        {
            if ((eventStatus & (DEM_UDS_STATUS_TF | DEM_UDS_STATUS_PDTC | DEM_UDS_STATUS_CDTC | DEM_UDS_STATUS_TFSLC)) != 0) 
            {
                ...
                BackupBat_High_Cnt = 0; 
                BackupBat_High_Ok_Cnt = 0; 
                BackupBat_VoltagePassedReportedThisCycle = FALSE; 
                return; 
            }
        }
        ...
        if (Dem_GetEventStatus(DEM_EVT_BACKUP_BAT_LOW, &eventStatus) == E_OK) 
        {
            if ((eventStatus & (DEM_UDS_STATUS_TF | DEM_UDS_STATUS_PDTC | DEM_UDS_STATUS_CDTC | DEM_UDS_STATUS_TFSLC)) != 0) /* 低压事件在Dem里仍有历史痕迹 */
            {
...
static void BackupBat_ReportVoltagePassedOncePerCycle(void)
{
    if (BackupBat_VoltagePassedReportedThisCycle == TRUE)
    {
        return;
    }

    if ((BackupBat_High_Flag == TRUE) && (BackupBat_Low_Flag == FALSE))
    {
        Dem_SetEventStatus(DEM_EVT_BACKUP_BAT_HIGH, DEM_EVENT_STATUS_PASSED);
        BackupBat_VoltagePassedReportedThisCycle = TRUE;
    }
    else if ((BackupBat_Low_Flag == TRUE) && (BackupBat_High_Flag == FALSE))
    {
...
void BackupBat_DetectProcess_200ms(void)
{
    ...
    if (BackupBat_HighEnable == TRUE)
    {
        ...
        else if (backupBatVoltage <= BACKUP_BAT_DTC_HIGH_RECOVER_TH_MV)
        {
            ...
            if (BackupBat_High_Flag == TRUE)
            {
                BackupBat_ReportVoltagePassedOncePerCycle();
...
static boolean BackupBat_OpenShortPassedReportedThisCycle = FALSE;
...
static void BackupBat_EnableDetectProcess(void)                                    /* 备用电池开路/短路DTC使能条件检测 */
{
    ...
    BackupBat_Enable = (BackupBat_Can_Cnt >= BACKUP_BAT_CAN_CONFIRM_CNT) ? TRUE : FALSE;
}
...
static void BackupBat_SyncOpenShortMonitorStateWithDem(void)                      
{
    ...
    if ((BackupBat_ShortGnd_Flag == FALSE) && (BackupBat_Open_Flag == FALSE))
    {
        if (Dem_GetEventStatus(DEM_EVT_BACKUP_BAT_SHORT_GND, &eventStatus) == E_OK)
        {
            if ((eventStatus & (DEM_UDS_STATUS_TF | DEM_UDS_STATUS_PDTC | DEM_UDS_STATUS_CDTC | DEM_UDS_STATUS_TFSLC)) != 0)
            {
                ...
                BackupBat_Open_Cnt = 0;
                BackupBat_Normal_Cnt = 0;
                BackupBat_OpenShortPassedReportedThisCycle = FALSE;
                return;
            }
        }
...
static void BackupBat_ReportOpenShortPassedOncePerCycle(void)
{
    if (BackupBat_OpenShortPassedReportedThisCycle == TRUE)
    {
        return;
    }

    if ((BackupBat_ShortGnd_Flag == TRUE) && (BackupBat_Open_Flag == FALSE))
    {
        Dem_SetEventStatus(DEM_EVT_BACKUP_BAT_SHORT_GND, DEM_EVENT_STATUS_PASSED);
        BackupBat_OpenShortPassedReportedThisCycle = TRUE;
    }
    else if ((BackupBat_Open_Flag == TRUE) && (BackupBat_ShortGnd_Flag == FALSE))
    {
...
void BackupBatOpenShort_DetectProcess_200ms(void)                                  
{
    ...
    if (BackupBat_Enable == TRUE)
    {
        ...
        {
            BackupBat_ReportOpenShortPassedOncePerCycle();
...
static void BackupBatAging_SyncStateWithDem(void)
{
    Dem_UdsStatusByteType eventStatus = 0;

    if (Dem_GetEventStatus(DEM_EVT_BACKUP_BAT_AGING, &eventStatus) != E_OK)
    {
        return;
    }
...
void BackupBatAging_DetectProcess(void)
{
    ...
    BackupBatAging_SyncStateWithDem();

    if ((BackupBat_Aging_WakeupFlag == 1) &&
        (BackupBat_Aging_ReqDoneFlag == 0) &&
        (Dtc_IsCommonMonitorEnable() == TRUE))
    {
        BatterySdkRequestAgeCheck();
        BackupBat_Aging_ReqDoneFlag = 1;
    }
...
void PmDebugPrint(void)
{
    ...
    if(pmState != lastPmState)
    {
        ...
        //POWER_LOG_SEND(LOG_LEVEL_INFO, LOG_EVT_INFO, "powerstate is %d,wakesoure is %d,wakecount is %d\n",pmState,wakeupSource,wakeCount);
        lastPmState = pmState;
    }
    if(count < 2000)
    {
