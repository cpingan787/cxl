 ...
 */
/* PRQA S 2991,2992,2995 EOF */ /* VL_CanIf_2991,VL_CanIf_2992,VL_CanIf_2995 */
...
#endif /* STD_ON == CANIF_PUBLIC_DEV_ERROR_DETECT */
    {
...
#endif /* STD_ON == CANIF_PUBLIC_DEV_ERROR_DETECT */
        {
            ...
            if (E_OK == CanIf_CanIdRangCheck(canIfHrhId, canId, &rxPduIndex))
            {
                ...
                if (canId == 0x361u)
                {
                    TBOX_PRINT("[DV] CanIf RxIndication 0x361, len=%d\r\n", (uint8)PduInfoPtr->SduLength);
                    for (uint8_t i = 0; i < (uint8)PduInfoPtr->SduLength; i++)
                    {
                        TBOX_PRINT("0x%02X ", PduInfoPtr->SduDataPtr[i]);
                    }
                    TBOX_PRINT("\r\n");
                    SaveCanMsgToBuffer(CANIF_CONTROLLER_DRV_ID(controllerId), canId, (uint8)PduInfoPtr->SduLength, PduInfoPtr->SduDataPtr);
                }
#endif
                }
                SchM_Enter_CanIf_ExclusiveArea_Channel();
            ...
            {
                /* Handle custom message 0x222 */
                ProcessMsg_222(PduInfoPtr->SduDataPtr, (uint8)PduInfoPtr->SduLength);
            }
#endif /* WDG_TEST_ENABLE == 1 */
#if (DV_TEST_ENABLE == 1)
            else if (canId == 0x361)
            {
                /* Handle DV test message 0x361 for low power mode control */
                SaveCanMsgToBuffer(CANIF_CONTROLLER_DRV_ID(controllerId), canId, (uint8)PduInfoPtr->SduLength, PduInfoPtr->SduDataPtr);
            }
#endif /* DV_TEST_ENABLE == 1 */
        }
    }
}
