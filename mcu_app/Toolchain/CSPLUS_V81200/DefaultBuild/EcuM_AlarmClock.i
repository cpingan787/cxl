...
static  uint8 EcuM_FindAlarmIdByUser(EcuM_UserType user);

 
static  EcuM_TimeType EcuM_FindNextEarliestAlarm(void);
 ...
 Std_ReturnType EcuM_SetClock(EcuM_UserType user, EcuM_TimeType time)
{
    ...
    {
        SchM_Enter_EcuM_GlobalClock();
        if (EcuM_UserCfgs[user].setClkAllowed == 1u)
        {
...
EcuM_UpdateEcuMClock(void)
{
    ...
    if ((1000u) <= EcuM_TimeRecord)
    {
        ...
        {
            (*pRt->Wks.Validated) |= (uint32)((uint32)0x01u << (7));
            pRt->MasterAlarm = EcuM_FindNextEarliestAlarm();
        }
        SchM_Exit_EcuM_GlobalClock();
    }
...
EcuM_CancellAlarms(void)
{
    ...
    for (alarmIdx = 0; alarmIdx < 1; alarmIdx++)
    {
...
static  EcuM_TimeType EcuM_FindNextEarliestAlarm(void)
{
    ...
    boolean firstFlag = 1u;
    uint8 alarmIdx;

    for (alarmIdx = 0; (alarmIdx < 1) && (pRt->UserAlarm[alarmIdx] != pRt->MasterAlarm);
         alarmIdx++)
    {
