...
static my_queue_t queue_pool[5];

static my_mutex_t mutex_pool[8];
static uint8_t osHalInitFlag = 0;

 
volatile uint32_t ulCriticalNesting = 0;

static void OsHalInit(void)
{
    uint32_t i;
    ...
    for(i = 0; i < 5; i++)
    {
        queue_pool[i].state = QUEUE_UNUSED;
        queue_pool[i].item_size = 0;
        queue_pool[i].capacity = 0;
        queue_pool[i].head = 0;
        queue_pool[i].tail = 0;
    ...
    for(i = 0; i < 8; i++)
    {
        mutex_pool[i].state = MUTEX_UNUSED;
...
static my_queue_t *GetQueueHandle(QueueHandle_t xQueue)
{
    uint32_t queueIndex;

    if((xQueue == 0) || (xQueue > 5))
    {
        return ((void *) 0);
    }

    queueIndex = (uint32_t)xQueue - 1;
    if(queue_pool[queueIndex].state != QUEUE_USED)
    {
        return ((void *) 0);
    }

    return &queue_pool[queueIndex];
}
...
uint32_t xQueueSend(QueueHandle_t xQueue, const void *pvItemToQueue, uint32_t xTicksToWait)
{
    ...
    if (queue == ((void *) 0) || pvItemToQueue == ((void *) 0)) {
        if(printFalg == 0)
        {
            printFalg = 1;
        }
...
uint32_t xQueueReceive(QueueHandle_t xQueue, void *pvBuffer, uint32_t xTicksToWait)
{
    ...
    if (queue == ((void *) 0) || pvBuffer == ((void *) 0)) {
        return 0;
    }
...
uint32_t xSemaphoreTake(SemaphoreHandle_t xSemaphore, uint32_t xBlockTime)
{
    ...
    if (mutex == ((void *) 0) || mutex->state == MUTEX_UNUSED) {
        return 0;
    }
