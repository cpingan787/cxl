...
uint8_t Crc8(uint8_t initValue,uint8_t Polynomial,uint8_t *input, int len);
...
volatile uint8_t g_uartReceiveData0[200];
volatile uint8_t g_uartReceiveData1[200];
...
static void MpuHalGpioInit(void)
{
    ...
    Dio_WriteChannel((Dio_ChannelType)22, 0U);
    Dio_WriteChannel((Dio_ChannelType)26, 0U);
    ...
    Dio_WriteChannel((Dio_ChannelType)23, 0U);
    Dio_WriteChannel((Dio_ChannelType)5, 1U);
}
...
static void MpuHalSetPower(uint8_t flag)
{
    if(0==flag)
    {
        Dio_WriteChannel((Dio_ChannelType)19, 0U);
        Dio_WriteChannel((Dio_ChannelType)21, 0U);
    }
    else
    {
        Dio_WriteChannel((Dio_ChannelType)19, 1U);
...
static void MpuHalSetPowerkey(uint8_t flag)
{
    ...
    {
        Dio_WriteChannel((Dio_ChannelType)22, 1U);
    }
}
...
static void MpuHalSetWakeOut(uint8_t flag)
{
    ...
    {
        Dio_WriteChannel((Dio_ChannelType)26, 1U);
    }
}
static void MpuDataDispatch(const uint8_t *pData, uint16_t length, uint8_t irq)
{
    ...
    for (i = 0; i < instanceMax; i++)
    {
        ...
        if ((pHandleInstance->useFlag == 1) && (pHandleInstance->pDataBufferRx != ((void *) 0)))
        {
...
static void MpuUartRingCopyOut(uint8_t *pDst, uint16_t startAddress, uint16_t length)
{
    ...
    if ((pDst == ((void *) 0)) || (length == 0))
    {
        return;
    }

    firstCopyLength = 2048 - startAddress;
    ...
    if ((handle >= 0) && (handle < 15))
    {
        if (pTxMsg != ((void *) 0))
        {
            if (pTxMsg->dataLength < 4000)
            {
                ...
                index = 0;
                memcpy(sTxBuffer + index, packHeader, 8);
                index += 8;
                ...
                index++;
                if (MpuUartTransmit(sTxBuffer, index) != 0)
                {
                    ret = -1;
                }
...
int16_t MpuHalReceive(int16_t handle, MpuHalDataPack_t *pRxMsg, uint32_t waitTime)
{
    ...
    if ((handle >= 0) && (handle < 15))
    {
        ...
        if (pRxMsg != ((void *) 0))
        {
            ...
            if (xQueueReceive(queHandle, &data, waitTime) == 1)
            {
                index = (data >> 16) & 0xFFFF;
                len = data & 0xFFFF;
                if ((pRxMsg->dataBufferSize + 8 + 2) >= len)
                {
...
void UartProtocalProcess(uint8_t *pData, uint16_t dataLength, uint8_t IsrFlag)
{
    ...
    for (i = 0; i < dataLength; i++)
    {
        ...
        else if (pProtocalData->dataCount < 8)
        {
            pProtocalData->data[pProtocalData->dataCount] = pData[i];
            pProtocalData->dataCount++;
            if (8 == pProtocalData->dataCount)
            {
                pProtocalData->dataLength = ((uint16_t)(pProtocalData->data[5]) << 8) + pProtocalData->data[6];
                if (pProtocalData->dataLength > (sizeof(pProtocalData->data) - (8 + 2)))
                {
                    pProtocalData->dataCount = 0;
                    pProtocalData->dataLength = 0;
                }
        ...
        else
        {
            pProtocalData->data[pProtocalData->dataCount] = pData[i];
            pProtocalData->dataCount++;
            if (pProtocalData->dataCount >= (pProtocalData->dataLength + (8 + 2)))
            {
...
void MpuHalTxTaskInit(void)
{
    ...
    g_mpuUartTxBuffer.txQueueHandle = xQueueCreate(20, 
                                                   sizeof(uint32_t));
}
...
void MpuHalTxTask(void)
{
    ...
    
    {
        ...
        if (xQueueReceive(g_mpuUartTxBuffer.txQueueHandle, &queueData, 0) != 1)
        {
            return;
        }
        ...
        if ((length == 0) || (length > 2048) || (length > g_mpuUartTxBuffer.usedSize))
        {
            return;
        }
        ...
        if (R_UART5_Send(g_mpuUartTxBuffer.activeBuffer, length) == ((0x00U) + 0x00U))
        {
...
static int16_t MpuUartTransmit(const uint8_t *pTxData, uint16_t txLength)
{
    ...
    if ((pTxData == ((void *) 0)) || (txLength == 0) || (txLength > 2048))
    {
        return -1;
    }

    if (txLength > (2048 - g_mpuUartTxBuffer.usedSize))
    {
        return -1;
    }
    ...
    g_mpuUartTxBuffer.index = (startAddress + txLength) % 2048;
    g_mpuUartTxBuffer.usedSize += txLength;
    data |= txLength;
    if (xQueueSend(g_mpuUartTxBuffer.txQueueHandle, &data, 0) != 1)
    {
