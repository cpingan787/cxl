...
static const struc_udsParameter m_udsParameter =
    {
        .canChannel = TBOX_CAN_CHANNEL_2, // can channel 1
        .physicalResponseId = 0x76A,
        .physicalRequestId = 0x762,
...
} struct_TpReceiveData;

static struct_TpReceiveData m_physicalRxData;
static struct_TpReceiveData m_functionalRxData;

static volatile uint8_t m_TxMulitFrameFlag = 0;
static volatile uint32_t m_bTxMulitFrameNumber = 0;
static volatile uint32_t m_bTxMulitFrameCount = 0;
static volatile uint32_t m_txWaitTimeoutCounter = 0;
static volatile uint8_t m_txReceiveFCFlag = 0;
static volatile uint8_t m_txBlockSize = 0;
static volatile uint8_t m_txSTmin = 0;
static volatile int32_t m_udsTpTransmitStatus;
...
static const uint32_t g_physicalReceiveCanId = 0x77E;
static const uint32_t g_functionalReceiveCanId = 0x760;
static const uint32_t g_physicalTransmitCanId = 0x7BE;
...
static const Diagnostic_t g_serviceECUFunGroup[] =
    {
        ...
        {0x2E, {SESSION_BIT_DEFAULT | SESSION_BIT_EXTENDED | SESSION_BIT_EXTENDED}, {ADDRESSING_BIT_PHYSICAL | ADDRESSING_BIT_FUNCTION}, Service0x2EProcess},
};

static const uint16_t g_localOnlyDidList_2E[] = {
    0x010D, // ESKey
    0xB2E5, // JtagStatus
};
...
static const WdidInfor_t g_Service2EFunMapList[] =
    {
        ...
        {0xF190, 17, Service2EWriteVIN, E_UDS_NONE_SECURITY_LEVEL},
        {0xF1A1, 17, Service2EWriteVehicleNetworkConfigurationF1A1, E_UDS_SECURITY_LEVEL1},
        ...
        {0x88CC, 20, Service2EWriteSerialNumber, E_UDS_SECURITY_LEVEL1},
        ...
        // {0xF1B2,     4,       Service2EWriteInstallationDate,               E_UDS_SECURITY_LEVEL1},  //Vehicle Manufacturing Date
        // {0xF184,     9,       Service2EWriteAppSoftwareFingerprint,         E_UDS_SECURITY_LEVEL1},
        // {0xF1B9,     4,       Service2EWriteSubnetConfigListOnHighSpeedCan, E_UDS_SECURITY_LEVEL1},

        // {0x0FC0,     16,      Service2EWriteESKey, 			        E_UDS_SECURITY_LEVEL2},  //ESK
        // {0xC110,     1,       Service2EWriteDiagnosticCanReport,    	E_UDS_SECURITY_LEVEL1},  //Diagnostic CAN report
        // {0xC101,     12,      Service2EWriteEOLconfig, 			E_UDS_SECURITY_LEVEL1},

        //{0xCF1E,     16,      Service2EWriteProlinesChannelKey,             E_UDS_SECURITY_LEVEL1},
        //{0xCF1F,     1,       Service2EWritePhoneKeyOffLineUseTimes,        E_UDS_SECURITY_LEVEL1},
        //{0xCF20,     12,      Service2EWriteNFCCardID,                      E_UDS_SECURITY_LEVEL1},
};
...
static const RdidInfor_t g_Service22FunMapList[] =
    {
        ...
        {0xF18C, 27, Service22ReadSerialNumber},                                                  // 电控单元序列号(ECU)
...
static int16_t Service0x10Process(uint8_t *udsData, uint16_t udsLen, uint8_t functionAddressFlag)
{
  ...
  if (0X02 == udsLen)
  {
    ...
    {
      if ((g_currentSession == E_EXTEND_SESSION) || (g_currentSession == E_DEFAULT_SESSION))
      {
...
static uint8_t Service0x27Process0x09(uint8_t *udsData, uint16_t udsLen, uint8_t functionAddressFlag)
{
  uint8_t negativeNum = 0;
  uint8_t responseData[32];
  responseData[0] = udsData[0] + 0x40;
  ...
  else if (g_currentSecurityLevel == E_UDS_SECURITY_LEVEL2)
  {
  ...
  else
  {
    ...
    {
      // if(g_u8ConsecutiveSeedCount[1]==0)
      {
        UdsGetSeed(g_securitySeed, &g_securitySeedLength);
      }
      UdsSeedtoKey(g_securitySeed, g_securitySeedLength, g_securityKey, &g_securityKeyLength, E_SECURITY_CALCULATE_LEVEL2);
      memcpy((responseData + 2), g_securitySeed, g_securitySeedLength);
      DiagnosticDataTransmit(g_tpHandle, g_physicalTransmitCanId, responseData, 2 + g_securitySeedLength, functionAddressFlag);
      g_securitySendSeedLevel = E_UDS_SECURITY_LEVEL2;
    }
    else
    {
      if (g_securityTimerDelayType != E_SECURITYTIMER_ACCESSERRORDELAY)
      {
        negativeNum = 0x36;
        // start delay access timer 180S
        TimerHalStartTime(g_ecuSecurityTimerHandle_2, 180000);
        g_securityTimerDelayType_2 = E_SECURITYTIMER_ACCESSERRORDELAY;
      }
      else
      {
        negativeNum = 0x37;
...
static uint8_t Service0x27Process0x02(uint8_t *udsData, uint16_t udsLen, uint8_t functionAddressFlag)
{
  ...
  if (E_UDS_SECURITY_LEVEL1 == g_securitySendSeedLevel)
  {
    ...
    else
    {
      ...
      // g_currentSecurityLevel = NONE_SECURITY_LEVEL;
    }
...
static uint8_t Service0x27Process0x0A(uint8_t *udsData, uint16_t udsLen, uint8_t functionAddressFlag)
{
  uint8_t negativeNum = 0;
  uint8_t responseData[32];
  responseData[0] = udsData[0] + 0x40;
  ...
  // DiagnosticDataTransmit(g_tpHandle,g_physicalTransmitCanId,&u8Busy7f78CMD[1],3,0);
  if (E_UDS_SECURITY_LEVEL2 == g_securitySendSeedLevel)
  {
    if (ByteArrayCheckIsSame(&udsData[2], g_securityKey, g_securityKeyLength) == 0)
    {
      DiagnosticDataTransmit(g_tpHandle, g_physicalTransmitCanId, responseData, 2, 0);
      g_currentSecurityLevel = E_UDS_SECURITY_LEVEL2;
      // clear seed get state
...
static int16_t DiagnosticResponseProcess(uint8_t *udsData, uint16_t udsLen, uint8_t functionAddressFlag)
{
  ...
  if ((funNum == 0x11) || (funNum == 0x28) || (funNum == 0x2f) || (funNum == 0x31) ||
...
int16_t CanTransmitData(uint32_t canId, uint8_t canExtended, uint8_t *pCanData, uint8_t dlc)
{
  int8_t ret;
  ret = CanHalDiagnosticTransmit(m_udsParameter.canChannel, canId, dlc, pCanData, 0);
  return ret;
}

static void CanUdsTxManagement(uint8_t *canData, uint8_t dataLength, uint8_t functionalAddress)
{
  ...
  m_txBlockSize = canData[1];
  m_txSTmin = canData[2];
  m_txReceiveFCFlag = 1;
}

static void CanUdsRxManagement(uint8_t *canData, uint8_t dataLength, uint8_t functionalAddress)
{
  uint8_t u8FcCmd[8] = {0x30, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00};
  uint8_t lastFrameByte = 0; //?????????????
  volatile struct_TpReceiveData *pTpReceive;

  if (functionalAddress)
  {
    pTpReceive = &m_functionalRxData;
  }
  else
  {
    pTpReceive = &m_physicalRxData;
  }

  if (((canData[0] & 0xF0) == 0x00) && (pTpReceive->rxMulitFrameFlag == false)) // single frame
  {
    // UpgradeFlag = 1;
    // Timeout_counter = 0;
    // WaitTimeout_counter = 0;
    ...
    // TBOX_PRINT("scan %x %x %x %x %x %x %x %x",canData[0],canData[1],canData[2],canData[3],canData[4],canData[5],canData[6],canData[7]);
  }
  else
  {
    if ((0x10 == (canData[0] & 0xF0)) && (pTpReceive->rxMulitFrameFlag == false)) // fist frame of the multi frame
    {
      ...
      u8FcCmd[1] = m_udsParameter.blockSize;
      u8FcCmd[2] = m_udsParameter.STmin;
      u8FcCmd[3] = m_udsParameter.fillByte;
      u8FcCmd[4] = m_udsParameter.fillByte;
      u8FcCmd[5] = m_udsParameter.fillByte;
      u8FcCmd[6] = m_udsParameter.fillByte;
      u8FcCmd[7] = m_udsParameter.fillByte;
      CanTransmitData(m_udsParameter.physicalResponseId, m_udsParameter.canIdExtended & 0x01, u8FcCmd, 8);
      ...
      if (pTpReceive->rxMulitFrameNumber)
      {
        if ((canData[0] & 0xF0) == 0x20) // consecutive frame
        {
          ...
          else
          {
            ...
            pTpReceive->rxMulitFrameFlag = false;
            pTpReceive->receiveFlag = 1;
            // TBOX_PRINT("mcan %x %x %x %x %x %x %x %x",canData[0],canData[1],canData[2],canData[3],canData[4],canData[5],canData[6],canData[7]);
          }
        }
      }
      else
...
static void CanRxCallback(uint8_t u8CANChannel, uint32_t rxId, uint8_t *canData, uint8_t dlc)
{
  uint32_t canId;
  uint8_t extended;

  canId = rxId & 0x7FFFFFFF;
  extended = (rxId >> 31) & 0x01;

  if (m_udsParameter.canChannel == u8CANChannel)
  {
    if (m_udsParameter.canIdExtended == extended)
    {
      if ((canId) == m_udsParameter.physicalRequestId)
      {

        if (m_TxMulitFrameFlag)
        {
          CanUdsTxManagement(canData, dlc, 0);
        }
        else
        {
          CanUdsRxManagement(canData, dlc, 0);
        }
      }
      else if (canId == m_udsParameter.functionalRequestId)
      {
        if (m_TxMulitFrameFlag)
        {
          CanUdsTxManagement(canData, dlc, 1);
        }
        else
        {
          CanUdsRxManagement(canData, dlc, 1);
        }
      }
    }
  }
...
int16_t TaskEcuDiagnosticInit(void)
{
  ...
  CanHalSetReceiveCallBack(CanRxCallback);
  return 0;
}
...
void TaskEcuDiagnostic(void *pvParameters)
{
  ...
  g_tpHandle = CanTpSdkInitialize(TBOX_CAN_CHANNEL_D, &g_tpParameter, &g_tpBuffer);

  if (g_tpHandle < 0)
  {
    return;
  }
  ServiceTestSetDiagnosticCan(TBOX_CAN_CHANNEL_D);
  ...
  while (1)
  {
    ...
    if (0 == ret)
    {
      if (g_sleepFlag == 0)
      {
        ...
        if (g_currentSession == E_PROGROM_SESSION)
        {
          UdsServiceProcess(g_udsRxData, g_udsRxDataLen, functionalFlag);
        }
        else
        {
    ...
    //     if (g_currentSession == E_PROGROM_SESSION)
    //     {
    //         UdsServiceProcess(g_udsRxData, g_udsRxDataLen, 0);
    //     }
    //     else
    //     {
...
static int8_t FlashCheckSumIsOk(uint32_t checkSum) // 0x31?????hecksum
{
  ...
  g_flashCrc32Value = Crc32(&g_crc32Object, g_flashCrc32Value, (uint8_t *)FLASH_APP_BANKB_BASE_ADDRESS, g_flashSize);
  g_flashCrc32Value ^= 0xFFFFFFFF;
