...
#define CANFD1_RX_PORT                PORT_E
#define CANFD1_RX_PIN                 GPIO_11
#define CANFD1_TX_PORT                PORT_D
#define CANFD1_TX_PIN                 GPIO_1
#define CANFD1_RX_MUX                 PTE11_CAN0_RX
#define CANFD1_TX_MUX                 PTD1_CAN0_TX
#define CANFD1_IRQ_MUX                PTE11_GPIO
#else
...
#define CANFD4_TYPE                   CY_CANFD1_1_TYPE   
#define CANFD4_RX_PORT                GPIO_PRT17
#define CANFD4_RX_PIN                 1 
#define CANFD4_TX_PORT                GPIO_PRT17
#define CANFD4_TX_PIN                 0 
#define CANFD4_RX_MUX                 P17_1_CANFD1_TTCAN_RX1 
#define CANFD4_TX_MUX                 P17_0_CANFD1_TTCAN_TX1 
#define CANFD4_PCLK                   PCLK_CANFD1_CLOCK_CAN1
#define CANFD4_IRQN                   canfd_1_interrupts0_1_IRQn

#define CANFD5_TYPE                   CY_CANFD1_2_TYPE   
#define CANFD5_RX_PORT                GPIO_PRT18
#define CANFD5_RX_PIN                 7 
#define CANFD5_TX_PORT                GPIO_PRT18
#define CANFD5_TX_PIN                 6 
#define CANFD5_RX_MUX                 P18_7_CANFD1_TTCAN_RX2 
#define CANFD5_TX_MUX                 P18_6_CANFD1_TTCAN_TX2 
#define CANFD5_PCLK                   PCLK_CANFD1_CLOCK_CAN2
#define CANFD5_IRQN                   canfd_1_interrupts0_2_IRQn

#define CANFD6_TYPE                   CY_CANFD1_0_TYPE   
#define CANFD6_RX_PORT                GPIO_PRT14
#define CANFD6_RX_PIN                 1 
#define CANFD6_TX_PORT                GPIO_PRT14
#define CANFD6_TX_PIN                 0 
#define CANFD6_RX_MUX                 P14_1_CANFD1_TTCAN_RX0 
#define CANFD6_TX_MUX                 P14_0_CANFD1_TTCAN_TX0 
#define CANFD6_PCLK                   PCLK_CANFD1_CLOCK_CAN0
#define CANFD6_IRQN                   canfd_1_interrupts0_0_IRQn
#endif

#ifndef EVB_BOARD_TEST
#define CAN0FD_STB_PORT    PORT_E
#define CAN0FD_STB_PIN     GPIO_13
#define CAN0FD_STB_PIN_MUX PTE13_GPIO
#else
#define CAN0FD_STB_PORT    PORT_B
#define CAN0FD_STB_PIN     GPIO_2
#define CAN0FD_STB_PIN_MUX PTB2_GPIO
#endif

#define CAN1FD_STB_PORT    PORT_A
#define CAN1FD_STB_PIN     GPIO_14
#define CAN1FD_STB_PIN_MUX PTA14_GPIO

#define CAN2FD_STB_PORT    PORT_B
#define CAN2FD_STB_PIN     GPIO_12
#define CAN2FD_STB_PIN_MUX PTB12_GPIO
...
#else
#define CAN0_IRQ_MUX PTB0_GPIO
#endif
#define CAN1_IRQ_MUX PTA12_GPIO
#define CAN2_IRQ_MUX PTD4_GPIO
#if(CAN_CHANNEL_NUMBER_MAX == 6)
#define CAN3_IRQ_MUX P14_1_GPIO
#define CAN4_IRQ_MUX P17_1_GPIO
#define CAN5_IRQ_MUX P18_7_GPIO
#endif
typedef enum  
{
...
static can_tx_msg_buffer_t g_can0TxBuffer;
static can_tx_msg_buffer_t g_can1TxBuffer;
...
// CAN_MsgBuf_t CAN0_RxBuf;			//定义CAN0接收使用的缓存变量
// CAN_MsgBuf_t CAN1_RxBuf;			//定义CAN1接收使用的缓存变量
// CAN_MsgBuf_t CAN2_RxBuf;			//定义CAN2接收使用的缓存变量

// unsigned int CAN0_RxIntFlag;		//定义CAN0接收一帧数据完成标志
// unsigned int CAN1_RxIntFlag;		//定义CAN1接收一帧数据完成标志
// unsigned int CAN2_RxIntFlag;		//定义CAN2接收一帧数据完成标志
...
void CAN0_Init(uint8_t canIndex,uint8_t canfdFlag,CanBaudType_e idBandrate,CanBaudType_e dataBandrateRate)
{
    ...
    PORT_PinmuxConfig(CAN0FD_STB_PORT, CAN0FD_STB_PIN, CAN0FD_STB_PIN_MUX);	//设置 为GPIO功能。为 TJA1042 的 STB
	GPIO_SetPinDir(CAN0FD_STB_PORT, CAN0FD_STB_PIN, GPIO_OUTPUT);	//设置 为输出功能，用作TJA1042 的 STB
	GPIO_WritePinOutput(CAN0FD_STB_PORT, CAN0FD_STB_PIN, GPIO_HIGH);	//设置 输出高电平，STB 设为高电平，JA1042 为 Standby 状态
	...
	*/
    ...
    g_can0TxBuffer.txCount = 0;
    g_can0TxBuffer.txIndexIn = 0;
    g_can0TxBuffer.txIndexOut = 0;
    ...
    case 1:
        ...
        */
        ...
        g_can1TxBuffer.txCount = 0;
        g_can1TxBuffer.txIndexIn = 0;
        g_can1TxBuffer.txIndexOut = 0;
...
void CAN0_HOST_MEM_ERR_ISR(void)
{
	...
	for(i=0; i<14; i++)
	{
		CAN1RW->CAN_MB[i].MB0 = 0x00000000;
		CAN1RW->CAN_MB[i].MB1 = 0x00000000;
		CAN1RW->CAN_MB[i].MB2 = 0x00000000;
		CAN1RW->CAN_MB[i].MB3 = 0x00000000;
	}
...
static CanRxCallBackFun_t *m_pCanRxCallFun = NULL;
void CanHalSetReceiveCallBack(CanRxCallBackFun_t fun)
{
	m_pCanRxCallFun = fun;
}
 ...
 * @return none
 */
static void CAN_HandleMsg(uint8_t instance, const stc_canfd_msg_t *msgInfo)
{
    // uint8_t i = 0;
    // CanRxInterruptProcessMsg(instance, msgInfo->ID, msgInfo->DATA, msgInfo->DLC, msgInfo->FDF, 0);
		if(m_pCanRxCallFun!=NULL)
		{
			m_pCanRxCallFun(instance, msgInfo->canId, (uint8_t *)msgInfo->data, msgInfo->dlc);
		}
}
/*****************************************************************************
 * 函数: CAN0_MB0TO15_ISR
...
void CAN0_MB0TO15_ISR(uint32_t mbIdx)
{
    ...
    if(mbIdx <= 4)
    {
        if(g_can0TxBuffer.txCount>0)
        {
            CAN_Send(CAN_ID_0, g_canControllerTxBufferIndex[0], &g_Can0TxRxInfo, g_can0TxBuffer.canMsg[g_can0TxBuffer.txIndexOut].canId, g_can0TxBuffer.canMsg[g_can0TxBuffer.txIndexOut].data, 0);	//使用邮箱2发送报文
                //参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
            g_canControllerTxBufferIndex[0]++;
            if(g_canControllerTxBufferIndex[0] >= 5)
            {
                g_canControllerTxBufferIndex[0] = 0;
            }

            g_can0TxBuffer.txCount--;
            g_can0TxBuffer.txIndexOut++;
            if(g_can0TxBuffer.txIndexOut >= CAN_TX_BUFFER_SIZE)
            {
                g_can0TxBuffer.txIndexOut = 0;
            }
        }
    ...
    CAN_GetMsgBuff(CAN_ID_0, mbIdx, &can0RxBuf);	//读取指定邮箱(MB)中的数据，放入 can0RxBuf 中
		//参数：CAN模块序号、邮箱序号、存储数据的数组
...
void CAN1_MB0TO15_ISR(uint32_t mbIdx)
{
    ...
    if(mbIdx <= 4)
    {
        if(g_can1TxBuffer.txCount>0)
        {
            CAN_Send(CAN_ID_1, g_canControllerTxBufferIndex[0], &g_Can1TxRxInfo, g_can1TxBuffer.canMsg[g_can1TxBuffer.txIndexOut].canId, g_can1TxBuffer.canMsg[g_can1TxBuffer.txIndexOut].data, 0);	//使用邮箱2发送报文
                //参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
            g_canControllerTxBufferIndex[1]++;
            if(g_canControllerTxBufferIndex[1] >= 5)
            {
                g_canControllerTxBufferIndex[1] = 0;
            }

            g_can1TxBuffer.txCount--;
            g_can1TxBuffer.txIndexOut++;
            if(g_can1TxBuffer.txIndexOut >= CAN_TX_BUFFER_SIZE)
            {
                g_can1TxBuffer.txIndexOut = 0;
            }
        }
    ...
    {
        stc_canfd_msg_t stcCanFDmsg = {0};
        //TBOX_PRINT("Can Interrupt!\n");
        CAN_GetMsgBuff(CAN_ID_1, mbIdx, &can1RxBuf);	//读取指定邮箱(MB)中的数据，放入 can1RxBuf 中
            //参数：CAN模块序号、邮箱序号、存储数据的数组
...
void CAN2_MB0TO15_ISR(uint32_t mbIdx)
{
    ...
    CAN_GetMsgBuff(CAN_ID_2, mbIdx, &can2RxBuf);	//读取指定邮箱(MB)中的数据，放入 can2RxBuf 中
		//参数：CAN模块序号、邮箱序号、存储数据的数组
...
ResultStatus_t CAN0_SendNM(unsigned int msg_id, unsigned char *dataP)
{
	...
	//发送 CAN 报文
	can0_Ret = CAN_Send(CAN_ID_0, 0, &g_Can0TxRxInfo, msg_id, dataP, 0);	//优先使用MB2发送报文
				//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
...
ResultStatus_t CAN1_SendNM(unsigned int msg_id, unsigned char *dataP)
{
    ...
    //发送 CAN 报文
    can1_Ret = CAN_Send(CAN_ID_1, 0, &g_Can1TxRxInfo, msg_id, dataP, 0);	//优先使用MB2发送报文
                //参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
...
ResultStatus_t CAN2_SendNM(unsigned int msg_id, unsigned char *dataP)
{
    ...
    //发送 CAN 报文
    can2_Ret = CAN_Send(CAN_ID_2, 0, &g_Can2TxRxInfo, msg_id, dataP, 0);	//优先使用MB2发送报文
                //参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
...
ResultStatus_t CAN0_SendDiag(unsigned int msg_id, unsigned char *dataP)
{
	ResultStatus_t can0_Ret;		//定义CAN0 发送函数的返回变量
	//发送 CAN 报文
	can0_Ret = CAN_Send(CAN_ID_0, 1, &g_Can0TxRxInfo, msg_id, dataP, 1);	//优先使用MB2发送报文
				//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
...
ResultStatus_t CAN0_SendApp(unsigned int msg_id, unsigned char *dataP)
{
	ResultStatus_t can0_Ret;		//定义CAN0 发送函数的返回变量
	//发送 CAN 报文
	can0_Ret = CAN_Send(CAN_ID_0, 2, &g_Can0TxRxInfo, msg_id, dataP, 2);	//优先使用MB2发送报文
				//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
	if(can0_Ret == BUSY)	//如果MB2为忙状态，使用MB3发送报文
	{
		can0_Ret = CAN_Send(CAN_ID_0, 3, &g_Can0TxRxInfo, msg_id, dataP, 3);//使用MB3发送报文
					//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
	}
	
	if(can0_Ret == BUSY)	//如果MB3为忙状态，使用MB4发送报文
	{
		can0_Ret = CAN_Send(CAN_ID_0, 4, &g_Can0TxRxInfo, msg_id, dataP, 4);//使用MB3发送报文
					//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
	}
...
static int16_t CanHalSetCANMode(uint8_t u8Channel, uint8_t mode)
{
    ...
    switch(u8Channel)
    {
        case 0:
        {
            ...
            {
                GPIO_ClearPinOutput(CAN0FD_STB_PORT,CAN0FD_STB_PIN);
            }
            else
            {
                GPIO_SetPinOutput(CAN0FD_STB_PORT,CAN0FD_STB_PIN);// Go-to-Sleep mode
            }
        }
        break;
        case 1:
...
int16_t CanHalSetMode(uint8_t mode)
{
  ...
  if(mode == 0)
  {
    ...
    if(g_canWakeUpFlag[2])
    {
        GPIO_SetPinDir(CANFD3_RX_PORT, CANFD3_RX_PIN, GPIO_INPUT);
        // PORT_PullConfig(CANFD3_RX_PORT, T2G_CANFD1_RX_PIN, PORT_PULL_DOWN);
        PORT_PinIntConfig(CANFD3_RX_PORT, CANFD3_RX_PIN, PORT_ISF_INT_FALLING_EDGE);
        INT_SetPriority(CANFD3_IRQN, 0x3);
        INT_EnableIRQ(CANFD3_IRQN);
    }
#if(CAN_CHANNEL_NUMBER_MAX == 6)
...
{
    CAN_Id_t pstCanType;
    can_tx_msg_buffer_t *pTxBuffer = NULL;
    ...
    case 0:
      pstCanType = CAN_ID_0;
      pTxBuffer = &g_can0TxBuffer;
      pCAN_MessageInfo = &g_Can0TxRxInfo;
      break;
    case 1:
      pstCanType = CAN_ID_1;
      pTxBuffer = &g_can1TxBuffer;
...
#if 0
    can0SendRet = CAN_Send(pstCanType, g_canControllerTxBufferIndex[u8Channel], pCAN_MessageInfo, id, pu8CmdData, 0);	//使用邮箱2发送报文
				//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级

    g_canControllerTxBufferIndex[u8Channel]++;
    if(g_canControllerTxBufferIndex[u8Channel]>=5)
    {
...
#else
    if(pTxBuffer->txCount>=CAN_TX_BUFFER_SIZE)
    {
        COMMON_ENABLE_INTERRUPTS();
        return CAN_ERROR_TX_BUFFER_FULL;
    }

    if(pTxBuffer->txCount==0)
    {
        can0SendRet = CAN_Send(pstCanType, g_canControllerTxBufferIndex[u8Channel], pCAN_MessageInfo, id, pu8CmdData, 0);	//使用邮箱2发送报文
            //参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
        ...
        if(can0SendRet != 0)
        {
            ...
            // if(can0SendRet == BUSY)
            // {
            //     // 对于BUSY状态，主动尝试重置当前使用的邮箱
            //     CAN_ClearMbIntStatus(pstCanType, g_canControllerTxBufferIndex[u8Channel]);
            //     // 将邮箱配置为非激活状态，以便下次可以使用
            //     CAN_ConfigTxMb(pstCanType, g_canControllerTxBufferIndex[u8Channel], pCAN_MessageInfo, 0x000, 0);
            // }
            
            // 将消息添加到发送缓冲区
            pTxBuffer->canMsg[pTxBuffer->txIndexIn].canId = id;
            pTxBuffer->canMsg[pTxBuffer->txIndexIn].dlc = u8Len;
            memcpy(pTxBuffer->canMsg[pTxBuffer->txIndexIn].data,pu8CmdData,u8Len);
            pTxBuffer->txIndexIn++;
            if(pTxBuffer->txIndexIn>=CAN_TX_BUFFER_SIZE)
            {
                pTxBuffer->txIndexIn = 0;
            }
            pTxBuffer->txCount++;
        }
    }
...
static void CanControllerReset1(uint8_t canChannel)
{
    volatile CAN_Id_t pstCanType; 
    // get can type
...
static int16_t CanControllerBusOffError(uint8_t canChannel)
{
    ...
    {
    case 0:
        // busErr = (CAN_GetESR1BufForCbf(CAN_ID_0) >> 2) & 0x01;
        busErr = CAN0_BusOffIntFlag;
        CAN0_BusOffIntFlag = 0;
        break;
    case 1:
        // busErr = (CAN_GetESR1BufForCbf(CAN_ID_1) >> 2) & 0x01;
        break;
    case 2:
...
int16_t CanHalDiagnosticTransmit(int16_t canHandle,uint32_t canId,uint8_t dlc,uint8_t *canData,uint8_t fdFlag)
{
    uint8_t canChannel;
    int16_t ret;
    if(canHandle<0)
...
static void CanControllerReset(uint8_t canChannel)
{
    ...
    {
        case 0:
            // CAN0_Init(0,1,E_CAN_500K,E_CAN_500K);
            // CAN_RecoverFromBusOffManually(CAN_ID_0);
            break;
        case 1:
            // CAN0_Init(1,1,E_CAN_500K,E_CAN_500K);
            break;
        case 2:
            // CAN0_Init(2,1,E_CAN_500K,E_CAN_500K);
            break;
#if(CAN_CHANNEL_NUMBER_MAX == 6)
//     case 3://ECAN
//             pstCanType = &T2G_CANFD4_TYPE->M_TTCAN;
...
void CanHalTestMain(void)
{
    ...
    // while(1)
	{
		...
		//发送 CAN 报文的举例
		can0SendRet = CAN_Send(CAN_ID_0, 2, &g_Can0TxRxInfo, 0x389, CAN0_TxData, 0);	//使用邮箱2发送报文
					//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
		if(can0SendRet == BUSY)	//CAN0 发送函数的返回变量
		{
			can0SendRet = CAN_Send(CAN_ID_0, 3, &g_Can0TxRxInfo, 0x389, CAN0_TxData, 1);	//使用邮箱3发送报文
						//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
		}
		if(can0SendRet == BUSY)	//CAN0 发送函数的返回变量
		{
			can0SendRet = CAN_Send(CAN_ID_0, 4, &g_Can0TxRxInfo, 0x389, CAN0_TxData, 2);	//使用邮箱4发送报文
						//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
		}

		// COMMON_Delay_ms(100);	//延时100毫秒
		can0SendRet = CAN_Send(CAN_ID_0, 3, &g_Can0TxRxInfo, 0x589, CAN0_TxData, 3);	//使用邮箱3发送报文
					//参数：CAN模块序号、邮箱序号、信息类型数组、信息ID、存储数据的数组、本地优先级
