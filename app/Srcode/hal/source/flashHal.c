...
static int WorkFlashWaitUntilRdy(uint8_t delayWaitFlag)
{
    uint32_t count;
    count = 0;
...
int16_t FlashHalDataBlockWrite(uint32_t blockAddress,uint32_t writeOffset,const uint8_t *pWriteData,uint32_t writeLength)
{
    ...
    g_workFlashBuffer.u8Data[4094] = newSectorFlag;      
    ...
    for(i=0;i<1024;i++)
    {
      Cy_FlashWriteWork(alternativeAddress+i*4,&g_workFlashBuffer.u32Data[i] , CY_FLASH_DRIVER_NON_BLOCKING);
...
int16_t FlashHalSmallDataBlockWrite(uint32_t blockAddress,uint32_t writeOffset,const uint8_t *pWriteData,uint32_t writeLength)
{
    ...
    for(i=0;i<writeLength;i++)
    {
        g_workFlashBuffer.u8Data[i] = pWriteData[i];
    }   
    ...
    {
      Cy_FlashWriteWork(blockAddress + i*4,&g_workFlashBuffer.u32Data[i] , CY_FLASH_DRIVER_NON_BLOCKING);
      WorkFlashWaitUntilRdy(0);
    }	
    // Disable write access
