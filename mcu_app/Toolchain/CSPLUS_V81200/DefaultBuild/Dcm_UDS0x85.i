...
static  Std_ReturnType Dcm_Uds0X85ServiceConditionCheck(
    uint8 ProtocolCtrlId,
    Dcm_NegativeResponseCodeType * ErrorCode)
{
    uint8 MsgCtrlId;
    ...
    if (((1u) != Dcm_MsgCtrl[MsgCtrlId].Subfunction)
        && ((2u) != Dcm_MsgCtrl[MsgCtrlId].Subfunction))
    {
    ...
    if (0U == ret)
    {
        ...
        ret = DsdInternal_SubSesCheck(ProtocolCtrlId, 0x85u, ErrorCode);
    }
    if (0U == ret)
    {
         
        ret = DsdInternal_SubSecurityCheck(ProtocolCtrlId, 0x85u, ErrorCode);
    }
    if (0U == ret)
    {
        {
    ...
    Dcm_NegativeResponseCodeType * ErrorCode)
{
    ...
    uint32 Offset;
    Std_ReturnType ret;
    Dem_ReturnControlDTCSettingType returnCtrlDtcSetting = 0x00u;
    const Dcm_MsgContextType* pMsgContext;
    const Dcm_DslBufferType* pDcmChannelCfg;
    ...
    if (0U == ret)
    {
        ...
        if ((2U) == pMsgContext->ReqDataLen)
        {
            ...
            if ((1u) == subFunc)
            {
                ...
                returnCtrlDtcSetting = Dem_DcmEnableDTCSetting(0xFFFFFFUL, 0x01u);
                if (0x00u == returnCtrlDtcSetting)
                {
                    (void)SchM_Switch_DcmControlDTCSetting(((Rte_ModeType_DcmControlDTCSetting)0x00u));
                }
            }
            if ((2u) == subFunc)
            {
                 
                returnCtrlDtcSetting = Dem_DcmDisableDTCSetting(0xFFFFFFUL, 0x01u);
            }
        }
        else
        {
