...
const FlashParamMap_t FlashParamMap[E_PARAMETER_INFO_MAX_NUM] = {
    {E_PARAMETER_INFO_ICCID,                20,     10U,   ((void *) 0)},
    {E_PARAMETER_INFO_IMEI,                 24,     10U,   ((void *) 0)},
    {E_PARAMETER_INFO_SN,                   16,     6U,   NvMBlockRamBuffer6},
    {E_PARAMETER_INFO_VIN,                  17,     7U,   NvMBlockRamBuffer7},
    {E_PARAMETER_INFO_TSPAddr,              32,     17U,   NvMBlockRamBuffer17},
    {E_PARAMETER_INFO_TSPPort,              6,      19U,   NvMBlockRamBuffer19},
    {E_PARAMETER_INFO_ECallNumber,          15,     10U,   ((void *) 0)},
    {E_PARAMETER_INFO_ManufactureData,      3,      5U,   NvMBlockRamBuffer5},
    {E_PARAMETER_INFO_ParatNumber,          5,      10U,   ((void *) 0)},
    {E_PARAMETER_INFO_SupIdentifier,        5,      10U,    ((void *) 0)},
...
int16_t WorkFlashVehicleInforStore(FlashParaId_e parameterId, uint8_t *data, uint32_t dataLength)
{
    ...
    if ((ret != 0) || (dataLength > paramInfo.length) || paramInfo.address == ((void *) 0)) 
    {
        ret = (-2);
        return ret;
    }
...
int16_t WorkFlashVehicleInforRead(FlashParaId_e parameterId, uint8_t *data, uint32_t *dataLength)
{
    ...
    ret = VehicleInforGetDataOffsetAddressAndLength(parameterId, &paramInfo);
    if (ret != 0 || paramInfo.length == 0 || paramInfo.address == ((void *) 0))
    {
        ret = (-2);
        return ret;
    }
...
void PrintHexArray(const uint8_t *data, uint32_t length, const char *prefix)
{
    static char logBuffer[384];   
    uint32_t i = 0;
    uint32_t offset = 0;
    uint32_t printLen = (length > 128) ? 128 : length;   
