 ...
 *                              improved.
 * V2.3.3  08-Jul-2019   :  Following changes are made:
 *                          1. ARDAABD-3760 - In Fls_ProcessReadImm() API,
 *                          The assignment operation of 'LucErrorStatus'
 *                          changes from "=" to "|=".
 * V2.3.4  14-Dec-2020   :  Following changes are made:
 *                          1. As part of ARDAABD-4906
 *                            Fixed an issue where the FLS write command
 *                            could not be completed and the Flash state
 *                            would get stuck on MEMIF_BUSY in FAST mode.
 *                            Corrected the discription of the function
 *                            Fls_DisableFaciEndInt().
 *                          2. As part of ARDAABD-4965
 *                            Removed critical section in Fls_ClearBackUpData.
 *                          3. As part of ARDAABD-4958
 *                            Added dummy read and SYNCP processing between
 *                            write operation of PDMAnDCENm and FPSYS
 *                            registers in Fls_InitiateDmaWriteJob function.
 *                            Added dummy read and SYNCP processing between
 *                            write operation of INTC2 and FPSYS
 *                            registers in Fls_ProcessJobResult function.
 *                          4. As part of ARDAABD-4063
 *                            Added the check process of
 *                            Fls_UseFlashControllerPreHook and
 *                            Fls_UseFlashControllerEndHook API to
 *                            Fls_MainErase/Fls_MainWrite/Fls_MainRead/
 *                            Fls_MainBlankCheck internal function.
 *                            Added the reference UD ID in comment.
 * V2.3.5  31-Mar-2021   :  As part of ARDAABD-5040, the following changes are
 *                          made:
 *                          a) Critical section protection added in functions
 *                             Fls_PerformBlankCheckForReadOp,
 *                             Fls_ProcessReadImm and Fls_SetFHVE.
 *                          b) 'Function(s) invoked' field is updated in the
 *                             function banner of
 *                             Fls_PerformBlankCheckForReadOp,
 *                             Fls_ProcessReadImm and Fls_SetFHVE.
 *                          c) Copyright information updated.
 * V2.3.6  12-Apr-2021   :  As part of review based on ARDAABD-5040, requirement
 *                          EAAR_PN0034_FR_0013 is mapped in newly added
 *                          Critical section protection in functions
 *                          Fls_PerformBlankCheckForReadOp,
 *                          Fls_ProcessReadImm and Fls_SetFHVE.
 * V2.3.7  03-Aug-2021   :  As part of ARDAABD-6326, the following changes are
 *                          made:
 *                          a) Function 'Fls_ProcessReadImm' is updated to
 *                             notify ECC errors reported from
 *                             'Fls_FcuOffsetDataCopy' function.
 *                          b) A local variable 'LucOffsetErrorStatus' is added
 *                             to store the Data Flash error status register
 *                             value returned from 'Fls_FcuOffsetDataCopy'
 *                             function.
 * V2.3.8  17-Aug-2021   :  As part of review based on ARDAABD-6326, the
 *                          following changes are made:
 *                          a) Updated the condition for checking ECC Status
 *                             Register for Read Errors in 'Fls_ProcessReadImm'
 *                             function.
 *                          b) 'Fls_ProcessReadImm' function is updated to
 *                             add a check when no error is returned from
 *                             'Fls_FcuOffsetDataCopy' function and error from
 *                             'Fls_FcuOffsetDataCopy' shall be copied to
 *                             LucErrorStatus only when LucErrorStatus is either
 *                             no error or SED.
 *                          c) In 'Fls_ProcessReadImm' function, the check to
 *                             update LenStatus and LblSECError is updated
 *                             such that LenStatus is updated as
 *                             FLS_FCU_ERR_ECC_SED only if LucErrorStatus is
 *                             FLS_FCU_REGBIT_DFERSTR_SEDF else it is updated as
 *                             FLS_FCU_ERR_ECC_DED inorder to notify DED if both
 *                             DED and SED occurs.
 * V2.3.9  24-Aug-2021   :  As part of ARDAABD-6991, APIs
 *                          Fls_DisableDmaChannel, Fls_ProcessJobResult,
 *                          Fls_DisableFaciEndInt, Fls_InitiateDmaWriteJob
 *                          and Fls_EnableFaciEndInt are updated such that
 *                          ICRxxx register will be accessed in 8 bits when
 *                          setting MKxxx bit using 'RH850_SV_MODE_ICR_xxx'
 *                          macros.
 * V2.3.10 14-Sep-2021   :  As part of review based ARDAABD-6991, the following
 *                          changes are made:
 *                          a) Updated 'Registers Used' for the functions
 *                             Fls_MainErase, Fls_InitiateDmaWriteJob,
 *                             Fls_DisableDmaChannel, Fls_MainWrite,
 *                             Fls_MainRead, Fls_MainBlankCheck,
 *                             Fls_ProcessJobResult, Fls_ProcessResume and
 *                             Fls_TimeOutCheckAndProcessing to add the register
 *                             ICDMAm.
 *                          b) Updated 'Registers Used' for the function
 *                             Fls_DmaIsr to add the register ICFLENDNM.
 *                          c) Removed Local variable LusRegvalue and updated
 *                             functions Fls_DisableFaciEndInt and
 *                             Fls_EnableFaciEndInt
 * V2.3.11 22-Sep-2021   :  As part of review based on ARDAABD-6326, the
 *                          following changes are made:
 *                          a) Updated the function Fls_ProcessReadImm to report
 *                             FLS_FCU_ERR_ECC_DED in-order to notify DED or
 *                             both DED and SED occurs and to report
 *                             FLS_FCU_ERR_ECC_SED if LucErrorStatus is
 *                             FLS_FCU_REGBIT_DFERSTR_SEDF.
 *                          b) Updated the function Fls_ProcessReadImm
 *                             to remove LucOffsetErrorStatus and the return
 *                             value of the function Fls_FcuOffsetDataCopy is
 *                             stored in the variable LucErrorStatus.
 * V2.3.12 20-Apr-2022   :  As part of ARDAABD-4786, the following changes
 *                          are made:
 *                          a) Updated function 'Fls_MainErase' to remove
 *                             DET Runtime and DEM error 'FLS_E_ERASE_FAILED'
 *                             reported for PE/user mode switching failure
 *                             while initiating erase and verifying erase
 *                             operation.
 *                          b) Updated function 'Fls_MainWrite' to remove
 *                             DET Runtime and DEM error 'FLS_E_WRITE_FAILED'
 *                             reported for PE/user mode switching failure
 *                             while initiating write operation.
 *                          c) Updated function 'Fls_ProcessJobResult' to report
 *                             DET Runtime and DEM error 'FLS_E_ERASE_FAILED'
 *                             and 'FLS_E_WRITE_FAILED' for PE/user mode
 *                             switching failure while initiating/resuming
 *                             erase/write operation.
 *                          d) Updated function 'Fls_ProcessJobResult' to report
 *                             DET error 'FLS_E_VERIFY_ERASE_FAILED' for PE/user
 *                             mode switching failure while verifying erase
 *                             operation.
 *                          e) Updated function 'Fls_MainBlankCheck' to save FCU
 *                             status if PE/user mode switching failed.
 *                          f) Updated copyright information.
 *                          g) Updated 'Function(s) invoked' field in the
 *                             function banner of functions 'Fls_MainErase',
 *                             'Fls_MainWrite' and 'Fls_ProcessJobResult'.
 *                          h) Updated 'Global Variables Used' field in the
 *                             function banner of 'Fls_MainBlankCheck'
 *                             function.
 *                          i) Mapped FLS_ESDD_UD_272 to 'Fls_MainErase' and
 *                             'Fls_ProcessJobResult' function.
 *                          j) Mapped FLS315, FLS104, SWS_Fls_00315,
 *                             SWS_Fls_00104, FLS316, FLS105, SWS_Fls_00316,
 *                             SWS_Fls_00105, FLS022, FLS313, SWS_Fls_00022,
 *                             SWS_Fls_00313, FLS_ESDD_UD_206,
 *                             FLS_ESDD_UD_256, FLS_ESDD_UD_249,
 *                             FLS_ESDD_UD_259, FLS_ESDD_UD_252 to
 *                             'Fls_ProcessJobResult' function.
 * V2.3.13 03-Oct-2025   :  a) As part of ARDAABD-10763, the following changes
 *                          are made:
 *                          Updated function 'Fls_InitiateBlankCheckJob' to check
 *                          end address exceed 64KB boundary and recalulate end address 
 *                          invoke Fls_FcuCalculateBlankCheckAddr 
 *                          b) As part of ARDAABD-10735, the following changes
 *                          are made:
 *                          c) Add macro FLS_TIMEOUT_MONITORING to functions 
 *                          'Fls_MainErase', 'Fls_MainWrite', 'Fls_MainBlankCheck'
 */
/******************************************************************************/
...
FUNC(Std_ReturnType, FLS_PRIVATE_CODE) Fls_InitiateEraseJob(void)
{
  ...
  /* Check if FCU is ready to initiate Job */
  ...
  if (FLS_FCU_OK == LenStatus)
  {
    ...
    /* Implements: FLS_ESDD_UD_015, FLS_ESDD_UD_236 */
    if (FLS_FCU_OK == LenStatus)
    {
      #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON)
      ...
      LulCurrentStartAddr = Fls_GstVar.ulJobStartAddress -
                                                            FLS_DF_BASE_ADDRESS;
      LulCurrentEndAddr = ((LulCurrentStartAddr + (uint32)FLS_FCU_BLOCK_SIZE) -
                                                  (uint32)FLS_FCU_ONE);
      ...
      (
        &FLS_GpFACIReg_BaseAddress->ulFEADDR,
        (LulCurrentEndAddr & RAM_MASK_FEADDR),
        RAM_MASK_FEADDR,
        FLS_ERASE_SID
      )
      /* END Msg(2:2814)-3 */
...
**                             Fls_UseFlashControllerPreHook,
**                             Fls_UseFlashControllerEndHook
**
** Registers Used            : FHVE3,FHVE15,FSADDR,FEADDR,FSTATR,FENTRYR,FASTAT,
**                             DCSTCm,DCENm,DTCTm,ICFLENDNM,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PRIVATE_CODE
/* MISRA Violation: START Msg(2:0832)-9 */
#include FLS_MEMMAP_FILE
...
FUNC(void, FLS_PRIVATE_CODE) Fls_MainErase(void)
{
  ...
  /* Implements: FLS_ESDD_UD_269, FLS_ESDD_UD_270, FLS_ESDD_UD_271 */
  /* Implements: FLS_ESDD_UD_268 */
  #if (STD_ON == FLS_USEFLASHCONTROLLERHOOK)
  /* Local variable to hold the Hook function return value */
  Fls_AccessStatusType LenAccessStatusType;

  /* Before invoke PreHook API, check EndHook status */
  LenAccessStatusType = Fls_UseFlashControllerEndHook();
  if( FLS_ACCESS_OK == LenAccessStatusType )
  {
    if (Fls_GstVar.ulJobStartAddress < Fls_GstVar.ulJobEndAddress)
    {
      /* before performing erase process, invoke PreHook API */
      LenAccessStatusType = Fls_UseFlashControllerPreHook(FLS_COMMAND_ERASE);
    }
    else
    {
      #if (FLS_DEV_ERROR_DETECT == STD_ON)
      /* before performing blank check process, invoke PreHook API */
      LenAccessStatusType = Fls_UseFlashControllerPreHook
                                                  (FLS_COMMAND_BLANKCHECK);
      #else
      /* Set as OK when no blank check process */
      LenAccessStatusType = FLS_ACCESS_OK;
      #endif
    }
  }
  else
  ...
  if (FLS_ACCESS_NG == LenAccessStatusType)
  {
    /* Process Job Failure */
    Fls_ProcessJobResult(MEMIF_JOB_FAILED);
  }
  #if ((STD_OFF == FLS_INTERRUPT_MODE) && (STD_ON == FLS_TIMEOUT_MONITORING))
  else if (FLS_ACCESS_BUSY == LenAccessStatusType)
  {
  ...
  #endif /* (STD_ON == FLS_USEFLASHCONTROLLERHOOK) */
  {
    ...
    if (FLS_FCU_OK == LenStatus)
    {
      ...
      /* END Msg(2:3416)-2 */
      {
        ...
        else
        {
          ...
          /* Implements: FLS_ESDD_UD_268, FLS_ESDD_UD_271 */
          #if (STD_ON == FLS_USEFLASHCONTROLLERHOOK)
          /* Invoke End Hook for last call */
          (void)Fls_UseFlashControllerEndHook();
          #endif
    ...
    if ((E_NOT_OK == LucReturnValue) || \
        (FLS_FCU_ERR_ERASE == LenStatus) || \
        (FLS_FCU_ERR_INTERNAL == LenStatus))
    {
      /* Process Job Failure */
      Fls_ProcessJobResult(MEMIF_JOB_FAILED);
      /* Implements: FLS_ESDD_UD_269, FLS_ESDD_UD_271 */
      #if (STD_ON == FLS_USEFLASHCONTROLLERHOOK)
      /* Invoke End Hook for last call */
      (void)Fls_UseFlashControllerEndHook();
      #endif
  ...
  if (FLS_FCU_OK == LenStatus)
  {
    ...
    if (FLS_FCU_OK == LenStatus)
    {
      ...
      FLS_WRITE_REG_AND_MIRROR
        (
          &FLS_GpFACIReg_BaseAddress->ulFEADDR,
          (LulCurrentStartAddr + FLS_FCU_WRITE_SIZE) - FLS_FCU_ONE,
          &Fls_GaaRamMirror[RAM_IDX_FEADDR]
        )
      FLS_CHECK_WRITE_VERIFY_RUNTIME
      (
        ...
        (
          (
            (LulCurrentStartAddr + FLS_FCU_WRITE_SIZE) -
            FLS_FCU_ONE
          ) &
          RAM_MASK_FEADDR
        ),
      ...
      FLS_FCU_DF_ISSUE_BYTE_CMD(FLS_FACI_CMD_DF_WRITE);
      ...
      FLS_ENTER_CRITICAL_SECTION(FLS_DRIVERSTATE_DATA_PROTECTION);
      #endif /* End of #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON) */
      Fls_GstVar.ulJobStartAddress+= FLS_FCU_WRITE_SIZE;
...
FUNC(Std_ReturnType, FLS_PRIVATE_CODE) Fls_InitiateDmaWriteJob(void)
{
  ...
  if (FLS_FCU_OK == LenStatus)
  {
    ...
    if (FLS_FCU_OK == LenStatus)
    {
      ...
      RH850_SV_MODE_ICR_AND(8, (LpIntpCntrlReg),(FLS_RESET_INT_MK));
      #endif /* (FLS_INTERRUPT_MODE == STD_ON) */
      ...
      /* Dummy read */
      RH850_SV_MODE_REG_READ_ONLY(32, &LpDmaRegisters->ulDCEN);
...
**
** Registers Used            : DCENm, DTCTm, ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PRIVATE_CODE
/* MISRA Violation: START Msg(2:0832)-9 */
#include FLS_MEMMAP_FILE
...
FUNC(void, FLS_PRIVATE_CODE) Fls_DisableDmaChannel(void)
{
  ...
  RH850_SV_MODE_ICR_OR(8, LpDmaConfig->pDmaIntCntlReg,
                         (FLS_SET_INT_MK));
...
**
** Registers Used            : FHVE3,FHVE15,FSADDR,FEADDR,FSTATR,FENTRYR,FASTAT,
**                             DCSTCm,DCENm,DTCTm,DCSTm,ICFLENDNM,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PRIVATE_CODE
/* MISRA Violation: START Msg(2:0832)-9 */
#include FLS_MEMMAP_FILE
...
FUNC(void, FLS_PRIVATE_CODE) Fls_MainWrite(void)
{
  ...
  if( FLS_ACCESS_OK == LenAccessStatusType )
  {
    ...
    {
      /* before performing DMA write process, invoke PreHook API */
      LenAccessStatusType = Fls_UseFlashControllerPreHook
                          ...
                          (FLS_DMA_INSTANCES != FLS_DMA_ZERO)) */
    {
      /* before performing write process, invoke PreHook API */
      LenAccessStatusType = Fls_UseFlashControllerPreHook(FLS_COMMAND_WRITE);
    }
  }
  else
  {
...
**
** Registers Used            : FHVE3,FHVE15,FSADDR,FEADDR,FSTATR,FENTRYR,FASTAT,
**                             DFERRINT,FBCSTAT,DFERSTC,DFERSTR,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PRIVATE_CODE
/* MISRA Violation: START Msg(2:0832)-9 */
#include FLS_MEMMAP_FILE
...
FUNC(void, FLS_PRIVATE_CODE) Fls_MainRead(void)
{
  ...
  if( FLS_ACCESS_OK == LenAccessStatusType )
  {
    /* before performing read process, invoke PreHook API */
    LenAccessStatusType = Fls_UseFlashControllerPreHook(FLS_COMMAND_READ);
  }
  else
  {
    /* Do nothing */
...
**                            FLS_CHECK_WRITE_VERIFY_RUNTIME,
**                            Fls_FcuCheckSequencerStatus, Fls_FcuForcedStop,
**                            Fls_FcuClearStatus
**
...
(const uint32 LulStartAddr, const uint16 Luscount)
{
  ...
  /* Implements FLS_ESDD_UD_114, FLS_ESDD_UD_163 */
  ...
  if (FLS_FCU_OK == LenStatus)
  {
    ...
    /* QAC Warning: START Msg(2:2814)-3 */
    /* MISRA Violation: START Msg(4:0303)-1 */
    FLS_WRITE_REG_AND_MIRROR(&FLS_GpFACIReg_BaseAddress->ulFSADDR, LulStartAddr,
                                              &Fls_GaaRamMirror[RAM_IDX_FSADDR])
    /* For write verification */
    /* Implements FLS_ESDD_UD_108 */
    FLS_CHECK_WRITE_VERIFY_RUNTIME(&FLS_GpFACIReg_BaseAddress->ulFSADDR,
                                   (LulStartAddr & RAM_MASK_FSADDR),
                                   RAM_MASK_FSADDR, FLS_READ_SID)
    FLS_WRITE_REG_AND_MIRROR(&FLS_GpFACIReg_BaseAddress->ulFEADDR,LulEndAddr,
                             &Fls_GaaRamMirror[RAM_IDX_FEADDR])
    /* For write verification */
    /* Implements FLS_ESDD_UD_108 */
    FLS_CHECK_WRITE_VERIFY_RUNTIME(&FLS_GpFACIReg_BaseAddress->ulFEADDR,
    (LulEndAddr & RAM_MASK_FEADDR), RAM_MASK_FEADDR, FLS_READ_SID)
...
/* END Msg(2:3227)-1*/
{
  ...
  if (FLS_FCU_REGVAL_DFERSTR_NOERR != LucErrorRegValue)
  {
...
**                             Det_ReportRuntimeError, FLS_WRITE_REG_ONLY,
**                             FLS_WRITE_REG_AND_MIRROR
**                             FLS_CHECK_WRITE_VERIFY_RUNTIME,
**                             FLS_ENTER_CRITICAL_SECTION,
**                             FLS_EXIT_CRITICAL_SECTION, FLS_DF_READ32,
**                             FLS_DF_WRITE32, FLS_DF_READ8,FLS_DF_READ16,
**                             FLS_DF_WRITE8, FLS_DF_WRITE16
**
...
**
** Function(s) invoked       : FLS_ENTER_CRITICAL_SECTION,
**                             FLS_EXIT_CRITICAL_SECTION,
**                             FLS_WRITE_REG_AND_MIRROR,
**                             FLS_CHECK_WRITE_VERIFY_RUNTIME
**
...
FUNC(Std_ReturnType, FLS_PRIVATE_CODE) Fls_InitiateBlankCheckJob(void)
{
  ...
  LulBCEndAddress = Fls_GstVar.ulJobEndAddress - FLS_DF_BASE_ADDRESS;
  /* Check if end address exceed 64KB boundary and recalulate end address */
  ...
  {
      Fls_GstVar.ulSrcDestAddress = LulBCEndAddress + FLS_DF_BASE_ADDRESS + \
                                                           (uint32)FLS_FCU_ONE;
  }
...
** Preconditions             : None
**
** Global Variables Used     : Fls_GblVerifyCompletedJob,Fls_GstVar.enDFStatus
**
** Function(s) invoked       : Fls_FcuCheckBCJobStatus,Fls_FcuBlankCheckResult,
**                             Fls_ProcessJobResult,Fls_FcuSwitchMode,
**                             Det_ReportError,Fls_UseFlashControllerPreHook,
**                             Fls_UseFlashControllerEndHook
**
** Registers Used            : FHVE3,FHVE15,FSADDR,FEADDR,FSTATR,FENTRYR,FASTAT,
**                             DCENm,DTCTm,FBCSTAT,ICFLENDNM,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PRIVATE_CODE
/* MISRA Violation: START Msg(2:0832)-9 */
#include FLS_MEMMAP_FILE
    ...
    if (FLS_FCU_OK == LenStatus)
    {
      ...
      if (FLS_FCU_OK == LenStatus)
      {
        ...
        if (FLS_FCU_OK == LenStatus)
        {
          ...
          {
            /* For subsequent blocks of erase, invoke Fls_InitiateBlankCheckJob */
            LucReturnValue = Fls_InitiateBlankCheckJob();
      ...
      #if (STD_ON == FLS_USEFLASHCONTROLLERHOOK)
      if ((E_NOT_OK == LucReturnValue) || \
        (FLS_FCU_ERR_BLANKCHECK == LenStatus) || \
        (FLS_FCU_ERR_INTERNAL == LenStatus))
      {
        /* Invoke End Hook for last call */
        (void)Fls_UseFlashControllerEndHook();
...
**                             Fls_DisableDmaChannel, Det_ReportError
**
** Registers Used            : FHVE3,FHVE15,DCENm,DTCTm,ICFLENDNM,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PRIVATE_CODE
/* MISRA Violation: START Msg(2:0832)-9 */
#include FLS_MEMMAP_FILE
/* END Msg(2:0832)-9 */
/* Implements: FLS315, FLS104, SWS_Fls_00315, SWS_Fls_00104 */
/* Implements: FLS316, FLS105, SWS_Fls_00316, SWS_Fls_00105 */
/* Implements: FLS022, FLS313, SWS_Fls_00022, SWS_Fls_00313 */
/* Implements: FLS_ESDD_UD_206, FLS_ESDD_UD_272 */
/* Implements: FLS_ESDD_UD_256, FLS_ESDD_UD_249 */
/* Implements: FLS_ESDD_UD_259, FLS_ESDD_UD_252 */
...
/* END Msg(2:3227)-1*/
{
  ...
  /* Implements: FLS_ESDD_UD_103 */
  RH850_SV_MODE_ICR_OR(8, &(FLS_ICFLENDNM), (FLS_SET_INT_MK));
  /* END Msg(4:0303)-1 */
  /* END Msg(2:2814)-3 */
  /* MISRA Violation: END Msg(4:4397)-7 */
  Fls_RestoreIntReq();
  ...
  RH850_DUMMY_READ_SYNCP(16, &(FLS_ICFLENDNM));
  ...
  if (FLS_FCU_ERR_INTERNAL == Fls_GstVar.enDFStatus)
  {
    ...
    {
      #if (FLS_AR_VERSION == FLS_AR_LOWER_VERSION)
      /* Report DEM error as Erase Job Failed */
      Dem_ReportErrorStatus(FLS_E_ERASE_FAILED, DEM_EVENT_STATUS_FAILED);
      #else
      /* Report DET Runtime error as Erase Job Failed */
      (void)Det_ReportRuntimeError(FLS_MODULE_ID, FLS_INSTANCE_ID,
    ...
    #if (FLS_DEV_ERROR_DETECT == STD_ON)
    /* QAC Warning: START Msg(2:3416)-2 */
    else if (FLS_TRUE == Fls_GblVerifyCompletedJob)
    /* END Msg(2:3416)-2 */
    {
      /* Report DET error as Verify Erase Failed */
      (void)Det_ReportError(FLS_MODULE_ID, FLS_INSTANCE_ID,
                       FLS_MAINFUNCTION_SID, FLS_E_VERIFY_ERASE_FAILED);
    }
    #endif /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
    else if ((FLS_COMMAND_WRITE == Fls_GstVar.enCommand) || \
            (FLS_COMMAND_DMA_WRITE == Fls_GstVar.enCommand))
    {
      #if (FLS_AR_VERSION == FLS_AR_LOWER_VERSION)
      /* Report DEM error as Write Job Failed */
      Dem_ReportErrorStatus(FLS_E_WRITE_FAILED, DEM_EVENT_STATUS_FAILED);
      #else
      /* Report DET Runtime error as Write Job Failed */
      (void)Det_ReportRuntimeError(FLS_MODULE_ID, FLS_INSTANCE_ID,
  ...
  } /* End of if (FLS_FCU_ERR_INTERNAL == Fls_GstVar.enDFStatus) */
...
** Service ID                : NA
**
** Description               : This function sets interrupt mask flag and
...
** Service ID                : NA
**
** Description               : This function clears interrupt mask flag
**                             from ICFLENDNM register
**
** Sync/Async                : Synchronous
**
...
FUNC(void, FLS_PRIVATE_CODE) Fls_EnableFaciEndInt(void)
{
  ...
  /* Implements: FLS_ESDD_UD_103 */
  RH850_SV_MODE_ICR_AND(8, &(FLS_ICFLENDNM), (FLS_RESET_INT_MK));
  /* MISRA Violation: END Msg(4:0303)-1 */
}/* End of function Fls_SetIntReq */
