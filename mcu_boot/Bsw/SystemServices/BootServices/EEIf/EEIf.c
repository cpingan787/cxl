...
static uint32 EEIf_FindLatestAddress(uint8 findType)
{
    ...
    // 按照ACTUAL_DATALEN_SIZE长度遍历整个SECTOR空间，定位最后一个包含VALIDBLOCK_FLAG标志的地址
    latestAddress = 0;
    ...
    else if(findType == FIND_LATEST_WRITE_ADDRESS)
    {
        ...
        // 执行边界检测：验证当前SECTOR剩余空间是否能容纳ACTUAL_DATALEN_SIZE长度的数据
        ...
        else
        {
            ...
            // 将起始地址为latestAddress、长度为ACTUAL_DATALEN_SIZE的内存数据复制到nextSector
            ...
            // 处理剩余数据（如果ACTUAL_DATALEN_SIZE不是BLOCKSIZE的整数倍）
            uint32 remainingData = ACTUAL_DATALEN_SIZE % BLOCKSIZE;
            if(remainingData > 0)
            {
                // 读取剩余数据到临时缓冲区
                DataReadProcess(latestAddress - FEE_SECTOR0_STARTADDRESS + blockCount * BLOCKSIZE, blockBuffer, remainingData);
                
                // 写入到目标地址
                DataWriteProcess(nextSector - FEE_SECTOR0_STARTADDRESS + blockCount * BLOCKSIZE, blockBuffer, remainingData);
            }
...
/* BEGIN_FUNCTION_HDR
************************************************************************************************
* Function Name : EEIf_FindLatestDataAddress
* 
* Description   : 查找flash中对应数据的最新地址
* 
* Inputs        : addridx - 数据索引
* 
* Outputs       : 成功时返回该addridx的当前真实地址，失败时返回0xFFFFFFFF
* 
* Limitations   : None
*                 
************************************************************************************************
END_FUNCTION_HDR */
uint32 EEIf_FindLatestDataAddress(uint32 addridx)
{
    uint32 latestAddress = 0;
    uint32 realAddress = 0;
    
    // 查找最新的可读取地址
    latestAddress = EEIf_FindLatestAddress(FIND_LATEST_READ_ADDRESS);
    if(latestAddress == 0xFFFFFFFF)
    {
        // 未找到有效的可读取地址，返回错误
        return 0xFFFFFFFF;
    }
    ...
    // 计算addridx对应的真实地址
    // 每个block包含BLOCK_DATA_SIZE字节的用户数据
    // 计算目标block的索引
    uint32 targetBlockIndex = addridx / BLOCK_DATA_SIZE;
    
    // 计算用户数据在block中的偏移
    uint32 offsetInBlock = addridx % BLOCK_DATA_SIZE;
    
    // 计算真实地址：最新地址 + block索引 * BLOCKSIZE + 校验位大小 + 偏移
    realAddress = latestAddress + targetBlockIndex * BLOCKSIZE + USER_DATA_START + offsetInBlock;
