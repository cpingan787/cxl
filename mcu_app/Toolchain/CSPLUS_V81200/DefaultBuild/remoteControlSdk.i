...
int16_t RemoteControlSendAck(uint8_t controlCmd)
{
    g_mpuRespPack.aid = 0x31;
    g_mpuRespPack.mid = 0x05;
    
    g_mpuRespPack.subcommand = 0x6E;
    ...
    LogHalPrint("remote: RemoteControlSendAck - ");
    for(uint8_t i = 0; i < g_mpuRespPack.dataLength; i++)
    {
        LogHalPrint(" %02X", g_mpuRespPack . pDataBuffer[i]);
    }
    LogHalPrint("\r\n");
    return 0;
}
...
int16_t RemoteControlSendResult(uint8_t controlCmd, uint8_t* cmdData, uint16_t result, uint8_t vehicleResp, uint8_t type)
{
    ...
    if (0 == type)
    {
        ...
        LogHalPrint("remote: RemoteControlSendResult - ");
        for(uint8_t i = 0; i < g_mpuRespPack.dataLength; i++)
        {
            LogHalPrint(" %02X", g_mpuRespPack . pDataBuffer[i]);
        }
