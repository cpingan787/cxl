 ...
 *  V2.0.26   2025-06-17  xiaojian.liang
 *      Fixed CPT-14491, state machine stuck in ReadySleepState due to nmTimeoutTimer is 0 but nmToutFlg is FALSE.
 *  V2.0.26   2025-06-19  caihong.liu
 *      Fixed CPT-14266, Redundant passive wake-up event flags lead to repeated wake-up actions.
 *  V2.0.27   2025-10-10  caihong.liu
 *      Fixed CPT-14866, Fixed race condition in ReadySleepState causing unhandled CanNm_RepeatMessageRequest due to
...
#define CANNM_C_SW_PATCH_VERSION         27u
/*******************************************************************************
**                      Include Section                                       **
*******************************************************************************/
#include "CanNm.h"
...
static FUNC(void, CANNM_CODE) CanNm_RxCarWakeUpHadle(uint8 chIndex, const uint8* rxDataPtr);
...
static FUNC(void, CANNM_CODE) CanNm_RxPnEiraEraHandle(uint8 chIndex, const uint8* rxPnDtaPtr);
#endif
...
static FUNC(void, CANNM_CODE) CanNm_StateManagement(uint8 chIndex, const CanNm_EventType* eventFlags);

static FUNC(boolean, CANNM_CODE) CanNm_WakeUpSignalHandle(CanNm_ChannelIndexType chIndex, boolean rxIndication);

static FUNC(void, CANNM_CODE) CanNm_BusSleepStateHandle(uint8 chIndex, const CanNm_EventType* eventFlags);

static FUNC(void, CANNM_CODE) CanNm_PrepareBusSleepStateHandle(uint8 chIndex, const CanNm_EventType* eventFlags);

static FUNC(void, CANNM_CODE) CanNm_RepeatMessageStateHandle(uint8 chIndex);

#if CANNM_NODE_DETECTION_ENABLED == STD_ON
static FUNC(boolean, CANNM_CODE) CanNm_NodeDetectStateHandle(CanNm_ChannelIndexType chIndex, boolean rxRMRFlag);
#endif

static FUNC(void, CANNM_CODE) CanNm_NormalOperationStateHandle(uint8 chIndex, const CanNm_EventType* eventFlags);

static FUNC(void, CANNM_CODE) CanNm_ReadySleepStateHandle(uint8 chIndex, const CanNm_EventType* eventFlags);

static FUNC(void, CANNM_CODE) CanNm_RxDataMainHandle(uint8 chIndex, CanNm_EventType* eventFlags);
...
static FUNC(void, CANNM_CODE) CanNm_EnterPrepareBusSleepModeHandle(CanNm_ChannelIndexType chIndex);
...
/* pn fill PNI bit */
            ...
            {
                chRTPtr->aggRawPnData[loop] = 0x0u;
 ...
 */
/******************************************************************************/
uint8 validNmMsgFlag = FALSE;//manually add
...
/* @req SWS_CanNm_00337, @req SWS_CanNm_00339*/
...
#endif
            {
                ...
                if (chCfgPtr->PduCbvPosition != CANNM_PDU_OFF)
                {
                    ...
                    if ((rawCbvData & CANNM_CBV_BIT_RMR_MASK) > 0u)
                    {
                        chRTPtr->rmrBitFlg = TRUE;
                    }
                }
#endif
                chRTPtr->rxPduFlg = TRUE;
#if CANNM_CAR_WAKEUP_RX_ENABLED == STD_ON
                CanNm_RxCarWakeUpHadle(chIndex, PduInfoPtr->SduDataPtr);
#endif
                validNmMsgFlag = TRUE;//manually add
/* @req SWS_CanNm_00037 */
...
#endif
    {
        ...
        if (PduInfoPtr->SduLength >= CANNM_DEFAULT_PDU_LENGTH)
        {
            ...
            if (chIndex < CANNM_NUMBER_OF_CHANNEL)
            {
                ...
                /* @req SWS_CanNm_00351 */
                SchM_Enter_CanNm_Context();
                (void)ILib_memcpy(PduInfoPtr->SduDataPtr, CanNm_ChRunTime[chIndex].txPduData, CANNM_DEFAULT_PDU_LENGTH);
                SchM_Exit_CanNm_Context();
...
static FUNC(void, CANNM_CODE) CanNm_RxCarWakeUpHadle(uint8 chIndex, const uint8* rxDataPtr)
{
    ...
    SchM_Enter_CanNm_Context();
    uint8 carWakeupByte = rxDataPtr[chCfgPtr->CarWakeUpBytePosition];
    SchM_Exit_CanNm_Context();
    ...
    if (chCfgPtr->CarWakeUpRxEnabled && ((carWakeupByte & carWakeupMask) > 0u))
    {
        ...
        if (CanNm_CfgPtr->ChannelConfig[chIndex].CarWakeUpFilterEnabled)
        {
            ...
            {
                SchM_Enter_CanNm_Context();
                uint8 rxNodeId = rxDataPtr[(uint8)chCfgPtr->PduNidPosition];
                SchM_Exit_CanNm_Context();
    ...
    P2CONST(PduInfoType, AUTOMATIC, CANNM_APPL_DATA) PduInfoPtr)
{
    ...
    if (chCfgPtr->PnEnabled)
    {
        ...
        if (pnRxIndFlg)
        {
            uint8 pnDataLength = CanNm_CfgPtr->PnInfo->CanNmPnInfoLength;
            uint8 pnDataOffset = CanNm_CfgPtr->PnInfo->CanNmPnInfoOffset;
            for (uint8 byteIndex = 0u; byteIndex < pnDataLength; byteIndex++)
            {
                chRTPtr->aggRawPnData[byteIndex] |= PduInfoPtr->SduDataPtr[byteIndex + pnDataOffset];
            }
        }
        SchM_Exit_CanNm_Context();
#endif
...
static FUNC(boolean, CANNM_CODE) CanNm_WakeUpSignalHandle(CanNm_ChannelIndexType chIndex, boolean rxIndication)
{
    ...
    /* In BusSleep mode, receiving a message will trigger a passive wake-up event, but it must wait for ComM to initiate
     * the state transition for BusNm. */
    boolean rxWakeup = (rxIndication && (chRTPtr->canNmState == NM_STATE_PREPARE_BUS_SLEEP)) ? TRUE : FALSE;
...
static FUNC(void, CANNM_CODE) CanNm_BusSleepStateHandle(uint8 chIndex, const CanNm_EventType* eventFlags)
{
    ...
    SchM_Enter_CanNm_Context();
    /* @req SWS_CanNm_00127 */
    /* The wake-up flag is used as a conditional indicator before being cleared in the following steps */
    if ((!chRTPtr->netRequestFlg || !chRTPtr->passiveStartUp) && eventFlags->rxIndication)
    {
        /* To prevent duplicate reporting of Nm_NetworkStartIndication, this function shall be executed exclusively with
         * Nm_PassiveStartUp */
...
static FUNC(void, CANNM_CODE) CanNm_RepeatMessageStateHandle(uint8 chIndex)
{
    ...
    if (CanNm_TestAndClear(&chRTPtr->netRequestFlg))
    {
        ...
        {
            CanNm_EntryRepeatMessageState(chIndex, chRTPtr->canNmState, TRUE);
            goto exit; /* PRQA S 2001 */ /* VL_CanNm_2010 */
        }
#endif /*STD_ON == CANNM_GLOBAL_PN_SUPPORT*/
    }
#endif /* STD_OFF == CANNM_PASSIVE_MODE_ENABLED */
 ...
 */
        ...
        {
            CanNm_ClrTxPduCbvBit(chIndex, CANNM_CBV_BIT_RMR_MASK);
        }
#endif
    }
 ...
 * Param-Name[out]     None
 * Param-Name[in/out]  None
 * Return              TRUE: The operating state of the channel has changed.
 *                     FALSE: The operating state of the channel has not changed.
 */
/******************************************************************************/
#if CANNM_NODE_DETECTION_ENABLED == STD_ON
static FUNC(boolean, CANNM_CODE) CanNm_NodeDetectStateHandle(CanNm_ChannelIndexType chIndex, boolean rxRMRFlag)
{
    ...
    if (inReqFlag || rxRMRFlag)
    {
        if (inReqFlag)
        {
            /* @req SWS_CanNm_00121 SWS_CanNm_00113 */
            CanNm_SetTxPduCbvBit(chIndex, CANNM_CBV_BIT_RMR_MASK);
        }
...
static FUNC(void, CANNM_CODE) CanNm_NormalOperationStateHandle(uint8 chIndex, const CanNm_EventType* eventFlags)
{
    ...
    if (chCfgPtr->NodeDetectionEnabled)
    {
        /* @req SWS_CanNm_00120 SWS_CanNm_00119 */
        if (CanNm_NodeDetectStateHandle(chIndex, eventFlags->repeateMessageIndication))
        {
            CanNm_EntryRepeatMessageState(chIndex, chRTPtr->canNmState, FALSE);
        }
    }
...
/*the busload reduction is only work in Normal operation mode*/
    ...
    if ((NM_STATE_NORMAL_OPERATION == chRTPtr->canNmState) && (eventFlags->rxIndication))
    {
        /* @req SWS_CanNm_00069 */
...
#if (CANNM_NODE_DETECTION_ENABLED == STD_OFF) && (CANNM_BUSLOAD_REDUCTION_ENABLED == STD_OFF)
    CANNM_UNUSED(eventFlags);
#endif
}
...
static FUNC(void, CANNM_CODE) CanNm_ReadySleepStateHandle(uint8 chIndex, const CanNm_EventType* eventFlags)
{
    const CanNm_ChannelConfigType* chCfgPtr = &CanNm_CfgPtr->ChannelConfig[chIndex];
    CanNm_InnerChannelType* chRTPtr = &CanNm_ChRunTime[chIndex];
    boolean netRequestFlg = FALSE;

    /*
     * Atomically determine and update the next channel state within this critical section
     * to prevent receiving requests during the window between state evaluation and transition,
     * which may not be properly handled due to the imminent state change.
     */
    SchM_Enter_CanNm_Context();
#if (STD_OFF == CANNM_PASSIVE_MODE_ENABLED)
    if (CanNm_TestAndClear(&chRTPtr->netRequestFlg))
 ...
 */
    ...
    {
        /* @req SWS_CanNm_00112 SWS_CanNm_00111 */
        if (CanNm_NodeDetectStateHandle(chIndex, eventFlags->repeateMessageIndication))
        {
            chRTPtr->canNmState = NM_STATE_REPEAT_MESSAGE;
        }
    ...
    if ((chRTPtr->canNmState == NM_STATE_READY_SLEEP)
        && ((chRTPtr->nmTimeoutTimer == 0u)
            ...
            || ((FALSE == chRTPtr->canNmTxEnable) && (CANNM_NETWORK_RELEASED == chRTPtr->netRequestStatus))
#endif
                ))
    {
        /* @req SWS_CanNm_00109 */
...
/* Detection of Remote Sleep Indication Handle */
    ...
    if (chCfgPtr->NodeDetectionEnabled && chRTPtr->rmrBitFlg)
    {
        ...
        if ((NM_STATE_NORMAL_OPERATION == chRTPtr->canNmState) || (NM_STATE_READY_SLEEP == chRTPtr->canNmState))
        {
            eventFlags->repeateMessageIndication = TRUE;
        }
    ...
    if (chCfgPtr->PnEnabled)
    {
        uint8 rxPnData[CANNM_PN_INFO_LENGTH] = {0};
        SchM_Enter_CanNm_Context();
        (void)ILib_memcpy(rxPnData, chRTPtr->aggRawPnData, CANNM_PN_INFO_LENGTH);
        (void)ILib_memset(chRTPtr->aggRawPnData, 0u, CANNM_PN_INFO_LENGTH);
        SchM_Exit_CanNm_Context();

        /* @req SWS_CanNm_00428 @req SWS_CanNm_00429 */
        CanNm_RxPnEiraEraHandle(chIndex, rxPnData);
    }
#endif
}
