 ...
 
{
    ...
    uint32* Os_ArchTopStkPtr = (uint32*)((((uint32)Os_TaskCfg[TaskID]. TaskStack . StackTop) & 0xFFFFFFFCUL));
     

     
     
       
    Os_ArchTopStkPtr      = Os_ArchTopStkPtr - (28U);    
    *(--Os_ArchTopStkPtr) = (uint32)Os_TaskErrBack;               
    *(--Os_ArchTopStkPtr) = (uint32)Os_TaskCfg[TaskID].TaskEntry;                      

     
    Os_ArchTopStkPtr = Os_ArchTopStkPtr - 1U;
    *Os_ArchTopStkPtr = (1U << (15U));  

     

     
    Os_TaskCBExt[TaskID] = (uint32)Os_ArchTopStkPtr;    
}
...
void OS_GetSystemStackPointer(void)
{
    ...
    Os_ArchTempSp = (((uint32)(Os_SystemStack_Inf[Os_Hal_GetCoreID()]->StackTop)) & 0xFFFFFFFCUL);
