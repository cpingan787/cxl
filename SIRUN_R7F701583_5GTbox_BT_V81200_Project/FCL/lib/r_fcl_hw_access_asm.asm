...
;-- Mod. Date     : $Date: 2019/04/19 18:52:13JST $
...
; Macro for functions R_FCL_Fct_*_CalcRange
CALC_SECTION_RANGE .macro FUNC_NAME, SECT_NAME, SECT_ATTR
    R_FCL_CODE_ROM.~SECT_ATTR .CSEG SECT_ATTR
    .PUBLIC FUNC_NAME

    FUNC_NAME:
        ; /* calculate section start address */
        mov32   #__s~SECT_NAME~.~SECT_ATTR, r10
        
        ; /* calculate section size */
        mov32   #__e~SECT_NAME~.~SECT_ATTR, r11
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  .section    "R_FCL_CODE_USRINT.pctext", pctext
  .align 4
  .L.dummy.R_FCL_CODE_USRINT.pctext:

  .section    "R_FCL_CODE_USR.pctext", pctext
  .align 4
  .L.dummy.R_FCL_CODE_USR.pctext:

  .section    "R_FCL_CODE_RAM.pctext", pctext
  .align 4
  .L.dummy.R_FCL_CODE_RAM.pctext:

  .section    "R_FCL_CODE_ROMRAM.pctext", pctext
  .align 4
  .L.dummy.R_FCL_CODE_ROMRAM.pctext:

  .section    "R_FCL_CODE_RAM_EX_PROT.pctext", pctext
  .align 4
  .L.dummy.R_FCL_CODE_RAM_EX_PROT.pctext:
$else
...
;-------------------------------------------------------------------------------------------------------------
;-- Initialization of the section R_FCL_CODE_RAM_EX_PROT (because of prefetch issues)
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  R_FCL_CODE_RAM_EX_PROT.pctext .CSEG PCTEXT
$else
  R_FCL_CODE_RAM_EX_PROT.text .CSEG TEXT
$endif
.dw     0x0
.dw     0x0
.dw     0x0
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  CALC_SECTION_RANGE _R_FCL_Fct_CodeRomRam_CalcRange, R_FCL_CODE_ROMRAM, pctext
$else
  CALC_SECTION_RANGE _R_FCL_Fct_CodeRomRam_CalcRange, R_FCL_CODE_ROMRAM, text
$endif
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  CALC_SECTION_RANGE _R_FCL_Fct_CodeRam_CalcRange, R_FCL_CODE_RAM, pctext
$else
  CALC_SECTION_RANGE _R_FCL_Fct_CodeRam_CalcRange, R_FCL_CODE_RAM, text
$endif
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  CALC_SECTION_RANGE _R_FCL_Fct_CodeUsr_CalcRange, R_FCL_CODE_USR, pctext
$else
  CALC_SECTION_RANGE _R_FCL_Fct_CodeUsr_CalcRange, R_FCL_CODE_USR, text
$endif
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  CALC_SECTION_RANGE _R_FCL_Fct_CodeUsrInt_CalcRange, R_FCL_CODE_USRINT, pctext
$else
  CALC_SECTION_RANGE _R_FCL_Fct_CodeUsrInt_CalcRange, R_FCL_CODE_USRINT, text
$endif
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  CALC_SECTION_RANGE _R_FCL_Fct_CodeExProt_CalcRange, R_FCL_CODE_RAM_EX_PROT, pctext
$else
  CALC_SECTION_RANGE _R_FCL_Fct_CodeExProt_CalcRange, R_FCL_CODE_RAM_EX_PROT, text
$endif
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  R_FCL_CODE_ROM.pctext .CSEG PCTEXT
$else
...
;-------------------------------------------------------------------------------------------------------------
$ifdef __PIC
  R_FCL_CODE_RAM.pctext .CSEG PCTEXT
$else
...
$else
  R_FCL_CODE_RAM.text .CSEG TEXT
$endif
.PUBLIC _R_FCL_FCUFct_Clear_Cache_Asm
...
;-------------------------------------------------------------------------------------------------------------
;-- Function name: R_FCL_Fct_Get_PID_Asm
;-------------------------------------------------------------------------------------------------------------
;--
;-- Function read out the value of system register (PID - Processor ID).
;--
...
$else
  R_FCL_CODE_RAM.text .CSEG TEXT
$endif
.PUBLIC _R_FCL_Fct_Get_PID_Asm

_R_FCL_Fct_Get_PID_Asm:
    stsr    6, r10, 1      ; /* system register 6, 1 is PID */
