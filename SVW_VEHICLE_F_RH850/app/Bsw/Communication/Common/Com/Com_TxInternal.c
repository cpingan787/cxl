...
/*delay transmit of the MDT */
        ...
        if (TMS != Com_TX_TMS_EN)
        {
            ...
            txModeIndex = TxIpduPtr->ComTxModeFalseRefId; /* PRQA S 2983 */ /*VL_Com_2983 */
            ...
            if (ipduGroupRefNumber > 0u)
            {
                ...
                if (ipduGroupRefIndex < ipduGroupRefNumber)
                {
                    if (Com_TX_ACTIVE_EN != activeEnable)
                    {
                        ...
                        if ((COM_UNUSED_COM_PDUID == txIpduPtr->ComTxModeFalseRefId)
...
#if (0u < COM_SIGNAL_8BITBUFF_SIZE)
    SchM_Enter_Com_Context();
    if (0u != ILib_memcmp(&Com_Signal8BitRuntimeBuff[SignalBufferId], (const uint8*)SignalNewDataPtr, Length))
    {
    ...
    for (; cnt < txSignalNumber; ++cnt)
    {
        ...
        if (COM_TX_SIGNAL_NOTIFICATION_EN == (txSignalPtr->ComTxSignalNotificationType & COM_TX_SIGNAL_NOTIFICATION_EN))
        {
            Com_TxSigErrAndNotifyIdType txSignalErrAndNotifyFncId =
                (COM_TX_SIGNAL_ERROR_AND_NOTIFICATION == txSignalPtr->ComTxSignalNotificationType)
...
/******************************************************************************/
FUNC(void, COM_CODE)
Com_TpTxConfirmationHandle(PduIdType id, Std_ReturnType result)
{
    ...
    P2VAR(PduLengthType, AUTOMATIC, COM_APPL_DATA) availableDataPtr)
{
    ...
    if ((Com_TX_ACTIVE_EN == activeEnable) && (Com_TX_TRANSMITING_EN == Transmiting))
    {
        ...
        {
            txIpduStatePtr->TxOffset =
                (txIpduStatePtr->TxOffset >= retry->TxTpDataCnt) ? (txIpduStatePtr->TxOffset - retry->TxTpDataCnt) : 0u;
