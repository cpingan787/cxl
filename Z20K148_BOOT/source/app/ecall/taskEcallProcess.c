...
static uint8_t g_buttonHardfault = 0;
...
void AirbagSingleProcess(void)
{
    ...
    CanParseSdkReadSignal(g_CanSignalFormat,&pCan0SignalConfigure->ABM_AirbagSignal,&dataVaule);   
#else
    pCan1SignalConfigure = GetCan1SignalConfigure();
    CanParseSdkReadSignal(g_CanSignalFormat,&pCan1SignalConfigure->ABM_AirbagSignal,&dataVaule);
#endif
