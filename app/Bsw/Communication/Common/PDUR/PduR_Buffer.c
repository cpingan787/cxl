...
static boolean PduR_QueueIsFull(const PduR_QueueType* queuePtr);
static void PduR_QueueEnqueue(const PduR_QueueType* queuePtr, PduR_BufferType* bufferPtr);
static void PduR_BufferRelease(PduR_BufferType* bufferPtr);
/*******************************************************************************
...
/* get Queue full state. */
static boolean PduR_QueueIsFull(const PduR_QueueType* queuePtr)
{
    /* state of last one to represent full state of the queue. */
    return (queuePtr->bufferPtrs[queuePtr->depth - 1u] != NULL_PTR) ? TRUE : FALSE;
}

/* to return whether Queue is empty. */
boolean PduR_QueueIsEmpty(const PduR_QueueType* queuePtr)
{
    /* state of first one to represent empty state of the queue. */
...
/* Update size of data whitch stored in this buffer. */
...
{
    /* Copy data to current available position of data buffer and update size of data store in.  */
...
{
    pduInfoPtr->MetaDataPtr = (metaDataLength == 0u) ? NULL_PTR : bufferPtr->data; /* PRQA S 0317 */ /* VL_QAC_0317*/
...
/* get remain buffer size of buffer pointed by bufferPtr.*/
