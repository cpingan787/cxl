...
/**
 * @brief 获取当前是否处于ECALL拨打过程中
 * @return 1: 正在ECALL拨打过程中
 *         0: 不在ECALL拨打过程中（空闲或已结束）
 */
uint8_t AlarmSdkIsEcallInProgress(void)
{
    EcallCore_t *core = &g_ecallCore;
    /* 状态非IDLE即表示正在ECALL拨打过程中 */
    return (core->state != ECALL_CORE_IDLE) ? 1 : 0;
}
/* ========================= 回调函数实现 ========================= */
...
int16_t AlarmSdkSelfchackPeriSend(void)
{
    ...
    g_alarmPack.aid = ALARM_SDK_AID;
    g_alarmPack.mid = ALARM_SDK_MID_ECALL;
    g_alarmPack.subcommand = ALARM_SDK_SUBCMD_ECALL_SELFCHK_PERIOD;
...
int16_t AlarmSdkLogReport(uint8_t *data, uint16_t length)
{
    ...
    g_alarmPack.aid = ALARM_SDK_AID;
    g_alarmPack.mid = ALARM_SDK_MID_LOG_REPORT;
    g_alarmPack.subcommand = ALARM_SDK_SUBCMD_LOG_REPORT;
    memset(g_packData, 0, sizeof(g_packData));
    g_alarmPack.dataBufferSize = sizeof(g_packData);
...
int16_t EcallSatausRequest(void)
{
    ...
    g_alarmPack.aid = ALARM_SDK_AID;
    g_alarmPack.mid = ALARM_SDK_MID_ECALL;
    g_alarmPack.subcommand = ALARM_SDK_SUBCMD_ECALL_STATUS;
    memset(g_packData, 0, sizeof(g_packData));
    g_alarmPack.dataBufferSize = sizeof(g_packData);
    g_packData[0] = 0x04;
    g_alarmPack.pDataBuffer = g_packData;
...
int16_t EcallSatausSyncResponse(uint8_t status)
{
    EcallCore_t *core = &g_ecallCore;
    if (core->mpuHandle < 0) return -1;

    g_alarmPack.aid = ALARM_SDK_AID;
    g_alarmPack.mid = ALARM_SDK_MID_ECALL_TRIIGRD;
    g_alarmPack.subcommand = ALARM_SDK_SUBCMD_ECALL_TRIIGRD_RESPONSE;
    memset(g_packData, 0, sizeof(g_packData));
    g_alarmPack.dataBufferSize = sizeof(g_packData);
    g_packData[0] = status;
    g_alarmPack.pDataBuffer = g_packData;
...
int16_t AlarmSdkInit(uint16_t cycleTime)
{
    ...
    core->cfg.timeoutTriggerMs    = (2*60*1000);
    core->cfg.timeoutAwaitStateMs = (2*60*1000);
    ...
    core->cfg.testModeTimeMs      = (10*1000);
    ...
    filter.aid = ALARM_SDK_AID;
    filter.midMin = ALARM_SDK_MID_LOG_REPORT;
    filter.midMax = 0xFF;
...
static void EcallCore_ProcessEvent(EcallCore_t *core, const EcallEvent_t *evt)
{
    EcallLogReport(12,(uint8_t*)&core->state,1);
    EcallLogReport(13,(uint8_t*)&evt->event,1);
    switch (core->state) {
    case ECALL_CORE_IDLE:
        if (evt->event == ECALL_EVT_TRIGGER_REQ) {
            ...
            core->ledCtrl(E_SOS_LED_STATE_IN_CALL); // SOS灯亮

            // 日志上报
            if (g_ecallSendCount < 0xFF) {
                g_ecallSendCount++;
            } else {
                g_ecallSendCount = 1;
            }
            EcallLogReport(2,&g_ecallSendCount, sizeof(g_ecallSendCount));
        }
        break;
    ...
    case ECALL_CORE_TRIGGERING:
        ...
        } else if (evt->event == ECALL_EVT_TRIGGER_REQ) {
            core->triggerType = evt->param;
            core->enterStateTick = xTaskGetTickCount();
            core->lastRetryTime = core->enterStateTick;
            core->lastTriggerTick = core->enterStateTick;
            EcallCore_SendTrigger(core, core->triggerType);
        }
        break;
    case ECALL_CORE_MPU_ACTIVE:
        if (evt->event == ECALL_EVT_TRIGGER_REQ) {
            ...
            core->lastTriggerTick = core->enterStateTick;
            EcallCore_SendTrigger(core, core->triggerType);
        } else if (evt->event == ECALL_EVT_MPU_ACK_SUCCESS) {
    ...
    case ECALL_CORE_AWAIT_STATE:
        if (evt->event == ECALL_EVT_TRIGGER_REQ) {
            ...
            core->lastTriggerTick = core->enterStateTick;
            EcallCore_SendTrigger(core, core->triggerType);
        } else if (evt->event == ECALL_EVT_MPU_STATE_UPDATE) {
...
static void EcallCore_SendTrigger(EcallCore_t *core, uint8_t type)
{
    ...
    g_alarmPack.aid = ALARM_SDK_AID;
    g_alarmPack.mid = ALARM_SDK_MID_ECALL;
    g_alarmPack.subcommand = ALARM_SDK_SUBCMD_ECALL_TRIGGER;
    memset(g_packData, 0, sizeof(g_packData));
    g_packData[0] = type;
    g_alarmPack.pDataBuffer = g_packData;
    g_alarmPack.dataLength = 1;
...
static void EcallCore_ReSendTrigger(EcallCore_t *core)
{
    if (core->triggerType > 2) return;  // 安全检查
    EcallCore_SendTrigger(core, core->triggerType);
}

/* ========================= 对外触发接口 ========================= */
uint8_t AlarmSdkEcallTriger(uint8_t type)
{
    EcallCore_t *core = &g_ecallCore;
    uint32_t now = xTaskGetTickCount();
...
int16_t AlarmSdkEcallStop(void)
{
    ...
    g_alarmPack.aid = ALARM_SDK_AID;
    g_alarmPack.mid = ALARM_SDK_MID_ECALL;
    g_alarmPack.subcommand = ALARM_SDK_SUBCMD_ECALL_STOP;
    memset(g_packData, 0, sizeof(g_packData));
...
/* ========================= MPU消息解析 ========================= */
static uint8_t g_controlAmpStatus = 1;
static void EcallCore_ParseMpuMsg(EcallCore_t *core, MpuHalDataPack_t *pack)
{
    if (pack->aid == ALARM_SDK_AID && pack->mid == ALARM_SDK_MID_ECALL) {
        if ((pack->subcommand & 0x7F) == ALARM_SDK_SUBCMD_ECALL_STATUS)
        {
            ...
            {   // ACK
                ...
                if (pack->pDataBuffer[1] == ECALL_TRIGGER_RESULT_SUCC &&
                    pack->dataLength >= 3 &&
                    pack->pDataBuffer[2] == E_ECALL_TRIGGER_TEST_MODE) {
                    // 测试模式，主动触发自检，这里只做超时处理
                        core->globalStartTick = xTaskGetTickCount();
                        EcallCore_TransitionTo(core, ECALL_CORE_TEST_MODE);
                        TBOX_PRINT("enter TEST_MODE state\r\n");
                }
                
                // 然后发送ACK事件，让状态机处理（MPU_ACTIVE状态下会优先跳转到AWAIT_STATE）
                EcallEvent_t evt;
                evt.event = (pack->pDataBuffer[1] == ECALL_TRIGGER_RESULT_SUCC) ?
                             ECALL_EVT_MPU_ACK_SUCCESS : ECALL_EVT_MPU_ACK_FAIL;
                g_controlAmpStatus = 0;//收到ACK后，立即控制功放
                EcallCore_ProcessEvent(core, &evt);
            } 
        ...
        else if((pack->subcommand & 0x7F) == ALARM_SDK_SUBCMD_ECALL_AMP_SDZ)
        {
            if (pack->pDataBuffer[0] == 1)
            {
                g_controlAmpStatus = 1;
            }
            else if (pack->pDataBuffer[0] == 0)
            {
                g_controlAmpStatus = 0;
            }
            else
            {
                g_controlAmpStatus = 1;
    ...
    /* SYNC ECALL SATAUS  */
    if (pack->aid == ALARM_SDK_AID && pack->mid == ALARM_SDK_MID_ECALL_TRIIGRD) 
    {TBOX_PRINT("aid:%x mid:%x sub:%x data:%x\r\n", pack->aid, pack->mid, pack->subcommand, pack->pDataBuffer[0]);
         if ((pack->subcommand & 0x7F) == ALARM_SDK_SUBCMD_ECALL_TRIIGRD_REQUEST)
         {
            if ((core->state == ECALL_CORE_IN_CALL) || (core->state == ECALL_CORE_AWAIT_STATE)) 
            {
                EcallSatausSyncResponse(ECALL_SYNC_TRIIGED);
            } 
            else
            {
                EcallSatausSyncResponse(ECALL_SYNC_NO_TRIIGED);
            }
        }
    }
}
uint8_t AlarmGetControlAmpStatus(void)
{
    return g_controlAmpStatus;
}
/*************************************************
