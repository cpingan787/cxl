...
extern volatile  PortReg * const Dio_GaaPortReg_BaseAddress[5U];
...
typedef struct
{
    ...
    uint16_t dataBufferSize;
    volatile uint16_t rxIndexIn;     
    volatile uint16_t rxIndexOut;    
...
volatile uint8_t g_uartRxRingBuf[4096];
volatile uint16_t g_uartRxHead = 0;   
volatile uint16_t g_uartRxTail = 0;   

 
static uint8_t g_uartTxRingBuf[4096];
static volatile uint16_t g_uartTxHead = 0;    
static volatile uint16_t g_uartTxTail = 0;    
static volatile uint16_t g_uartTxLastLen = 0; 

 
static volatile uint16_t g_txFrameLens[16];
static volatile uint8_t  g_txLenHead = 0;
static volatile uint8_t  g_txLenTail = 0;

 
static uint8_t s_flatTxBuffer[4096];
...
static void MpuHalGpioInit(void)
{
    Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_NAD_V2X_5V0__EN_Pin1_7, 1U);
    Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_NAD_V2X_3V8_EN_Pin18_3, 1U);
}
...
static void MpuDataDispatch(const uint8_t *pData, uint16_t length, uint8_t irq)
{
    ...
    for (i = 0; i < 15; i++)
    {
        ...
        if ((pHandle->useFlag == 1) && (pHandle->pDataBufferRx != ((void *) 0)))
        {
...
static void WriteToTxRingBuffer(const uint8_t *src, uint16_t len)
{
    uint16_t space_to_end = 4096 - g_uartTxHead;
    if (len <= space_to_end)
    {
        memcpy(&g_uartTxRingBuf[g_uartTxHead], src, len);
        g_uartTxHead = (g_uartTxHead + len) & (4096 - 1);
    }
    else
    {
...
int16_t MpuHalTransmit(int16_t handle, const MpuHalDataPack_t *pTxMsg)
{
    ...
    uint8_t packHeader[8];
    uint8_t crcBuf[2];
    uint8_t nextLenHead;
    int16_t ret = 0;

    if ((handle >= 0) && (handle < 15) && (pTxMsg != ((void *) 0)))
    {
        totalLen = 8 + pTxMsg->dataLength + 2;
        ...
        if (totalLen < (4096 - 10))
        {
            ...
            freeSpace = (g_uartTxTail - g_uartTxHead - 1) & (4096 - 1);
            nextLenHead = (g_txLenHead + 1) & (16 - 1);

            if ((totalLen <= freeSpace) && (nextLenHead != g_txLenTail))
            {
                WriteToTxRingBuffer(packHeader, 8);
...
int16_t MpuHalReceive(int16_t handle, MpuHalDataPack_t *pRxMsg, uint32_t waitTime)
{
    ...
    if ((handle < 0) || (handle >= 15) || (pRxMsg == ((void *) 0)))
    {
        return -1;
    }
    ...
    if (pHandle->rxFrameCount > 0)
    {
        ...
        ReadFromRingBuffer(packHeader, pHandle->pDataBufferRx, pHandle->dataBufferSize, &readIndex, 8);
        ...
        if (pRxMsg->dataBufferSize >= payloadLen)
        {
            ...
            if (crcRx == crc)
            {
                ...
                LogHalPrint("MPU HAL: CRC error, expect %04x, get %04x\r\n", crc, crcRx);
            }
        }
        else
        {
...
void MpuHalUartInterruptCallback(uint8_t data)
{
    uint16_t next_head = (g_uartRxHead + 1) & (4096 - 1);
...
void MpuHalUartTimerCallback(void)
{
    ...
    while (g_uartRxTail != g_uartRxHead)
    {
        ...
        g_uartRxTail = (g_uartRxTail + 1) & (4096 - 1);
...
static inline void UartProtocalProcess_Byte(uint8_t rx_data, uint8_t IsrFlag)
{
    ...
    else if (8 == pBuf->dataCount)
    {
        pBuf->dataLength = ((uint16_t)(pBuf->data[5]) << 8) | pBuf->data[6];
        
        if (pBuf->dataLength > (sizeof(pBuf->data) - 8 - 2))
        {
            LogHalPrint("MPU HAL: Invalid dataLength %d, drop frame\r\n", pBuf->dataLength);
            pBuf->dataCount = 0; 
            pBuf->dataLength = 0;
        }
    }
    else if (pBuf->dataCount >= (pBuf->dataLength + 8 + 2))
    {
...
void MpuHalTxTask(void)
{
    ...
    if (g_txLenHead != g_txLenTail)
    {
        ...
        g_txLenTail = (g_txLenTail + 1) & (16 - 1);
        ReadFromRingBuffer(s_flatTxBuffer, g_uartTxRingBuf, 4096, (uint16_t*)&g_uartTxTail, frameLen);
        MD_STATUS status = R_UART5_Send(s_flatTxBuffer, frameLen);
        if(status != ((0x00U) + 0x00U))
        {
            LogHalPrint("MPU Send failed\r\n");
            return;
        }
        g_MpuEndFlag = 1;
    }
...
static void MpuHalSetPowerkey(uint8_t flag)
{
    if(0==flag)
    {
        Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_AG591_POWERKEY_EN_Pin18_1, 0U);
    }
    else
    {
        Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_AG591_POWERKEY_EN_Pin18_1, 1U);
    }
}
...
static void MpuHalSetPower(uint8_t flag)
{
    if(0==flag)
    {
        Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_NAD_V2X_5V0__EN_Pin1_7, 0U);
        Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_NAD_V2X_3V8_EN_Pin18_3, 0U);
    }
    else
    {
        Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_NAD_V2X_5V0__EN_Pin1_7, 1U);
...
static void MpuHalSetWakeOut(uint8_t flag)
{
    if(0==flag)
    {
        Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_MCU_WAKEUP_NAD_Pin12_0, 0U);
    }
    else
    {
        Dio_WriteChannel(DioConf_DioChannel_DIO_Channel_MCU_WAKEUP_NAD_Pin12_0, 1U);
    }
}
