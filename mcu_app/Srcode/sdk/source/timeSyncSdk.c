...
void TimeSyncSdkCycleProcess(MpuHalDataPack_t *msgData)
{
    ...
    else
    {
        if (msgData != NULL && msgData->pDataBuffer != NULL && msgData->dataLength > 0)
        {
            ...
            **************/
            if (((pRxData[5] & 0xF0) == 0x10) || ((pRxData[5] & 0xF0) == 0x20))
            {
                g_timeData.timeSyncStat = pRxData[5] & 0x0F; // TODO guanyua
                g_timeData.timeSrc = (pRxData[5] >> 4) & 0x0F;
                if (g_timeData.timeSyncStat == 0)
                {
                    g_timeData.timestamp = 0;
                }
                else
                {
                    g_timeData.timestamp = (pRxData[0] << 24) + (pRxData[1] << 16) + (pRxData[2] << 8) + pRxData[3];
                }
                g_timeData.timeZone = pRxData[4];
...
int16_t TimeSyncSdkGetRealTime(uint8_t *timeSrc, uint32_t *pYear, uint8_t *pMonth, uint8_t *pDay, uint8_t *pHour, uint8_t *pMin, uint8_t *pSecond)
{
    ...
    *timeSrc = g_timeData.timeSrc;
    if (g_timeData.timeValidity == 0)
    {
        TimeSyncGetLocalTimeFromRtc(g_timeData.timestamp,8, pYear, pMonth, pDay, pHour, pMin, pSecond);
        return -1;
    }
    // TimerHalGetRtcTime(&time);    // TODO guanyuan
    int16_t ret = TimeSyncGetLocalTimeFromRtc(g_timeData.timestamp, 8, pYear, pMonth, pDay, pHour, pMin, pSecond);
    if (ret == 0)
    {
        return 0;
    }
