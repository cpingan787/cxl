...
typedef struct
{
	...
	uint16_t svwVdop;						
	double svwAltitude;					
	float svwEastVelocity;				
	float svwNorthVelocity;				
	float svwUpVelocity;				
	float svwEastVelocityStdDeviation;	
	float svwNorthVelocityStdDeviation; 
	float svwUpVelocityStdDeviation;	
	uint64_t svwTimestamp;				
	float svwHorizontalAccuracy;		
	float svwMagneticDeviation;			
...
int16_t StateSyncGetMpuFaultExtStatus(MpuFaultExtStatus_t *extStatus); 
...
typedef enum                                                       
{
    ...
    E_DTC_QUERY_STATE_UNKNOWN = 0xFF                              
} DtcQueryState_e; 


DtcQueryState_e DtcGetObjState(DtcQueryObj_e obj);   
...
static uint16_t g_processCycleTime = 10; 


static CpuLocationInfo_t g_cpuLocationInfo;   
static CpuSatelliteInfo_t g_cpuSatelliteInfo; 
static CpuNetInfoSync_t g_cpuNetInfo;         
static CpuHalStateSync_t g_cpuHalState;       
static CpuTspStateSync_t g_cpuTspState;       

static CpuDtcSync_t g_cpuMpuDtcInfo;                
static MpuFaultExtStatusSync_t g_mpuFaultExtStatus; 
...
static uint16_t FloatToUint16Trunc(float f)
{
    ...
    uint32_t mant = u & 0x7FFFFF | 0x800000; 
...
void StateSyncSdkCycleProcess(MpuHalDataPack_t *msgData)
{
    if (msgData != ((void *) 0) && msgData->aid == 0x01)
    {
...
static int16_t g_mcuDtcSyncMpuHandle = -1;  
static uint16_t g_mcuDtcSyncCycleTime = 0;  
static uint16_t g_mcuDtcSyncTimeCount = 0;  
static uint8_t g_mcuDtcSyncData[4] = {0};
static MpuHalDataPack_t g_mcuDtcSyncPack;

static const McuSendCpuFaultMap_t g_mcuSendCpuFaultMap[14] =
    {
        {0, (Dem_EventIdType)23u},
        {1, (Dem_EventIdType)24u},
        {2, (Dem_EventIdType)25u},
        {3, (Dem_EventIdType)26u},
        {4, (Dem_EventIdType)29u},
        {5, (Dem_EventIdType)30u},
        {6, (Dem_EventIdType)31u},
        {7, (Dem_EventIdType)32u},
        {8, (Dem_EventIdType)35u},
        {9, (Dem_EventIdType)36u},
        {10, (Dem_EventIdType)7u},
        {11, (Dem_EventIdType)9u},
        {12, (Dem_EventIdType)8u},
        {13, (Dem_EventIdType)1u}};

static uint8_t McuSendCpuFaultIsFailed(Dem_EventIdType eventId) 
{
    ...
    if (Dem_GetEventStatus(eventId, &eventStatus) != 0U) 
    {
        return 0;
    }
    return ((eventStatus & 0x01u) != 0) ? 1u : 0;
}
...
static uint32_t McuSendCpuFaultBuildBitmap(uint8_t *pData)
{
    ...
    for (i = 0u; i < 14; i++)
    {
        if (McuSendCpuFaultIsFailed(g_mcuSendCpuFaultMap[i].eventId) != 0) 
        {
            McuSendCpuFaultSetBit(pData, g_mcuSendCpuFaultMap[i].bitIndex); 
        }
    }
    bitmap |= ((uint32_t)pData[0] << 24); 
...
void McuSendCpuFaultSyncCycleProcess(void)
{
    ...
    g_mcuDtcSyncPack.subcommand = 0x02;
    g_mcuDtcSyncPack.pDataBuffer = g_mcuDtcSyncData;
    g_mcuDtcSyncPack.dataBufferSize = sizeof(g_mcuDtcSyncData);
    g_mcuDtcSyncPack.dataLength = 4;
    MpuHalTransmit(g_mcuDtcSyncMpuHandle, &g_mcuDtcSyncPack);
    LogHalPrint("tx mcu fault bitmap = 0x%08X\r\n", bitmap);  
    LogHalPrint("tx raw data: %02X %02X %02X %02X\r\n", g_mcuDtcSyncData[0], g_mcuDtcSyncData[1], g_mcuDtcSyncData[2], g_mcuDtcSyncData[3]);
}
...
static McuRecvMpuFaultMap_t g_mcuRecvMpuFaultMap[21];

static void InitMcuRecvMpuFaultMap(void)
{
    g_mcuRecvMpuFaultMap[0].bitIndex = 0;
    g_mcuRecvMpuFaultMap[0].eventId = 21u;
    g_mcuRecvMpuFaultMap[1].bitIndex = 1;
    g_mcuRecvMpuFaultMap[1].eventId = 22u;
    g_mcuRecvMpuFaultMap[2].bitIndex = 2;
    g_mcuRecvMpuFaultMap[2].eventId = 5u;
    g_mcuRecvMpuFaultMap[3].bitIndex = 3;
    g_mcuRecvMpuFaultMap[3].eventId = 6u;
    g_mcuRecvMpuFaultMap[4].bitIndex = 4;
    g_mcuRecvMpuFaultMap[4].eventId = 13u;
    g_mcuRecvMpuFaultMap[5].bitIndex = 5;
    g_mcuRecvMpuFaultMap[5].eventId = 14u;
    g_mcuRecvMpuFaultMap[6].bitIndex = 6;
    g_mcuRecvMpuFaultMap[6].eventId = 15u;
    g_mcuRecvMpuFaultMap[7].bitIndex = 7;
    g_mcuRecvMpuFaultMap[7].eventId = 50u;
    g_mcuRecvMpuFaultMap[8].bitIndex = 8;
    g_mcuRecvMpuFaultMap[8].eventId = 56u;
    g_mcuRecvMpuFaultMap[9].bitIndex = 9;
    g_mcuRecvMpuFaultMap[9].eventId = 57u;
    g_mcuRecvMpuFaultMap[10].bitIndex = 10;
    g_mcuRecvMpuFaultMap[10].eventId = 54u;
    g_mcuRecvMpuFaultMap[11].bitIndex = 11;
    g_mcuRecvMpuFaultMap[11].eventId = 58u;
    g_mcuRecvMpuFaultMap[12].bitIndex = 12;
    g_mcuRecvMpuFaultMap[12].eventId = 55u;
    g_mcuRecvMpuFaultMap[13].bitIndex = 13;
    g_mcuRecvMpuFaultMap[13].eventId = 53u;
    g_mcuRecvMpuFaultMap[14].bitIndex = 14;
    g_mcuRecvMpuFaultMap[14].eventId = 51u;
    g_mcuRecvMpuFaultMap[15].bitIndex = 15;
    g_mcuRecvMpuFaultMap[15].eventId = 52u;
...
static uint32_t McuRecvMpuFaultBitmapToU32(const uint8_t *pData) 
{
    uint32_t value = 0;
    value |= ((uint32_t)pData[0] << 24); 
    value |= ((uint32_t)pData[1] << 16); 
    value |= ((uint32_t)pData[2] << 8);  
    value |= ((uint32_t)pData[3] << 0);  
    return value;
}
...
void MpuDtcSyncSdkCycleProcess(MpuHalDataPack_t *msgData)
{
    uint32_t faultBitmap = 0; 
    uint32_t i = 0;
    static uint8_t s_faultMapInitialized = 0; 
