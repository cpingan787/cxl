...
uint8 g_BootM_SIBTimeout = 0u;
uint8 g_BootM_SIBData[(3u)];
static const uint8 gs_StayInBootFrame[(3u)] =
{
    0x02, 0x10, 0x03
};
...
static void BootM_GetCounterValue(uint32 counterId, uint16* value);
...
static void BootM_AppJump(void)
{
    ...
    AppEntry = (theApp)(*((uint32 *)(0x00080602UL)) + (0x00080600UL));
...
uint8 BootM_GetFlag(void)
{
    ...
    if(Diag_FlagCompare(0x00u) == 0U)
    {
         flag = (0x00u);
    }
    else if((BootM_IsAllLBA_Valid() == 0U) && (SecureBootCurrentStatus == SECURE_BOOT_SUCCESS))
    {
         BootM_AppGo();
         flag = (0x03u);
    }
...
uint8 BootM_IsAllLBA_Valid(void)
{
    ...
    retValue = Diag_FlagCompare(0x04u);
...
static void BootM_ResetRespond(uint8 reason)
{
    ...
    if(reason == (0x00u))
    {
        ...
        ResBuffer[1] = 0x50;
        ResBuffer[2] = 0x02;
        ResBuffer[3] = (uint16)(50U) >> (uint8)0x08u;
        ResBuffer[4] = (uint16)(50U) & (uint8)0xFFu;
        ResBuffer[5] = ((uint16)(5000u)/(10U)) >> (uint8)0x08u;
        ResBuffer[6] = ((uint16)(5000u)/(10U)) & (uint8)0xFFu;
        RemainBytes = 1;
    }
    else if(reason == (0x01u))
    {
        ResBuffer[0] = 0x06;
        ResBuffer[1] = 0x50;
        ResBuffer[2] = 0x01;
...
void BootM_ReprogramRespond(void)
{
    if(Diag_FlagCompare(0x03u) == 0U)
    {
        Diag_FlagClear(0x03u);
    }
    else
    {
        BootM_ResetRespond((0x00u));
    }
}
...
static void BootM_MCUResetRespond(uint8 reason)
{
    ...
    else
    {
        if(reason == (0x01u))
        {
            BootM_ResetRespond((0x01u));
        }
        else if(reason == (0x02u))
        {
            BootM_ResetRespond((0x02u));
        }
        else
        {
...
static uint8 BootM_StayInBootProcess(void)
{
    uint16 StartSIBTimer = 0;
    uint16 CurrentSIBTimer = 0;
    uint8 isValidSIBRequest = 0;
    BootM_GetCounterValue(SystemTimer_Core_0, &StartSIBTimer);
    do
    {
        if(BooM_IsSIBValid() == 0U)
        {
            BootM_ResetRespond((0x03u));
            isValidSIBRequest = 1;
        }
        BootM_GetCounterValue(SystemTimer_Core_0, &CurrentSIBTimer);
        Wdg_59_DriverB_TriggerFunc((boolean)0x01);
    }
    while(((CurrentSIBTimer - StartSIBTimer) < (20u)) && (isValidSIBRequest == 0));
