...
static boolean g_secocTripCntFaultFlag = FALSE;
static boolean g_secocTripCntPassedReportedThisCycle = FALSE;
...
static void SecocTripCntDtcSyncStateWithDem(void)                       
{
    Dem_UdsStatusByteType eventStatus = 0;                             

    if (Dem_GetEventStatus(EventParameter_0xE00444, &eventStatus) == E_OK) 
    {
        if ((eventStatus & (DEM_UDS_STATUS_TF |                         
                            DEM_UDS_STATUS_PDTC |                      
                            DEM_UDS_STATUS_CDTC)) != 0U)               
        {
            g_secocTripCntFaultFlag = TRUE;                             
            g_secocTripCntPassedReportedThisCycle = FALSE;              
        }
        else if ((eventStatus & DEM_UDS_STATUS_TFSLC) == 0U)           
        {
            g_secocTripCntFaultFlag = FALSE;                            
        }
    }
}

static void SecocTripCntReportPassedOncePerCycle(void)                  
{
    if (g_secocTripCntPassedReportedThisCycle == TRUE)                  
    {
        return;                                                         
    }

    if (g_secocTripCntFaultFlag == TRUE)                               
    {
        Dem_SetEventStatus(EventParameter_0xE00444, DEM_EVENT_STATUS_PASSED); 
        g_secocTripCntPassedReportedThisCycle = TRUE;                   
    }
}
...
void SecocTripCntDtcMonitor_200ms(void)                                         //200ms周期监控SecOc_count这个block当前状态
{
    ...
    SecocTripCntDtcSyncStateWithDem();
    ...
    {
        SecocTripCntReportPassedOncePerCycle();
        Dem_SetEventStatus(EventParameter_0xE00444, DEM_EVENT_STATUS_PASSED);
        g_secocTripCntFaultFlag = FALSE;
        g_secocTripCntPassedReportedThisCycle = TRUE;   
    }
