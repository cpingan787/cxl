...
CanTp_ConnectionChannelType CanTp_Channels[0x1u];
...
CanTp_GetRxSduCfgByNpdu(
    ...
    uint8 * ChannelId)
{
    const CanTp_RxNSduType* result = ((void *)0);
    ...
    for (chnIdx = 0; (((void *)0) == result) && (chnIdx < CanTp_ConfigPtr->ChannelNum); chnIdx++)
    {
        ...
        for (rxNSduIdx = 0; rxNSduIdx < channelPtr->RxNSduNum; rxNSduIdx++)
        {
            ...
            case CANTP_NORMALFIXED:
                frameType = CanTpRxPduPtr->SduDataPtr[(uint8)0x00] & (uint8)0xF0;
                if ((RxNSdus->RxNPduId == CanTpRxPduId)
                    && (((uint8)0x20 == frameType) || (0x00u == frameType)
                        || ((uint8)0x10 == frameType)))
                {
                    result = RxNSdus;
    ...
    uint8 * totaloffset)
{
    ...
    sfDl = PduInfoPtr->SduDataPtr[offset] & (uint8)0x0F;
    ...
    const CanTp_RxNSduType * RxNSduCfgPtr)
{
    ...
    ffDl = (uint32)PduInfoPtr[pcioffest] & (uint32)(uint8)0x0F;
    ffDl <<= 8u;
    ...
    PduLengthType bufferSize)
{
    ...
    PduLengthType length = 0xFF;
    uint8 offset = 0xFF;
    uint8 bs;
    Std_ReturnType ret = 0U;
     
    if (RxchannelPtr->CurrentBs == 0u)  
    {
        bs = (0u != ((RxchannelPtr->EventFlags) & ((uint8)0x01)))
                 ? RxchannelPtr->ChangedBs
                 : ((RxNSduCfgPtr->Bs != ((void *)0)) ? *(RxNSduCfgPtr->Bs) : 0x0u);
        if (0u == bs)
        {
        ...
        else
        {
            ...
            length =
                (CANTP_CAN_20 == RxNSduCfgPtr->CanFrameType) ? length : (RxchannelPtr->FDDlc - (PduLengthType)offset);
            


 
            blockDataLen = length * (PduLengthType)bs;
             
            if ((bufferSize < blockDataLen) && (bufferSize < RxchannelPtr->SduDataRemaining))
            {
                RxchannelPtr->CurrentBs = (uint8)(bufferSize / length);
                ret = (RxchannelPtr->CurrentBs >= 1u) ? 0U : 1U;
            }
            else
            {
    ...
    PduLengthType bufferSize)
{
    ...
    {
        blockDataLen = (PduLengthType)RxchannelPtr->CurrentBs * ((PduLengthType)8 - offset);
    }
    else
    {
 ...
 Std_ReturnType
CanTp_SendFC(const CanTp_RxNSduType* RxNSduCfgPtr, const CanTp_ConnectionType* RxchannelPtr, CanTp_FCInfoType CanTp_FC)

{
    ...
    {
        FcBuf[offset] = (uint8)0x30 | CanTp_FC.FcFs;
        FcBuf[offset + 1u] = CanTp_FC.FcBS;
        FcBuf[offset + 2u] = (uint8)CanTp_FC.FcSTMin;
    }
    if (CANTP_PADDING_ON == RxNSduCfgPtr->RxPaddingActivation)
    {
        CanTp_MemorySet(&FcBuf[fcFrameLen], 0xaau, ((uint16)(PduLengthType)8 - fcFrameLen));
        fcFrameLen = (uint16)(PduLengthType)8;
    }
    ...
    uint8 * pcioffset)
{
    ...
    case CANTP_STANDARD:
    case CANTP_NORMALFIXED:
        *pcioffset = (uint8)0x00;
        *dataoffset = (uint8)0x01;
        break;
    default:
 ...
 const CanTp_TxNSduType *
CanTp_GetTxSduCfgByFCNpdu(
    PduIdType CanTpRxPduId,
    const PduInfoType * CanTpRxPduPtr,
    uint8 * ChannelId)
{
    const CanTp_TxNSduType* result = ((void *)0);
    const CanTp_TxNSduType* TxNSdus;
    const CanTp_ChannelType* channelPtr;
    uint8 txNSduIdx;
    uint8 chnIdx;
    ...
    {
        channelPtr = &CanTp_ConfigPtr->CanTpChannel[chnIdx];
        for (txNSduIdx = 0; (((void *)0) == result) && (txNSduIdx < channelPtr->TxNsduNum); txNSduIdx++)
        {
    ...
    CanTp_FCInfoType * FcInfoPtr)
{
    ...
    FcInfoPtr->FcFs = PduInfoPtr->SduDataPtr[offset] & (uint8)0x0F;
    FcInfoPtr->FcBS = PduInfoPtr->SduDataPtr[offset + 1u];
    
 
    stMinRaw = PduInfoPtr->SduDataPtr[offset + 2u];
    if ((uint8)0x7F >= stMinRaw)
    {
         
        stMinUs = (uint32)stMinRaw * (uint32)1000;
    }
    else if (((uint8)0xF1 <= stMinRaw) && ((uint8)0xF9 >= stMinRaw))
    {
         
        stMinUs = ((uint32)((uint32)stMinRaw - 0xF0UL)) * (uint32)100;
    }
    else
    {
         
        stMinUs = (uint32)(uint8)0x7F * (uint32)1000;
    }
    ...
    const PduInfoType * PduInfoPtr)
{
    ...
    case CANTP_FUNCTIONAL_TX:
        ...
        case CANTP_NORMALFIXED:
            ...
            if (((uint8)0x07 >= PduInfoPtr->SduLength)
                || ((CANTP_CAN_FD == TxNSduCfgPtr->CanFrameType)
                    && ((TxNSduCfgPtr->TxNPduDLC - (uint8)0x02) > PduInfoPtr->SduLength)))
            {
...
)
{
    ...
    if (offset != 0xFFu)
    {
        ...
        if (len < TxSubchannelPtr->SduDataTotalCount)
        {
            ...
            TxSubchannelPtr->HandleType = (uint8)0x10;
        ...
        else
        {
            ...
            TxSubchannelPtr->HandleType = 0x00u;
    ...
    uint8* totalOffset)
{
    ...
    if (pciDl <= (uint32)4095)
    {
         
         
        dataPtr[offset] = (uint8)0x10 | (uint8)(pciDl >> 8u);
        offset++;
    ...
    uint8* totalOffset)
{
    ...
    dataPtr[offset] = (uint8)0x20 | TxSubchannelPtr->CurrentCfSn;
    offset++;
    *totalOffset = offset;
}
    ...
    uint8* totalOffset)
{
    ...
    if (CANDL <= 8u)
    {
        ...
        dataPtr[offset] = ((uint8)pciDl & (uint8)0x0F);  
        offset++;
        *totalOffset = offset;
    }
    else
    ...
    PduInfoType* pduInfoPtr)
{
    ...
    case CANTP_STANDARD:
    case CANTP_NORMALFIXED:
        can20DataFieldLen = (uint8)0x07;
        canFdDataOffset = (uint8)0x02;
        break;
    default:
