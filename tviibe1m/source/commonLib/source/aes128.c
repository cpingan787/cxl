...
/*************************************************************************
key-密钥，keyLen-密钥长度；pt-加密前明文；ct-加密后密文；len-明文长度

*************************************************************************/
/*int aes_pkcs7_en(const uint8_t *key, uint32_t keyLen, uint8_t *pt, uint8_t *ct, uint32_t len)
{
    uint8_t i=0,n=0;
    ...
    Aes128EcbEncrypt(key,keyLen,pt,ct,16); 
	return 0;
}*/

/*************************************************************************
key-密钥，keyLen-密钥长度；pt-解密前密文；ct-解密后明文；len-解密前密文长度

*************************************************************************/
/*int aes_pkcs7_de(const uint8_t *key, uint32_t keyLen, const uint8_t *pt, uint8_t *ct, uint32_t len)
{

    Aes128EcbDecrypt(key,keyLen,pt,ct,16); 
...
void Aes128CmacLoadMacKey(Aes128Cmac_t *pData,uint8_t *key)
{
	...
	for (i = 0; i < 16; i++)
	{
		pData->MACkey[i] = key[i]; // set MAC key
	}
	//mbedtls_aes_setkey_enc(&aes, MACkey, 128);
	//mbedtls_aes_crypt_ecb(&aes, AES_ENCRYPT, plain, c0);
	Aes128EcbEncrypt(pData->MACkey,16,plain,c0,16);
	if (c0[0]<0x80) //generate k1
	{
		leftshift(16, c0, pData->k1);
	}
	else
	{
		leftshift(16, c0, pData->k1);
		ArrayXor(16, pData->k1, Rb, pData->k1);
	}
	if (pData->k1[0] < 0x80) //generate k2
	{
		leftshift(16, pData->k1, pData->k2);
	}
	else
	{
		leftshift(16, pData->k1, pData->k2);
		ArrayXor(16, pData->k2, Rb, pData->k2);
	}
}
...
void Aes128CmacGenerateMAC(Aes128Cmac_t *pData,int len, uint8_t *add, uint8_t *macvalue)
{
	...
	else
	{
		if (len==0)
		{
			...
			ArrayXor(16, Blocktemp, pData->k2, Blocktemp);
			//mbedtls_aes_crypt_ecb(&aes, AES_ENCRYPT, Blocktemp, macvalue);
			Aes128EcbEncrypt(pData->MACkey,16,Blocktemp,macvalue,16);
		}
		else
