...
/* PRQA S 2469, 2463,2985 EOF */ /* VL_Dem_2469, VL_Dem_2463, VL_Dem_2985 */
/*******************************************************************************
...
/* PRQA S 6030 -- */
{
    ...
    else
    {
        ...
        {
            for (uint8 permanentStart = 0; permanentStart < DEM_MAX_NUMBER_EVENT_ENTRY_PERMANENT; permanentStart++)
            {
                Dem_EventIdType eventId = DemPermanentMemory[permanentStart];
                if (eventId != 0x00u)
                {
                    boolean find = FALSE;
                    uint16 dtcRef = DemPbCfgPtr->pDemEventParameter[Dem_GetEventInternalId(eventId)].DemDTCRef;
                    if (dtcRef != DEM_DTC_REF_INVALID)
                    {
                        for (uint16 iloop = 0; iloop < pFilter->DemDemDTCReportNum; iloop++)
                        {
                            if (pFilter->DemDTCReport[iloop] == dtcRef)
                            {
                                find = TRUE;
                            }
                        }
                        if (find == FALSE)
                        {
                            pFilter->DemDTCReport[pFilter->DemDemDTCReportNum] = dtcRef;
                    ...
                    if (((DtcStatus & 0x0Cu) != 0u) && ((DtcStatus & 0xF3u) == 0u))
                    {
                        for (uint8 index = 0; index < EntryNum; index++)
                        {
                            if (pEntry->EventId != 0u)
                            {
                                ...
                                if ((DEM_MEM_DEST_INVALID != Dem_CheckEventInternalMemDest(DTCOrigin, pEvent->IntId))
                                    && (iloop == DTCRef))
                                {
                                    dtcflag = TRUE;
...
Dem_DcmCheckClearParameter(uint32 DTC, Dem_DTCFormatType DTCFormat, Dem_DTCOriginType DTCOrigin)
{
    ...
    else
    {
        ...
        if (DTC != DEM_DTC_GROUP_ALL_DTCS)
        {
            ...
            if (Dem_GetDTCIndex(DTC, DTCFormat) == DEM_DTC_REF_INVALID)
            {
...
Dem_DcmClearDTC(uint32 DTC, Dem_DTCFormatType DTCFormat, Dem_DTCOriginType DTCOrigin)
{
    ...
    else
    {
        ...
        if (Dem_ClearDTCLock == DEM_CLEAR_LOCK)
        {
            ...
            {
#if (DEM_CLEAR_DTCBEHAVIOR == DEM_CLRRESP_NONVOLATILE_FINISH)
                Dem_ClearNonvolatileStatus = DEM_ClEAR_NONVOLATILE_FINISHED;//yjp add
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_SetDTCSettingProcess(void)
{
    uint16 iloop;
    ...
    for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
    {
        ...
        {
            if (DemPbCfgPtr->DemCfgDTC[pEventCfg->DemDTCRef].GroupRef != DEM_GROUP_OF_DTC_INVALID)
            {
#if (DEM_GROUP_OF_DTC_NUM > 0)
                if (DEM_BITS_ISNSET(
                        DemDTCSettingInfo.DTCGroupStatus,
                        DemPbCfgPtr->DemCfgDTC[pEventCfg->DemDTCRef].GroupRef))
                {
...
#endif
                {
                    if (0x00u == DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_DTC_SETTING))
                    {
                        ...
                        if (DemPbCfgPtr->pDemEventParameter[iloop].DemCallbackInitMForE != NULL_PTR)
                        {
                            DemPbCfgPtr->pDemEventParameter[iloop].DemCallbackInitMForE(DEM_INIT_MONITOR_REENABLED);
                        }
                    }
                }
            }
 ...
 */
/*************************************************************************/
/* PRQA S 6020, 6010 ++ */ /* VL_MTR_Dem_STLIN, VL_MTR_Dem_STCYC */
...
/* PRQA S 6020, 6010 -- */
{
    ...
    if ((0x00u == (pEvent->UdsStatus & DEM_UDS_STATUS_TF)) && (pDTCAttr != NULL_PTR))
    {
        for (uint8 iloop = 0; (iloop < DEM_MEM_DEST_MAX_NUM_OF_DTC)
                              && (DEM_MEM_DEST_INVALID != pDTCAttr->DemMemoryDestinationRef[iloop]);
             iloop++)
        {
            if (DemMemDestCfg[pDTCAttr->DemMemoryDestinationRef[iloop]].ExtId == DEM_DTC_ORIGIN_PRIMARY_MEMORY)
            {
                /* req SWS_Dem_00219 */
    ...
    if (0x00u == DEM_FLAGS_ISSET(pEvent->EventStatusFlag, DEM_EVENT_FLAGS_THISOPISFAILED))
    {
        ...
        /* In each operation cycle the counter shall be incremented as soon as FDC10 has reached a value that is equal
         * to or greater than its uncomfiramationLimit for the first time. */
...
#if (DEM_OCC6EANLE == STD_ON)
                    /*  The counter may also be cleared when the confirmedDTCLimit is reached. */
                    pEvent->OCC6 = 0;
#endif /* DEM_OCC6EANLE == STD_ON */
                }
...
FUNC(void, DEM_CODE) Dem_IntWriteNvRAM(void)
{
    ...
    DemOBDDataStorage.StoreIgnUpCycleCounter = IgnUpCycleCounter;
    DemOBDDataStorage.StoreContinuousMICounter = ContinuousMICounter;
    DemOBDDataStorage.StoreWarmUpCycleCounter = WarmUpCycleCounter;
    DemOBDDataStorage.StoreOBDDistanceMILOn = OBDDistanceMILOn;
    DemOBDDataStorage.StoreDistSinceDtcCleared = DistSinceDtcCleared;
    DemOBDDataStorage.StoreOBDTimeMILOn = OBDTimeMILOn;
    DemOBDDataStorage.StoreOBDTimeDTCClear = OBDTimeDTCClear;
    DemOBDDataStorage.StoreOBDB1Counter = OBDB1Counter;
#if (DEM_OBD_SUPPORT == DEM_OBD_MASTER_ECU)
    DemOBDDataStorage.StoreMasterContinuousMICounter = MasterContinuousMICounter;
#endif
    blockIndex++;
    blockRef = DemNvRamBlockId[blockIndex];
    (void)NvM_CancelJobs(blockRef);
