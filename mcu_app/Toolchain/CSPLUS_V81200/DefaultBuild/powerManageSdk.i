...
typedef enum
{
    ...
    E_PM_STATE_POWER_ON,
    E_PM_STATE_NMPU_SLEEP,
    E_PM_STATE_NMPU_PRE_CHECK_CAN,
    E_PM_STATE_NMPU_WAKE,
    E_PM_STATE_NMPU_DELAY,
    E_PM_STATE_NMPU_CHECK_NM_STATUS,
    E_PM_STATE_WAKE,
    E_PM_STATE_WAKE_DELAY,
    E_PM_STATE_CHECK_NM_STATUS,
    E_PM_STATE_PRE_SLEEP_NOTICE,
    E_PM_STATE_PRE_SLEEP_WAIT,
    E_PM_STATE_MCU_SLEEP,
...
typedef struct
{
    ...
    uint32_t restart24hTimer;       
    uint32_t listenWakeupTimer;     
    uint8_t mpuPowerOffFlag;        
...
void ResetListenTimer(void)
{
    uint32_t listenTimer = 14*24*3600;  

    if(NvM_ReadBlock(29U, NvMBlockRamBuffer29) == 1U)
    {
    ...
    if(NvMBlockRamBuffer29[5] == 0) 
    {
        listenTimer = NvMBlockRamBuffer29[0] * 24 * 3600;
    }
    else if(NvMBlockRamBuffer29[5] == 1) 
    {
        listenTimer = NvMBlockRamBuffer29[0] * 3600;
    }
    else if(NvMBlockRamBuffer29[5] == 2) 
    {
...
static uint32_t GetListenTimerParam(void)
{
    ...
    if(NvM_ReadBlock(49U, NvMBlockRamBuffer49) == 1U)
    {
        return listenTimer;
    }
    ...
    LogHalPrint("Get listen timer from flash: %u seconds\n", listenTimer);
    return listenTimer;
}
...
static uint32_t SaveListenTimerParam(void)
{
    ...
    {
        NvMBlockRamBuffer49[4] = UsgMd_1_Standby;   
    }

    if(NvM_WriteBlock(49U, NvMBlockRamBuffer49) == 1U)
    {
        return 1U;
    }
}
...
uint8_t GetStoredUserMode(void)
{
    uint8_t userMode = UsgMd_1_Standby;   

    if(NvM_ReadBlock(49U, NvMBlockRamBuffer49) == 0U)
    {
        userMode = NvMBlockRamBuffer49[4];
         
        if(userMode > UsgMd_6_XOTA)
        {
            userMode = UsgMd_1_Standby;   
        }
    }
...
int16_t PowerManageSdkInit(const PmSdkConfig_t* pmConfig)
{
    if(pmConfig!=((void *) 0))
    {
        ...
        g_pmManage.restart24hTimer = (120 * 1000);
...
static void WakeDelayProcess(uint8_t mcuWakeSource,uint8_t mpuWakeSource,uint32_t *pWakeDelayTime)
{
    ...
    if(g_pmManage.wakeupFun!=((void *) 0))
    {
...
static void PmStateNmpuDelay(uint32_t cycleTime)
{
    ...
    {
        g_pmManage.pmState = E_PM_STATE_NMPU_WAKE;
        WakeDelayProcess(10,0x00,&g_pmManage.wakeDelayTime);
    }
    else if(PmMpuStartIsFinished()==0)
    {
        g_pmManage.pmState = E_PM_STATE_WAKE;
...
static uint8_t CheckVinIsInvalid(void)
{
    ...
    if(NvM_ReadBlock(7U, NvMBlockRamBuffer7) == 1U)
    {
        return 1;
    }

    if ((memcmp(NvMBlockRamBuffer7, vinZero, 17) == 0) ||
        (memcmp(NvMBlockRamBuffer7, vinFF, 17) == 0))
    {
        return 1;
    }
...
static void PmStatePreSleepNoticeProcess(uint32_t cycleTime)
{
    ...
    if(g_pmManage.restart24hTimer == 0)
    {
        ...
        LogHalPrint("24H timer expired, system reset\r\n");
        LogHalPrint("24H timer expired, system reset\r\n");
        LogHalPrint("24H timer expired, system reset\r\n");
...
static void PmStateCheckWakeupSourceProcess(uint32_t cycleTime)
{
    ...
    LogHalPrint("Wakeup source is : %d\r\n",g_pmManage . wakeupSource);
    TimerHalSetMode(1);
...
void PowerManageSdkGetPowerInfo(uint8_t* pPmState,uint8_t* pWakeupSource,uint32_t* pWakeCount)
{
    ...
    if(pWakeupSource!=((void *) 0))
    {
         
        *pWakeupSource = g_pmManage.wakeupSource;
    }
    if(pWakeCount!=((void *) 0))
    {
