...
static pMcuParametReadFun_t g_mcuParameterReadCbFunc = ((void *) 0) ;   
static pMcuParametWriteFun_t g_mcuParameterWriteCbFunc = ((void *) 0) ;  
static uint16_t g_cycleTime = 0 ;                           

static uint8_t g_syncMpuParamIdList[26] = {0} ;             
static uint8_t g_syncMpuParamSize = 0 ;                     
static uint8_t g_syncMpuParamRequstFlag = 0 ;               
static uint8_t g_syncMpuParamResultFlag = 0 ;               
static uint16_t g_syncMpuParamTimeCount = 0 ;               
static uint8_t g_syncParamToMpuRequstFlag = 0 ;             
static uint16_t g_syncParamToMpuTimeCount = 0 ;             
static uint8_t g_syncParamToMpuRequstCount = 0 ;            
static uint8_t g_syncParamToMpuData[64] = {0} ;             
static MpuHalDataPack_t g_syncParamToMpuPack ;              
static uint8_t g_mpuDataBuffer[200] = {0} ;                
static MpuHalDataPack_t g_mpuDataPack ;                     

typedef enum {
    PARAM_GET_FROM_FLASH = 0,    
    PARAM_GET_FROM_CONFIG,       
    PARAM_GET_METHOD_NONE        
} ParamGetMethod_e;

typedef struct {
    ParameterItemId_e paramId;           
    ParamGetMethod_e getMethod;          
    FlashParaId_e flashParaId;           
    ConfigItemType_e configItemType;     
} ParamMappingEntry_t;

static const ParamMappingEntry_t g_paramMappingTable[E_PARAMETER_ITEM_NUM_MAX] = {
    ...
    [E_PARAMID_TSPAddr]             = {E_PARAMID_TSPAddr,               PARAM_GET_FROM_FLASH,   E_PARAMETER_INFO_TSPAddr,               CONFIG_ITEM_MAX},
    [E_PARAMID_TSPPort]             = {E_PARAMID_TSPPort,               PARAM_GET_FROM_FLASH,   E_PARAMETER_INFO_TSPPort,               CONFIG_ITEM_MAX},
    ...
    [E_PARAMID_SupIdentifier]       = {E_PARAMID_SupIdentifier,         PARAM_GET_FROM_FLASH,   E_PARAMETER_INFO_SupIdentifier,         CONFIG_ITEM_MAX},
...
static const ParamWriteCheck_t g_paramWriteCheck[] = {
    ...
    [E_PARAMID_SW_Version]          = {E_PARAMID_SW_Version,        3},
    [E_PARAMID_HW_Version]          = {E_PARAMID_HW_Version,        3},
    [E_PARAMID_CustomSW_Version]    = {E_PARAMID_CustomSW_Version,  3},
    [E_PARAMID_ManufactureData]     = {E_PARAMID_ManufactureData,   3},
    [E_PARAMID_ParatNumber]         = {E_PARAMID_ParatNumber,       5},
    [E_PARAMID_SupIdentifier]       = {E_PARAMID_SupIdentifier,     5},
...
int16_t ParameterSyncSdkInit(int16_t mpuHandle,pMcuParametReadFun_t mcuParamReadCb,pMcuParametWriteFun_t mcuParamWriteCb,uint16_t cycleTime,uint8_t *syncParamList,uint8_t syncParamSize)
{
    if(cycleTime == 0 || syncParamList == ((void *) 0) || syncParamSize == 0)
    {
        return -1;
    }
...
void ParameterSyncSdkCycleProcess(MpuHalDataPack_t *recvDataPack)
{
    ...
    uint8_t currentParamId = 0;
    uint8_t mpuDataLen = 0;
    uint8_t *pMpuData = ((void *) 0);
    
    if(recvDataPack!=((void *) 0))
    {
        LogHalPrint("Parameter sync : aid %d ,mid %d, subcommond %d\r\n",recvDataPack->aid,recvDataPack->mid,(recvDataPack->subcommand & 0x7F));
        if((recvDataPack->subcommand & 0x7F) == 2)
        {
            ...
            for(i = 0;i<recvDataPack->pDataBuffer[0];i++)
            {
                ...
                if(IsValidParamData(currentParamId,pMpuData,mpuDataLen) == 1) 
                {
                    ...
                    if(IsFlashDataValid(paramData,lenth) == 0) 
                    {
                        if(currentParamId == E_PARAMID_SW_Version) 
                        {
                        ...
                        LogHalPrint("change param %d\r\n",currentParamId);
                    }
                    else
                    {
                        
                        LogHalPrint("param %d has value\r\n",currentParamId);
                    }
                }
                else
                {
                    
                    LogHalPrint("paramdata error %d\r\n",currentParamId);
                }
    ...
    if((g_syncMpuParamResultFlag == 1)&&(g_syncParamToMpuRequstFlag==1))
    {
...
{
    int16_t ret = 0;
    if ((parameterItem >= E_PARAMETER_ITEM_NUM_MAX) || (flashParaId == ((void *) 0)))
    {
        ret = (-1);
    }
    else
...
int16_t McuParameterWrite(uint8_t parameterItem, uint8_t *parameterData, uint16_t pDataLength)
{
    ...
    do {
        ...
        if(flashParaId == E_PARAMETER_INFO_ECallNumber) 
        {
