...
void PowerManageHalInit(uint8_t mode,uint32_t time,uint32_t maxvoltage,uint32_t minvoltage)
{
    ...
    // PeripheralHalRegisterSetWakeupSourceCallback(PowerManageHalWakeupSourceChangeFun);   // TODO guanyuan
    if(mode == 1)
    {
      g_sleepmode = 1;
    }
