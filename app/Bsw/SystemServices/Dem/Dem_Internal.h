...
#define ENTRY_STORAGE_NUM       (DEM_MAX_NUMBER_EVENT_ENTRY_PRIMARY)
...
extern VAR(Dem_EventIdType, AUTOMATIC) DemPermanentMemory[DEM_MAX_NUMBER_EVENT_ENTRY_PERMANENT];
...
DEM_LOCAL_INLINE FUNC(uint8, DEM_CODE) Dem_GetDTCGroupIndex(uint32 DTCGroup)
{
    ...
    uint8 iloop;
    for (iloop = 0; iloop < DEM_GROUP_OF_DTC_NUM; iloop++)
    {
        if (DemPbCfg.GroupOfDTC[iloop] == DTCGroup)
        {
            ret = iloop;
            break;
        }
