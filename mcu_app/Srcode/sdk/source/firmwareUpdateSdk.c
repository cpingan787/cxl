...
static void FirmwareUpdateSdkEraseFlash(MpuHalDataPack_t *dataPack)
{
    uint8_t bank = 2;
...
static void FirmwareUpdateSdkLoadCode(MpuHalDataPack_t *dataPack)
{
    uint8_t *data = NULL;
    uint32_t len = 0;
    uint32_t addr = 0;
...
static void FirmwareUpdateSdkCodeCheck(MpuHalDataPack_t *dataPack)
{
    TBOX_PRINT("--------------------07 下载结束------------------\r\n");
    uint32_t checksum = 0;
    uint8_t bank = 2;
    //g_crcData ^= 0xFFFFFFFF;
    g_crcData = Crc32FinalResult(&g_crc32Object,g_crcData);
...
//        g_fotaModeFlag = 1;
    ...
    {
        FirmwareUpdateSdkEraseFlash(&dataPack);
    }
    else if(dataPack.mid == 0x06)  //下载升级数据
    {
        FirmwareUpdateSdkLoadCode(&dataPack);
    }
    else if(dataPack.mid == 0x07)  //下载结束
    {
        FirmwareUpdateSdkCodeCheck(&dataPack);
    }
