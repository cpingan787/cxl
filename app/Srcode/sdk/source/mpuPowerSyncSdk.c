...
/****************************** Macro Definitions ******************************/
#define ALLOW_SLEEP_DELAY_COUNT (30)             //MPU发送允许休眠后延时计数，单位秒
...
static uint8_t g_rkMode = 1;                     //功能模式  0：Init  1:正常模式  2：运输模式  3：生产模式
...
static uint32_t g_sleepDisableDelayCount = 0;    //切换到允许休眠后的延时计数
static uint8_t g_mpuKeepAliveLoseFlag = 1;       //mpu 心跳丢失标记 0：未丢失，1：丢失
static MpuErrorCbFun_t g_mpuErrorCallBackFunc = NULL;
static uint8_t g_mpuErrorResetCount = 0;        
static WakeUpSourceSet_t g_wakeUpSource;         //设置唤醒源
static uint8_t g_mpuWakeUpSource = 0;            //mpu唤醒源
static uint8_t g_mpuSubWakeUpSource = 0;         //mpu子唤醒源

/****************************** Public Function Implementations ******************************/
/*************************************************
  Function:       MpuPowerSyncSdkRequstSleep
  Description:    请求休眠接口
  Input:          sleepFlag:休眠标记 0：正常眠 1：深度休眠
  Output:         无
  Return:         0:执行成功
                  -1：执行失败
  Others:         
...
/*************************************************
  Function:       MpuPowerSyncSdkSendWakeSource
  Description:    发送唤醒源接口
...
/*************************************************
  Function:       MpuPowerSyncSdkSendPowerHalState
  Description:    发送车辆硬线状态接口
  Input:          无
  Output:         无
  Return:         0:执行成功
                  -1：执行失败
...
static int16_t MpuPowerSyncSdkSendPowerHalState(void)
{
    ...
    /* KL15状态 */
    g_packData[0] = PeripheralHalGetKl15Status() + 1;
    /* KL30电压 */
    PeripheralHalAdGet(AD0_CHANNEL_KL30,(uint32_t *)&voltage);
    ...
    g_packData[6] = 0;
    /* MCU启动原因 */
    resetReason = PowerManageSdkGetResetReason();
    g_packData[7] = (resetReason >> 24) & 0xFF;
    g_packData[8] = (resetReason >> 16) & 0xFF;
    g_packData[9] = (resetReason >> 8) & 0xFF;
    g_packData[10] = resetReason & 0xFF;

    g_powerSyncPack.pDataBuffer = g_packData;
    g_powerSyncPack.dataLength = 11;
...
/*************************************************
  Function:       MpuPowerSyncSdkSendBatteryState
  Description:    发送电池状态接口
  Input:          无
  Output:         无
  Return:         0:执行成功
                  -1：执行失败
...
/*************************************************
  Function:       MpuPowerSyncSdkSendPowerState
  Description:    发送电源状态接口
  Input:          无
  Output:         无
  Return:         0:执行成功
                  -1：执行失败
...
{
    GSensorHalData_t gSensorData;
    if(g_mpuHandle < 0)
    {
        return -1;
    }
    g_powerSyncPack.aid = 0x01;
    g_powerSyncPack.mid = 0x14;
    g_powerSyncPack.subcommand = 0x7F;
    ...
    if(GSensorHalGetValue(&gSensorData) == 0)
    {
        g_packData[0] = 0;
        g_packData[1] = (gSensorData.xAcceleratedData+30*1000) >= 0?(gSensorData.xAcceleratedData+30*1000):0;
        g_packData[2] = (gSensorData.yAcceleratedData+30*1000) >= 0?(gSensorData.yAcceleratedData+30*1000):0;
        g_packData[3] = (gSensorData.zAcceleratedData+30*1000) >= 0?(gSensorData.zAcceleratedData+30*1000):0;
    }
    else
    {
        g_packData[0] = 1; //错误
...
void MpuPowerSyncSdkCycleProcess(MpuHalDataPack_t *msgData)
{
    ...
    if(msgData != NULL)
    {
        ...
        if(msgData->aid == 0x01 && msgData->mid == 0x04)
        {
            ...
            else if((msgData->subcommand&0x7F) == 0x04) //MPU响应mcu的唤醒源
            {
                ...
                {
                    TBOX_PRINT("mpu wakeup %02X %02X %02X %02X \r\n", msgData->pDataBuffer[0], msgData->pDataBuffer[1], msgData->pDataBuffer[2], msgData->pDataBuffer[3]);
                    ...
                    {
                        if(msgData->pDataBuffer[2] != 0x0)
                        {
                            g_mpuWakeUpSource = msgData->pDataBuffer[2];
                            g_mpuSubWakeUpSource = msgData->pDataBuffer[3];
                        }
                    }
                }
                xSemaphoreGive(g_mutexHandle);
            ...
            {
                xSemaphoreTake(g_mutexHandle, portMAX_DELAY);
                /* 检测到从阻止休眠切换到允许休眠，启动延时 */
                if(g_lastSleepDisableEvent != 0 && msgData->pDataBuffer[1] == 0)
                {
                    g_sleepDisableDelayCount = ALLOW_SLEEP_DELAY_COUNT;  // 启动延时
                }
                /* 如果收到新的阻止休眠，清除延时 */
                if(msgData->pDataBuffer[1] != 0)
                {
                    g_sleepDisableDelayCount = 0;
                }
                g_lastSleepDisableEvent = msgData->pDataBuffer[1];
                g_sleepDisableEvent = msgData->pDataBuffer[1];
                xSemaphoreGive(g_mutexHandle);
                mpuKeepAliveTimeCount = 0;
                g_mpuErrorResetCount = 0;
    ...
    if(mpuKeepAliveTimeCount >= ((5 * 60 * 1000)/g_processCycleTime))
    {
        ...
        g_sleepDisableDelayCount = 0;  //清除允许休眠延时计数
...
/*************************************************
  Function:       MpuPowerSyncSdkSetSleepMode
  Description:    设置电源状态为深度休眠
  Input:          sleepMode：0:唤醒 1:正常休眠 2:深度休眠
  Output:         无
  Return:         无
  Others:         
*************************************************/
void MpuPowerSyncSdkSetSleepMode(uint8_t sleepMode)
{
...
int16_t MpuPowerSyncSdkGetSleepDisableState(void)
{
    ...
    /* 当前允许休眠，但停发阻止休眠后延时未结束，则返回阻止休眠 */
    if(sleepDisableEvent == 0 && g_sleepDisableDelayCount > 0)
    {
        effectiveSleepDisable = 1;  /* 停发阻止休眠后延时期内，阻止休眠 */
    }
    else
    {
...
/*************************************************
  Function:       MpuPowerSyncSdkGetRawSleepDisableEvent
  Description:    获取MPU禁止休眠事件
  Input:          无
  Output:         无
  Return:         0：无阻值休眠时间
                  非零值：有阻止休眠事件
...
/*************************************************
  Function:       MpuPowerSyncSdkGetMpuWakeUpSource
  Description:    获取MPU的唤醒源
  Input:          无
  Output:         无
  Return:         0：无唤醒源
  Others:         
*************************************************/
uint8_t MpuPowerSyncSdkGetMpuWakeUpSource(void)
{
    if(g_wakeUpSource.successFlag == 1)
    {
        return g_mpuWakeUpSource;
    }
    return 0;
}

/*************************************************
  Function:       MpuPowerSyncSdkGetMpuSubWakeUpSource
  Description:    获取MPU的子唤醒源
  Input:          无
  Output:         无
  Return:         0：无子唤醒源
  Others:         
*************************************************/
uint8_t MpuPowerSyncSdkGetMpuSubWakeUpSource(void)
{
    if(g_wakeUpSource.successFlag == 1)
    {
        return g_mpuSubWakeUpSource;
    }
    return 0;
}
