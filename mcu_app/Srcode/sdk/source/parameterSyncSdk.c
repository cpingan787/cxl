...
static const ParamWriteCheck_t g_paramWriteCheck[] = {
    ...
    [E_PARAMID_SN]                  = {E_PARAMID_SN,                27},//可删除
    [E_PARAMID_VIN]                 = {E_PARAMID_VIN,               17},//可删除
    [E_PARAMID_ECallNumber]         = {E_PARAMID_ECallNumber,       17},
    [E_PARAMID_SW_Version]          = {E_PARAMID_SW_Version,        64},
    [E_PARAMID_ParatNumber]         = {E_PARAMID_ParatNumber,       64},//可删除
        ...
        if((recvDataPack->subcommand & 0x7F) == 2)
        {
            ...
            for(i = 0;i<recvDataPack->pDataBuffer[0];i++)
            {
                ...
                if(IsValidParamData(currentParamId,pMpuData,mpuDataLen) == 1)/* 传入校验参数数据及长度是否有效 */
                {
                    ...
                    {
                        isWriteOK = ProjectConfigSetMpuVersion(pMpuData,mpuDataLen);
                        if(isWriteOK == 0)
                        {
                    ...
                    {
                        isWriteOK = g_mcuParameterWriteCbFunc(currentParamId,pMpuData,mpuDataLen);
