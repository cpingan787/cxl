...
#define DEM_START_SEC_VAR_NO_INIT_UNSPECIFIED
#include "Dem_MemMap.h"
VAR(DemJ1939DTCGetNodeType, AUTOMATIC) DemJ1939DTCGetNode; /* PRQA S 1504 */ /* VL_Dem_1504 */
#define DEM_STOP_SEC_VAR_NO_INIT_UNSPECIFIED
#include "Dem_MemMap.h"
#endif
/*******************************************************************************
...
FUNC(boolean, DEM_CODE) Dem_J1939CeckNode(uint16 dtcRef, uint8 node)
{
    boolean ret = FALSE;
    uint8 J1939NodeRef =
        DemPbCfgPtr->pDemDTCAttributes[DemPbCfgPtr->DemCfgDTC[dtcRef].DemDTCAttributesRef].DemJ1939DTC_J1939NodeRef;
    const J1939NodeType* lJ1939Node = &DemPbCfgPtr->pJ1939Node[J1939NodeRef];
    const uint8* nodeIndex = lJ1939Node->NodeIDRef;
    uint8 nodeNum = lJ1939Node->NodeNum;
    ...
    P2VAR(Dem_J1939DcmLampStatusType, AUTOMATIC, DEM_APPL_DATA) LampStatus)
{
    ...
    else
    {
        ...
        {
            DemJ1939DTCFilter.DTCOrigin = DEM_DTC_ORIGIN_PERMANENT_MEMORY;
        }
        else
        {
            DemJ1939DTCFilter.DTCOrigin = DEM_DTC_ORIGIN_PRIMARY_MEMORY;
        }
...
Dem_J1939DcmGetNumberOfFilteredDTC(P2VAR(uint16, AUTOMATIC, DEM_APPL_DATA) NumberOfFilteredDTC)
{
    ...
    else
    {
        ...
        {
            entryNum = DEM_MAX_NUMBER_EVENT_ENTRY_PERMANENT;
        }
        else
        {
            DTCOrigin = Dem_GetInternalMemDest(DEM_DTC_ORIGIN_PRIMARY_MEMORY);
            entryNum = DemMemDestCfg[DTCOrigin].EntryNum;
        }
        for (iloop = 0; iloop < entryNum; iloop++)
        {
            ...
            if (pFilter->DTCOrigin == DEM_DTC_ORIGIN_PERMANENT_MEMORY)
            {
                eventId = DemPermanentMemory[iloop];
                pEntry = &DemMemDestCfg[0u].EntryList[0u];
            }
            else
#endif
            {
                pEntry = &DemMemDestCfg[DTCOrigin].EntryList[iloop];
            ...
            if (eventId != 0x00u)
            {
                ...
                const Dem_DTCType* pDTC = &DemPbCfgPtr->DemCfgDTC[dtcRef];
...
/* PRQA S 6030 -- */
{
    ...
    else
    {
             ...
             ReadinessGroupIndex++)
        {
            ...
            {
                tempRef = DemPbCfgPtr->pDemEventParameter[iloop].DemDTCRef;
                if (DEM_DTC_REF_INVALID != tempRef)
                {
                        ...
                        && ((pObdDTC->DemEventOBDReadinessGroup == ReadinessGroupIndex)))
                    {
                        ...
                        if (J1939Dtc != 0UL)
                        {
                            ...
                            uint8 DemOperationCycleRef = DemPbCfgPtr->pDemEventParameter[iloop].DemOperationCycleRef;
