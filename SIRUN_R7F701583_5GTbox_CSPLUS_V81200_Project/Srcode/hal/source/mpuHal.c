...
#define MPU_HAL_TX_BUFFER                     3072//4096
#define MPU_HAL_RX_BUFFER                     4096
#define MPU_HAL_UART_INT_RX_BUF_LEN           500
...
static void MpuHalSetPower(uint8_t flag)
{
    ...
    {
        R_PORT_SetGpioOutput(Port1, 7, 1);
        R_PORT_SetGpioOutput(Port18, 3, 1);
    }
}
...
static void MpuHalSetPowerkey(uint8_t flag)
{
    ...
    {
        R_PORT_SetGpioOutput(Port18, 1, 1);
    }
}
        ...
        if((pHandleInstance->useFlag==1)&&(pHandleInstance->pDataBufferRx!=NULL))
        {
            if((pHandleInstance->useRxFilter==1)&&(pData[2]==pHandleInstance->rxFilterConfig.aid))
            {
                if((pData[3]>=pHandleInstance->rxFilterConfig.midMin)&&(pData[3]<=pHandleInstance->rxFilterConfig.midMax))
                {
                    ...
                    /******** dispatch data*******************************************/
                    data += length;
                    pHandleInstance->rxFrameLength = data;
#if(0)  // TODO guanyuan
                    if(irq)
                    {
...
int16_t MpuHalSetRxBuffer(int16_t handle,uint8_t* pBuffer,uint32_t bufferSize)
{
    if((handle>=0)&&(handle<MPU_HAL_HANDLE_INSTANSE_MAX))
    {
        if(pBuffer!=NULL)
        {
            // g_mpuManage.rxHandle[handle].rxQueueHandle = xQueueCreate(10,sizeof(uint32_t));  // TODO guanyuan
            g_mpuManage.rxHandle[handle].rxFrameLength = 0;
...
int16_t MpuHalReceive(int16_t handle,MpuHalDataPack_t* pRxMsg,uint32_t waitTime )
{
    ...
    // QueueHandle_t queHandle; // TODO guanyuan
    int16_t ret = MPU_HAL_STATUS_OK;
    ...
    if((handle>=0)&&(handle<MPU_HAL_HANDLE_INSTANSE_MAX))
    {
        ...
        if(pRxMsg!=NULL)
        {
            ...
            // if(xQueueReceive( queHandle, &data, waitTime)== pdPASS)  // TODO guanyuan
            if(g_mpuManage.rxHandle[index].rxFrameLength != 0)
            {
                data = g_mpuManage.rxHandle[index].rxFrameLength;
                g_mpuManage.rxHandle[index].rxFrameLength = 0;
...
void MpuHalTxTask(void)
{
    ...
    // while(1)
    {
        ...
        if(g_mpuUartTxBuffer.txFrameLength == 0)
        {
          return;
        }
        queueData = g_mpuUartTxBuffer.txFrameLength;
        g_mpuUartTxBuffer.txFrameLength = 0;
        //发送FIFO中的数据;
        i=0;
...
static int16_t MpuUartTransmit(const uint8_t* pTxData,uint16_t txLength)
{
    ...
    data |= txLength;
    // xQueueSend(g_mpuUartTxBuffer.txQueueHandle,&data,0); // TODO guanyuan
    g_mpuUartTxBuffer.txFrameLength = data;
    return 0;
}
...
void MpuHalUartPrintErrState(uint16_t cycleTime)
{
    ...
    {
        TBOX_PRINT("mpu uart error type: %d \r\n", g_mpuUartErrorType);
        g_mpuUartErrorFlag = 0;
        g_mpuUartErrorType = 0;
        if(g_mpuUartErrorType == 4)
        {
            errCount++;
            if(errCount > 10)  // TODO guanyuan，临时调试，接收持续错误，则复位MCU
            {
                errCount = 0;
