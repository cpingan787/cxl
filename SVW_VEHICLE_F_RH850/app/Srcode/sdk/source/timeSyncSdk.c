...
typedef struct
{
    ...
    uint8_t lastTimeSrc;        // 上一次时间源
    uint8_t timeSrcCnt;         // 时间源相同连续计数
} MpuTime_t;
...
static void TimeSyncSdkSendTimeSignalToCan(void)
{
    ...
    int16_t ret = TimeSyncGetFormatTimeFromUnix(g_systemTimeData.timestamp, TIME_ZONE, &sysTime);
    ...
    if(g_systemTimeData.timeSrc == TIME_SYNC_TIMESRC_GPS)
    {
        ...
        if (ret != 0)
        {
            ...
            g_systemTimeData.timeValidity = 0;
            Com_SendSignal(IGPSTiSts_IAM_CONNCAN_200ms_FrP01_CONTROLLER_0_IAM_Tx, &g_systemTimeData.timeValidity);
            return;
        }
        uint8 year8 = sysTime.year - 2000;
    ...
    else if(g_systemTimeData.timeSrc == TIME_SYNC_TIMESRC_NTP)
    {
        ...
        {
            g_systemTimeData.timeValidity = 0;
            Com_SendSignal(INTPTiSts_IAM_CONNCAN_200ms_FrP02_CONTROLLER_0_IAM_Tx, &g_systemTimeData.timeValidity);
            return;
        }
        uint8 year8 = sysTime.year - 2000;
...
void TimeSyncSdkCycleProcess(MpuHalDataPack_t *msgData)
{
    ...
    if (msgData != NULL && msgData->pDataBuffer != NULL && msgData->dataLength > 0)
    {
        ...
        if((msgData->subcommand & 0x7F) == TIME_SYNC_SUNBCOMMAND_MPU_TIME_TO_MCU_SYNC && msgData->dataLength >= 6)
        {
            ...
            **************/
            uint32_t timestamp = (pRxData[0] << 24) + (pRxData[1] << 16) + (pRxData[2] << 8) + pRxData[3];//时间戳
            uint8_t timeZone = pRxData[4];                                                                //时区
            uint8_t syncStatus = pRxData[5] & 0x0F;                                                       //时间同步状态
            uint8_t timeSrc = (pRxData[5] >> 4) & 0x0F;                                                   //时间源
            uint32_t rtcTime = 0;
            ...
            if (syncStatus == 0 || timeSrc == 0)
            {
                /* 未同步 清空时间同步数据 */
                g_systemTimeData.timestamp = 0;
                g_systemTimeData.timeZone = 0;
                g_systemTimeData.timeSyncStat = 0;
                g_systemTimeData.timeSrc = 0;
                g_systemTimeData.timeValidity = 0;
            ...
            {
                /* 已同步 更新时间同步数据 */
                g_systemTimeData.timestamp = timestamp;
                g_systemTimeData.timeZone = timeZone;
                g_systemTimeData.timeSyncStat = syncStatus;
                g_systemTimeData.timeSrc = timeSrc;
                g_systemTimeData.timeValidity = 1;

                /* 同步状态连续有效判断 */
                if(g_systemTimeData.timeValidityCnt < TIME_SYNC_TIME_SYNC_TO_RTC_CNT)
                {
                    g_systemTimeData.timeValidityCnt ++;         /* 时间同步计数器增加 */
                }

                /* 时间源连续相同判断 */
                if(g_systemTimeData.lastTimeSrc == timeSrc)
                {
                    if(g_systemTimeData.timeSrcCnt < TIME_SYNC_TIME_SYNC_TO_RTC_CNT)
                    {
                        g_systemTimeData.timeSrcCnt ++;         /* 时间源相同连续计数增加 */
                    }
                }
                else
                {
                    g_systemTimeData.lastTimeSrc = timeSrc;
                    g_systemTimeData.timeSrcCnt = 0;         /* 时间源相同连续计数清零 */
                }
...
int16_t TimeSyncSdkGetRealTime(FormatTime_t *formatTime)
{
    ...
    {
        if(TimeSyncGetFormatTimeFromUnix(time, TIME_ZONE, formatTime))
        {
            ret = 0;
        }
        else
...
void TimeSyncSdkSendTimeSignalToCanWithoutMpu(void)
{
    ...
    uint8_t year8 = 0;
    FormatTime_t sysTime = {0};
    static uint32_t canSignalTimeWithoutMpu = UTC_2000_BASE_TIME;// 时间未同步时，外发报文时间从2000年1月1日 00:00:00起始
    canSignalTimeWithoutMpu++;
    
    TimeSyncGetFormatTimeFromUnix(canSignalTimeWithoutMpu, TIME_ZONE, &sysTime);
