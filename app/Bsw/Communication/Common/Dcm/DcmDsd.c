...
FUNC(void, DCM_CODE) DsdInternal_ProcessPage(uint8 ProtocolCtrlId)
{
    ...
    if (Dcm_PageBufferData.TimeOut != TRUE)
    {
        ...
        pMsgContext = &Dcm_MsgCtrl[MsgCtrlIndexx].MsgContext;
        /** Start first page transmission*/
        pMsgContext->ResMaxDataLen = Dcm_PageBufferData.TotalSize;
        pMsgContext->ResDataLen = Dcm_PageBufferData.FilledSize;
        pMsgContext->pResData = Dcm_PageBufferData.PagedBuffer;
#if (STD_ON == DCM_GENERIC_CONNECTION)
        DcmRxPduIdx = pMsgContext->DcmRxPduId;
        ...
        PduInfo.SduLength = (PduLengthType)pMsgContext->ResMaxDataLen;
