 ...
 *         06-Oct-2019   : Modified the column width of Line210,211
 * V2.5.7: 14-Dec-2020   : 1.As part of ARDAABD-4680
 *                           To compare the ulJobStartAddress and
 *                           ulJobEndAddress before set the suspendRequest flag
 *                           in Api Fls_Suspend()
 *                         2.As part of ARDAABD-4636
 *                           In Api Fls_Write/Fls_Erase, after setting up the
 *                           "Fls_GstVar.enCommand", before calling the job
 *                           initialization function, set the global
 *                           variables Fls_GenJobResult to "MEMIF_JOB_PENDING"
 *                           and set Fls_GenState to "MEMIF_BUSY".
 *                         3.As part of ARDAABD-4961
 *                           Deleted the check of DFERSTR register
 *                           in Fls_CheckHWConsistency API.
 *                         4.As part of ARDAABD-4063
 *                           Added the check process of
 *                           Fls_UseFlashControllerPreHook and
 *                           Fls_UseFlashControllerEndHook API to
 *                           Fls_Erase/Fls_Write/Fls_Read/Fls_BlankCheck API.
 *                         5.As part of ARDAABD-4958
 *                           Added dummy read and SYNCP processing between
 *                           write operation of INTC2 and FPSYS
 *                           registers in Fls_Erase, Fls_Write and
 *                           Fls_BlankCheck API.
 *                           Added the reference UD ID in comment.
 * V2.5.8: 31-Mar-2021   : As part of ARDAABD-5040, the following changes are
 *                         made:
 *                         a) Critical section protection added in API
 *                            Fls_CheckHWConsistency.
 *                         b) 'FLS_ENTER_CRITICAL_SECTION' and
 *                            'FLS_EXIT_CRITICAL_SECTION' are added in the
 *                            'Function(s) invoked' field in the function
 *                            banner of Fls_CheckHWConsistency API.
 *                         c) Copyright information updated.
 * V2.5.9: 12-Apr-2021   : As part of review based on ARDAABD-5040, requirement
 *                         EAAR_PN0034_FR_0013 is mapped in newly added
 *                         Critical section protection in API
 *                         Fls_CheckHWConsistency.
 * V2.5.10: 24-Aug-2021  : As part of ARDAABD-6991, APIs Fls_Cancel, Fls_Erase,
 *                         Fls_Write, Fls_BlankCheck and Fls_Resume are
 *                         updated such that ICRxxx register will be accessed
 *                         in 8 bits when setting MKxxx bit using
 *                         'RH850_SV_CLEAR_ICR_SYNCP' and
 *                         'RH850_SV_MODE_ICR_xxx' macros.
 * V2.5.11: 14-Sep-2021  : As part of review based on ARDAABD-6991,
 *                         the following changes are made:
 *                         a) Updated 'Registers Used' for the APIs
 *                            Fls_Erase, Fls_Write, Fls_Cancel,
 *                            Fls_MainFunction and Fls_BlankCheck to add the
 *                            register ICDMAm.
 *                         b) Updated 'Registers Used' for the API Fls_Cancel
 *                            to add the register ICFLENDNM.
 *                         c) MISRA violation justification and tags for
 *                            Msg(2:1006) has been added.
 * V2.5.12: 22-Sep-2021  : As part of review based on ARDAABD-6991, updated the
 *                         tag of Msg(2:1006) as (4:1006).
 * V2.5.13: 20-Apr-2022  : As part of ARDAABD-8673, the following changes
 *                         are made:
 *                         a) Updated 'Fls_Cancel' API to call
 *                            'Fls_CallJobNotification' function after
 *                            exiting critical section.
 *                         b) Updated copyright information.
 *                         c) Updated 'Function(s) invoked' field in the
 *                            function banner of 'Fls_Cancel' API.
 * V2.5.14:  25-Sep-2025  :  Following changes are made:
 *                          1. As part of ARDAABD-10559, updated the type 
 *                          declaration of the argument TargetAddressPtr 
 *                          in Fls_ReadImmediate from 'pointer to constant data'
 *                          to 'pointer to variable data'."
 *                          2. As part of ARDAABD-10673, updated Local variable 
 *                          to hold the blank check start/end address in Fls_BlankCheck 
 */
/******************************************************************************/
/*******************************************************************************
...
/* Verification  : However, part of the code is verified manually and it is   */
/*                 not having any impact.                                     */
/* Reference     : Look for START Msg(4:1006)-10  and                         */
/*                 END Msg(4:1006)-10  tags in the code.                      */
/******************************************************************************/
/**                         QAC Warning                                      **/
/******************************************************************************/
/* 1. QAC Warning:                                                            */
...
)
{
  ...
  if (E_OK == LucReturnValue)
  {
    ...
    {
     /* MISRA Violation: END Msg(4:4397)-1 */
      /* Report Error to DET */
      (void)Det_ReportError (
                              FLS_MODULE_ID,
                              FLS_INSTANCE_ID,
  ...
  /* Implements: FLS268, SWS_Fls_00268 */
  ...
  #endif /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
     ...
     /* Implements: FLS191, SWS_Fls_00191, FLS_ESDD_UD_031, FLS_ESDD_UD_147 */
     Fls_GpConfigPtr  = ConfigPtr;
     #if ((FLS_SET_MODE_API == STD_ON) &&\
          (FLS_DMA_INSTANCES != FLS_DMA_ZERO))
     /* QAC Warning: START Msg(2:2814)-3 */
     if (NULL_PTR != ConfigPtr->pDmaUnitConfig)
     {
     /* END Msg(2:2814)-3 */
       /* Pointer to DMA channel configuration */
       Fls_GpDmaUnitConfig = ConfigPtr->pDmaUnitConfig;
    ...
    /* Data flash initialization */
     ...
     if ((FLS_FCU_OK == Fls_GstVar.enDFStatus)
         #if (STD_ON == FLS_ECC_INITCHK_SUPPORT)
         &&(FLS_FCU_OK == LenECCStatus)
         #endif /* #if (STD_ON == FLS_ECC_INITCHK_SUPPORT) */
         )
     {
        ...
        /* Implements: FLS323, SWS_Fls_00323 */
        /* Set the driver status to idle */
        Fls_GenState = MEMIF_IDLE;
        /* Implements: FLS324, SWS_Fls_00324 */
        ...
        /* Initialize the global variables requested length and current length
           length to zero. */
        /* Implements: FLS_ESDD_UD_191, FLS_ESDD_UD_190 */
...
**                             FASTAT,DCENm,DTCTm,ICFLENDNM,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PUBLIC_CODE
/* MISRA Violation: START Msg(2:0832)-5 */
#include FLS_MEMMAP_FILE
...
)
{
  ...
  LucReturnValue = Fls_DetErrorCheck(FLS_ERASE_SID);
  #endif /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  ...
  Fls_GstVar.ulJobEndAddress = (TargetAddress + Length) - (uint32)FLS_ONE;
  /* Implements: FLS_ESDD_UD_036 */
  #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON)
  ...
  #if (FLS_DEV_ERROR_DETECT == STD_ON)
  /* Implements: FLS_ESDD_UD_019, FLS_ESDD_UD_023, FLS_ESDD_UD_111 */
  /* Implements: FLS_ESDD_UD_024 */
  if (
      (
        (uint32)FLS_ZERO !=
        (Fls_GstVar.ulJobStartAddress & (uint32)(FLS_DF_BLOCK_SIZE - FLS_ONE))
      ) ||
      (Fls_GstVar.ulJobStartAddress < FLS_DF_SECTOR_START_ADDRESS) ||
      (
        (Fls_GstVar.ulJobStartAddress - FLS_DF_SECTOR_START_ADDRESS) >
  ...
  if (
      ...
      /* check for block upper boundary */
      (
        (
          (Fls_GstVar.ulJobEndAddress - FLS_DF_SECTOR_START_ADDRESS) +
          (uint32)FLS_ONE
        ) >
        (uint32)FLS_DF_TOTAL_SIZE
      ) ||
      /* check for alignment of length to block boundary */
      (
        (uint32)FLS_ZERO !=
        (
          (Fls_GstVar.ulJobEndAddress + (uint32)FLS_ONE) &
  ...
  #endif /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
    /* Implements: FLS_ESDD_UD_268, FLS_ESDD_UD_270, FLS_ESDD_UD_271 */
    #if (STD_ON == FLS_USEFLASHCONTROLLERHOOK)
...

    {
      ...
      if ((uint32)FLS_ZERO != (Length & (uint32)(FLS_DF_BLOCK_SIZE - FLS_ONE)))
      {
        Length = (Length + (uint32)(FLS_DF_BLOCK_SIZE - FLS_ONE)) &
                                (~((uint32)(FLS_DF_BLOCK_SIZE - FLS_ONE)));
        Fls_GstVar.ulJobEndAddress = (TargetAddress + Length) - (uint32)FLS_ONE;
      }
      else
      ...
      /* MISRA Violation: START Msg(4:1006)-10 */
      RH850_SV_CLEAR_ICR_SYNCP(8, &(FLS_ICFLENDNM), (FLS_RESET_INT_MK));
      /* END Msg(4:1006)-10 */
      /* END Msg(4:0303)-9 */
      /* END Msg(2:2814)-3 */
      #endif /* End of #if (FLS_INTERRUPT_MODE == STD_ON) */
      /* Implements: FLS_ESDD_UD_191 */
      /* Set the command as Erase command */
      Fls_GstVar.enCommand = FLS_COMMAND_ERASE;
      ...
      #endif /* End of #if (FLS_SUSPEND_API == STD_ON) */
      {
             ...
             (STD_ON == FLS_USEFLASHCONTROLLERHOOK))
        if (FLS_ACCESS_BUSY == LenAccessStatusType)
        {
          LucReturnValue = E_OK;
        }
        else
...
)
{
  ...
  #if (FLS_DEV_ERROR_DETECT == STD_ON)
  /* Invoking function to check whether driver is initialised/busy */
  LucReturnValue = Fls_DetErrorCheck(FLS_WRITE_SID);
  ...
  Fls_GstVar.ulJobStartAddress = TargetAddress;
  Fls_GstVar.ulJobEndAddress = (Fls_GstVar.ulJobStartAddress + Length) -
    (Fls_LengthType)FLS_ONE;
  ...
  #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON)
  /* Enable relevant interrupts to exit this critical section */
  /* Implements: EAAR_PN0034_FR_0013, FLS_ESDD_UD_023, FLS_ESDD_UD_027 */
  FLS_EXIT_CRITICAL_SECTION(FLS_DRIVERSTATE_DATA_PROTECTION);
  #endif
  #if (FLS_DEV_ERROR_DETECT == STD_ON)
  ...
  if (
      ((uint32)FLS_ZERO == Length) ||
      (((TargetAddress + Length) & (uint32)(FLS_PAGE_SIZE - FLS_ONE)) !=
          (uint32)FLS_ZERO) ||
      (
        ((TargetAddress - FLS_DF_SECTOR_START_ADDRESS) + Length) >
        (uint32)FLS_DF_TOTAL_SIZE
      )
    )
  {
  ...
  /* Implements: FLS_ESDD_UD_031, FLS_ESDD_UD_147 */
  #if ((FLS_SET_MODE_API == STD_ON) &&\
       (FLS_DMA_INSTANCES != FLS_DMA_ZERO))
  if ((MEMIF_MODE_FAST == Fls_GenCurrentMode) &&
      (NULL_PTR == Fls_GpDmaUnitConfig))
  {
    /* Report error to DET */
    (void)Det_ReportError (
                            FLS_MODULE_ID,
  ...
  #endif  /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    #endif /* (STD_ON == FLS_USEFLASHCONTROLLERHOOK) */
    {
      ...
      if (MEMIF_MODE_FAST == Fls_GenCurrentMode)
      {
        LucReturnValue = Fls_SuspendPreCheck(FLS_COMMAND_DMA_WRITE);
      }
      else
      #endif /* End of #if ((FLS_SET_MODE_API == STD_ON) &&\
                            (FLS_DMA_INSTANCES != FLS_DMA_ZERO)) */
      ...
      #endif /* End of #if (FLS_SUSPEND_API == STD_ON) */
      {
        ...
        if (MEMIF_MODE_FAST == Fls_GenCurrentMode)
        {
          /* Set the command as DMA Write command */
          Fls_GstVar.enCommand = FLS_COMMAND_DMA_WRITE;
          /* Set the job result to Job pending */
          Fls_GenJobResult = MEMIF_JOB_PENDING;
          /* Set the driver state as Busy */
          Fls_GenState = MEMIF_BUSY;
          ...
          #endif /* (STD_OFF == FLS_INTERRUPT_MODE) */
          {
            /* Initiate DMA Write Job */
            LucReturnValue = Fls_InitiateDmaWriteJob();
          }
        }
        else
        #endif /* End of #if ((FLS_SET_MODE_API == STD_ON) &&\
                              (FLS_DMA_INSTANCES != FLS_DMA_ZERO)) */
        {
          ...
          Fls_GstVar.enCommand = FLS_COMMAND_WRITE;
          /* Set the job result to Job pending */
          Fls_GenJobResult = MEMIF_JOB_PENDING;
          /* Set the driver state as Busy */
          Fls_GenState = MEMIF_BUSY;
...
**                              Fls_CallJobNotification,Fls_RestoreIntReq,
**                              Fls_ClearBackUpData,Fls_DisableDmaChannel
**
** Registers Used             : DTCTm,DCENm,FSADDR,FEADDR,FSTATR,
**                              FENTRYR,FASTAT,ICFLENDNM,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PUBLIC_CODE
/* MISRA Violation: START Msg(2:0832)-5 */
#include FLS_MEMMAP_FILE
...
FUNC(void, FLS_PUBLIC_CODE)Fls_Cancel(void)
{
  ...
  #endif  /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    if ((MEMIF_JOB_PENDING == Fls_GenJobResult)  ||
                 (FLS_COMMAND_NONE != Fls_GstBackUpVar.enCommand))
    #else
    if (MEMIF_JOB_PENDING == Fls_GenJobResult)
    /* END Msg(2:3416)-2 */
    #endif
    {
      ...
      FLS_ENTER_CRITICAL_SECTION(FLS_DRIVERSTATE_DATA_PROTECTION);
        ...
        #if ((FLS_SUSPEND_API == STD_ON) || (FLS_RESUME_API == STD_ON))
        if (FLS_COMMAND_NONE != Fls_GstBackUpVar.enCommand)
        {
          LenCmdToCancel = Fls_GstBackUpVar.enCommand;
        }
        else
        #endif
        {
          LenCmdToCancel = Fls_GstVar.enCommand;
        }
      ...
      case FLS_COMMAND_DMA_WRITE:
            ...
            #endif /* End of #if ((FLS_SET_MODE_API == STD_ON) &&
                 (FLS_DMA_INSTANCES != FLS_DMA_ZERO)) */
      /* MISRA Violation: START Msg(4:2003)-7 */
      ...
      case FLS_COMMAND_BLANKCHECK:
            ...
            {
              /* If internal error occurs due to hardware failure
                 set the Driver State to IDLE */
              Fls_GenState = MEMIF_IDLE;
      ...
      /* Call the JOB end call back function */
...
**                             ICFLENDNM,DFERSTC,DFERSTR,ICDMAm
*******************************************************************************/
#define FLS_START_SEC_PUBLIC_CODE
/* MISRA Violation: START Msg(2:0832)-5 */
#include FLS_MEMMAP_FILE
...
FUNC(void, FLS_PUBLIC_CODE)Fls_MainFunction(void)
{
  ...
  {
    #if (FLS_DEV_ERROR_DETECT == STD_ON)
    /* Implements: SWS311, SWS_Fls_00311 */
    /* Report error to DET that module is not initialized */
    (void)Det_ReportError (
                            FLS_MODULE_ID,
                            FLS_INSTANCE_ID,
                            FLS_MAINFUNCTION_SID,
                            FLS_E_UNINIT
                          );
    #endif /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  }
  else
  {
    ...
    /* END Msg(2:3416)-2 */
    {
      ...
      case FLS_COMMAND_ERASE :
          ...
          Fls_MainErase();
          #endif /* End of #if (FLS_INTERRUPT_MODE == STD_OFF) */
      ...
      case FLS_COMMAND_DMA_WRITE :
          #if (FLS_INTERRUPT_MODE == STD_OFF)
          Fls_MainWrite();
          #elif ((FLS_SET_MODE_API == STD_ON) &&\
                (FLS_DMA_INSTANCES != FLS_DMA_ZERO))
          if (MEMIF_MODE_FAST == Fls_GenCurrentMode)
          {
            LenStatus = Fls_FcuCheckJobStatus();
            if ((FLS_FCU_ERR_WRITE == LenStatus) ||
...
)
{
  ...
  #if (FLS_DEV_ERROR_DETECT == STD_ON)
  /* Invoking function to check whether driver is initialized/busy */
  LucReturnValue = Fls_DetErrorCheck(FLS_READ_SID);
  ...
  #endif /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON)
    /* Disable relevant interrupts to protect this critical section */
    /* Implements EAAR_PN0034_FR_0013, FLS_ESDD_UD_023, FLS_ESDD_UD_111 */
    FLS_EXIT_CRITICAL_SECTION(FLS_DRIVERSTATE_DATA_PROTECTION);
    #endif /* End of #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON) */
     ...
     * address boundary.
     */
    /* Implements: FLS_ESDD_UD_024, FLS_ESDD_UD_026 */
    if (
        ((uint32)FLS_ZERO == Length) ||
        (
          (
    ...
    #endif  /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
    {
      ...
      #endif /* (STD_ON == FLS_USEFLASHCONTROLLERHOOK) */
      {
        ...
        if ((uint32)FLS_ZERO != (Length & (uint32)(FLS_PAGE_SIZE - FLS_ONE)))
        {
          /* Round off the length value to the page boundary */
          Length = Length + ((uint32)FLS_PAGE_SIZE -
                             (Length & (uint32)(FLS_PAGE_SIZE - FLS_ONE)));
        }
        else
        {
          /* No action required */
        ...
        Fls_GstVar.enCommand = FLS_COMMAND_READ;
...
)
{
  ...
  #if (FLS_DEV_ERROR_DETECT == STD_ON)
  /* Invoking function to check whether driver is initialised/busy */
  LucReturnValue = Fls_DetErrorCheck(FLS_COMPARE_SID);
  ...
  #endif /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    #endif  /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
    {
      ...
      Fls_GstVar.enCommand = FLS_COMMAND_COMPARE;
      /* set the driver state to busy */
      Fls_GenState = MEMIF_BUSY;
      /* set the job result as pending */
      Fls_GenJobResult = MEMIF_JOB_PENDING;
...
**                             Fls_GstVar.ulSrcDestAddress,
**                             Fls_GstVar.enCommand,Fls_GenState,
**                             Fls_GenJobResult,Fls_UseFlashControllerPreHook,
**                             Fls_UseFlashControllerEndHook
**
...
)
{
  ...
  Fls_GstVar.ulRequestedLength = Length;
  /* Local variable to hold the blank check start/end address */
  ...
  #endif  /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
    ...
    LenAccessStatusType = Fls_UseFlashControllerPreHook(FLS_COMMAND_BLANKCHECK);
...
FUNC(Std_ReturnType, FLS_PUBLIC_CODE) Fls_Suspend(void)
{
  ...
  #endif  /* End of #if (FLS_DEV_ERROR_DETECT == STD_ON) */
  {
      ...
      )
    {
      ...
      FLS_ENTER_CRITICAL_SECTION(FLS_DRIVERSTATE_DATA_PROTECTION);
      #endif /* End of #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON) */
      /* Check if the last block is writing or erasing */
...
**
** Function(s) invoked   : Det_ReportError, FLS_ENTER_CRITICAL_SECTION,
**                         FLS_EXIT_CRITICAL_SECTION
**
** Registers Used        : DFERRINT,FASTAT,FEADDR,FENTRYR,FSADDR,FPCKAR,
...
)
{
  ...
  if (FLS_UNINITIALIZED == Fls_GblInitStatus)
  {
    (void)Det_ReportError (
                            FLS_MODULE_ID,
                            FLS_INSTANCE_ID,
                            FLS_CHECK_HW_CONSISTENCY_SID,
                            FLS_E_UNINIT
                          );
    LucReturnValue = E_NOT_OK;
  }
  else if (
            (LenHWConsistencyMode != FLS_DYNAMIC) &&
            (LenHWConsistencyMode != FLS_STATIC)
          )
  {
    (void)Det_ReportError (
                            FLS_MODULE_ID,
  ...
  {
    /* Implements: EAAR_PN0034_FSR_0005, FLS_ESDD_UD_237, FLS_ESDD_UD_236 */
    if (FLS_DYNAMIC == LenHWConsistencyMode)
    {
      #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON)
      /* Disable relevant interrupts to protect this critical section */
      /* Implements EAAR_PN0034_FR_0013 */
      ...
      if (
          (
            ...
            ((uint8)Fls_GaaRamMirror[RAM_IDX_DFERRINT] & RAM_MASK_DFERRINT)
          )                                                                   ||
          (
            FLS_GpFACIReg_BaseAddress->ucFASTAT   !=
            ((uint8)Fls_GaaRamMirror[RAM_IDX_FASTAT]  & RAM_MASK_FASTAT)
          )                                                                   ||
          (
            FLS_GpFACIReg_BaseAddress->ulFEADDR   !=
            (Fls_GaaRamMirror[RAM_IDX_FEADDR]  & RAM_MASK_FEADDR)
          )                                                                   ||
          (
            FLS_GpFACIReg_BaseAddress->usFENTRYR  !=
            ((uint16)Fls_GaaRamMirror[RAM_IDX_FENTRYR] & RAM_MASK_FENTRYR)
          )                                                                   ||
          (
            FLS_GpFACIReg_BaseAddress->ulFSADDR  !=
            (Fls_GaaRamMirror[RAM_IDX_FSADDR] & RAM_MASK_FSADDR)
          )                                                                   ||
          (
            (FLS_GpFACIReg_BaseAddress->usFPCKAR) !=
            ((uint16)Fls_GaaRamMirror[RAM_IDX_FPCKAR] & RAM_MASK_FPCKAR)
          )
        #if ((FLS_SET_MODE_API == STD_ON) &&\
             (FLS_DMA_INSTANCES != FLS_DMA_ZERO))
                                                                              ||
          (LpDmaRegisters->ulDTCT != Fls_GaaRamMirror[RAM_IDX_DTCT])          ||
          (LpDmaRegisters->ulDCEN != Fls_GaaRamMirror[RAM_IDX_DCEN])
        #endif /* End of #if ((FLS_SET_MODE_API == STD_ON) &&\
                              (FLS_DMA_INSTANCES != FLS_DMA_ZERO)) */
        )
...
/* Only DMA transfer feature has STATIC registers */
#if ((FLS_SET_MODE_API == STD_ON) && (FLS_DMA_INSTANCES != FLS_DMA_ZERO))
    else
    {
      #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON)
      /* Disable relevant interrupts to protect this critical section */
      ...
      if (
         (
          ...
          (LpDmaRegisters->ulDRDA != FLS_FCU_DFLASH_CMD_ADD ) ||
          (LpDmaRegisters->ulDRTC != FLS_DMA_DRTC_REG_VALUE )
        )
      {
        LucReturnValue = E_NOT_OK;
      }
