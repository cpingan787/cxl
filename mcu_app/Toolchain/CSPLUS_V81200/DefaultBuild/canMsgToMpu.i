...
CanMsgRxConfigureBuffer_t g_canRxToCpuConfigureBuffer[1][100];
static CanIdConfigure_t g_canRxToCpuConfigure[1];

static StackBuffer_t g_stack;
static CpuCanMsgBuffer_t g_cpuCanRxQueue[10];
static volatile uint16_t g_cpuCanRxQueueWrite = 0;
static volatile uint16_t g_cpuCanRxQueueRead = 0;

uint8_t g_txBuffer[100*14+3+8];
...
{
  if((pQueueMsg == ((void *) 0)) || (pCanMsg == ((void *) 0)))
  {
    return;
  }

  if(pCanChannel != ((void *) 0))
  {
...
int16_t CanMsgTransmitToCpu(int16_t mpuHandle)
{
    ...
    while (CpuCanRxQueuePeek(&queueMsg) == 0)
    {
        ...
        {
            LogHalPrint("CAN ID 0x361, DLC: %d, Data: ", canMsg . dlc);
            for (uint8_t j = 0; j < canMsg.dlc; j++)
            {
                LogHalPrint("0x%02X ", canMsg . canData[j]);
            }
            LogHalPrint("\n");
        }
...
int16_t CanMsgConfigureBufferAdd(uint8_t canChannel,CanMsgRxConfigure_t *pElement)
{
  ...
  if(g_canRxToCpuConfigure[canChannel].elementCount>=100)
  {
    return -1;
  }
