...
{
    switch (DemPbCfgPtr->OperationCycle[OperationId].DemOperationCycleType)
    {
#if (DEM_OBD_SUPPORT != DEM_OBD_NO_OBD_SUPPORT)
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_CalOBDRelatedValue(uint16 OperationId)
{
    uint16 iloop;
    const Dem_EventParameterType* pEventCfg = DemPbCfgPtr->pDemEventParameter;
...
FUNC(void, DEM_CODE) Dem_UpdateOBDMilStatus(uint8 indicatorRef, uint16 IntID)
{
    ...
    DemDTCRef = DemPbCfgPtr->pDemEventParameter[IntID].DemDTCRef;
    uint32 DemDtcValue = DemPbCfgPtr->DemCfgDTC[DemDTCRef].DemDtcValue;
    uint16 DemObdDTCRef = DemPbCfgPtr->DemCfgDTC[DemDTCRef].DemObdDTCRef;
    ...
    if ((indicatorRef == DEM_MALFUNCTION_LAMP_INDICATOR) && (DemDTCRef != DEM_DTC_REF_INVALID))
    {
        ...
        if (currentMilStatus != DEM_INDICATOR_OFF)
        {
            ...
            if ((DemDtcValue < 0xFFFF33UL) || (pObdDTC[DemObdDTCRef].DemDtcValue != DEM_OBD_DTC_CFG_INVALID))
            {
                /* req SWS_Dem_01139 */
...
Dem_DcmReadDataOfPID01(P2VAR(uint8, AUTOMATIC, DEM_APPL_DATA) PID01value)
{
    ...
    else
    {
        ...
        for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
        {
            ...
            const Dem_DTCType* pDTC = &DemPbCfgPtr->DemCfgDTC[tempRef];
             ...
             ReadinessGroupIndex++)
        {
            ...
            for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
            {
                ...
                if ((pEventCfg[iloop].DemEventAvailable == TRUE) && (tempRef != DEM_DTC_REF_INVALID))
                {
                    obdDtcRef = DemPbCfgPtr->DemCfgDTC[tempRef].DemObdDTCRef;
                    /* req SWS_Dem_01101 */
                    if ((DemDTCGeneralStatus[tempRef].SuppressionStatus != TRUE) && (obdDtcRef != DEM_OBD_DTC_INVALID)
    ...
    else
    {
             ...
             ReadinessGroupIndex++)
        {
            for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
            {
                ...
                {
                    obdDtcRef = DemPbCfgPtr->DemCfgDTC[tempRef].DemObdDTCRef;
                    const DemObdDTCType* pObdDTC = &DemPbCfgPtr->ObdDTC[obdDtcRef];
                    /* req SWS_Dem_01101*/
                    if ((DemDTCGeneralStatus[tempRef].SuppressionStatus != TRUE) && (obdDtcRef != DEM_OBD_DTC_INVALID)
                        && (pObdDTC->DemEventOBDReadinessGroup == ReadinessGroupIndex))
                    {
                        ...
                        if (obdDtc != DEM_OBD_DTC_CFG_INVALID)
                        {
                            ...
                            if (((DEM_BITS_ISSET(DemOperationCycleStatus, OCRef)))
                                && (DemPbCfgPtr->OperationCycle[OCRef].DemOperationCycleType == DEM_OPCYC_POWER)
        ...
        if ((0u == FrameNumber) && (DEM_DTC_FORMAT_OBD == DTCFormat))
        {
            ...
            if ((pEntry != NULL_PTR) && (IntId != DemPbCfgPtr->DemEventNum))
            {
                ...
                if ((pEventCfg->DemDTCRef != DEM_DTC_REF_INVALID)
                    && (DEM_OBD_DTC_INVALID != DemPbCfgPtr->DemCfgDTC[pEventCfg->DemDTCRef].DemObdDTCRef)
                    && (DemDTCGeneralStatus[pEventCfg->DemDTCRef].SuppressionStatus == FALSE))
                {
                    *DTC = (uint32)DemPbCfgPtr->ObdDTC[DemPbCfgPtr->DemCfgDTC[pEventCfg->DemDTCRef].DemObdDTCRef]
                               .DemDtcValue;
                    ret = E_OK;
                }
            }
...
Dem_ReadDataOfPID01(P2VAR(uint8, AUTOMATIC, DEM_APPL_DATA) PID01value)
{
    ...
    else
    {
        ...
        for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
        {
            ...
            {
                tempRef = pEventCfg->DemDTCRef;
                pDTC = &DemPbCfgPtr->DemCfgDTC[tempRef];
                ObdDTCRef = pDTC->DemObdDTCRef;
             ...
             ReadinessGroupIndex++)
        {
            ...
            for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
            {
                ...
                if (pEventCfg->DemEventAvailable == TRUE)
                {
                    ...
                    if ((DEM_DTC_REF_INVALID != tempRef) && (DEM_OBD_DTC_INVALID != ObdDTCRef)
                        && (DemDTCGeneralStatus[tempRef].SuppressionStatus != TRUE)
                        && (DemPbCfgPtr->ObdDTC[ObdDTCRef].DemEventOBDReadinessGroup == ReadinessGroupIndex))
                    {
                        obdDtc = DemPbCfgPtr->ObdDTC[ObdDTCRef].DemDtcValue;
                        if (obdDtc != DEM_OBD_DTC_CFG_INVALID)
                        {
                            const Dem_EventRelateInformationType* pEvent = Dem_GetEventInfo(iloop);
                            DEM_BITS_SET(supportedGroups, ReadinessGroupIndex);
...
/*************************************************************************/
FUNC(Std_ReturnType, DEM_CODE)
Dem_SetDataOfPID21(P2CONST(uint8, AUTOMATIC, DEM_APPL_DATA) PID21value)
{
    Std_ReturnType ret = E_NOT_OK;
    if (DEM_STATE_INIT != Dem_InitState)
    {
...
/*************************************************************************/
FUNC(Std_ReturnType, DEM_CODE)
Dem_SetDataOfPID31(P2CONST(uint8, AUTOMATIC, DEM_APPL_DATA) PID31value)
{
    Std_ReturnType ret = E_NOT_OK;
    if (DEM_STATE_INIT != Dem_InitState)
    {
...
/*************************************************************************/
FUNC(Std_ReturnType, DEM_CODE)
Dem_SetDataOfPID4D(P2CONST(uint8, AUTOMATIC, DEM_APPL_DATA) PID4Dvalue)
{
    Std_ReturnType ret = E_NOT_OK;
    if (DEM_STATE_INIT != Dem_InitState)
    {
...
Dem_SetPfcCycleQualified(void)
{
    ...
    else
    {
        ...
        {
            if (DemPbCfgPtr->OperationCycle[iloop].DemOperationCycleType == DEM_OPCYC_OBD_DCY)
            {
                PFCStatu = TRUE;
            }
        }
...
Dem_GetPfcCycleQualified(P2VAR(boolean, AUTOMATIC, DEM_APPL_DATA) isqualified)
{
    ...
    else
    {
        for (iloop = 0; iloop < DemPbCfgPtr->DemOperationCycleNum; iloop++)
        {
            ...
            {
                if (Dem_GetDemWIRStatus(DEM_MALFUNCTION_LAMP_INDICATOR) != DEM_INDICATOR_OFF)
                {
                    PFCStatu = TRUE;
                }
            }
...
Dem_SelectOBDFreezeFrame(P2VAR(Dem_EventIdType, AUTOMATIC, DEM_APPL_DATA) IntId)
{
    ...
    *IntId = DemPbCfgPtr->DemEventNum;
    ...
    for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
    {
        ...
        {
            uint8 DemWWHOBDDTCClass = DemPbCfgPtr->DemCfgDTC[DTCRef].DemWWHOBDDTCClass;
...
DEM_LOCAL FUNC(void, DEM_CODE) Dem_B1CounterProcess(void)
{
    ...
    for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
    {
        ...
        if ((DEM_DTC_REF_INVALID != DTCRef)
            && (DemPbCfgPtr->DemCfgDTC[DTCRef].DemWWHOBDDTCClass == DEM_DTC_WWHOBD_CLASS_B1)
