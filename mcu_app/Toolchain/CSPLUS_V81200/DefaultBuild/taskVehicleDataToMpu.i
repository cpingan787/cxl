...
static void TboxCanRxCycleProcess(void)
{
    ...
    ret = Com_ReceiveSignalGroup(16u);
    if(ret != 0U)
    {
    ...
    ret = Com_ReceiveSignal(738u, &signalValue);
    if(ret == 0U)
    {
        g_vehicleInfor.userModeValid = signalValue;
    }
    ret = Com_ReceiveSignal(739u, &signalValue);
    if(ret == 0U)
    {
        g_vehicleInfor.userMode = signalValue;
    }
    ret = Com_ReceiveSignal(737u, &signalValue);
    if(ret == 0U)
    {
        g_vehicleInfor.vehicleModeValid = signalValue;
    }
    ret = Com_ReceiveSignal(734u, &signalValue);
    if(ret == 0U)
    {
...
void TaskVehicleDataToCpu(void)
{
    ...
    {     
        ...
        {      
            ...
            {
                CanMsgToCpuConfigureSyncCycleProcess(((void *) 0),10);
            }
        }
    }  	
}
