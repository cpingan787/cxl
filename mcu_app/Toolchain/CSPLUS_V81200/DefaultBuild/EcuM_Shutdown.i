...
static  void EcuM_SetSdtg(
    EcuM_RunTimeType * pRt,
    EcuM_ShutdownTargetType target,
    EcuM_ShutdownModeType mode);

static  void EcuM_GetSdtg(
    const EcuM_RunTimeType * pRt,
    EcuM_ShutdownTargetType * pTarget,
    EcuM_ShutdownModeType * pMode);
 ...
 void EcuM_Shutdown(void)
{
    const EcuM_RunTimeType * pRt = &EcuMRunData;
    {
...

        {
            ...
            if ((uint8)1u == (pRt)->SdtgNext.Target)
            {
                EcuM_AL_Reset((EcuM_ResetType)((pRt->SdtgNext.Mode) - 256u));
            }
            else if ((uint8)2u == (pRt->SdtgNext.Target))
            {
...
EcuM_DoShutDownPhase(uint16 caller)
{
    ...
    if ((Std_ReturnType)0U == ret)
    {
        if ((boolean)1u == EcuM_UserCfgs[userIndex].goDownAllowed)
        {
            EcuM_OffPreOS();
        }
    }
...
EcuM_OffPreOS(void)
{
...

    {
        ...
        BswM_EcuM_CurrentState((EcuM_StateType)0x40);
    }
    ...
    SchM_Deinit();
    {
        ...
        if (0U != pendingWkSrcs)
        {
            (void)EcuM_SelectShutdownTarget((uint8)1u, EcuM_ConfigPtr->defaultShutdownMode);
        }
    }
