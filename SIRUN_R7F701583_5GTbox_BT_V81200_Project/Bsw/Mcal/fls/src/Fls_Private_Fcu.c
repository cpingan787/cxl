 ...
 *                             improved.
 *         06-Oct-2019   :  Modified the Revision history year of Line 105
 * V2.5.9  22-Jun-2020   :  ARDAABD-4725 - Modify the temporary variable
 *                          LucLoopCount(uint8) to LulLoopCount(uint32) in
 *                          Function Fls_FcuSwitchMode and Fls_FcuForcedStop.
 *                          ARDAABD-4015 - Changes name of variable "Luscount"
 *                          which is used in Function Fls_FcuPerformBlankCheck
 *                          to "LulCount".
 * V2.5.10 31-Mar-2021   :  As part of ARDAABD-5040, the following changes are
 *                          made:
 *                          a) Critical section protection added in functions
 *                             Fls_FcuSwitchMode, Fls_FcuPerformBlankCheck,
 *                             Fls_FcuDataCopy and Fls_FcuReturnFromCmdLock.
 *                          b) 'Function(s) invoked' field is updated in the
 *                             function banner of Fls_FcuSwitchMode,
 *                             Fls_FcuPerformBlankCheck, Fls_FcuDataCopy
 *                             and Fls_FcuReturnFromCmdLock.
 *                          c) Copyright information updated.
 * V2.5.11 12-Apr-2021   :  As part of review based on ARDAABD-5040, requirement
 *                          EAAR_PN0034_FR_0013 is mapped in newly added
 *                          Critical section protection in functions
 *                          Fls_FcuSwitchMode, Fls_FcuPerformBlankCheck,
 *                          Fls_FcuDataCopy and Fls_FcuReturnFromCmdLock.
 * V2.5.12 03-Aug-2021   :  As part of ARDAABD-6326, the following changes are
 *                          made:
 *                          a) Return type of function 'Fls_FcuOffsetDataCopy'
 *                             is updated as uint8 to return the value of Data
 *                             Flash error status register.
 *                          b) Function 'Fls_FcuOffsetDataCopy' is updated to
 *                             notify the ECC error that raised when the
 *                             argument Length for API 'Fls_ReadImmediate' is
 *                             not set as multiple of 4.
 *                          c) Function banner of function
 *                             'Fls_FcuOffsetDataCopy' is updated.
 * V2.5.13 14-Sep-2021   :  As part of review based on ARDAABD-6326, update of
 *                          global variable made inside critical section
 *                          protection.
 * V2.5.14 03-Oct-2025   :  As part of review based on ARDAABD-10673, update of
 *                          Updated the syntax type of Fls_FcuPerformBlankCheck
 *                          protection.
 *                          Update QAC message: 1330 in function Fls_FcuPerformBlankCheck
 */
/******************************************************************************/
...
/* Message       : (2:1330) The parameter identifiers in this function        */
/*                 declaration differ from those in a previous declaration.   */
/* Rule          : MISRA-C:2012 Rule-8.3                                      */
/* Justification : Parameter names differ between prototype and definition,   */
/*                 but types are identical. No functional or safety impact    */
/* Verification  : However, part of the code is verified manually and it is   */
/*                 not having any impact.                                     */
/* Reference     : Look for START Msg(2:1330)-2 and                           */
/*                 END Msg(2:1330)-2 tags in the code.                        */
/*******************************************************************************
**                         QAC Warning                                        **
*******************************************************************************/
/* 1. QAC Warning:                                                            */
...
FUNC(Fls_FcuStatusType, FLS_PRIVATE_CODE) Fls_PreFcuInitCheck(void)
{
  ...
  if (
      (FLS_DFECCCTL_RESET_VAL != FLS_GpECCReg_BaseAddress->usDFECCCTL) ||
      (FLS_DFERRINT_RESET_VAL != FLS_GpECCReg_BaseAddress->ucDFERRINT) ||
      (FLS_DFTSTCTL_RESET_VAL != FLS_GpECCReg_BaseAddress->usDFTSTCTL)
    )
  /* END Msg(2:2814)-1 */
  {
...
FUNC(Fls_FcuStatusType, FLS_PRIVATE_CODE) Fls_FcuWritePreCheck(void)
{
  ...
  /* QAC Warning: START Msg(2:2814)-1 */
  if (FLS_FCU_REGBIT_FSTATR_FRDY != (FLS_GpFACIReg_BaseAddress->ulFSTATR &
...
FUNC(Fls_FcuStatusType, FLS_PRIVATE_CODE) Fls_FcuCheckSequencerStatus(void)
{
  ...
  do
  {
    /* check if FRDY == 1, bit 15 */
    /* QAC Warning: START Msg(2:2814)-1 */
    if ((FLS_GpFACIReg_BaseAddress->ulFSTATR & FLS_FCU_REGBIT_FSTATR_FRDY) !=
        (uint32)FLS_ZERO)
    /* END Msg(2:2814)-1 */
    {
  ...
  } while (((uint32)FLS_ZERO < LulTimeout) && (FLS_FALSE == LblFrdyCheck));
  if ((FLS_GpFACIReg_BaseAddress->ulFSTATR & FLS_FCU_REGBIT_FSTATR_ILGERR) !=
                              (uint32)FLS_ZERO)
  {
  ...
  if (FLS_TRUE == LblCmdLockCheck)
  {
    /* Check ILGLERR, DFAE bits, write DFAE bit if required */
    Fls_FcuReturnFromCmdLock();
  }
  else
  {
    /*do nothing */
    }
  if (((uint32)FLS_ZERO < LulTimeout) && (FLS_FALSE == LblCmdLockCheck))
  {
    LenRetVal = FLS_FCU_OK;
  }
  else
  {
...
(VAR(const uint16, FLS_APPL_DATA) LusMode)
{
  ...
  uint32 LulLoopCount;
  LulLoopCount = (uint32)FLS_FCU_WAIT_COUNT;
  /* QAC Warning: START Msg(2:2814)-1 */
  if (LusMode != FLS_GpFACIReg_BaseAddress->usFENTRYR)
  /* END Msg(2:2814)-1 */
  {
    if (FLS_FCU_MODE_USER == LusMode)
    {
      /* check FRDY bit and cmd lock status */
      LenRetVal = Fls_FcuCheckSequencerStatus();

      if (FLS_FCU_OK != LenRetVal)
      {
        /* cmd timeout, issue a cmd stop */
        /* :- if any error occurs here, it will be returned */
        /* :- retVal must not be overwritten with FLS_FCU_OK */
        /* :- any other error report is ok */
        LenRetVal = Fls_FcuForcedStop();
      }
      else
      {
        /* do nothing */
      ...
      FLS_WRITE_REG_AND_MIRROR(
                                ...
                                FLS_FCU_MODE_USER + FLS_FCU_REGBIT_FENTRY_KEY,
                                &Fls_GaaRamMirror[RAM_IDX_FENTRYR]
                              )

      /* Check if value of the FENTRYD bit is set */
      /* Dummy read as the mode is not changed immediately on some devices */
      do
      {
        LulLoopCount--;
      } while (
                (FLS_FCU_ZERO < LulLoopCount) &&
                (
                  LusMode != FLS_GpFACIReg_BaseAddress->usFENTRYR
                )
              );
    ...
    else
    {
      ...
      FLS_WRITE_REG_AND_MIRROR(
                                &FLS_GpFACIReg_BaseAddress->usFENTRYR,
                                FLS_FCU_MODE_PE + FLS_FCU_REGBIT_FENTRY_KEY,
                                &Fls_GaaRamMirror[RAM_IDX_FENTRYR]
                              )
      ...
      /* END Msg(2:3892)-4 */
      /* Also clear FSADDR when entering P/E mode */
      FLS_WRITE_REG_AND_MIRROR(
      ...
      /* Also clear FEADDR when entering P/E mode */
      FLS_WRITE_REG_AND_MIRROR(
      ...
      FLS_EXIT_CRITICAL_SECTION(FLS_REGISTER_PROTECTION);
      #endif /* End of #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON) */
      /* mode switch SHOULD BE succesfull, check will be performed */
      LenRetVal = FLS_FCU_OK;
    }

    /* If Time out occurs */
    if (FLS_FCU_ZERO == LulLoopCount)
    {
      LenRetVal = FLS_FCU_ERR_INTERNAL;
    }
    else
    {
      Fls_GstFcuVar.usFcuFlashMode = LusMode;
    }
  }
  else
  {
...
FUNC(Fls_FcuStatusType, FLS_PRIVATE_CODE) Fls_FcuForcedStop(void)
{
  ...
  {
      /* QAC Warning: START Msg(2:2814)-1 */
     LulReturnValue = FLS_GpFACIReg_BaseAddress->ulFSTATR;
      /* END Msg(2:2814)-1 */
  ...
  while((LulLoopCount > FLS_FCU_ZERO) && (FLS_FCU_BUSY == LenStatus));

  LucFlsStatusRegValue = FLS_GpFACIReg_BaseAddress->ucFASTAT;
  /* If Time out occurs or FCU is not released from command lock state */
  if ( (FLS_FCU_ZERO == LulLoopCount) || (FLS_ZERO !=
      (uint8)(FLS_FCU_REGBIT_FASTAT_CMDLK & LucFlsStatusRegValue)))
  {
    LenStatus = FLS_FCU_ERR_INTERNAL;
  }
  else
...
FUNC(Fls_FcuStatusType, FLS_PRIVATE_CODE) Fls_FcuSetFrequency(void)
{
  ...
  LenStatus = FLS_FCU_OK;
  LusCpuFreq = FLS_CPU_FREQUENCY_MHZ;
  ...
  LulFreqData = Fls_FcuGetFWParam(FLS_FCU_EXTRA3_FDIV_FWVER_03);
  ...
  if (FLS_FCU_BYTE_RESET != LucFwVer)
  {
    ...
    if (LucFwVer < FLS_FCU_FWVER_04)
    {
      ...
      Fls_GstFcuVar.ulFcuBaseAddrECC = (uint32)FLS_GpECCReg_BaseAddress;
      /* END Msg(4:0303)-1 */
    }
    else
    {
      LucFreqDivider = FLS_CPUCLK_UL_DIV;
    ...
    if ((LusCpuFreq >= LulMinFreq) && (LusCpuFreq <= LulMaxFreq))
    {
        ...
        (FLS_FCU_ZERO != LucFreqDivider))
      {
        ...
        LusFaciFreq = (uint16)(((uint16)(LusCpuFreq) + (uint16)LucFreqDivider)
                                 - (uint16)FLS_FCU_ONE)/(uint16) LucFreqDivider;
      }
      else
      {
      ...
      FLS_WRITE_REG_AND_MIRROR(
                              ...
                              (uint16)(FLS_FCU_REGBIT_FPCKAR_KEY | LusFaciFreq),
                              &Fls_GaaRamMirror[RAM_IDX_FPCKAR]
                              )
      /* END Msg(2:2814)-1 */
      /* Set written frequency */
      FLS_CHECK_WRITE_VERIFY_INIT
      (
        &FLS_GpFACIReg_BaseAddress->usFPCKAR,
        LusFaciFreq,
...
(const uint32 LulSrcAddr, const uint32 LulBufferAddr)
{
  ...
  /* QAC Warning: START Msg(2:2814)-1 */
  LucDFERStatus = FLS_GpECCReg_BaseAddress->ucDFERSTR;
   /* END Msg(2:2814)-1 */
  if (FLS_FCU_REGVAL_DFERSTR_NOERR != LucDFERStatus)
  {
    ...
    FLS_WRITE_REG_AND_MIRROR(
                              ...
                              &Fls_GaaRamMirror[RAM_IDX_DFERSTR]
                            )
    FLS_CHECK_WRITE_VERIFY_RUNTIME(
...
** Output Parameters         : None
**
** Return parameter          : LucDFERStatus
**
...
**                             FLS_DF_WRITE16, FLS_ENTER_CRITICAL_SECTION,
**                             FLS_EXIT_CRITICAL_SECTION,
**                             FLS_WRITE_REG_AND_MIRROR,
**                             FLS_CHECK_WRITE_VERIFY_RUNTIME
**
...
FUNC(uint8, FLS_PRIVATE_CODE) Fls_FcuOffsetDataCopy
...
FUNC(Fls_FcuStatusType, FLS_PRIVATE_CODE) Fls_FcuCheckJobStatus(void)
{
  ...
  if ((uint32)FLS_ZERO != (FLS_FCU_FSTATR_PE_ERROR & LulRegValue))
  {
     if (Fls_GstVar.enCommand == FLS_COMMAND_ERASE)
     {
       LenStatus = FLS_FCU_ERR_ERASE;
     }
     else
  ...
  else if (FLS_FCU_REGBIT_FSTATR_FRDY ==
           (FLS_FCU_REGBIT_FSTATR_FRDY & LulRegValue))
  {
  ...
  /* QAC Warning: START Msg(2:2814)-1 */
  if (
      (MEMIF_MODE_FAST == Fls_GenCurrentMode) &&
      (Fls_GstVar.enCommand == FLS_COMMAND_DMA_WRITE) &&
      ((LpDmaRegisters->ulDCST & FLS_DMA_DCST_ERR) == FLS_DMA_DCST_ERR)
    )
  /* END Msg(2:2814)-1 */
  {
    LenStatus = FLS_FCU_ERR_WRITE;
    /* Clear DMA error bit */
    FLS_WRITE_REG_ONLY(&LpDmaRegisters->ulDCSTC, FLS_DMA_DCST_ERR)
    /* check result in status register */
    FLS_CHECK_WRITE_VERIFY_RUNTIME
    (
...
FUNC(Fls_FcuStatusType, FLS_PRIVATE_CODE) Fls_FcuBlankCheckResult(void)
{
  ...
  /* QAC Warning: START Msg(2:2814)-1 */
  LulRegValue = FLS_GpFACIReg_BaseAddress->ucFBCSTAT;
   /* END Msg(2:2814)-1 */
...
(
  ...
  /* MISRA Violation: START Msg(2:1330)-2 */
  const uint32 LulEndAddr
  /* END Msg(2:1330)-2 */
)
{
  Fls_FcuStatusType LenStatus;
  ...
  if (FLS_FCU_OK == LenStatus)
  {
    ...
    (
      &FLS_GpFACIReg_BaseAddress->ulFEADDR,
      (LulEndAddr & RAM_MASK_FEADDR),
      RAM_MASK_FEADDR,
      FLS_BLANK_CHECK_SID
    )
    /* MISRA Violation: START Msg(4:0303)-1 */
    /* Issuing BLANKCHECK Command */
...
FUNC(void, FLS_PRIVATE_CODE) Fls_FcuReturnFromCmdLock(void)
{
  ...
  /* QAC Warning: START Msg(2:2814)-1 */
  if (FLS_FCU_REGBIT_FSTATR_ILGERR == (FLS_FCU_REGBIT_FSTATR_ILGERR &
      FLS_GpFACIReg_BaseAddress->ulFSTATR))
  /* END Msg(2:2814)-1 */
  {
    /*  Checking DFAE bit */
    /* QAC Warning: START Msg(2:3892)-4 */
    if (FLS_FCU_REGBIT_FASTAT_DFAE == (FLS_FCU_REGBIT_FASTAT_DFAE &
    ...
    /* END Msg(2:3892)-4 */
    {
      ...
      FLS_ENTER_CRITICAL_SECTION(FLS_REGISTER_PROTECTION);
      #endif /* End of #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON) */
      /* Reset DFAE bit to Zero */
      FLS_WRITE_REG_AND_MIRROR(&FLS_GpFACIReg_BaseAddress->ucFASTAT,
                               FLS_FASTAT_RESET_VAL,
                               &Fls_GaaRamMirror[RAM_IDX_FASTAT])
      /* QAC Warning: START Msg(2:3892)-4 */
      /* For write verification */
      FLS_CHECK_WRITE_VERIFY_RUNTIME(&FLS_GpFACIReg_BaseAddress->ucFASTAT,
                                      FLS_FCU_REGBIT_FASTAT_DFAE,
                                      RAM_MASK_FASTAT,
                                      FLS_FCURETURNFROMCMDLOCK_API_ID)
      /* END Msg(2:3892)-4 */
      #if (FLS_CRITICAL_SECTION_PROTECTION == STD_ON)
      /* Enable relevant interrupts to exit this critical section */
      /* Implements EAAR_PN0034_FR_0013 */
...
/*******************************************************************************
** Function Name             : Fls_FcuCalculateBlankCheckAddr
**
** Service ID                : NA
**
...
** Output Parameters         : None
**
** Return parameter          : LulCntEndAddr
**
** Preconditions             : None
**
** Global Variables Used     : None
...
/* END Msg(4:5087)-4 */
/* END Msg(2:0832)-1 */
FUNC(uint32, FLS_PRIVATE_CODE) Fls_FcuCalculateBlankCheckAddr
                                      (const uint32 LulStartAddr, const uint32 LulEndAddr)
{
  uint32 LulCntEndAddr;
  uint32 LulBoundryAddr;

  /* Calculate Boundry End Address */
  LulBoundryAddr = LulStartAddr / (uint32)FLS_64_KB;
  /* Calculate Boundry DF start address */
  /* QAC Warning: START Msg(1:3383)-4 */
  LulBoundryAddr *= (uint32)FLS_64_KB;
  /* END Msg(1:3383)-4 */
  /* Calculate Boundry END end address */
  /* QAC Warning: START Msg(1:3383)-4 */
  LulBoundryAddr += (uint32)FLS_64_KB - (uint32)FLS_FCU_ONE;
  /* END Msg(1:3383)-4 */
...
} /* End of function Fls_FcuCalculateBlankCheckAddr */
#define FLS_STOP_SEC_PRIVATE_CODE
/* QAC Warning: START Msg(2:0832)-1 */
/* MISRA Violation: START Msg(4:5087)-4 */
#include FLS_MEMMAP_FILE
