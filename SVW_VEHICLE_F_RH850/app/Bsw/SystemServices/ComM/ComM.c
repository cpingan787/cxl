 ...
 *   V2.1.15  20250618 caihong.liu
 *      1.CPT-14266 - Redundant passive wake-up event flags lead to repeated wake-up actions.
 *      2.CPT-14903 - ComM channel can enter FULL_COMM state without requesting ComM_CommunicationAllowed.
 *   V2.1.16  20250618 caihong.liu
 *      1.CPT-15270 - Fix: Clear passive wakeup request for all ComMNmVariant types upon entering
...
#include "ComM_MemMap.h"
ComM_ModeType ComM_UserReqMode[COMM_USER_NUMBER]; /* PRQA S 1504 */ /* VL_ComM_NoStaticVar */
 ...
 */
/* PRQA S 1505, 1520 ++ */ /* VL_ComM_NoStaticFunc, VL_ComM_1520 */
...
/* PRQA S 1505, 1520 -- */ /* VL_ComM_NoStaticFunc, VL_ComM_1520 */
{
    ...
    if (ComM_ConfigPtr->ComMPncEnabled == TRUE)
    {
        ...
        for (uint32 index = 0u; index < (uint32)(userCfgPtr->pncListNum); index++)
        {
...
/* PRQA S 1520 -- */ /* VL_ComM_1520 */
{
...
#endif
    {
        ...
        ComM_UserReqMode[User] = ComMode;
#endif
        ret = ComM_HandleUserReqest(User, ComMode); /* PRQA S 1520 */ /* VL_ComM_1520 */
#if ((COMM_MODE_LIMITATION_ENABLED == STD_ON) || (COMM_WAKEUP_INHIBITION_ENABLED == STD_ON))
...
Std_ReturnType ComM_LimitECUToNoComMode(boolean Status)
{
...
#endif
    {
        if (0u != (ComM_StoreData.ecuGroupClass & COMM_LIMITED_TO_NO_COM_BflMask))
        {
            for (uint32 chIdex = 0u; chIdex < COMM_CHANNEL_NUMBER; chIdex++)
            {
...
FUNC(void, COMM_CODE) ComM_EcuM_WakeUpIndication(NetworkHandleType Channel)
{
...
#endif
    {
      ...
      */
         ...
         */
        for (uint32 chIdx = 0u; chIdx < COMM_CHANNEL_NUMBER; chIdx++)
#else
