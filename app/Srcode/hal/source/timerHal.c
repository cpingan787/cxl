...
#define DEC2BCD(dec) ((((dec) / 10) << 4) | ((dec) % 10))  //十进制数转换为BCD码
#define BCD2DEC(bcd) (((bcd) >> 4) * 10 + ((bcd) & 0x0F))  //BCD码转换为十进制数
/* 闰年判断宏: 能被400整除，或能被4整除且不能被100整除 */
...
static uint32_t g_sleepStartUtc = 0; // 休眠开始时间UTC时间
static SemaphoreHandle_t g_rtcMutexHandle = NULL;

static void TimerHalSetRtcAlarmFlagInNvm(void);

/**************************************** RTC相关 ************************/
/*************************************************
  Function:     TimerHalRtcLockInit
  Description:  创建RTC互斥锁
  Input:        None
  Output:       None
  Return:       None
  Others:       None
...
/*************************************************
  Function:     TimerHalRtcRead
  Description:  从RTC寄存器读取数据
  Input:        regAddr: 寄存器地址
                buffer: 数据缓冲区
                length: 数据长度
  Return:       0: 成功
...
/*************************************************
  Function:     TimerHalRtcWrite
  Description:  向RTC寄存器写入数据
  Input:        regAddr: 寄存器地址
                buffer: 数据缓冲区
                length: 数据长度
  Return:       0: 成功
...
/*************************************************
  Function:     RtcToTimestamp
  Description:  将RTC读取的BCD时间转换为Unix时间戳（秒）
  Input:        year_bcd: 年（BCD，如0x25表示2025年）
                month_bcd: 月（BCD，1~12）
                day_bcd: 日（BCD，1~31）
                hour_bcd: 时（BCD，0~23）
                minute_bcd: 分（BCD，0~59）
                second_bcd: 秒（BCD，0~59）
  Output:       None
  Return:       从1970-01-01 00:00:00开始的秒数（uint32_t）
  Others:       年份仅支持2000~2099年，若超出范围需调整世纪处理
*************************************************/
static uint32_t RtcToTimestamp(uint8_t year_bcd, uint8_t month_bcd, uint8_t day_bcd,
...
/*************************************************
  Function:     TimerHalWeekdayToRtcMask
  Description:  将从1970-01-01开始的天数转换为RTC寄存器中的星期掩码
  Input:        days_from_1970: 从1970-01-01开始的天数（0表示星期四）
  Output:       None
  Return:       RTC寄存器中的星期掩码（0x01-0x40）
  Others:       None
*************************************************/
...
/*************************************************
 * Function:     TimestampToRtcRegisters
 * Description:  将Unix时间戳转换为RTC寄存器值（适用于2000~2099年）
 * Input:        timestamp: 从1970-01-01 00:00:00开始的秒数（UTC）
 * Output:       regs: 输出数组，长度至少 7，按 RTC 寄存器顺序填充：
 ...
 * Others:       None
*************************************************/
static void TimestampToRtcRegisters(uint32_t timestamp, uint8_t regs[7]) 
{
...
/*************************************************
  Function:     TimerHalConfigDateAlarm
  Description:  配置RTC日期报警
  Input:        utc :UTC时间
  Output:       None
  Return:       None
  Others:       None
...
/*************************************************
  ...
  Description:  清除RTC闹钟标志位(RTC闹钟触发后调用以清除标志位)
  Input:        None
  Output:       None
  Return:       None
  Others:       None
...
/*************************************************
  Function:     TimerHalDisableRtcAlarm
  Description:  禁用RTC闹钟
  Input:        None
  Output:       None
  Return:       None
...
{
    uint8_t flag = 0;
    uint8_t regAddr = RTC_REG_ADDR_CTR2;
    ...
    flag &= (uint8_t)(~RTC_CTR2_INTAE);   /* 只清 INTAE */

    regAddr = RTC_REG_ADDR_CTR2;
    if (TimerHalRtcWrite(regAddr, &flag, 1) != 0)
    {
        TBOX_PRINT("clear INTAE fail\r\n");
        return;
    }

    TimerHalClearRtcAlarmFlagInNvm();
}
...
/*************************************************
  Function:     TimerHalSetRtcAlarmFlagInNvm
  Description:  设置RTC闹钟标志位 (标记当前已有RTC闹钟)
  Input:        None
  Output:       None
  Return:       None
  Others:       None
*************************************************/
void TimerHalSetRtcAlarmFlagInNvm(void)
{
    uint8_t flag[2] = {0};
    flag[0] = 0x01; /* 有RTC闹钟，清除为1 */
    flag[1] = 0x00; // 刷新标志位  调用此接口时即为刚设置闹钟，必定不需要刷新
    for(uint8_t i = 0; i < 2; i++)
    {
        NvMBlockRamBuffer42[i] = flag[i];
    }
    NvM_WriteBlock(NvMBlock_DIDCE01, NvMBlockRamBuffer42);
}

/*************************************************
  Function:     TimerHalClearRtcAlarmFlagInNvm
  Description:  清除RTC闹钟标志位 (标记当前无RTC闹钟)
  Input:        None
  Output:       None
  Return:       None
  Others:       None
...
{
    uint8_t flag[2] = {0};
    flag[0] = 0x00;                      /* 无RTC闹钟，清除为0 */
    flag[1] = GetListenTimerResetFlag(); // 刷新标志位
    for(uint8_t i = 0; i < 2; i++)
    {
        NvMBlockRamBuffer42[i] = flag[i];
    }
...
/*************************************************
  Function:     TimerHalGetRtcAlarmFlagInNvm
  Description:  获取RTC闹钟标志位 (标记当前是否有RTC闹钟 1:有 0:无)
  Input:        None
  Output:       None
  Return:       uint8_t 0:无RTC闹钟 1:有RTC闹钟
  Others:       None
*************************************************/
uint8_t TimerHalGetRtcAlarmFlagInNvm(void)
{
    ...
    for(uint8_t i = 0; i < 2; i++)
    {
        flag[i] = NvMBlockRamBuffer42[i];
    }
    return flag[0];
}

/*************************************************
  Function:     TimerHalGetRtcAlarmResetFlagInNvm
  Description:  获取RTC闹钟刷新标志位 (标记当前是否需要刷新RTC闹钟 1:需要 0:不需要)
  Input:        None
  Output:       None
  Return:       uint8_t 0:不需要刷新 1:需要刷新
  Others:       None
*************************************************/
uint8_t TimerHalGetRtcAlarmResetFlagInNvm(void)
{
    uint8_t flag[2] = {0};
    NvM_ReadBlock(NvMBlock_DIDCE01, NvMBlockRamBuffer42);
    for(uint8_t i = 0; i < 2; i++)
