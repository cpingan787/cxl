...
uint8 APP_WakeupHold = 	1u;
uint8 APP_WakeupNotified = 0u;
...
Mcu_WakeSourceType Mcu_WakeSource = {
	...
	.Crash_WakeFlag = 0u,
	.Ecall_WakeFlag = 0u,
	.Imu_Int1_WakeFlag = 0u,
	.Imu_Int2_WakeFlag = 0u,
	.Rtc_WakeFlag = 0u,
};
uint8 NmSleepMode = 0;
...
EcuM_WakeupSourceType Mcu_CheckWakeSource(void)
{
	...
	if(Dio_ReadChannel((Dio_ChannelType)13) == 0U)
		Mcu_WakeSource.Can_WakeFlag = 1u;
	else
		Mcu_WakeSource.Can_WakeFlag = 0u;
	...
	if(Dio_ReadChannel((Dio_ChannelType)10) == 0U)
		Mcu_WakeSource.NAD_WakeFlag = 1u;
	else
		Mcu_WakeSource.NAD_WakeFlag = 0u;
	...
	if(Dio_ReadChannel((Dio_ChannelType)2) == 1U)
	{
		Mcu_WakeSource.Ecall_WakeFlag = 1u;
	}
	else
	{
		Mcu_WakeSource.Ecall_WakeFlag = 0u;
	}
	 
	if(Dio_ReadChannel((Dio_ChannelType)8) == 0U)
		Mcu_WakeSource.Imu_Int1_WakeFlag = 1u;
	else
		Mcu_WakeSource.Imu_Int1_WakeFlag = 0u;

	 
	if(Dio_ReadChannel((Dio_ChannelType)9) == 0U)
		Mcu_WakeSource.Imu_Int2_WakeFlag = 1u;
	else
		Mcu_WakeSource.Imu_Int2_WakeFlag = 0u;

	 
	if(Dio_ReadChannel((Dio_ChannelType)16) == 0U)
		Mcu_WakeSource.Rtc_WakeFlag = 1u;
	else
		Mcu_WakeSource.Rtc_WakeFlag = 0u;



	if(Power_WakeUpSrcIsValid() == 1u)
	{
		if(Mcu_WakeSource.Can_WakeFlag == 1u)
		{
			WK_Type |= (uint32)((uint32)0x01u << (5)); 
			EcuM_SetWakeupEvent((uint32)((uint32)0x01u << (5)));
		}
		else
		{
			WK_Type |= (uint32)((uint32)0x01u << (6));
			EcuM_SetWakeupEvent((uint32)((uint32)0x01u << (6)));
		}

		BswM_RequestMode(3,1);
	}
	else
	{
...
{
    Mcu_WakeSource.Crash_WakeFlag = 1u;
    Icu_CheckWakeup((uint32)((uint32)0x01u << (6)));
}

void Icu_Edge_Detect_KL30 (void)
{
	Mcu_WakeSource.KL30_WakeFlag = 1u;
    Icu_CheckWakeup((uint32)((uint32)0x01u << (6)));
}
...
{
    Mcu_WakeSource.Rtc_WakeFlag = 1u;
    Icu_CheckWakeup((uint32)((uint32)0x01u << (7)));
}
...
void Power_Init(void)
{
	...
	validNmMsgFlag = 0;
	Mcu_WakeSource.Can_WakeFlag = 0u;
	Mcu_WakeSource.KL30_WakeFlag = 0u;
	Mcu_WakeSource.NAD_WakeFlag = 0u;
	Mcu_WakeSource.Crash_WakeFlag = 0u;
	Mcu_WakeSource.Ecall_WakeFlag = 0u;
	Mcu_WakeSource.Imu_Int1_WakeFlag = 0u;
	Mcu_WakeSource.Imu_Int2_WakeFlag = 0u;
	Mcu_WakeSource.Rtc_WakeFlag = 0u;
...
void Power_Detect(void)
{
    ...
    gIOHwAbDI_KL30_St = Dio_ReadChannel((Dio_ChannelType)18);

    gIOHwAbDI_CanRx_St = Dio_ReadChannel((Dio_ChannelType)13);
    gIOHwAbDI_NAD_WAKEUP_MCU_St = Dio_ReadChannel((Dio_ChannelType)10);
    gIOHwAbDI_CDS_St = Dio_ReadChannel((Dio_ChannelType)15);
    gIOHwAbDI_ECALL_BUTTON_St = Dio_ReadChannel((Dio_ChannelType)2);
    gIOHwAbDI_IMU_INT1_St = Dio_ReadChannel((Dio_ChannelType)8);
    gIOHwAbDI_IMU_INT2_St = Dio_ReadChannel((Dio_ChannelType)9);
    gIOHwAbDI_RTC_St = Dio_ReadChannel((Dio_ChannelType)16);
	...
	{
		if((APP_GetWakeupHold() == 1u) && (APP_WakeupNotified == 0u)) 
		{
			Mcu_CheckWakeSource();   
		}
		else
...
{   
	...
	Power_Init();
	EcuM_OnGoOffOne();
	Dio_WriteChannel((Dio_ChannelType)6, 1U);
    BswM_RequestMode(3,2);
	BswM_RequestMode(2,0);         
    BswM_RequestMode(1,0);   
	BswM_RequestMode(3,0);    
