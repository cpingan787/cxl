...
static uint32_t* calcTxEventBufAdrs(cy_pstc_canfd_type_t pstcCanFD,
                                    uint8_t u8MsgBuf)
{
    uint32_t* pu32Adrs;
    ...
    {
        /* Set the message buffer address to the return value if the index is available */
        //pu32Adrs = (uint32_t *)((uint32_t)pstcCanFD + (uint32_t)CY_CANFD_MSGRAM_START) + pstcCanFD->M_TTCAN.unTXEFC.stcField.u14EFSA+ (2) * u8MsgBuf;
        pu32Adrs = (uint32_t*)(((uint32_t)pstcCanFD & 0xFFFF0000ul) + (uint32_t)CY_CANFD_MSGRAM_START);
        pu32Adrs += pstcCanFD->M_TTCAN.unTXEFC.stcField.u14EFSA+(2 * u8MsgBuf);
    }
    return pu32Adrs;
...
void Cy_CANFD_IrqHandler( cy_pstc_canfd_type_t pstcCanFD )
{
    ...
    {
      if(pstcCanFDChMTTCAN->unIR.u32Register & 0x38000000)
      {
          if (pstcCanFDContext->canFDNotificationCb.canFDErrorInterruptFunction != NULL)
          {
              pstcCanFDContext->canFDNotificationCb.canFDErrorInterruptFunction(pstcCanFDChMTTCAN->unPSR.stcField.u3LEC);
          }
      }
      else
      {
        Cy_CANFD_ErrorHandling(pstcCanFD);
        /*pstcCanFDChMTTCAN->unCCCR.stcField.u1INIT = 0;        
        while ( pstcCanFDChMTTCAN->unCCCR.stcField.u1INIT != 0 )
        {
          
        }   
        if (pstcCanFDChMTTCAN->unIR.u32Register & 0x00000001)
        {
          pstcCanFDChMTTCAN->unIR.u32Register = 0X3FFFFFFE;
        }
        else
        {
          pstcCanFDChMTTCAN->unIR.u32Register = 0X3FFFFFFF;
        }*/
      }        
    }
...
#else
    //by xwm 2023.4.25
    if(pstcCanFDChMTTCAN->unIR.stcField.u1TEFL_||pstcCanFDChMTTCAN->unIR.stcField.u1TEFF||pstcCanFDChMTTCAN->unIR.stcField.u1TEFW||pstcCanFDChMTTCAN->unIR.stcField.u1TEFN)
    {
      //stc_canfd_tx_event_t stcCanFDTxEventMsg;
      uint8_t txMsgMarker;
      //uint32_t ei;
      pstcCanFDChMTTCAN->unIR.stcField.u1TEFL_ = 0x01;
      pstcCanFDChMTTCAN->unIR.stcField.u1TEFF = 0x01;
      pstcCanFDChMTTCAN->unIR.stcField.u1TEFW = 0x01;
      pstcCanFDChMTTCAN->unIR.stcField.u1TEFN = 0x01;
      while(pstcCanFDChMTTCAN->unTXEFS.stcField.u6EFFL!=0)
      {
#if(1)
        pu32Adrs = calcTxEventBufAdrs(pstcCanFD,pstcCanFDChMTTCAN->unTXEFS.stcField.u5EFGI);
        //update the index of Tx event FIFO 
        pstcCanFDChMTTCAN->unTXEFA.stcField.u5EFAI = pstcCanFDChMTTCAN->unTXEFS.stcField.u5EFGI;
        if (pu32Adrs)
        {
            /* Save received data */
            /* XTD : Extended Identifier */
            txMsgMarker = ((cy_stc_canfd_tx_buffer_t *) pu32Adrs)->t1_f.mm;
            //ei = ((cy_stc_canfd_tx_buffer_t *) pu32Adrs)->t1_f.efc;
            //if(ei==01)
            {
              if (pstcCanFDContext->canFDInterruptHandling.canFDTxInterruptFunction != NULL)
              {
                  pstcCanFDContext->canFDInterruptHandling.canFDTxInterruptFunction(txMsgMarker);
              }              
            }
...
cy_en_canfd_status_t Cy_CANFD_Init(cy_pstc_canfd_type_t pstcCanFD, const cy_stc_canfd_config_t* pstcConfig)
{
    ...
    unTXEFC.stcField.u6EFWM = 1; /* Watermark interrupt disabled .       xwm 2023.4 .25   */
    unTXEFC.stcField.u6EFS = 16;   /* Tx Event FIFO disabled . -xwm 2023.4.25 */
    ...
    unTXBC.stcField.u6TFQS = 32;   /* No Tx FIFO/Queue xwm 2023.4.5 */
    ...
    unTXBC.stcField.u14TBSA = pstcCanFDChMTTCAN->unTXEFC.stcField.u14EFSA + 
      (20 * SIZE_OF_TXEVENT_FIFO_IN_WORD);    // Reserving memory for 10 TxEvent Fifo elements for easy future use
    ...
    unIE.stcField.u1DRXE  = 0; /* Message stored to Dedicated Rx Buffer xwm 2023.4.25 */
...
cy_en_canfd_status_t Cy_CANFD_UpdateAndTransmitMsgBuffer( cy_pstc_canfd_type_t pstcCanFD, uint8_t u8MsgBuf, cy_stc_canfd_msg_t* pstcMsg )
{
    ...
    pstcCanFDTxBuffer->t1_f.efc = 1; /* Tx Event Fifo not used,xwm 2023.4.25 */
    pstcCanFDTxBuffer->t1_f.mm  = u8MsgBuf; /* Not used ,xwm 2023.4.25*/
    pstcCanFDTxBuffer->t1_f.dlc = pstcMsg->dataConfig.dataLengthCode;
    pstcCanFDTxBuffer->t1_f.fdf = (pstcMsg->canFDFormat == true) ? 1 : 0;
    pstcCanFDTxBuffer->t1_f.brs = (pstcMsg->canFDFormatBrs == true) ? 1 : 0;
...
cy_en_canfd_status_t CanFD_GetMsgFromTransmitMsgBuffer( cy_pstc_canfd_type_t pstcCanFD,
                                            uint8_t u8MsgBuf,
                                            cy_stc_canfd_msg_t* pstcMsg
                                          )
{
    uint8_t u8DataLengthWord;
    cy_stc_canfd_tx_buffer_t* pstcCanFDTxBuffer;
    ...
    if(pstcCanFDTxBuffer == NULL)
    {
     ...
     //pstcCanFDTxBuffer->T0_f.RTR; /* Transmit data frame. */
    if(pstcCanFDTxBuffer->t0_f.xtd)
    {
      pstcMsg->idConfig.extended = true;
      pstcMsg->idConfig.identifier = pstcCanFDTxBuffer->t0_f.id;
    }
    else
    {
      pstcMsg->idConfig.extended = false;
      pstcMsg->idConfig.identifier = pstcCanFDTxBuffer->t0_f.id>>18;
    } 
    pstcMsg->dataConfig.dataLengthCode = pstcCanFDTxBuffer->t1_f.dlc;
    if(pstcCanFDTxBuffer->t1_f.fdf)
    {
      pstcMsg->canFDFormat = true;
    }
    else
    {
      pstcMsg->canFDFormat = false;
    }
    ...
    {
        u16dlcTmp = pstcMsg->dataConfig.dataLengthCode - 8;
    }
    u8DataLengthWord = dataBufferSizeInWord[u16dlcTmp];

    /* Data set */
    for ( u16count = 0; u16count < u8DataLengthWord; u16count++ )
    {
        pstcMsg->dataConfig.data[u16count] = pstcCanFDTxBuffer->data_area_f[u16count];
    }
