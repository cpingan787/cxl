...
#define MPU_LTE_POWERKEY_ON_PULSE_MS     10000U    /* 原理图要求 Power_ON pulse = 1S */
...
static void MpuHalSetPower(uint8_t flag)
{
    if (flag == 0U)
    {
        TBOX_PRINT("!!! WARN: Cutting off MPU Power (3.8V & Level Shift OFF) !!!\r\n");
        MpuHalSetPowerkey(0U);
        //MpuHalSetReset(0U);
        MpuHalSetWakeOut(1U);
...
void MpuHalCycleProcess(uint32_t cycleTime)
{
    ...
    {
        uint32_t otaFlagData = 0;
        EEIf_Read(2, 4, (uint8*)&otaFlagData);
        //TBOX_PRINT("0000 %08X.\n", otaFlagData);
        if (otaFlagData == 0xD5u) //0xD5 OTA 成功标志位
        {
            uint32_t resetFlag = 0x00u;
            EEIf_Write(2, 4, (uint8*)&resetFlag);
            //TBOX_PRINT("2222\n");
