...
typedef struct
{
    boolean sleepSuspend;                         


 
    Mcu_ModeType mcuMode;                          
    uint8 numberOfWakeSource;                      
    const uint8 * wkSrcIdx;  
    EcuM_WakeupSourceType wkMask;                  
} EcuM_SleepModeCfgType;

 
typedef struct
{
    uint32 checkWkupTimeout;        
...
typedef struct
{
    uint32 alarmClkTimeout;      
    EcuM_UserType alarmClkUser;  
} EcuM_AlarmClockCfgType;

 
typedef struct
{
    boolean goDownAllowed;  
    boolean setClkAllowed;  
    EcuM_UserType usrId;  
} EcuM_UserCfgType;
...
typedef struct
{
    ...
    EcuM_TargetType SdtgNext;  
    EcuM_TargetType SdtgLast;  
...
extern const EcuM_SleepModeCfgType EcuM_SleepModeCfgs[1];

 
extern const EcuM_WakeupSourceCfgType EcuM_WkSourceCfgs[3];

 
extern const Mcu_ModeType EcuM_NormalMcuModeCfg;

 
extern const EcuM_UserCfgType EcuM_UserCfgs[1];


 
extern const EcuM_AlarmClockCfgType EcuM_AlarmClkCfgs[1];



 

extern EcuM_RunTimeType EcuMRunData;
...
static  void EcuM_HaltSequence(void);

 
static  void EcuM_PollSequence(void);

 
static  void EcuM_HaltSequenceExit(void);

 
static  void EcuM_SleepNotifyBswM(void);

 
static  void EcuM_WakeupRestart(void);



 
static  void EcuM_ValidateWkEvAction(uint8 wkSrcIdx);

 
static  Std_ReturnType EcuM_DoSleepPhase(void);
 ...
 Std_ReturnType EcuM_GoDownHaltPoll(uint16 caller)
{
 ...
 EcuM_WakeupSourceType EcuM_GetPendingWakeupEvents(void)
{
    EcuM_WakeupSourceType wks = ((uint32)0u);
...
{
    {
        ...
        SchM_Exit_EcuM_WkEv();
        BswM_EcuM_CurrentWakeup(sources, (EcuM_WakeupStatusType)0);
    }
    return;
}
 ...
 EcuM_WakeupSourceType EcuM_GetExpiredWakeupEvents(void)
{
 ...
 void EcuM_SetWakeupEvent(EcuM_WakeupSourceType sources)
{
    EcuM_RunTimeType * pRt = &EcuMRunData;
    uint32 spanTime;
...

    {
        ...
        if ((sources & EcuM_SleepModeCfgs[sleepModeId].wkMask) != ((uint32)0u))
        {
            SchM_Enter_EcuM_WkEv();
            ...
            if (wksIdx != (uint8)3)
            {
                if (EcuM_WkSourceCfgs[wksIdx].checkWkupTimeout != 0u)
                {
                    ...
                    )
                    {
                        ...
                        BswM_EcuM_CurrentWakeup(sources, (EcuM_WakeupStatusType)3);
                ...
                else  
                {
                    ...
                    if (EcuMRunData.State >= (EcuM_StateType)0x10)
                    {
...
SET_WAKEUP_EVENT:
 ...
 void EcuM_ValidateWakeupEvent(EcuM_WakeupSourceType sources)
{
    const EcuM_RunTimeType * pRt = &EcuMRunData;
    const EcuM_WakeupSourceCfgType * pWks;
    uint8 wkIdx;

    {
        wkIdx = EcuM_WkSrcMap2CfgWkIdx(sources);
        pWks = &(EcuM_WkSourceCfgs[wkIdx]);
        if (((EcuM_StateType)0x30 == (pRt->State & (EcuM_StateType)0x30)) && (((void *)0) == pWks->comMChnl)
        )
        {
...
static  void EcuM_GoSleep(void)
{
    ...
    sleepModeId = pRt->SdtgNext.Mode;
    if (sleepModeId <= (EcuM_ShutdownModeType)1)
    {
        wksNum = EcuM_SleepModeCfgs[sleepModeId].numberOfWakeSource;
        {
            for (index = 0; index < wksNum; index++)
            {
                ...
                BswM_EcuM_CurrentWakeup(EcuM_WkSourceCfgs[refWkSrcIdx].wkSource, (EcuM_WakeupStatusType)0);
            }
            osRet = GetResource(EcuM_OSResource);
        }
         
        EcuMRunData.State = (EcuM_StateType)0x50;
        BswM_EcuM_CurrentState((EcuM_StateType)0x50);
    }
    if ((StatusType)0U != osRet)
    {
        EcuM_ErrorHook(11);
    }
}
...
static  void EcuM_HaltSequence(void)
{
    ...
    while (1u)
    {
        ...
        if (((((uint32)((uint32)((uint32)0x01u << (5))|(uint32)((uint32)0x01u << (6)))) & pendWks) == 0u) && ((((uint32)((uint32)((uint32)0x01u << (5))|(uint32)((uint32)0x01u << (6)))) & ValidatedWks) == 0u))
        {
...
static  void EcuM_PollSequence(void)
{
    ...
    do
    {
        ...
        for (index = 0; index < (uint8)3; index++)
        {
            if ((boolean)1u == EcuM_WkSourceCfgs[index].isPolling)
            {
        ...
        if (((uint32)((uint32)0x01u << (7)) == (((*EcuMRunData.Wks.Validated)) & (uint32)((uint32)0x01u << (7))))
    ...
    } while (0u == (((uint32)((uint32)((uint32)0x01u << (5))|(uint32)((uint32)0x01u << (6)))) & pendingWkup));
    ...
    for (index = 0; index < (uint8)3; index++)
    {
        if ((EcuM_WkSourceCfgs[index].wkSource & (*pRt->Wks.Pending)) != 0u)
        {
                ...
                && (EcuM_WkSourceCfgs[index].validationTimeout > 0u))
            {
                ...
                BswM_EcuM_CurrentWakeup(EcuM_WkSourceCfgs[index].wkSource, (EcuM_WakeupStatusType)1);
            }
            else
            {
                 
                if (EcuM_WkSourceCfgs[index].wkSource < (EcuM_WakeupSourceType)0x00000010)
                {
                    BswM_EcuM_CurrentWakeup(EcuM_WkSourceCfgs[index].wkSource, (EcuM_WakeupStatusType)2);
                }
            }
        }
    }
...
static  void EcuM_WakeupRestart(void)
{
    ...
    if (EcuM_NormalMcuModeCfg != ((Mcu_ModeType)0xffu))
    {
        DisableAllInterrupts();
...
static  void EcuM_ValidateWkEvAction(uint8 wkSrcIdx)
{
    ...
    if (((void *)0) != pWks->comMChnl)
    {
        ComM_EcuM_WakeUpIndication(*(pWks->comMChnl));
    }
    BswM_EcuM_CurrentWakeup(pWks->wkSource, (EcuM_WakeupStatusType)2);
}
