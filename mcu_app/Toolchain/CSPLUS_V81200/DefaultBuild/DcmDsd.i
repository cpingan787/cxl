    ...
    uint16 * pSidTabServieCfgIndex)
{
    ...
    if ((((void *)0) != pSidTabIndex) && (((void *)0) != pSidTabServieCfgIndex))
    {
        ...
        if (0U == ret)
        {
            ...
            Flag = 0u;
            for (ServiceIndex = 0; (ServiceIndex < Service_Num) && (0u == Flag); ServiceIndex++)
            {
                if ((Sid == pDsdSidTab[ServiceIndex].DcmDsdServiceId)
                    && (pDsdSidTab[ServiceIndex].DcmDsdServiceUsed == 1u))
                {
                    Flag = 1u;
                }
            }
    ...
    uint16 * pSubServieCfgIndex)
{
    ...
    if ((SubService_Num > 0u) && (1u == pService->DcmDsdSubfuncAvial))
    {
        for (SubServiceIndex = 0; (SubServiceIndex < SubService_Num) && (0u == Flag); SubServiceIndex++)
        {
            const Dcm_DsdSubServiceCfgType* pSubService = &(pService->DcmDsdSubService[SubServiceIndex]);
            if ((SubFunctionId == pSubService->DcmDsdSubServiceId) && (1u == pSubService->DcmDsdSubServiceUsed))
            {
                Flag = 1u;
            }
        }
    ...
    DsdInternal_SuppressPosRsp(uint8 ProtocolCtrlId, uint16 SidTabIndex, uint16 SidTabServieCfgIndex)
{
    ...
    MsgCtrlIndexx = Dcm_ProtocolCtrl[ProtocolCtrlId].MsgCtrlIndex;
    if (1u == pService->DcmDsdSubfuncAvial)
    {
        SubFunction = Dcm_MsgCtrl[MsgCtrlIndexx].Subfunction;
        SchM_Enter_Dcm_ExclusiveArea();
        Dcm_MsgCtrl[MsgCtrlIndexx].MsgContext.MsgAddInfo.SuppressPosResponse = 0u;
        if ((0x80u == (SubFunction & 0x80u)) && (1u == pService->DcmDsdSuppressPosRsp))
        {
            Dcm_MsgCtrl[MsgCtrlIndexx].MsgContext.MsgAddInfo.SuppressPosResponse = 1u;
...
static  Std_ReturnType DsdInternal_RxIndication_Check(uint8 ProtocolCtrlId)
{
    uint8 Sid;
    uint8 MsgCtrlId;
    ...
    if (0U == ret)
    {
        ...
        if (1U == ret)
        {
            ...
            DsdInternal_SetNrc(ProtocolCtrlId, ((Dcm_NegativeResponseCodeType)0x11u));
            DsdInternal_ProcessingDone(ProtocolCtrlId);
        }
    }
    if (0U == ret)
    ...
    if (0U == ret)
    {
        ...
        if (1U == ret)
        {
            ...
            DsdInternal_SetNrc(ProtocolCtrlId, ((Dcm_NegativeResponseCodeType)0x7Fu));
            DsdInternal_ProcessingDone(ProtocolCtrlId);
        }
    }
    if (0U == ret)
    {
        ...
        if (1U == ret)
        {
            DsdInternal_SetNrc(ProtocolCtrlId, ((Dcm_NegativeResponseCodeType)0x33u));  
            DsdInternal_ProcessingDone(ProtocolCtrlId);
        }
    }
    if ((0U == ret) && (Sid >= 0x10u) && (Sid != 0x31u)
        && (1u
                   ...
                   .DcmDsdSubfuncAvial))
    {
        if ((2U) > Dcm_MsgCtrl[MsgCtrlId].MsgContext.ReqDataLen)
        {
            ...
            DsdInternal_SetNrc(ProtocolCtrlId, ((Dcm_NegativeResponseCodeType)0x13u));
            DsdInternal_ProcessingDone(ProtocolCtrlId);
            ret = 1U;
        }
        if (0U == ret)
        {
            ...
            if (1U == ret)
            {
                ...
                DsdInternal_SetNrc(ProtocolCtrlId, ((Dcm_NegativeResponseCodeType)0x12u));
                DsdInternal_ProcessingDone(ProtocolCtrlId);
            }
        }
    }
 ...
 Std_ReturnType DsdInternal_RxIndication(uint8 ProtocolCtrlId)
{
    Std_ReturnType ret;
 ...
 void DsdInternal_ProcessingDone(uint8 ProtocolCtrlId)
{
    uint8 Nrc;
    uint8 MsgCtrlIndexx;
    ...
    static uint8 Dcm_Nrc78Channel[8] = {0};
    const Dcm_DslCfgType* pDcmDslCfg = DcmPbCfgPtr->pDcmDslCfg;
    ...
    case DCM_NEG_RSP:  
        ...
        if (((uint8)DCM_FUNCTIONAL == ReqTypex)
            && ((((Dcm_NegativeResponseCodeType)0x11u) == Nrc) || (((Dcm_NegativeResponseCodeType)0x12u) == Nrc)
                || (((Dcm_NegativeResponseCodeType)0x31u) == Nrc) || (((Dcm_NegativeResponseCodeType)0x7Eu) == Nrc)
                || (((Dcm_NegativeResponseCodeType)0x7Fu) == Nrc)))
        {
            DslInternal_ResetResource(ProtocolCtrlId);
        }
        else if (((Dcm_NegativeResponseCodeType)0x78u) == Nrc)
        {
             
            Dcm_Nrc78Channel[0] = 0x7Fu;
            Dcm_Nrc78Channel[1] = Dcm_MsgCtrl[MsgCtrlIndexx].SID;
            Dcm_Nrc78Channel[2] = ((Dcm_NegativeResponseCodeType)0x78u);
            SchM_Enter_Dcm_ExclusiveArea();
        ...
        else
        {
            ...
            if (0x22u == Dcm_MsgCtrl[MsgCtrlIndexx].NRC)
            {
                ...
                if (0U == Dcm_GetSpecificCauseCode(&Code))
                {
 ...
 void DsdInternal_TxConfirmation(uint8 ProtocolCtrlId)
{
    DsdInternal_ManufacturerConfirmation(ProtocolCtrlId);
    DsdInternal_SupplierConfirmation(ProtocolCtrlId);
    DspInternal_DcmConfirmation(ProtocolCtrlId);
...
DsdInternal_ServiceAddressAndLengthFormatIdentifierCheck(const uint8 addressAndLengthFormatIdentifier)
{
    ...
    for (iloop = 0; (iloop < Num) && (ret == 1U); iloop++)
    {
