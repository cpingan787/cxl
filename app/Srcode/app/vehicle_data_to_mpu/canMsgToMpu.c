...
int16_t SaveCanMsgToBuffer(uint8_t canChannel, uint32_t canId, uint8_t dlc, const uint8_t *pCanData)
{
    ...
    TBOX_PRINT("[DV] SaveCanMsgToBuffer: canId=0x%X, dlc=%d, data=", canId, dlc);
    for (uint8_t i = 0; i < dlc; i++)
    {
        TBOX_PRINT("0x%02X ", pCanData[i]);
    }
    TBOX_PRINT("\r\n");
    
    /* Directly process 0x361 message in interrupt context */
    if (dlc >= 5)
    {
        uint8_t testMode = pCanData[4];
        TBOX_PRINT("[DV] 0x361 data[4]=0x%02X, setting testMode=%d\r\n", testMode, testMode);
        if (testMode <= 2)
        {
            PowerManageSdkSetTestMode(testMode);
        }
    }
    /* Also save to queue for other processing if needed */
...
#if (DV_TEST_ENABLE == 1)
        if (queueMsg.canId == 0x361)
        {
            /* Use queueMsg data directly since canMsg may have issues */
            TBOX_PRINT("[DV] CAN ID 0x361, DLC: %d, Data: ", queueMsg.dlc);
            for (uint8_t j = 0; j < queueMsg.dlc; j++)
            {
                TBOX_PRINT("0x%02X ", queueMsg.canData[j]);
            }
            TBOX_PRINT("\r\n");
            
            /* testMode already set in SaveCanMsgToBuffer, just send ACK */
            if (queueMsg.dlc >= 5)
            {
                CanHalMsg_t ackMsg;
                ackMsg.canId = queueMsg.canId;
                ackMsg.dlc = queueMsg.dlc;
                memcpy(ackMsg.canData, queueMsg.canData, queueMsg.dlc);
                SendCan361AckMsg(&ackMsg);
            }
        }
        return 0;
#endif
