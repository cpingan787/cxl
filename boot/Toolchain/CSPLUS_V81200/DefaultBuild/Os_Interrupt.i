...
static void Os_ICBInit(const Os_SCBType *pScb);
...
static void Os_DisableAllInterrupts(Os_SCBType *pScb);
...
static inline  void Os_ExitISR2SwitchTask(Os_SCBType *pScb);
...
static inline  void Os_EnterISR1StackMonitor(const Os_SCBType *pScb);
...
static inline  void Os_EnterISR2StackMonitor(const Os_SCBType *pScb);
...
static void Os_ICBInit(const Os_SCBType *pScb)
{
    ...
    if (Os_CfgStdResourceMax_Inf[sysCore] > 0U)
    {
        ...
        {
            pICB = Os_ICB[i];
            pICB->IsrC2ResCount = (Os_ResourceType)0u;    
...
void Os_DisableIntSourceInApp(const Os_ApplicationCfgType *posCurAppCfg, boolean *intSourceFlag)
{
    ...
    for (uint16 i = 0u; i < isrRefCnt; i++)
    {
        ...
        else
        {
            ...
            Os_Hal_DisableInterruptSource(IsrSrc, 0U);
...
void Os_ResumeIntSourceInApp(const Os_ApplicationCfgType *posCurAppCfg, const boolean *intSourceFlag)
{
    ...
    for (uint16 i = 0u; i < isrRefCnt; i++)
    {
        if (intSourceFlag[i] == 1u)
        {
            ...
            Os_Hal_EnableInterruptSource(IsrSrc, 0U);
...
void Os_ResumeAllInterrupts(Os_SCBType *pScb)    
{
...
void Os_SuspendAllInterrupts(Os_SCBType *pScb)   
{
...
void Os_ResumeOSInterrupts(Os_SCBType *pScb)    
{
