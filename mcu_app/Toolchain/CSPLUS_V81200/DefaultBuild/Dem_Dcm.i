...
Dem_DTCFilterInfoType DemDTCFilterInfo;
...
Dem_DTCSettingInfoType DemDTCSettingInfo;
...
Dem_DTCByOccurrenceTimeType DemDTCByOccurrenceTimeInfo;
...
static Dem_DTCRecordUpdateStatusInfoType DTCRecordUpdateStatusInfo;
...
Dem_FreezeFrameRecordFilterInfoType DemFreezeFrameRecordFilterInfo;
...
boolean Dem_DTCStatusChangedInd = 0u;
...
static boolean Dem_DTCDisable = 0u;
...
static  uint8 Dem_CheckEventInternalMemDest(Dem_DTCOriginType DTCOrigin, uint16 IntId);
static  boolean Dem_CheckDTCFormat(uint8 dtcFormat, uint16 dtcRef);
static  Dem_ReturnGetNextFilteredElementType Dem_DcmCheckDTCFilterStatus(
    const uint32 * DTC,
    const Dem_UdsStatusByteType * DTCStatus);
static  Dem_ReturnClearDTCType
...
Dem_DcmGetDTCStatusAvailabilityMask(Dem_UdsStatusByteType * DTCStatusMask)
{
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x16u, 0x20u);
    }
    else if (DTCStatusMask == ((void *)0))
    {
        DEM_DET_REPORT(0x16u, 0x11u);
    }
    else
    {
        *DTCStatusMask = 0xffu;
...
Dem_DcmGetStatusOfDTC(uint32 DTC, Dem_DTCOriginType DTCOrigin, uint8 * DTCStatus)
{
    uint8 memDest;
    uint16 dtcIndex;
    Dem_ReturnGetStatusOfDTCType ret = 0x03u;

    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x15u, 0x20u);
    }
    else if (DTCStatus == ((void *)0))
    {
        DEM_DET_REPORT(0x15u, 0x11u);
    }
    else
    {
         
        dtcIndex = Dem_GetDTCIndex(DTC, 0x01u);
        if (dtcIndex == 0xFFFFu)
        {
            ret = 0x01u;
        }
        else if (DemDTCGeneralStatus[dtcIndex].SuppressionStatus == 1u)
        {
            ret = 0x01u;
        }
        else
        {
            ...
            {
                *DTCStatus = (DemDTCGeneralStatus[dtcIndex].CbUdsStatus & 0xffu);
                ret = 0x00u;
            }
        }
    }
...
{
    uint16 dtcIndex;
    Dem_ReturnGetSeverityOfDTCType ret = 0x01u;
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x0eu, 0x20u);
    }
    else if (DTCSeverity == ((void *)0))
    {
        DEM_DET_REPORT(0x0eu, 0x11u);
    }
    else
    {
         
        dtcIndex = Dem_GetDTCIndex(DTC, 0x01u);
        if ((dtcIndex != 0xFFFFu) && (DemDTCGeneralStatus[dtcIndex].SuppressionStatus == 0u))
        {
            *DTCSeverity = DemPbCfgPtr->DemCfgDTC[dtcIndex].DemDTCSeverity;
            ret = 0x00u;
        }
    }
    return ret;
...
{
    uint16 dtcIndex;
    Dem_ReturnGetFunctionalUnitOfDTCType ret = 0x01u;
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x34u, 0x20u);
    }
    else if (DTCFunctionalUnit == ((void *)0))
    {
        DEM_DET_REPORT(0x34u, 0x11u);
    }
    else
    {
        ...
        if ((dtcIndex != 0xFFFFu) && (DemDTCGeneralStatus[dtcIndex].SuppressionStatus == 0u))
        {
            ...
            *DTCFunctionalUnit = DemPbCfgPtr->DemCfgDTC[dtcIndex].DemDTCFunctionalUnit;
            ret = 0x00u;
        }
    }
    return ret;
...
{
    Dem_ReturnSetFilterType ret = 0x01u;
    uint8 memDest = Dem_GetInternalMemDest(DTCOrigin);
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x13u, 0x20u);
    }
    else if (
        ((0x00u == DTCFormat) && (3u == 3u))
        || ((0x02u == DTCFormat) && (0U == 0U))
        || ((0u == 0u) && (DTCKind == 0x02u))
        || ((memDest == 0xFFu)
                ))
    {
        DEM_DET_REPORT(0x13u, 0x10u);
    }
    else
    {
         
        DemDTCFilterInfo.DTCStatusMask = (0u == DTCStatusMask) ? (0xFFu & 0xffu)
                                                               : (DTCStatusMask & 0xffu);
        ...
        Dem_MemSet(
            ...
            0x00u,
            (sizeof(Dem_DTCReportByTimeOrderType) * 59u));
        Dem_MemSet((uint8*)DemDTCFilterInfo.DemDTCReport, 0x00u, 59u);
        DemDTCFilterInfo.IsSet = 1u;
        ret = 0x00u;
    }
    return ret;
}
...
{
    Dem_ReturnGetNumberOfFilteredDTCType ret = 0x01u;
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x17u, 0x20u);
    }
    else if (NumberOfFilteredDTC == ((void *)0))
    {
        DEM_DET_REPORT(0x17u, 0x11u);
    }
    else if (DemDTCFilterInfo.IsSet == 0u)
    {
    ...
    else
    {
        ...
        Dem_DTCOriginType DTCOrigin;
        {
            const Dem_DTCType* pDemDTC = DemPbCfgPtr->DemCfgDTC;
            DTCOrigin = Dem_GetInternalMemDest(0x01u);
             
            for (uint16 iloop = 0; iloop < 59u; iloop++)
            {
                ...
                if (((0x00u != (pDemDTCGeneralStatus->CbUdsStatus & pFilter->DTCStatusMask))
                    ...
                    && (1u == Dem_CheckDTCFormat(pFilter->DTCFormat, iloop))
                    && ((pFilter->FilterWithSeverity == 0u)
                        || ((0x00u != (pFilter->DTCSeverityMask & pDemDTC[iloop].DemDTCSeverity))))
                    && (pFilter->FilterForFaultDetectionCounter == 0u)
                    && (0u == pDemDTCGeneralStatus->SuppressionStatus))
                {
                    pFilter->NumberOfFilteredDTC++;
                     
                    boolean dtcflag = 0u;
                    uint8 EntryNum = DemMemDestCfg[DTCOrigin].EntryNum;
                    const Dem_EventMemEntryType* pEntry = DemMemDestCfg[DTCOrigin].EntryList;
                    Dem_UdsStatusByteType DtcStatus =
                        (pDemDTCGeneralStatus->CbUdsStatus & 0xffu);
                    if (((DtcStatus & 0x0Cu) != 0u) && ((DtcStatus & 0xF3u) == 0u))
                    {
                        for (uint8 index = 0; index < EntryNum; index++)
                        {
                            if (pEntry->EventId != 0u)
                            {
                                ...
                                uint16 DTCRef = DemPbCfgPtr->pDemEventParameter[pEvent->IntId].DemDTCRef;
                                if ((0xFFu != Dem_CheckEventInternalMemDest(DTCOrigin, pEvent->IntId))
                                    && (iloop == DTCRef))
                                {
                                    dtcflag = 1u;
    ...
    if (ret == 0x00u)
    {
        ...
        for (; pFilter->CurrentIndex < pFilter->DemDTCTimeOrderNum; pFilter->CurrentIndex++)
        {
            if (ret != 0x00u)
            {
                ...
                *DTCStatus = DemDTCGeneralStatus[dtcRef].CbUdsStatus & 0xffu;
                if (0u != *DTC)
                {
                    ret = 0x00u;
                }
...
Dem_DcmGetNextFilteredDTCAndFDC(
    ...
    sint8 * DTCFaultDetectionCounter)
{
    Dem_ReturnGetNextFilteredElementType ret = 0x01u;
    Dem_DTCFilterInfoType* pFilter = &DemDTCFilterInfo;
    uint16 dtcRef;
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x3bu, 0x20u);
    }
    else if ((DTCFaultDetectionCounter == ((void *)0)) || (DTC == ((void *)0)))
    {
        DEM_DET_REPORT(0x3bu, 0x11u);
    }
    else
    {
        for (; (ret != 0x00u) && (pFilter->CurrentIndex < 59u); pFilter->CurrentIndex++)
        {
            dtcRef = pFilter->CurrentIndex;
            if (dtcRef != 0xFFFFu)
            {
                 
                if ((DemDTCGeneralStatus[dtcRef].SuppressionStatus == 1u)
                    || (0u == Dem_CheckDTCFormat(pFilter->DTCFormat, dtcRef)))
                {
                    ret = 0x01u;
                }
                else
                {
                    ...
                    *DTCFaultDetectionCounter = FDCInfo[dtcRef].FDC;
                    if ((0u
                         != (DemDTCGeneralStatus[dtcRef].CbUdsStatus & 0xffu
    ...
    uint8 * DTCFunctionalUnit)
{
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x3du, 0x20u);
    }
    else if (
        (DTC == ((void *)0)) || (DTCStatus == ((void *)0)) || (DTCSeverity == ((void *)0)) || (DTCFunctionalUnit == ((void *)0)))
    {
        DEM_DET_REPORT(0x3du, 0x11u);
    }
    else
    {
        for (; (ret != 0x00u) && (pFilter->CurrentIndex < 59u); pFilter->CurrentIndex++)
        {
            ...
            if (dtcRef != 0xFFFFu)
            {
                ...
                else
                {
                    ...
                    Dem_GetEventDTC(dtcRef, pFilter->DTCFormat, DTC);
                    *DTCStatus = (DemDTCGeneralStatus[dtcRef].CbUdsStatus & 0xffu);
                    *DTCSeverity = DemPbCfgPtr->DemCfgDTC[dtcRef].DemDTCSeverity;
                    *DTCFunctionalUnit = DemPbCfgPtr->DemCfgDTC[dtcRef].DemDTCFunctionalUnit;
    ...
    uint16 * NumberOfFilteredRecords)
{
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x3fu, 0x20u);
    }
    else if (0x01u != DTCFormat)
    {
        DEM_DET_REPORT(0x3fu, 0x10u);
    }
    else if (NumberOfFilteredRecords == ((void *)0))
    {
        DEM_DET_REPORT(0x3fu, 0x11u);
    }
    else
    {
    ...
    uint8 * RecordNumber)
{
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x3au, 0x20u);
    }
    else if (DTC == ((void *)0))
    {
        DEM_DET_REPORT(0x3au, 0x11u);
    }
    else if (RecordNumber == ((void *)0))
    {
        DEM_DET_REPORT(0x3au, 0x11u);
    }
    else
...
{
    Dem_EventIdType IntId;
    Dem_ReturnGetDTCByOccurrenceTimeType ret = 0x01u;

    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x19u, 0x20u);
    }
    else if (DTC == ((void *)0))
    {
        DEM_DET_REPORT(0x19u, 0x11u);
    }
    else
    {
        switch (DTCRequest)
        ...
        if ((IntId != 0xFFFFu)
            && (DemPbCfgPtr->pDemEventParameter[IntId].DemDTCRef != 0xFFFFu))
        {
            uint16 DTCRef = DemPbCfgPtr->pDemEventParameter[IntId].DemDTCRef;
             
            if (DemDTCGeneralStatus[DTCRef].SuppressionStatus == 0u)
            {
                Dem_GetEventDTC(DTCRef, 0x01u, DTC);
            }
            ret = 0x00u;
        }
    }
...
{
    uint16 dtcIndex;
    Dem_ReturnDisableDTCRecordUpdateType ret = 0x01u;
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x1au, 0x20u);
    }
    else if (Dem_DTCDisable == 1u)
    {
        DEM_DET_REPORT(0x1au, 0x40u);
        ret = 1U;
    }
    else
    {
    ...
    uint16 * BufSize)
{
    uint8 memDest;
    uint16 dtcIndex;
    Dem_ReturnGetFreezeFrameDataByDTCType ret = 0x01u;
    uint16 memDestAndRecordNumber;

    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x1du, 0x20u);
    }
    else
    {
        dtcIndex = Dem_GetDTCIndex(DTC, 0x01u);
        memDest = Dem_GetInternalMemDest(DTCOrigin);
        if ((DestBuffer == ((void *)0)) || (BufSize == ((void *)0)))
        {
            DEM_DET_REPORT(0x1du, 0x11u);
        }
        else if (RecordNumber == 0xFFu)
        {
            DEM_DET_REPORT(0x1du, 0x12u);
            ret = 0x03u;
        }
         
        else if ((dtcIndex == 0xFFFFu) || (DemDTCGeneralStatus[dtcIndex].SuppressionStatus == 1u))
        {
...
Dem_DcmGetSizeOfFreezeFrameByDTC(
    ...
    uint16 * SizeOfFreezeFrame)
{
    Dem_ReturnGetSizeOfDataByDTCType ret = 0x01u;
    uint16 dtcRef;
    uint8 memDest;

    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x1fu, 0x20u);
    }
    else if (SizeOfFreezeFrame == ((void *)0))
    {
        DEM_DET_REPORT(0x1fu, 0x11u);
    }
    else
    {
         
        dtcRef = Dem_GetDTCIndex(DTC, 0x01u);
        memDest = Dem_GetInternalMemDest(DTCOrigin);
        if ((dtcRef == 0xFFFFu) || (DemDTCGeneralStatus[dtcRef].SuppressionStatus == 1u))
        {
...
{
    uint8 memDest;
    Dem_ReturnGetExtendedDataRecordByDTCType ret = 0x01u;
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x20u, 0x20u);
    }
    else if ((DestBuffer == ((void *)0)) || (BufSize == ((void *)0)))
    {
        DEM_DET_REPORT(0x20u, 0x11u);
    }
    else if (ExtendedDataNumber >= 0xFEu)
    {
        DEM_DET_REPORT(0x20u, 0x12u);
        ret = 0x03u;
    }
    else
    {
        ...
        dtcRef = Dem_GetDTCIndex(DTC, 0x01u);
        pDTCAttrCfg = &DemPbCfgPtr->pDemDTCAttributes[DemPbCfgPtr->DemCfgDTC[dtcRef].DemDTCAttributesRef];
        if (memDest == 0xFFu)
        {
             
            ret = 0x02u;
        }
        else if (pExtDataCfg == ((void *)0))
        {
            ret = 0x03u;
        }
        ...
        else if (0xFFFFu == pDTCAttrCfg->DemExtendedDataClassRef)
        {
            ret = 0x03u;
        }
        else if ((dtcRef != 0xFFFFu) && (DemDTCGeneralStatus[dtcRef].SuppressionStatus == 0u))
        {
            *BufSize = 0u;
    ...
    uint16 * SizeOfExtendedDataRecord)
{
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x21u, 0x20u);
    }
    else if (SizeOfExtendedDataRecord == ((void *)0))
    {
        DEM_DET_REPORT(0x21u, 0x11u);
    }
    else
    {
        dtcRef = Dem_GetDTCIndex(DTC, 0x01u);
...
Dem_DcmCheckClearParameter(uint32 DTC, Dem_DTCFormatType DTCFormat, Dem_DTCOriginType DTCOrigin)
{
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x7bu, 0x20u);
    }
    else if (Dem_ClearDTCLock == DEM_CLEAR_LOCK)
    {
        ret = 0x04u;
    }
    else if (0x01u != DTCOrigin)
    {
        ret = 0x02u;
    }
    else
    {
        ...
        if (DTC != 0xFFFFFFUL)
        {
            if (Dem_GetDTCIndex(DTC, DTCFormat) == 0xFFFFu)
            {
                    ret = 0x01u;
            }
        }
...
Dem_DcmClearDTC(uint32 DTC, Dem_DTCFormatType DTCFormat, Dem_DTCOriginType DTCOrigin)
{
    ...
    {
        if ((DTC == 0xFFFFFFUL) && (0x00u == DTCFormat))
        {
            Dem_ClearDTRInfoByEventId(0xFFFFu);
        }
        if (Dem_ClearDTCLock == DEM_CLEAR_LOCK)
        {
            if ((pClr->SaveDTCIndex == dtcRef) && (pClr->SaveDTCFormat == DTCFormat)
                && (pClr->SaveDTCOrigin == DTCOrigin) && (pClr->SID == 0x22u))
            {
...
Dem_DcmDisableDTCSetting(uint32 DTCGroup, Dem_DTCKindType DTCKind)
{
    Dem_ReturnControlDTCSettingType ret = 0x01u;
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x24u, 0x20u);
    }
    else
    {
...
Dem_DcmGetDTCSeverityAvailabilityMask(Dem_DTCSeverityType * DTCSeverityMask)
{
    Dem_ReturnClearDTCType ret = 1U;
    if (DEM_STATE_INIT != Dem_InitState)
    {
        DEM_DET_REPORT(0xb2u, 0x20u);
    }
    else if (((void *)0) == DTCSeverityMask)
    {
        DEM_DET_REPORT(0xb2u, 0x11u);
    }
    else
    {
 ...
 void Dem_SetDTCSettingProcess(void)
{
    uint16 iloop;
    ...
    for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
    {
        ...
        if ((pEventCfg->DemDTCRef != 0xFFFFu)
            && (DemDTCSettingInfo.DTCKind == DemPbCfgPtr->DemCfgDTC[pEventCfg->DemDTCRef].DTCKind))
        {
            if (DemPbCfgPtr->DemCfgDTC[pEventCfg->DemDTCRef].GroupRef != 0xFFu)
            {
                {
                    if (0x00u == DEM_FLAGS_ISSET(pEvent->Status, 0x01u))
                    {
                        ...
                        if (DemPbCfgPtr->pDemEventParameter[iloop].DemCallbackInitMForE != ((void *)0))
                        {
                            DemPbCfgPtr->pDemEventParameter[iloop].DemCallbackInitMForE(0x03u);
                        }
                    }
                }
            }
            ...
            {
                if (DemDTCSettingInfo.AllGroupIsEnabled == 0u)
                {
                     
                    (pEvent->Status) &= (uint8)(~(0x01u));
    ...
    *DTC = 0x00UL;
    if (DTCRef != 0xFFFFu)
    {
        ...
        case 0x02u:
            ...
            if ((DemObdDTCRef != 0xFFFFu)
                && (pDemObdDTC[DemObdDTCRef].DemJ1939DTCValue != 0xFFFFFFu))
            {
        ...
        case 0x00u:
            DemObdDTCRef = pDemDTC[DTCRef].DemObdDTCRef;
            if ((DemObdDTCRef != 0xFFFFu)
                && (pDemObdDTC[DemObdDTCRef].DemDtcValue != 0xFFFFu))
            {
    ...
    const Dem_UdsStatusByteType * DTCStatus)
{
    ...
    if (Dem_InitState != DEM_STATE_INIT)
    {
        DEM_DET_REPORT(0x18u, 0x20u);
    }
    else if ((DTCStatus == ((void *)0)) || (DTC == ((void *)0)))
    {
        DEM_DET_REPORT(0x18u, 0x11u);
    }
     
    else if (
        (pFilter->DTCKind == 0x02u)
        && ((pFilter->DTCStatusMask == 0x08u) || (pFilter->DTCStatusMask == 0x04u)
            || (pFilter->DTCOrigin == 0x03u)))
    {
        ret = 0x00u;
    }
    else
...
{
    uint8 iloop = 0;
    Std_ReturnType ret = 0xFFu;
    const Dem_DTCAttributesType *
    pDTCAttrCfg = Dem_EventDTCAttributesCfg(IntId);

    if (pDTCAttrCfg != ((void *)0))
    {
        while (iloop < 1u)
        {
...
static  boolean Dem_CheckDTCFormat(uint8 dtcFormat, uint16 dtcRef)
{
    ...
    if ((0x01u == dtcFormat) && (dtcRef != 0xFFFFu)
        && (pDemDTC[dtcRef].DemDtcValue != 0xFFFFFFu))
    {
        ret = 1u;
    }
    else if (
        (0x00u == dtcFormat)
        && ((dtcRef != 0xFFFFu) && (obdDtcRef != 0xFFFFu)
            && (pDemObdDTC[obdDtcRef].DemDtcValue != 0xFFFFu)))
    {
        ret = 1u;
    }
    else if (
        (0x02u == dtcFormat)
        && ((dtcRef != 0xFFFFu) && (obdDtcRef != 0xFFFFu)
            && (pDemObdDTC[obdDtcRef].DemJ1939DTCValue != 0xFFFFFFu)))
    {
        ret = 1u;
    }
    else
 ...
 void Dem_EventTestFailed(Dem_EventIdType IntId)
 
{
    ...
    if ((0x00u == (pEvent->UdsStatus & 0x01u)) && (pDTCAttr != ((void *)0)))
    {
        for (uint8 iloop = 0; (iloop < 1u)
                              && (0xFFu != pDTCAttr->DemMemoryDestinationRef[iloop]);
             iloop++)
        {
            if (DemMemDestCfg[pDTCAttr->DemMemoryDestinationRef[iloop]].ExtId == 0x01u)
            {
                 
                Dem_SetOccurrenceEvent(pEvent->IntId, 0x01u);
            }
        }
        (pEvent->Status) |= (0x20u);
    }
    ...
    (pEvent->UdsStatus) |= (0x01u | 0x02u | 0x20u);
     
    (pEvent->UdsStatus) &= (uint8)(~(0x40u | 0x10u));
    (pEvent->Status) |= (0x80u);



    Dem_EventTestFailedProcessOfComponent(pEvent, pEventCfg);
    if (DEM_FLAGS_ISSET(pEvent->Status, 0x20u) != 0x00u)
    {
        Dem_EventDataStorageTrigger(IntId, 0x05u);
    }

     
    pEvent->AgingCounter = 0;
     
    if (0x00u == DEM_FLAGS_ISSET(pEvent->UdsStatus, 0x04u))
    {
         
        (pEvent->UdsStatus) |= (0x04u);

    }
     
    if (0x00u == DEM_FLAGS_ISSET(pEvent->UdsStatus, 0x08u))
    {
         
        if ((pEvent->FailureCounter + 1u) >= pEventCfg->DemEventFailureCycleCounterThreshold)
        {
            {
                if ((0xFFFFu != DTCRef) && (pDTCAttr != ((void *)0)))
                {
                    const uint8* pMemDest = pDTCAttr->DemMemoryDestinationRef;
                    uint8 index = 0;
                    {
                        if ((pMemDest[index] != 0xFFu)
                            && (DemMemDestCfg[pMemDest[index]].ExtId == 0x01u))
                        {
                            Dem_SetOccurrenceEvent(IntId, 0x08u);  
                        }
                    }
                }
                ...
                else
                {
                    ...
                    (pEvent->UdsStatus) |= (0x08u);
                }
                 
                Dem_EventDataStorageTrigger(IntId, 0x00u);
            }
        }
    }
    else
    ...
    if ((pEvent->OccurrenceCounter < 0xFFu) && (((void *)0) != Dem_CheckEventMemEntryExistsAlready(IntId))
        && ((DEM_FLAGS_ISSET(pEvent->UdsStatus, 0x01u) != 0)
            && (DEM_FLAGS_ISSET(pEvent->Status, 0x20u) != 0))
        && (DEM_FLAGS_ISSET(pEvent->UdsStatus, 0x08u) != 0))
    {
    ...
    if (1u == Dem_CheckWIROn(pEvent, pEventCfg))
    {
 ...
 void Dem_EventTestPassed(Dem_EventIdType IntId)
{
    Dem_EventRelateInformationType* pEvent = &DemEventRelateInformation[IntId];
    ...
    (pEvent->UdsStatus) &= (uint8)(~(0x01u | 0x40u | 0x10u));
    (pEvent->Status) &= (uint8)(~(0x80u));
...
static  void Dem_EntryChronologicalOrderUpdate(
    Dem_EventIdType IntId,
    uint8 memDest,
    Dem_EventMemEntryType * pEntry)
{
...
Dem_MemEntryAllocate(Dem_EventIdType IntId, uint8 memDest)
{
    ...
    if ((0U != Dem_CheckEntryAlreadyAllocated(IntId, memDest, &pEntry))
        || (DEM_FLAGS_ISSET(pEvent->Status, 0x40u) != 0x00u))
    {
        pEntry = ((void *)0);
    }
    else if (
        (0x00u == DEM_FLAGS_ISSET(pEvent->Status, 0x08u))
        && (DEM_FLAGS_ISSET(pEvent->Status, 0x20u) != 0x00u))
    {
    ...
    else if (0x00u == DEM_FLAGS_ISSET(pEvent->Status, 0x01u))
    {
...
static  void Dem_CopyInfoFromInfo(
    const Dem_EventRelateInformationType * Info,
    Dem_EventMemEntryType * Entry)
{
 ...
 void Dem_IntWriteNvRAM(void)
{
    ...
    (void)NvM_WriteBlock(blockRef, ((void *)0));
