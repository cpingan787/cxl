...
static  void Dem_ExtendedDataStorage(
    Dem_EventIdType IntId,
    Dem_EventMemEntryType * pEntry,
    uint8 Trigger);
...
static  void Dem_GetFncData(DemDataElementReadFncType DataReadFnc, uint8* pBuffer, uint8 DataSize);
...
static  void Dem_FreezeFrameStorage(
    Dem_EventIdType IntId,
    Dem_EventMemEntryType * pEntry,
    uint8 Trigger);
...
static  void Dem_GetGeneralFreezeFrame(uint16 tempRef, uint8* pBuffer);
...
static  void Dem_GetJ1939FreezeFrame(uint16 tempRef, uint8 * pBuffer);
...
static  void Dem_GetObdFreezeFrame(uint8 * pBuffer);
...
static  void
    Dem_FreezeFrameCopy(Dem_EventIdType IntId, uint8 * pFFBuffer);
...
static  Dem_FreezeFrameInfoType *
    Dem_FreezeFrameGetByRecordNum(Dem_EventMemEntryType * pEntry, uint8 RecordNum);
    ...
    Dem_J1939DcmLampStatusType * LampStatus)
{
    ...
    if (DemIndicatorRef == 0xFFu)
    {
        ...
        if (DemWIRStatus[DemIndicatorRef] != 0x00u)
        {
            if (DemIndicatorBehaviour == 0x01u)
            {
                 
                *LampStatus |= 0x0C00u;
            }
            if (DemIndicatorBehaviour == 0x04u)
            {
                 
            }
            if (DemIndicatorBehaviour == 0x05u)
            {
...
static  Dem_EventStatusType Dem_DebouceProcessPrePassedCounter(
    const Dem_DebounceCounterBasedClassType * pCfgCounter,
    Dem_DebounceCounterInfoType * pCounter)
{
    Dem_EventStatusType status = 0x02u;
    ...
    {
        if (pCfgCounter->DemDebounceCounterJumpDown == 1u)  
        {
...
static  Dem_EventStatusType Dem_DebouceProcessPreFailedCounter(
    const Dem_DebounceCounterBasedClassType * pCfgCounter,
    Dem_DebounceCounterInfoType * pCounter)
{
    Dem_EventStatusType status = 0x03u;
    ...
    {
        if (pCfgCounter->DemDebounceCounterJumpUp == 1u)  
        {
...
static  void Dem_UpdateInternalData(uint16 IntId)
{
    ...
    if (DTCRef != 0xFFFFu)
    {
        ...
        if (pDTCAttrCfg->DemAgingAllowed == 0u)
        {
        ...
        uint8 iloop = 0;
        {
            ...
            {
                if (DemMemDestInfo[pMemDest[iloop]].OverFlow == 1u)
                {
    ...
    DemInternalData.FaultPendingCounter = DEM_GET_BIT(pEvent->UdsStatus, (uint8)0x04u);
...
static  void Dem_DcmGetFreezeFrameDataByDTCProcess(
    const uint8 * pFFData,
    uint8 * DestBuffer,
    uint16 * BufSize,
    Dem_EventIdType IntId,
    ...
    if (pDTCAttr != ((void *)0))
    {
        ...
        for (didRefNum = 0u; didRefNum < RefNum; didRefNum++)
        {
            ...
            for (csIndex = 0; csIndex < pDid->RefNum; csIndex++)
            {
                ...
                if (pDemDataElementClass->DemDataElementReadFnc != ((void *)0))
                {
...
static  void Dem_DcmGetSizeOfExtDataRecLoop(
    uint16 dtcRef,
    uint8 memDest,
    uint16 * SizeOfExtendedDataRecord,
    ...
    uint8 Trigger)
{
    ...
    if ((DemGlobalFreezeFrame.DemFreezeFrameRecordTrigger == Trigger)
        && ((0x02u != Trigger)
            || (DEM_FLAGS_ISSET(pEvent->Status, 0x02u) == 0x00u))
    ...
    if ((pDTCAttrCfg != ((void *)0)) && (pDTCAttrCfg->DemFreezeFrameRecNumClassRef != 0xFFu))
    {
        ...
        for (iloop = 0; iloop < RefNum; iloop++)
        {
            ...
            if (pFFRecordIndex != 0xFFu)
            {
                        ...
                        || (DEM_FLAGS_ISSET(pEvent->Status, 0x02u) == 0x00u)))
                {
                    ...
                    if (((pFF->RecordNum == 0xFFu)
                         && (pFFRecordCfg->DemFreezeFrameRecordUpdate == 0u))
                        || (pFFRecordCfg->DemFreezeFrameRecordUpdate == 1u))
                    {
                        {
...
static  Dem_FreezeFrameInfoType *
    Dem_FreezeFrameGetByRecordNum(Dem_EventMemEntryType * pEntry, uint8 RecordNum)
{
    Dem_FreezeFrameInfoType* ret = ((void *)0);
 ...
 void Dem_EventQueueProcessWithDebounce(Dem_EventIdType IntId)
{
    const Dem_EventParameterType* pEventCfg = &DemPbCfgPtr->pDemEventParameter[IntId];
    ...
    const Dem_EventParameterType * pEventCfg)
{
    ...
    for (iloop = 0; iloop < pEventCfg->AttrNum; iloop++)
    {
        ...
        {
            for (index = 0; ((index < 59u) && (Find == 0u)); index++)
            {
                for (cnt = 0; ((cnt < DemPbCfgPtr->pDemEventParameter[index].AttrNum) && (Find == 0u)); cnt++)
                {
                    if ((pWIRAttr->DemIndicatorRef
                        ...
                        && (0x00u != DEM_FLAGS_ISSET(DemEventRelateInformation[index].UdsStatus, 0x80u)))
                    {
                        Find = 1u;
                    }
                }
            ...
            if (Find == 0u)
            {
                DemWIRStatus[pWIRAttr->DemIndicatorRef] = 0x00u;
                ret = 1u;
            }
        }
    }
 ...
 void Dem_ResponseOnDtcStatusChange(Dem_EventIdType IntId)
{
    uint16 dtcRef = DemPbCfgPtr->pDemEventParameter[IntId].DemDTCRef;
    Dem_EventRelateInformationType* pEvent = Dem_GetEventInfo(IntId);

    if ((dtcRef != 0xFFFFu) && (pEvent->OldUdsStatus != pEvent->UdsStatus))
    {
...
Dem_InterEventTestPassed(Dem_EventIdType IntId)
{
    ...
    if ((0x00u == DEM_FLAGS_ISSET(pEvent->UdsStatus, 0x02u))
        && (0x00u != DEM_FLAGS_ISSET(pEvent->UdsStatus, 0x80u)))
    {
        const Dem_IndicatorAttributeType* pWIRAttr;
        boolean checkFlag = 1u;
 ...
 void Dem_InterOperationCycleStart(Dem_EventIdType IntId)
{
    ...
    if ((AlgorithmType == 0u) && (AlgorithmIndex < 59u))
    {
        DemDebounceCounterInfo[AlgorithmIndex].InternalDebounceCounter = 0;
    }
}
...
Dem_CallDataChangedOfMemEntryAllocate(Dem_EventIdType IntId)
{
    ...
    if (CallbackEventDataChanged != ((void *)0))
    {
 ...
 uint8 Dem_GetMemDestMaxNumOfDtc(void)
{
    return 1u;
}
...
void Dem_ClearEventRelateInformation(Dem_EventRelateInformationType * pEvent)
{
 ...
 void
Dem_DeleteRelatedData(Dem_EventMemEntryType * pEntry)
{
    Dem_EventRelateInformationType* pEvent;
    pEntry->FFNum = 0x00;
    pEntry->LatestFF = 0x00;
    Dem_MemSet((uint8*)pEntry->FFList, 0xFFu, sizeof(Dem_FreezeFrameInfoType) * 1u);
    Dem_MemSet((uint8*)&pEntry->GlobalFFList, 0xFFu, sizeof(Dem_GlobalFreezeFrameInfoType));
    Dem_MemSet(pEntry->ExtStatus, 0x00u, (((4u) + 7u) >> 3u));
    if ((pEntry->EventId != 0u) && (pEntry->EventId <= 59u))
    {
            ...
            uint8 iloop = 0;
            {
                    ...
                    && (DEM_BITS_ISNSET(DemMemDestInfo[MemDest].DisableDTCRecordUpdate, TempRef)))
                {
                    ...
                    if (pEntry != ((void *)0))
                    {
                        ...
                        if ((pDTCAttr->DemImmediateNvStorage == 1u)
                            && (DemEventRelateInformation[IntId].OccurrenceCounter < 0xffu))
                        {
                            Dem_IntWriteNvRAM();
                        }
                    }
 ...
 void Dem_DebounceInit(void)
{
    ...
    Dem_MemSet((uint8*)FDCInfo, 0x00u, (sizeof(FDCInfoType) * 59u));
}
 ...
 void Dem_DebounceReset(Dem_EventIdType IntId)
{
    const Dem_EventParameterType* pEventCfg = &DemPbCfgPtr->pDemEventParameter[IntId];
    Dem_DebounceCounterInfoType* pCounter;
...
Dem_GetInteralFDC(Dem_EventIdType IntId, sint8 * FaultDetectionCounter)
{
    ...
    *FaultDetectionCounter = 0;
    ...
    case 0u:
        ...
        if (((DemPbCfgPtr->pDemEventParameter[IntId].AlgorithmRef) < 1u)
            && ((pEventCfg->AlgorithmIndex) < 59u))
        {
...
Dem_DebounceProcess(Dem_EventDataBufferType pEventBuffer)
{
    ...
    pEvent->AgingCounter = (0x01u == Status) ? 0u : pEvent->AgingCounter;
    return Status;
}
 ...
 void Dem_DebounceTimerMain(void)
{
}
 ...
 void Dem_UpdateFDC(Dem_EventIdType IntId)    
{
    const Dem_EventParameterType* pEventCfg = &DemPbCfg.pDemEventParameter[IntId];
    const Dem_DebounceCounterInfoType* pCounter;
    const Dem_DebounceCounterBasedClassType* pCfgCounter;
    ...
    {
    case 0u:
        if (((DemPbCfg.pDemEventParameter[IntId].AlgorithmRef) < 1u)
            && ((pEventCfg->AlgorithmIndex) < 59u))
        {
    ...
    uint8 * RecordNumber)
{
    ...
    for (; (ret != 0x00u) && (pFilter->GetNum < pFilter->NumberOfFilteredRecords);)
    {
        ...
        else
        {
            ...
            else
            {
                if (pEntry->EventId != 0u)
                {
                    ...
                    uint16 DTCRef = DemPbCfgPtr->pDemEventParameter[IntId].DemDTCRef;
                    if ((DTCRef != 0xFFFFu) && (DemDTCGeneralStatus[DTCRef].SuppressionStatus == 0u))
                    {
    ...
    uint16 * NumberOfFilteredRecords)
{
    ...
    const Dem_EventMemEntryType* pEntry;
    Dem_FreezeFrameRecordFilterInfoType* pFilter = &DemFreezeFrameRecordFilterInfo;
    Dem_DTCOriginType DTCOrigin = 0x01u;
    ...
    *BufSize = 0u;
...

    {
        if (RecordNumber == DemGlobalFreezeFrame.DemFreezeFrameRecordNumber)
        {
            ...
            if ((pEntry != ((void *)0)) && (pEntry->GlobalFFList.RecordNum != 0xFFu))
            {
                getData = 1u;
        ...
        if (Find != 1u)
        {
            ...
            if ((FFClassRef != 0xFFFFu)
                && (0xFFu != pDTCAttrCfg->DemFreezeFrameRecNumClassRef)
            )
            {
                ...
                for (uint8 index = 0u; (index < RefNum) && (Find == 0u); index++)
                {
                    if (((*pFFRecordIndex) != 0xFFu)
                ...
                if (Find == 1u)
                {
                    ...
                    if (pEntry != ((void *)0))
                    {
                        ...
                        if (pFF != ((void *)0))
                        {
                            ...
                            if ((pDTCAttr != ((void *)0))
                                && (pDTCAttr->DemFreezeFrameClassRef != 0xFFFFu))
                            {
    ...
    uint16 * SizeOfFreezeFrame)
{
        ...
        if (pEntry != ((void *)0))
        {
            ...
            if (pDTCAttr != ((void *)0))
            {
                ...
                if ((RecordNumber == 0xFFu)
                    && (0xFFu != pDTCAttr->DemFreezeFrameRecNumClassRef)
                )
                {
 ...
 const Dem_ExtendedDataRecordClassType *
Dem_GetExtendedDataRecordCfg(uint8 ExtendedDataNumber)
{
    const Dem_ExtendedDataRecordClassType* ret = (const Dem_ExtendedDataRecordClassType*)((void *)0);
    uint8 iloop;

    for (iloop = 0; iloop < 4u; iloop++)
    {
...
Dem_GetExtendedRecordDataLoop(
    ...
    uint16 * BufSize)
{
    Dem_ReturnGetExtendedDataRecordByDTCType ret = 0x00u;
    uint16 dtcRef = Dem_GetDTCIndex(DTC, 0x01u);
    uint16 index;
    boolean Find = 0u;
...
Dem_InterDcmGetSizeOfExtendedDataRecordByDTC(
    ...
    uint16 * SizeOfExtendedDataRecord)
{
    Dem_ReturnGetSizeOfDataByDTCType ret = 0x00u;
    uint8 memDest;
    uint16 dtcRef;
    ...
    *SizeOfExtendedDataRecord = 0;
    ...
    for (; (Start < End) && (0x00u == ret); Start++)
    {
        const Dem_ExtendedDataRecordClassType* pExtDataCfg = ((void *)0);
        pExtDataCfg = Dem_GetExtendedDataRecordCfg(ExtendedDataNumber);
        if (pExtDataCfg == ((void *)0))
        {
            ret = 0x03u;
        }
        else
...
Dem_InterDcmDisableDTCSetting(uint32 DTCGroup, Dem_DTCKindType DTCKind)
{
    ...
    {
        DEM_DET_REPORT(0x24u, 0x10u);
        ret = 0x02u;
        ProcessOn = 0u;
    }
    else
    {
        DemDTCSettingInfo.AllGroupIsEnabled = 0u;
    }
    if (ProcessOn == 1u)
    {
...
Dem_InterDcmEnableDTCSetting(uint32 DTCGroup, Dem_DTCKindType DTCKind)
{
 ...
 uint8 Dem_GetDemMalfunctionLampIndicator(void)
{
    return 0xFFu;
}
...
static  void Dem_GetFncData(DemDataElementReadFncType DataReadFnc, uint8* pBuffer, uint8 DataSize)
{
    if (DataReadFnc != ((void *)0))
    {
        if (DataReadFnc(pBuffer) != 0U)
        {
            Dem_MemSet(pBuffer, 0xFFu, DataSize);
            DEM_DET_REPORT(0x55u, 0x30u);
        }
    }
}
...
Dem_CheckFreezeFrameIsStorage(Dem_EventIdType IntId, uint8 Trigger)
{
    ...
    if (pDTCAttrCfg != ((void *)0))
    {
        for (iloop = 0;
             (0xFFu != pDTCAttrCfg->DemFreezeFrameRecNumClassRef)
             && (iloop < DemFreezeFrameRecNumClass[pDTCAttrCfg->DemFreezeFrameRecNumClassRef].RefNum) && (res == 0u);
             iloop++)
        {
...
static  void Dem_FreezeFrameCopy(Dem_EventIdType IntId, uint8 * pFFBuffer)
{
    {
 ...
 void
Dem_GetFreezeFrame(Dem_EventIdType IntId, uint8 * pBuffer)
{
    uint16 tempRef;
    ...
    else
    {
        ...
        else
        {
            ...
            if ((tempRef != 0xFFu) && (obdDtcRef != 0xFFFFu)
                && (DemPbCfg.ObdDTC[obdDtcRef].DemJ1939DTCValue != 0xFFFFFFu))
            {
            ...
            else
            {
                ...
                else
                {
                    if ((obdDtcRef != 0xFFFFu)
                        && (DemPbCfg.ObdDTC[obdDtcRef].DemDtcValue != 0xFFFFu))
                    {
 ...
 
{
    ...
    if (pDTCAttrCfg != ((void *)0))
    {
        ...
        if (0xFFFFu != EDClassRef)
        {
            ...
            for (iloop = 0; (iloop < RefNum) && (ret == 0x31u); iloop++)
            {
                if (*pExtIndex != 0xFFu)
                {
                    ...
                    if (pExtRecord->DemExtendedDataRecordNumber == RecordNum)
                    {
                        ...
                        else
                        {
                            ...
                            for (uint8 index = 0; index < DataElementNum; index++)
                            {
                                ...
                                {
                                    uint8 ElementDataSize = pDemDataElementClass->DemDataElementDataSize;
                                    if (0u == pDemDataElementClass->DemInterFlag)
                                    {
                                        offset += ElementDataSize;
                                    }
                                    else
...
Dem_CheckExtendedDataIsStorage(Dem_EventIdType IntId, uint8 Trigger)
{
    ...
    if ((pDTCAttrCfg != ((void *)0)) && (pDTCAttrCfg->DemExtendedDataClassRef != 0xFFFFu))
    {
        for (iloop = 0; (iloop < DemExtendedDataClass[pDTCAttrCfg->DemExtendedDataClassRef].RefNum) && (res == 0u);
             iloop++)
        {
        ...
        for (iloop = 0; iloop < DemExtendedDataClass[DemExtendedDataClassRef].RefNum; iloop++)
        {
            ...
            if (pExtIndex != 0xFFu)
            {
                ...
                if ((RecordTrigger == Trigger)
                    && ((0x02u != Trigger)
                        || (DEM_FLAGS_ISSET(pEvent->Status, 0x02u) == 0x00u))
                    && ((Update == 1u) || (DEM_BITS_ISNSET(pEntry->ExtStatus, iloop))))
                {
    ...
    {
        {
            ...
            else
            {
                if (pEntry->EventId != 0u)
                {
                    ...
                    for (didRefNum = 0; (ret != 0U) && (didRefNum < DemFreezeFrameClass[Ref].RefNum); didRefNum++)
                    {
                        ...
                        if (pDid->DemDidIdentifier == Did)
                        {
                            ...
                            for (uint8 csIndex = 0; csIndex < pDid->RefNum; csIndex++)
                            {
                                ...
                                if (lDemDataElementClass->DemDataElementReadFnc != ((void *)0))
                                {
