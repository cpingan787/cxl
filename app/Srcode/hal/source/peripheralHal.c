...
/*===========================================================================*/
/* R7F701581 board pin mapping                                               */
/* Source: IAM_A车_4G_A0 schematic, MCU sheet 16                             */
/*===========================================================================*/
#define PERIPHERAL_KL30_DOWN_DET_PORT          Port0
#define PERIPHERAL_KL30_DOWN_DET_PIN           (9U)

#define PERIPHERAL_KL30_DET_EN_PORT            Port0
#define PERIPHERAL_KL30_DET_EN_PIN             (11U)

#define PERIPHERAL_ANT_DET_PWR_EN_PORT          Port0
#define PERIPHERAL_ANT_DET_PWR_EN_PIN           (12U)

#define PERIPHERAL_BECALL_RED_LED_EN_PORT       Port0
#define PERIPHERAL_BECALL_RED_LED_EN_PIN        (7U)

#define PERIPHERAL_GNSS_ANT_PWR_EN_PORT         Port10
#define PERIPHERAL_GNSS_ANT_PWR_EN_PIN          (5U)

#define PERIPHERAL_AMP_STB_PORT                 Port8
#define PERIPHERAL_AMP_STB_PIN                  (5U)

#define PERIPHERAL_CAN_STBY_PORT                Port8
#define PERIPHERAL_CAN_STBY_PIN                 (6U)

#define PERIPHERAL_HSM_WAKEUP_PORT              Port0
#define PERIPHERAL_HSM_WAKEUP_PIN               (5U)

#define PERIPHERAL_HSM_SPI_CS_PORT              Port8
#define PERIPHERAL_HSM_SPI_CS_PIN               (7U)

#define PERIPHERAL_DEEP_SLEEP_PWR_EN_PORT        Port8
#define PERIPHERAL_DEEP_SLEEP_PWR_EN_PIN         (9U)

#define PERIPHERAL_MCU_WDI_PORT                  Port9
#define PERIPHERAL_MCU_WDI_PIN                   (3U)

#define PERIPHERAL_KL30_PRESENT_LEVEL            (0U)
...
{
    /*
     * IAM_A车_4G_A0 has no dedicated KL15/ACC input. The old P8_5 ACC
     * mapping cannot be retained because P8_5 is AMP_STB on this board.
     * Use the available KL30 power-valid indication as the board power state.
     */
    return PeripheralHalGetKl30Status();
}
...
int16_t PeripheralHalGetKl30Status(void)
{
    ...
    /*
     * KL30_DOWN_DET is P0_9. The detector is active low:
     * low = KL30 present, high = KL30 absent.
     */
    pinLevel = R_PORT_GetLevel(PERIPHERAL_KL30_DOWN_DET_PORT,
                               PERIPHERAL_KL30_DOWN_DET_PIN);

    return (pinLevel == PERIPHERAL_KL30_PRESENT_LEVEL) ? 1 : 0;
}
...
int16_t PeripheralHalAdGet(uint8_t adChannel, uint32_t *pValue)
{
    ...
    if ((pValue == NULL) || (adChannel >= AD_CHANNEL_COUNT))
    {
        return -1;
    }

    if (g_adBuffer.adValid == 0U)
    {
        return -2;
    }

    adValue = g_adBuffer.adValue[adChannel];
    voltageMv = ((uint32_t)adValue * AD_REF_VOLTAGE) / AD_RESOLUTION_COUNTS;

    if (adChannel == AD0_CHANNEL_KL30)
    {
        /* Schematic sheet 7: 200 kOhm / 20 kOhm divider. */
        voltageMv *= AD_CHANNEL_KL30_FACTOR;
    }
    else if (adChannel == AD0_CHANNEL_BUB_VOLTAGE_ADC)
    {
        /* Schematic sheet 12: 200 kOhm / 100 kOhm divider. */
        voltageMv *= AD_CHANNEL_BATTERY_FACTOR;
    }
    else
    {
        /* Direct 0 - 3.3 V ADC input. */
    }
...
{
    /*
     * The old project used AP1_1, which does not exist on R7F701581.
     * The board LED enable is beCALL_RedLED_EN on P0_7 and is active high.
     */
    R_PORT_SetGpioOutput(PERIPHERAL_BECALL_RED_LED_EN_PORT,
                         PERIPHERAL_BECALL_RED_LED_EN_PIN,
                         High);
}
...
void PeripheralHalSetMode(uint8_t mode)
{
    if (mode != 0U)
    {
        /* AMP_STB=0 is standby, so high selects normal operation. */
        R_PORT_SetGpioOutput(PERIPHERAL_AMP_STB_PORT,
                             PERIPHERAL_AMP_STB_PIN,
                             High);

        /*
         * This single enable controls the KL30 ADC path and the
         * KL30_DOWN_DET power-failure detector.
         */
        R_PORT_SetGpioOutput(PERIPHERAL_KL30_DET_EN_PORT,
                             PERIPHERAL_KL30_DET_EN_PIN,
                             High);

        R_PORT_SetGpioOutput(PERIPHERAL_GNSS_ANT_PWR_EN_PORT,
                             PERIPHERAL_GNSS_ANT_PWR_EN_PIN,
                             High);

        R_PORT_SetGpioOutput(PERIPHERAL_ANT_DET_PWR_EN_PORT,
                             PERIPHERAL_ANT_DET_PWR_EN_PIN,
                             High);
    ...
    {
        /*
         * Keep KL30 detection enabled in sleep so power-loss status remains
         * available.
         */
        R_PORT_SetGpioOutput(PERIPHERAL_KL30_DET_EN_PORT,
                             PERIPHERAL_KL30_DET_EN_PIN,
        ...
        /* AMP_STB=0 enters amplifier standby. */
        R_PORT_SetGpioOutput(PERIPHERAL_AMP_STB_PORT,
                             PERIPHERAL_AMP_STB_PIN,
                             Low);
    }
...
void PeripheralHalMcuHardReset(void)
{
    ...
    do
    {
        resetValue = 0x00000001UL;
        WPROTR.PROTCMD0 = 0x000000A5UL;
        RESCTL.SWRESA = resetValue;
        RESCTL.SWRESA = ~resetValue;
        RESCTL.SWRESA = resetValue;
    } while (WPROTR.PROTS0 != 0x00000000UL);
}
...
{
    R_PORT_ToggleGpioOutput(PERIPHERAL_MCU_WDI_PORT,
                            PERIPHERAL_MCU_WDI_PIN);
}
...
{
    /* KL30_DOWN_DET is externally pulled up and active low. */
    R_PORT_SetGpioInput(PERIPHERAL_KL30_DOWN_DET_PORT,
                        PERIPHERAL_KL30_DOWN_DET_PIN);

    /*
     * CAN1 and CAN2 share CAN_STBY. SIT1044 S=0 is normal mode and
     * S=1 is standby.
     */
    R_PORT_SetGpioOutput(PERIPHERAL_CAN_STBY_PORT,
                         PERIPHERAL_CAN_STBY_PIN,
                         Low);

    /* HSM wake is inactive low; SPI chip select is inactive high. */
    R_PORT_SetGpioOutput(PERIPHERAL_HSM_WAKEUP_PORT,
                         PERIPHERAL_HSM_WAKEUP_PIN,
                         Low);

    R_PORT_SetGpioOutput(PERIPHERAL_HSM_SPI_CS_PORT,
                         PERIPHERAL_HSM_SPI_CS_PIN,
                         High);

    /* External watchdog WDI initial level. */
    R_PORT_SetGpioOutput(PERIPHERAL_MCU_WDI_PORT,
                         PERIPHERAL_MCU_WDI_PIN,
                         Low);

    /* Deep-sleep retention rail. */
    R_PORT_SetGpioOutput(PERIPHERAL_DEEP_SLEEP_PWR_EN_PORT,
                         PERIPHERAL_DEEP_SLEEP_PWR_EN_PIN,
                         High);

    /* KL30 monitoring stays enabled. */
    R_PORT_SetGpioOutput(PERIPHERAL_KL30_DET_EN_PORT,
                         PERIPHERAL_KL30_DET_EN_PIN,
                         High);
    ...
    /*
     * AMP_STB is controlled by the MCU on P8_5. AMP_MUTE is driven by the
     * NAD on this schematic and must not be driven through an MCU port.
     */
    R_PORT_SetGpioOutput(PERIPHERAL_AMP_STB_PORT,
                         PERIPHERAL_AMP_STB_PIN,
                         High);
