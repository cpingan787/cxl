...
static void SecocSaveTripCount(uint32_t value)
{
    ...
    ILib_memcpy(
                (const uint8*)NvM_BlockDescriptor[51U - 1u]
                .NvmRamBlockDataAddress,
                &value,
            4);
    Std_ReturnType ret = NvM_WriteBlock(51U,((void *)0));
    ...
    if(isEnable)
    {
        if(ret != 0U)
        {
            ...
            Dem_SetEventStatus(49u,0x01u);
            LogHalPrint("Fvm_SaveTripCount failed, ret = %d", ret);
        }
        else
        {
            
            Dem_SetEventStatus(49u,0x00u);
            LogHalPrint("Fvm_SaveTripCount success, value = %d", value);
        }
    }   
...
static void SecocReadTripCount(uint32_t* pValue)
{
    ...
    (void)NvM_GetErrorStatus(51U, &RequestResultPtr);
