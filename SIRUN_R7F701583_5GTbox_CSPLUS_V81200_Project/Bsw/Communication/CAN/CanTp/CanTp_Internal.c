    ...
    PduLengthType bufferSize)
{
    ...
    if (RxchannelPtr->CurrentBs == 0u) /**<first calculate*/
#endif
    {
...
#endif
            /**
             * The BS value is only a maximum value. For reasons of buffer length, the CAN Transport
             * Layer can adapt the BS value within the limit of the configured maximum value
             */
            blockDataLen = length * (PduLengthType)bs;
            /* buffer too small for next block  && buffer too small for last block*/
            if ((bufferSize < blockDataLen) && (bufferSize < RxchannelPtr->SduDataRemaining))
            {
                RxchannelPtr->CurrentBs = (uint8)(bufferSize / length);
                ret = (RxchannelPtr->CurrentBs >= 1u) ? E_OK : E_NOT_OK;
            }
            else
            {
