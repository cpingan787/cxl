...
#endif /* DEM_MEM_DEST_MAX_NUM_OF_DTC > 1u */
    {
        ...
        if (MemDest != DEM_MEM_DEST_INVALID)
        {
            ...
            for (uint8 iloop = 0; (iloop < EntryNum) && (pEntry == NULL_PTR); iloop++)
            {
                ...
                if ((MemEventId != 0u) && (pEventParameter[Dem_GetEventInternalId(MemEventId)].DemDTCRef == dtcRef))
                {
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_SetEnableCondictionProcess(void)
{
#if (DEM_ENABLE_CONDITION_NUM > 0)
    uint16 iloop = 0;
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_SetStorageCondictionProcess(void)
{
#if (DEM_STORAGE_CONDITION_NUM > 0)
    uint16 iloop = 0;
    Dem_EventRelateInformationType* pEvent;
    ...
    if (Dem_CheckStorageCondition != FALSE)
    {
        ...
        while (iloop < DemPbCfgPtr->DemEventNum)
        {
#if (DEM_STORAGE_CONDITION_NUM > 1u)
            boolean IsReplacement = FALSE;
#endif
            const Dem_EventParameterType* pEventParameter = DemPbCfgPtr->pDemEventParameter;
            const uint8 GroupRef = pEventParameter[iloop].DemStorageConditionGroupRef;
            pEvent = Dem_GetEventInfo(iloop);
            IsFulfilled = TRUE;
            ...
            if (IsFulfilled == TRUE)
            {
                if (0x00u == DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_STORAGE_CONDICTION))
                {
                    ...
                    index = 0;
                    Conditionindex = 0;
                    while ((index < DEM_STORAGE_CONDITION_NUM_BYTE)
#if (DEM_STORAGE_CONDITION_NUM > 1u)
                           && (IsReplacement == FALSE)
#endif
                    )
                    {
#if (DEM_STORAGE_CONDITION_NUM > 8u)
                        uint8 Counter = 0u;
#endif
                        Mask = 0x01;
                        while ((Conditionindex < DEM_STORAGE_CONDITION_NUM)
#if (DEM_STORAGE_CONDITION_NUM > 1u)
                               && (IsReplacement == FALSE)
#endif
#if (DEM_STORAGE_CONDITION_NUM > 8u)
                        ...
                        )
                        {
                            ...
                            if (((DemStorageConditionGroup[pEventParameter[iloop].DemStorageConditionGroupRef][index]
                                  & Mask)
                                 != 0u)
                                && (EventRef != DEM_EVENT_PARAMETER_INVALID))
                            {
                                const Dem_CallbackInitMForEType InitMForE = pEventParameter[iloop].DemCallbackInitMForE;
 ...
 */
/******************************************************************************/
FUNC(void, DEM_CODE) Dem_InterPreInit(void)
{
    uint16 iloop;
    ...
    for (iloop = 0; iloop < DEM_STORAGE_CONDITION_NUM; iloop++)
    {
        if (DemStorageCondition[iloop].StorageConditionStatus == TRUE)
        {
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_InterInit(void)
{
    uint16 iloop;
    Dem_EventRelateInformationType* pEvent;
    ...
    for (iloop = 0u; iloop < DEM_COMPONENT_NUM; iloop++)
    {
...
Dem_InterSetEventStatus(Dem_EventIdType* IntId, Dem_EventStatusType EventStatus)
{
    ...
    uint8 Mask;
    boolean IsReplacement = FALSE;
    const uint8 DemStorageConditionGroupRef = DemPbCfgPtr->pDemEventParameter[*IntId].DemStorageConditionGroupRef;
...
#endif
            Mask = 0x01;
            for (Conditionindex = 0; (Conditionindex < DEM_STORAGE_CONDITION_NUM) && (IsReplacement == FALSE)
#if (DEM_STORAGE_CONDITION_NUM > 8u)
                                     && (Counter < 8u)
#endif
                     ;
...
/*************************************************************************/
FUNC(Std_ReturnType, DEM_CODE)
Dem_InterGetIndicatorStatus(uint8 IndicatorId, P2VAR(Dem_IndicatorStatusType, AUTOMATIC, DEM_APPL_DATA) IndicatorStatus)
{
    Std_ReturnType ret = E_NOT_OK;
    ...
    P2VAR(uint16, AUTOMATIC, DEM_APPL_DATA) BufSize)
{
    ...
    if (dtcRef != DEM_DTC_REF_INVALID)
    {
        const uint8* pMemDest =
            DemPbCfgPtr->pDemDTCAttributes[DemPbCfgPtr->DemCfgDTC[dtcRef].DemDTCAttributesRef].DemMemoryDestinationRef;
...
Dem_InterSetComponentAvailable(Dem_ComponentIdType ComponentId, boolean AvailableStatus)
{
    ...
    if ((ComponentId > DEM_COMPONENT_NUM) || (ComponentId == 0u))
    {
...
FUNC(void, DEM_CODE) Dem_DebounceFreeze(Dem_EventIdType IntId)
{
        ...
        && (AlgorithmIndex < DEM_DEBOUNCE_TIME_BASED_EVENT_NUM))
    {
        ...
        if (DemPbCfg.DebounceTimeBaseClass[AlgorithmRef].DemDebounceBehavior == DEM_DEBOUNCE_FREEZE)
        {
            DemDebounceTimerInfo[AlgorithmIndex].IsFreezing = TRUE;
        }
        else
    ...
    {
        if (DemPbCfg.DebounceCounterBasedClass[AlgorithmRef].DemDebounceBehavior == DEM_DEBOUNCE_RESET)
        {
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_InterCalOBDRelatedValue(void)
{
#if (DEM_RATIO_NUM > 0)
    uint16 iloop;
    uint16 DemDTCRef;
    ...
    for (iloop = 0; iloop < DEM_RATIO_NUM; iloop++)
    {
        uint8 DemDiagnosticEventRef = DemPbCfgPtr->pDemRatio[iloop].DemDiagnosticEventRef;
        DemDTCRef = DemPbCfgPtr->pDemEventParameter[DemDiagnosticEventRef].DemDTCRef;
            ...
            && (0x00u != DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_AVAILABLE)))
        {
            /* req SWS_Dem_00709 */
#if (DEM_TRIGGER_FIM_REPORTS == STD_ON)
            FimFunctionId = DemPbCfgPtr->pDemRatio[iloop].DemFunctionIdRef;
            ...
            if (permission == TRUE)
            {
                const DemSecondaryFunctionIdType* pSecondaryFIdRef =
                    DemPbCfgPtr->pDemRatio[iloop].DemSecondaryFunctionIdRef;
            ...
            eventid = DemPbCfgPtr->pDemRatio[iloop].DemDiagnosticEventRef;
            ...
            const DemRatioType* pRatio = &DemPbCfgPtr->pDemRatio[iloop];
...
Dem_InterDcmGetAvailableOBDMIDs(uint8 Obdmid, P2VAR(uint32, AUTOMATIC, DEM_APPL_DATA) Obdmidvalue)
{
    ...
    *Obdmidvalue = 0;
    for (iloop = 0; iloop < DEM_DTR_NUM; iloop++)
    {
        uint8 DemDtrMid = DemPbCfgPtr->pDTR[iloop].DemDtrMid;
...
Dem_InterDcmGetNumTIDsOfOBDMID(uint8 Obdmid, P2VAR(uint8, AUTOMATIC, DEM_APPL_DATA) numberOfTIDs)
{
    ...
    *numberOfTIDs = 0u;
    for (iloop = 0; iloop < DEM_DTR_NUM; iloop++)
    {
        if (DemPbCfgPtr->pDTR[iloop].DemDtrMid == Obdmid)
        {
            /* req SWS_Dem_00761 */
            (*numberOfTIDs) += 1u;
        }
...
)
{
    ...
    if ((pEntry != NULL_PTR) && (IntID != DemPbCfgPtr->DemEventNum)
        && (DemPbCfgPtr->pDemEventParameter[IntID].DemDTCRef != DEM_DTC_REF_INVALID))
    {
        uint16 DTCRef = DemPbCfgPtr->pDemEventParameter[IntID].DemDTCRef;
        obdDtcRef = DemPbCfgPtr->DemCfgDTC[DTCRef].DemObdDTCRef;
        tempRef = DemPbCfgPtr->DemCfgDTC[DTCRef].DemDTCAttributesRef;
        const Dem_DTCAttributesType* pDTCAttr = &DemPbCfgPtr->pDemDTCAttributes[tempRef];
        ...
        {
            /* req SWS_Dem_01101 */
            if (DemDTCGeneralStatus[DemPbCfgPtr->pDemEventParameter[IntID].DemDTCRef].SuppressionStatus == FALSE)
            {
                for (pid_num = 0; (pid_num < DEM_PID_CLASS_NUM) && (ret == E_NOT_OK); pid_num++)
                {
                    pBuffer = &(pEntry->FFList[0]).Data[cnt];
                    pPid = &DemPbCfgPtr->PidClass[pid_num];
                        ...
                        for (DataElementIndex = 0; DataElementIndex < pPid->RefNum; DataElementIndex++)
                        {
                            ...
                            if (DataElementIndexOfPID == DataElementIndex)
                            {
                                const DemDataElementClassTypes* pDemDataElementClass =
                                    &DemPbCfgPtr->pDemDataElementClass[pPid->StartIndex + DataElementIndex];
...
#if (DEM_DTR_NUM > 0)
    uint16 iloop;
    const DTRType* lDTR = DemPbCfgPtr->pDTR;

    for (iloop = 0; iloop < DEM_DTR_NUM; iloop++)
    {
        if ((Obdmid == lDTR[iloop].DemDtrMid) && (TIDindex == iloop))
        {
            /* req SWS_Dem_00762 */
            *TIDvalue = lDTR[iloop].DemDtrTid;
            *UaSID = lDTR[iloop].DemDtrUasid;
...
Dem_InterRepIUMPRFaultDetect(Dem_RatioIdType RatioID)
{
    ...
    {
#if (DEM_RATIO_NUM > 0)
        if (DemPbCfgPtr->pDemRatio[RatioID].DemRatioKind == DEM_RATIO_API)
        {
 ...
 */
/*************************************************************************/
FUNC(Std_ReturnType, DEM_CODE) Dem_InterRepIUMPRDenLock(Dem_RatioIdType RatioID)
{
    Std_ReturnType ret = E_NOT_OK;
    if (RatioID > DEM_RATIO_NUM)
    {
...
#if (DEM_RATIO_NUM > 0)
        /* req SWS_Dem_00362 */
        if (DEM_IUMPR_DEN_PHYS_API == DemPbCfgPtr->pDemRatio[RatioID].DemIUMPRDenGroup)
        {
...
FUNC(Std_ReturnType, DEM_CODE) Dem_InterRepIUMPRDenRelease(Dem_RatioIdType RatioID)
{
    ...
    {
#if (DEM_RATIO_NUM > 0)
        if (DEM_IUMPR_DEN_PHYS_API == DemPbCfgPtr->pDemRatio[RatioID].DemIUMPRDenGroup) /*SWS_Dem_00362] */
        {
...
Dem_InterSetDTR(uint16 DTRId, sint32 TestResult, sint32 LowerLimit, sint32 UpperLimit, Dem_DTRControlType Ctrlval)
{
    Std_ReturnType ret = E_NOT_OK;
#if (DEM_DTR_NUM > 0)
    const Dem_EventRelateInformationType* pEvent = &DemEventRelateInformation[DemPbCfgPtr->pDTR[DTRId].DemDtrEventRef];
    uint8 DemDtrUpdateKind = DemPbCfgPtr->pDTR[DTRId].DemDtrUpdateKind;
...
/*read current distance*/
    ...
    const DemDataElementClassTypes* lDemDataElementClass = &DemPbCfgPtr->pDemDataElementClass[ref];
    if ((ref != DEM_DATA_ELEMENT_CLASS_NUM_INVALID) && (lDemDataElementClass->DemDataElementReadFnc != NULL_PTR))
    {
        lDemDataElementClass->DemDataElementReadFnc(data);
    }
#else
...
/* PRQA S 6010 -- */
{
    ...
    if (pFilter->DTCOrigin == DEM_DTC_ORIGIN_PERMANENT_MEMORY)
    {
#if (DEM_MAX_NUMBER_EVENT_ENTRY_PERMANENT > 0u)
        eventId = DemPermanentMemory[pFilter->CurrentIndex];
#else
        eventId = 0u;
#endif
    }
    else
    {
        ...
        pEntry = &DemMemDestCfg[DTCOrigin].EntryList[pFilter->CurrentIndex];
        eventId = pEntry->EventId;
    }
    if (eventId != 0u)
    {
    ...
    P2VAR(Dem_J1939DcmLampStatusType, AUTOMATIC, DEM_APPL_DATA) LampStatus)
{
    ...
    const Dem_IndicatorAttributeType* pWIRAttr =
        &DemIndicatorAttribute[DemPbCfgPtr->pDemEventParameter[pEvent->IntId].AttrStartIndex];
    for (iloop = 0; iloop < DemPbCfgPtr->pDemEventParameter[pEvent->IntId].AttrNum; iloop++)
    {
 ...
 */
/*************************************************************************/
/* PRQA S 6030, 6010 ++ */ /* VL_MTR_Dem_STMIF, VL_MTR_Dem_STCYC */
FUNC(Dem_ReturnGetNextFilteredElementType, DEM_CODE)
...
#endif /* DEM_J1939_NODE_NUM > 0u */
            {
                ...
                else
                {
                    ...
                    *OccurenceCounter = (pEvent->OccurrenceCounter > 0x7Eu) ? 0x7Eu : pEvent->OccurrenceCounter;
                    for (iloop = 0; iloop < DEM_MAX_NUMBER_FF_RECORDS; iloop++)
                    {
#if (DEM_J1939_FREEZE_FRAME_CLASS_NUM > 0)
                        uint16 DTCAttRef = DemPbCfgPtr->DemCfgDTC[dtcRef].DemDTCAttributesRef;
                        uint8 ClassRef = DemPbCfgPtr->pDemDTCAttributes[DTCAttRef].DemJ1939FreezeFrameClassRef;
                        uint8 ExClassRef =
                            DemPbCfgPtr->pDemDTCAttributes[DTCAttRef].DemJ1939ExpandedFreezeFrameClassRef;
                        uint8 FreezeRef = 0u;
                        uint16 DataSize;
...
#if (DEM_DATA_ELEMENT_CLASS_NUM > 0)
                            uint8 offset = 0;
                            for (uint8 SpnRefNum = 0; SpnRefNum < DemJ1939FreezeFrameClass[FreezeRef].RefNum;
                                 SpnRefNum++)
                            {
                                uint16 SpnIndex =
                                ...
                                const DemDataElementClassTypes* pDemDataElementClass =
                                    &DemPbCfgPtr->pDemDataElementClass[StartIndex];
...
#if (DEM_J1939_FREEZE_FRAME_CLASS_NUM > 0)
    uint8 DemJ1939ExFFClassRef = DemPbCfgPtr->pDemDTCAttributes[DemPbCfgPtr->DemCfgDTC[dtcRef].DemDTCAttributesRef]
        ...
        if (DemDTCGeneralStatus[dtcRef].SuppressionStatus == FALSE)
        {
            ...
            while (iloop < DEM_MAX_NUMBER_FF_RECORDS)
            {
                if ((DemJ1939DcmFreezeFrameFilter.IsOK == FALSE)
                    && (DemJ1939ExFFClassRef != DEM_J1939_FREEZE_FRAME_INVALID))
                {
    ...
    P2VAR(uint16, AUTOMATIC, DEM_APPL_DATA) Denominator)
{
    ...
    if (dtcRef != DEM_DTC_REF_INVALID)
    {
...
#endif
        {
            ...
            {
                /* req SWS_Dem_01104 */
                if ((DemPbCfgPtr->pDemRatio[Ratioindex].DemDiagnosticEventRef == pEvent->IntId)
                    && (0x00u != DEM_FLAGS_ISSET(pEvent->Status, DEM_EVENT_STATUS_AVAILABLE)))
                {
                    *SPN = (uint16)(((DemPbCfgPtr->ObdDTC[DemPbCfgPtr->DemCfgDTC[dtcRef].DemObdDTCRef].DemJ1939DTCValue
                                      & 0xff0000u)
                                     >> 16u)
                                    | (DemPbCfgPtr->ObdDTC[DemPbCfgPtr->DemCfgDTC[dtcRef].DemObdDTCRef].DemJ1939DTCValue
                                       & 0x00ff00u));
    ...
    uint8 node)
{
    ...
    pEvent = Dem_GetEventInfo(IntId);
    dtcRef = DemPbCfgPtr->pDemEventParameter[IntId].DemDTCRef;
#if (DEM_J1939_NODE_NUM > 0)
    if (dtcRef != DEM_DTC_REF_INVALID)
    {
    ...
    P2VAR(uint8, AUTOMATIC, DEM_APPL_DATA) notCompletedGroups)
{
    ...
    uint16 obdDtcRef;
    uint32 J1939Dtc;
    const Dem_DTCType* pDTC = DemPbCfgPtr->DemCfgDTC;
    const DemObdDTCType* pObdDTC = DemPbCfgPtr->ObdDTC;

    for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
    {
        dtcRef = DemPbCfgPtr->pDemEventParameter[iloop].DemDTCRef;
        /* req SWS_Dem_01101 */
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_ClearAllDTC(uint8 memDest)
{
    uint16 iloop;
    uint16 dtcRef;
    Dem_EventRelateInformationType* pEvent;
 ...
 */
/*************************************************************************/
FUNC(void, DEM_CODE) Dem_ClearGroupDTC(uint8 memDest, uint8 GroupIndex)
{
    ...
    for (iloop = 0; iloop < DemPbCfgPtr->DemEventNum; iloop++)
    {
        Ref = DemPbCfgPtr->pDemEventParameter[iloop].DemDTCRef;
        pEvent = &DemEventRelateInformation[iloop];
        if ((Ref != DEM_DTC_REF_INVALID) && (DemPbCfgPtr->DemCfgDTC[Ref].GroupRef == GroupIndex)
...
FUNC(void, DEM_CODE) Dem_ClearDTRInfoByEventId(Dem_EventIdType IntId)
{
    ...
    /* req SWS_Dem_00763 */
    const DTRType* lDTR = DemPbCfgPtr->pDTR;
    for (uint16 iloop = 0; iloop < DEM_DTR_NUM; iloop++)
    {
        if (IntId == lDTR[iloop].DemDtrEventRef)
        {
