 ...
 */
/*************************************************************************/
static FUNC(Std_ReturnType, DCM_CODE) DslInternal_ProtocolStart(uint8 ProtocolCtrlId)
{
    boolean Flag;
    uint8 Index;
    uint8 NotOkNum = 0u;
    ...
    P2VAR(PduLengthType, AUTOMATIC, DCM_VAR) availableDataPtr)
{
...
#endif
    {
        ...
        else
        {
            ...
            {
                bufRet = Dcm_CopyTxData_PagebufferDeal(ProtocolCfgId, info, TxChannelCtrlIndex);
            }
            else
#endif
            {
...
#if (STD_ON == DCM_PAGEDBUFFER_ENABLED)
        && (((Dcm_PageBufferData.TotalSize == 0UL) && (Dcm_PageBufferData.PagedBufferStarted == TRUE))
            || (Dcm_PageBufferData.PagedBufferStarted == FALSE))
#endif
    )
    {
    ...
    uint8 TxChannelCtrlIndex)
{
    ...
    uint32* Position = &Dcm_ChannelCtrl[TxChannelCtrlIndex].Dcm_BufferCunrentPosition;

    /*check the pagebuffer timer whether timeout*/
    if ((Dcm_PageBufferData.TimeOut == TRUE) || (Dcm_PageBufferData.PagedBufferCanceled == TRUE))
    {
    ...
    {
        /** need updata page */
        if (((*Position) + info->SduLength) > Dcm_PageBufferData.FilledSize)
        {
            if (Dcm_PageBufferData.IsNextFilled == TRUE)
            {
                uint32 curCopyLength = ((Dcm_PageBufferData.FilledSize - (*Position)) > 0u)
                                           ? (Dcm_PageBufferData.FilledSize - (*Position))
                                           : 0u;
                /**< copy data of the current page */
                Dcm_MemoryCopy(&Dcm_PageBufferData.PagedBuffer[*Position], info->SduDataPtr, curCopyLength);
                /** updata page */
                DspInternal_DcmUpdatePage(ProtocolCfgId, TRUE);
                /**< copy data of the next page */
                Dcm_MemoryCopy(
                    Dcm_PageBufferData.PagedBuffer,
                    &info->SduDataPtr[curCopyLength],
                    info->SduLength - curCopyLength);
                *Position = info->SduLength - curCopyLength;
                Dcm_PageBufferData.TotalSize -= info->SduLength;
                bufRet = BUFREQ_OK;
            }
            else
            {
        ...
        {
            Dcm_MemoryCopy(&(Dcm_PageBufferData.PagedBuffer[*Position]), info->SduDataPtr, info->SduLength);
            /** updata page */
            if ((((*Position) + info->SduLength) == Dcm_PageBufferData.FilledSize)
        ...
        if (Dcm_PageBufferData.TotalSize == 0U)
        {
            DslInternal_InitPageBuffer();
        }
    }
