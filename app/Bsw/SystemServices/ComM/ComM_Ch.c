...
COMM_LOCAL void ComM_ChStateTransitionToNoCom(uint32 chIdx, uint32 currentReqCnt);
COMM_LOCAL void ComM_ChBeh_EntryFullCom(uint32 chIdx, uint32 currentReqCnt);
COMM_LOCAL void ComM_ChBeh_EntryComReadySleep(uint32 chIdx);
#if (COMM_USED_MODULE_NM == STD_ON)
COMM_LOCAL void ComM_ChBeh_EntrySlientMode(uint32 chIdx);
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChBeh_EntryFullComReq(uint32 chIdx, uint32 currentReqCnt)
{
    ...
    /* ComM_CommonClrBit must be done after Nm_PassiveStartUp. In order to avoid unnecessary flag updates during
     * the execution of Nm_PassiveStartUp, where BusNm might still report passive wake-up events, all
     * corresponding passive wake-up flags shall be cleared at this time. */
    SchM_Enter_ComM_COMM_EXCLUSIVE_AREA_0();
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChBeh_EntryNoComRequestPending(uint32 chIdx, uint32 currentReqCnt)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChBeh_EntryNoCom(uint32 chIdx)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChBeh_EntryComReadySleep(uint32 chIdx)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChBeh_EntryFullCom(uint32 chIdx, uint32 currentReqCnt)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChBeh_EntrySlientMode(uint32 chIdx)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
    SchM_Enter_ComM_COMM_EXCLUSIVE_AREA_0();
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChRequestUpdate(uint32 chIdx, uint32 reqIdex, ComM_ModeType comMode)
{
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
    if ((NULL_PTR != chCfgPtr->fullComReqNotifyFunPtr) && (reqIdex < chCfgPtr->belongUserNum))
    {
...
#endif /* COMM_FUNC_CHECK */
        {
            ...
            uint32 usrNum = *(chCfgPtr->userFullComNotifyNumPtr);
            if (comMode == COMM_FULL_COMMUNICATION)
            {
                boolean findFlg = FALSE;
                for (uint32 index = 0u; index < usrNum; index++)
                {
 ...
 * @return      NA
 */
COMM_LOCAL FUNC(void, COMM_CODE) ComM_ChStateChangeHandle(uint32 chIdx)
{
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
    for (uint32 index = 0u; index < chCfgPtr->belongUserNum; index++)
    {
        ...
        for (uint32 i = 0u; i < usrCfgPtr->directChListNum; i++)
        {
            uint32 pollChIdx = usrCfgPtr->directChList[i];
...
void ComM_ChInit(const ComM_ChannelConfigType* channelConfigPtr, uint32 chNum)
{
    ...
    for (uint32 chIdx = 0u; chIdx < chNum; chIdx++)
    {
        ...
        {
            for (uint32 i = 0u; i < chCfgPtr->belongUserNum; i++)
            {
...
#if ((COMM_USER_NUMBER > 0u) && (COMM_USER_MODESWITCH_NOTIFY == STD_ON))
    for (uint32 index = 0u; index < COMM_USER_NUMBER; index++)
    {
 ...
 */
FUNC(void, COMM_CODE) ComM_ChComAllow(uint32 chIdx, boolean allowed)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    if (TRUE == allowed)
    {
 ...
 */
FUNC(void, COMM_CODE) ComM_ChBusSmModeInd(uint32 chIdx, ComM_ModeType comMode)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
 ...
 */
FUNC(Std_ReturnType, COMM_CODE) ComM_ChRequstCommMode(uint32 chIdx, uint32 reqIdex, ComM_ModeType comMode)
{
 ...
 */
FUNC(void, COMM_CODE) ComM_ChEcuMWakeup(uint32 chIdx)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
#if (COMM_MANAGED_CHANNEL_SUPPORT == STD_ON)
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
 ...
 */
FUNC(void, COMM_CODE) ComM_ChPassiveWakeup(uint32 chIdx)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
 ...
 */
void ComM_ChLimitWakeUp(uint32 chIdx, boolean stu)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    SchM_Enter_ComM_COMM_EXCLUSIVE_AREA_0();
    if (TRUE == stu)
 ...
 */
void ComM_ChLimitNoCom(uint32 chIdx, boolean stu)
{
    ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    if (TRUE == stu)
    {
 ...
 */
FUNC(Std_ReturnType, COMM_CODE)
ComM_ChRequstCommModeLimitCheck(uint32 chIdx, uint32 reqIdex, ComM_ModeType comMode)
{
    Std_ReturnType ret = E_OK;
#if ((COMM_MODE_LIMITATION_ENABLED == STD_ON) || (COMM_WAKEUP_INHIBITION_ENABLED == STD_ON))
    const ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
 ...
 */
FUNC(void, COMM_CODE) ComM_ChDiagIndication(uint32 chIdx, boolean flag)
{
    const ComM_ChannelConfigType* chCfgPtr = &ComM_ChannelConfigPtr[chIdx];
    if (TRUE == flag)
    {
 ...
 */
void ComM_ChGetInhibitionStu(uint32 chIdx, ComM_InhibitionStatusType* chStu)
{
    const ComM_ChVarType* chVarPtr = &ComM_ChVarTable[chIdx];
    *chStu = 0x0u;
    if (ComM_CommonGetBit(chVarPtr->funcMask, COMM_FUN_LIMIT_NOCOM_REQ))
