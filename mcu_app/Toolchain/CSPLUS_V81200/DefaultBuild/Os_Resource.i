...
static void Os_RCBInit(Os_RCBType *pRcb);
...
static inline  void Os_SaveResourceByTaskOrder(const Os_SCBType *pScb, Os_TCBType *pTCB, ResourceType resId);
...
static inline  void Os_SaveResourceByIsr2Order(const Os_SCBType *pScb, Os_ICBType *pICB, ResourceType resId);
...
static void Os_GetResourceByTask(Os_SCBType *pScb, ResourceType resId);
...
static void Os_GetResourceByTaskOrInterrupt(Os_SCBType *pScb, ResourceType resId);
...
static void Os_GetResourceByInterrupt(const Os_SCBType *pScb, Os_ICBType *pICB, ResourceType resId);
...
static StatusType Os_ReleaseResourceByTask(const Os_SCBType *pScb, ResourceType resId);
...
static StatusType Os_ReleaseResourceByTaskOrInterrupt(Os_SCBType *pScb, ResourceType resId);
...
static StatusType Os_ReleaseResourceByInterrupt(const Os_SCBType *pScb, ResourceType resId);
...
static StatusType Os_GetResourceSafetyCheck(const Os_SCBType *pScb, ResourceType resId);
...
static StatusType Os_ReleaseResourceSafetyCheck(ResourceType resId);
...
static StatusType Os_ResourceIdSafetyCheck(ResourceType resId);
...
{
    pRcb->SaveCount = 0u;
    pRcb->SavePrio = ((Os_PriorityType)0xFFFFU);
...
static StatusType Os_GetResourceSafetyCheck(const Os_SCBType *pScb, ResourceType resId)
{
    ...
    else if ((OS_LEVEL_TASK == pScb->SysOsLevel) && (0U == (Os_TaskResourceAccessMask[pScb->SysRunningTaskId][resId >> 4u] & ((uint32)1u << (resId & 0x0Fu)))))
     
    {
        err = 1U;
    }
       
    else if ((OS_LEVEL_ISR2 == pScb->SysOsLevel) && (0U == (Os_IsrResourceAccessMask[pScb->SysRunningIsrCat2Id][resId >> 4u] & ((uint32)1u << (resId & 0x0Fu)))))
...
static StatusType Os_ResourceIdSafetyCheck(ResourceType resId)
{
    ...
    if (Os_ObjectIDCheck((ObjectType)resId, (uint8)OS_OBJECT_RESOURCE) != 1u)
    {
        err = 3U;
    }
    else
    if (Os_GetCoreIdLocal() != (Os_ResourceCfg[resId]. HostCore))    
    {
        err = 21U;
    }
...
StatusType Os_ReleaseResource(Os_SCBType *pScb, ResourceType resId)    
 
{
    ...
    if ((StatusType)0U == err)
    {
        ...
        Os_RCB[resId]->SaveCount = 0u;
        Os_RCB[resId]->SavePrio = ((Os_PriorityType)0xFFFFU);
