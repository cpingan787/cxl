...
/*************************************************
 Function: VssAdapter_SM4_Challenge_Response
 Description: 基于SM4的8字节挑战-应答计算
 Input:  key        密钥（16字节）
         challenge  挑战值/随机数（8字节）
 Output: response   应答值（8字节截断输出）
 Return: 0-成功, 其他-失败
 Others: 
*************************************************/
uint32_t VssAdapter_SM4_Challenge_Response(const uint8_t *key, const uint8_t *challenge, uint8_t *response)
{
    int ret = Crypto_SM4_Challenge_Response(key, challenge, response);

    if (ret == CRYPTO_E_NOT_OK)
    {
...
uint32_t VssAdapter_SM2_Verify(const uint8_t *data, uint32_t len, const uint8_t *signature, uint8_t signature_len, const uint8_t *pub_key)
{
    ...
    // uint8_t sm2_za_value[32] = {0xDB,0x89,0x9,0xD1,0xDB,0x8,0xBC,0xD7,0xC4,0x7D,0xC0,0x46,0x2C,0x3A,0xF,0xFF,0x43,0x36,0xF8,0x18,0x42,0x7F,0x1D,0xA1,0xDB,0x15,0x7F,0x91,0xE0,0x89,0x18,0x37};
    uint8_t sm2_za_value[32] = {
    0x8D, 0x17, 0x93, 0xB0, 0x94, 0x7E, 0x70, 0x98,
    0x86, 0x44, 0x15, 0x77, 0xEF, 0x57, 0xEB, 0x2F,
    0xC4, 0x97, 0x32, 0xDD, 0xD2, 0xC2, 0x98, 0x34,
    0x7F, 0x94, 0x58, 0x98, 0xCF, 0x15, 0xA2, 0xFD,
    };
