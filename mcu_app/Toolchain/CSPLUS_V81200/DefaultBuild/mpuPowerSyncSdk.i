...
static uint8_t g_rkMode = 0;                     
static uint8_t g_sleepMode = 0;                  

static uint8_t g_packData[64] = {0};
static MpuHalDataPack_t g_powerSyncPack;         

static uint8_t g_reqIndex = 0;                   
static uint8_t g_mpuSleepAckFlag = 0;            
static uint8_t g_mcuReqCpuSleepFlag = 0;         
static uint32_t g_mcuReqCpuSleepTimeCount = 0;   
static uint32_t g_mcuReqCpuSleepCount = 0;       


static SemaphoreHandle_t g_mutexHandle = ((void *) 0);   
static uint8_t g_sleepDisableEvent = 0;          
static uint8_t g_lastSleepDisableEvent = 0;      
static uint32_t g_sleepDisableDelayCount = 0;    

static uint8_t g_mpuKeepAliveLoseFlag = 1;       

static MpuErrorCbFun_t g_mpuErrorCallBackFunc = ((void *) 0);
...
static WakeUpSourceSet_t g_wakeUpSource;         
static uint8_t g_mpuWakeUpSource = 0;   
static uint8_t g_mpuSubWakeUpSource = 0; 
...
static int16_t MpuPowerSyncSdkSendPowerHalState(void)
{
    ...
    PeripheralHalAdGet(13,(uint32_t *)&voltage);
...
void MpuPowerSyncSdkCycleProcess(MpuHalDataPack_t *msgData)
{
    ...
    if(msgData != ((void *) 0))
    {
        ...
        if(msgData->aid == 0x01 && msgData->mid == 0x04)
        {
            ...
            else if((msgData->subcommand&0x7F) == 0x04) 
            {
                xSemaphoreTake(g_mutexHandle, 0xFFFFFFFFUL);
            ...
            else if((msgData->subcommand&0x7F) == 0x06) 
            {
                xSemaphoreTake(g_mutexHandle, 0xFFFFFFFFUL);
                 
                if(g_lastSleepDisableEvent != 0 && msgData->pDataBuffer[0] == 0)
                {
                    g_sleepDisableDelayCount = 120;  
                }
    ...
    if(cycleTimeCount++ >= 1000/g_processCycleTime) 
    {
        cycleTimeCount = 0;
    ...
    if(g_wakeUpSource.setFlag == 1)
    {
        if(wakeSourceSyncTimeCount++ >= (200/10))   
        {
    ...
    {
        LogHalPrint("mpu keep alive time out!\r\n");
        LogHalPrint("mpu keep alive time out!\r\n");
        LogHalPrint("mpu keep alive time out!\r\n");
        g_sleepDisableEvent = 0;  
        g_sleepDisableDelayCount = 0;  
        ...
        if(g_mpuErrorCallBackFunc != ((void *) 0))
        {
