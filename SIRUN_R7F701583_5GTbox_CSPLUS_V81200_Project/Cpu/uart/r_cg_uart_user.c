...
extern volatile uint8_t g_mpuUartReciveOneByteBuffer[];
...
#else
    temp = g_uart4_rx_count;
    if (g_uart4_rx_length > temp)
    {
        *gp_uart4_rx_address = rx_data_8;
        gp_uart4_rx_address++;
        g_uart4_rx_count++;
...
* Description  : This function is a callback function called when UART4 completed data reception.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
...
{
    /* Start user code. Do not edit comment generated here */
    if(g_debugUartReciveCount < 10)
    {
        g_debugUartReciveCount += g_uart4_rx_count;
        R_UART4_Receive(&g_debugUartReciveData[g_debugUartReciveCount], 1);
    }
    /* End user code. Do not edit comment generated here */
}
/***********************************************************************************************************************
...
void r_uart5_interrupt_receive(void)
{
    ...
    g_mpuUartReciveOneByteBuffer[0] = rx_data_8;
    UartProtocalProcess(g_mpuUartReciveOneByteBuffer, 1, 1);
    // MpuHalUartInterruptCallback(g_mpuUartReciveOneByteBuffer[0]);
    g_mpuUartReciveOneByteBuffer[0] = 0;
    R_PORT_SetGpioOutput(APort1, 1, 0);
#else
