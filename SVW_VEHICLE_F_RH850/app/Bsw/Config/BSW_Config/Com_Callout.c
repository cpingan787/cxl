...
static boolean SyncMsgLoss_FaultFlag = FALSE;

static void SyncMsgLoss_SyncFlagWithDem(void)
{
    Dem_UdsStatusByteType eventStatus = 0;

    if (Dem_GetEventStatus(EventParameter_0xD60087, &eventStatus) != E_OK)
    {
        return;
    }
    ...
    {
        SyncMsgLoss_FaultFlag = FALSE;
    }
    else
    {
        SyncMsgLoss_FaultFlag = TRUE;
    }
}
