...
extern void OSShellhandler(void);    
...
static void Os_TerminateTaskInternal(Os_SCBType *pScb, TaskType runningTaskID, Os_TCBType *runningTCB);
...
static void Os_TerminateTask(TaskType runningTaskID);
...
static void Os_GetTaskState(TaskType taskId, TaskStateRefType State);
...
static inline  void Os_Schedule(Os_SCBType *pScb);
...
static void Os_InitTaskTCB(Os_SCBType *pScb, TaskType taskId);
...
static void Os_InitTaskTCB(Os_SCBType *pScb, TaskType taskId)
{
    ...
    if (0U != (pTaskCfg->TaskAutoStartMode & pScb->SysActiveAppMode))
    {
        ...
        if ((pScb->SysHighPrio == ((Os_PriorityType)0xFFFFU)) || (pTCB->TaskRunPrio > pScb->SysHighPrio))
        {
 ...
 
{
    ...
    for (uint16 i = (uint16)idStartRange; i < (uint16)idEndRange; i++)
    {
        Os_InitTaskTCB(pScb, (Os_TaskType)i);    
    }
...
static void Os_TerminateTaskInternal(
    Os_SCBType *pScb,
 ...
 
{
    ...
    if (Os_ObjectIDCheck((ObjectType)TaskID, (uint8)OS_OBJECT_TASK) != 1u)
    {
        status = 3U;
    }
    else
...
void Os_SetTaskToReady(TaskType taskId)    
 ...
 
{
    ...
    if (0x0U == Os_TaskCfg[pScb->SysRunningTaskId].TaskSchedule)
    {
