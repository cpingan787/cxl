...
BswM_DoComMAllowCom(ApplicationType partIdx, uint8 actionIdx)
{
    const BswM_ActionItemsLCfgType * actionLCfgPtr;
...
BswM_DoEcuMStateSwitch(ApplicationType partIdx, uint8 actionIdx)
{
 ...
 Std_ReturnType BswM_DoNMControl(ApplicationType partIdx, uint8 actionIdx)
{
    const BswM_ActionItemsLCfgType * actionLCfgPtr;
    Std_ReturnType ret;
...
BswM_DoPduGrpSwitch(ApplicationType partIdx, uint8 actionIdx)
{
    ...
    for (gruIdx = 0; gruIdx < numOfEnableIpduGru; gruIdx++)
    {
        ...
        if ((boolean)1u == actionLCfgPtr->pduGrpSwitchActionLCfgs[actionIdx].initialize)
        {
    ...
    Com_IpduGroupControl(*(BswM_RuntimeStatus.bswmPartPCCfgs[partIdx].ipduGroupReinitVecotr), 1u);
     
    Com_IpduGroupControl(*(BswM_RuntimeStatus.bswmPartPCCfgs[partIdx].ipduGroupAllVecotr), 0u);
       
     
    for (index = 0u; index < sizeof(Com_IpduGroupVector); index++)
    {
        uint8* pReinitVector = (uint8*)(BswM_RuntimeStatus.bswmPartPCCfgs[partIdx].ipduGroupReinitVecotr);
        uint8 const* pAllVector = (uint8*)(BswM_RuntimeStatus.bswmPartPCCfgs[partIdx].ipduGroupAllVecotr);
        pReinitVector[index] = pAllVector[index];
    }
    return ret;
}
...
BswM_DoPduRouterControl(ApplicationType partIdx, uint8 actionIdx)
{
    ...
    uint8 pduGrpNum;
    uint8 index = 0;
    boolean initialize = 0u;

    actionLCfgPtr = BswM_RuntimeStatus.bswmPartLCfgs[partIdx].acItemsLCfg;
    pduGrpNum = actionLCfgPtr->pdurCtrlActionLCfgs[actionIdx].numOfPathGrp;

    if (actionLCfgPtr->pdurCtrlActionLCfgs[actionIdx].initialize != ((void *)0))
    {
